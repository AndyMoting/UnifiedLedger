...
fun validateIncompleteOwnAssetTransferSource(
    ...
    val source =
        catalog.account(sourceAccountId)
...
fun createOwnAssetAccountTransfer(
    ...
    val creditedWithFee =
        checkedAdd(command.destinationCredit.minorUnits, command.fee.minorUnits)
    ...
    val sourceMinor =
        checkedNegate(command.sourceDebit.minorUnits)
            ?: return DomainResult.Failure(DomainViolation.ArithmeticOverflow)
    val typedPostings =
        listOf(
    ...
    val netWorthChange =
        checkedNegate(command.fee.minorUnits)
            ?: return DomainResult.Failure(DomainViolation.ArithmeticOverflow)
...
private fun validateTransferFeeCategory(
    ...
    val feeCategory =
        catalog.category(feeCategoryId)
            ?: return DomainResult.Failure(AccountTransferViolation.InvalidFeeCategory)
    val feeParent =
        feeCategory.parentId?.let(catalog::category)
            ?: return DomainResult.Failure(AccountTransferViolation.InvalidFeeCategory)
    val feeAccount =
        feeCategory.postingAccountId?.let(catalog::account)
            ?: return DomainResult.Failure(AccountTransferViolation.InvalidFeeCategory)
    if (
