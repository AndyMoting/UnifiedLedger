...
data class MixedPaymentPosting(
    val posting: Posting,
    val role: MixedPaymentPostingRole,
...
fun createMixedPaymentExpense(
    ...
    val fundingTotal =
        command.funding.fold(0L) { sum, item ->
    ...
    val accounts =
        command.funding.map { funding ->
            val account =
                catalog.account(funding.accountId)
    ...
    val cashOut =
        command.funding
            .zip(accounts)
            .filter { it.second.kind == AccountKind.ASSET }
            .sumOf { it.first.amount.minorUnits }
...
fun createCreditExpense(
    ...
    val liabilityMinor =
        checkedNegate(command.total.minorUnits)
