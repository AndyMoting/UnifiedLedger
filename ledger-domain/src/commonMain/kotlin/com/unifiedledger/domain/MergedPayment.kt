...
fun createMergedPaymentExpense(
    ...
    if (command.items.map { it.itemId }.toSet().size != command.items.size) return DomainResult.Failure(MergedPaymentViolation.DuplicateItemId)
    ...
    val funding = catalog.account(command.fundingAccountId)
    ...
    val expenseAccounts = command.items.map { item ->
        val category = catalog.category(item.categoryId)
        ...
        val account = category.postingAccountId?.let(catalog::account)
    ...
    val typed = command.items.zip(expenseAccounts).zip(ids.expensePostingIds).map { (pair, postingId) ->
    ...
    val postingSet = when (val result = PostingSet.create(ids.postingSetId, typed.map { it.posting })) {
