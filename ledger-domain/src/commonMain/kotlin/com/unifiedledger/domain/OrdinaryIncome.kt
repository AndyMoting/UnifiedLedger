...
fun createAssetReceivedOrdinaryIncome(
    ...
    val receivingAccount =
        catalog.account(command.receivingAccountId)
            ?: return DomainResult.Failure(DomainViolation.InvalidOrdinaryIncome)
    if (incomeAccount.ledgerId != command.ledgerId ||
        incomeAccount.kind != AccountKind.INCOME ||
        incomeAccount.realAccount ||
        incomeAccount.currency != command.amount.currency ||
        receivingAccount.ledgerId != command.ledgerId ||
        receivingAccount.kind != AccountKind.ASSET ||
        !receivingAccount.realAccount ||
        !receivingAccount.ownedByUser ||
        receivingAccount.currency != command.amount.currency
    ) {
        return DomainResult.Failure(DomainViolation.InvalidOrdinaryIncome)
    }
    val negative =
    ...
    val postingSet =
        when (postings) {
            is DomainResult.Success -> postings.value
