...
internal inline fun <reified T> success(result: DomainResult<T>): T =
    assertIs<DomainResult.Success<T>>(result).value

internal fun failure(result: DomainResult<*>): DomainViolation =
    assertIs<DomainResult.Failure>(result).violation

internal fun money(minorUnits: Long, currency: CurrencyUnit): Money =
    Money.ofMinor(minorUnits, currency)
...
internal class Rg01Fixture {
    ...
    val command = AssetPaidOrdinaryExpenseCommand(
        ledgerId = ledgerId,
        amount = amount,
    ...
    fun acceptedExpense(): FormalTransaction =
        success(createAssetPaidOrdinaryExpense(catalog, command, expenseIds))
    ...
    fun openingBalance(): FormalTransaction {
        ...
        val openingTimes = TransactionTimes.collapsed(
