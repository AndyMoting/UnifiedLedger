...
class OwnAssetPrincipalTransferTest {
    ...
    fun `principal transfer creates only two balanced internal postings`() {
        val transfer = assertIs<DomainResult.Success<OwnAssetPrincipalTransfer>>(
    ...
    fun `principal transfer rejects non-positive amount and same account`() {
        val base = OwnAssetPrincipalTransferCommand(
            ledgerId,
            AccountId("asset-b"),
            AccountId("asset-a"),
        ...
        val ids = OwnAssetPrincipalTransferIds(
            TransactionId("tx-transfer"), TransactionVersionId("version-transfer"),
            PostingSetId("posting-set-transfer"), PostingId("posting-out"), PostingId("posting-in"),
        )
        assertEquals(
