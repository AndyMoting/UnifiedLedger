...
class Rg03OperationBoundaryTest {
    ...
    fun `adapter preserves exact facts for every approved action without expected-state input`() {
        ...
        val commands = decoded.operations.filter { it.replayOf == null && it.expected !is Rg03ExpectedOutcome.Rejected }
    ...
    fun `adapter rejects invalid decimals timestamps ids currency and implicit confirmation`() {
        val manual = assertIs<Rg03Command.ManualTransfer>(
    ...
    fun `confirmation recovers persisted candidate and mirror uses operation scoped unique binding`() {
        ...
        val executor = ExecuteRg03Operation(
            candidateRecovery = Rg03CandidateRecoveryPort { ledgerId, candidateId ->
                recoveredLedger = ledgerId
                recoveredCandidateId = candidateId
                candidate
            },
            mirrorBinding = Rg03MirrorBindingPort { ledgerId, scope ->
            ...
            commitPort = Rg03PreparedOperationCommitPort { prepared ->
                committed += prepared
    ...
    fun `confirmed candidate and ambiguous mirror reach commit boundary for replay resolution`() {
        ...
        val executor = ExecuteRg03Operation(
            ...
            commitPort = Rg03PreparedOperationCommitPort { prepared ->
                commits++
                when (prepared) {
                    is Rg03PreparedOperation.ConfirmCandidate -> Rg03ExecutionResult.NoChange(
    ...
    fun `source result replaces stale lifecycle only from exactly one typed candidate id`() {
        ...
        cases.forEach { (sourceResult, expectedCandidateId) ->
            ...
            val executor = ExecuteRg03Operation(
                ...
                mirrorBinding = Rg03MirrorBindingPort { _, scope ->
            ...
            assertIs<Rg03ExecutionResult.Accepted>(
                executor.execute(
                    Rg03Command.ConfirmCandidate(
                        RequestId("confirm-a"), candidateA.candidateId, true, context.ledgerId,
                    ),
                ),
            )
    ...
    fun `every rejected root start clears a previously confirmed lifecycle scope`() {
        fun assertCleared(start: Rg03Command, expected: Rg03ExecutionResult) {
            ...
            assertIs<Rg03ExecutionResult.Accepted>(
                executor.execute(
                    Rg03Command.ConfirmCandidate(
                        RequestId("confirm-a"), candidate.candidateId, true, context.ledgerId,
                    ),
                ),
            )
        ...
        assertCleared(
            Rg03Command.ConfirmCandidate(
                RequestId("confirm-rejected"), CandidateId("candidate-b"), false, context.ledgerId,
            ),
    ...
    fun `recovery candidate id mismatch rejects without commit and clears stale scope`() {
        ...
        assertEquals(
            ...
            executor.execute(
                Rg03Command.ConfirmCandidate(
                    RequestId("confirm-b"), candidateB, true, context.ledgerId,
                ),
            ),
        )
    ...
    private fun persistedCandidate() = Rg03PersistedTransferCandidate(
    ...
    private fun sourceSnapshot() = Rg03SourceSnapshot(
        ledgerId = context.ledgerId,
