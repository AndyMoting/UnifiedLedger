...
class O2PrecisionRescaleTest {
    ...
    fun allApprovedFactoriesUseTheExplicitAccountCurrency() {
        ...
        val refund = formal(
            ...
            ImportConfirmDecisionFields.CreditExpenseRefundFlow(
                CategoryId("category-food"), AccountId("liability"), TransactionId("original"),
            ),
            ids("refund", 2),
        )
        ...
        val mixed = formal(
            ...
            ImportConfirmDecisionFields.MixedPaymentFlow(
                CategoryId("category-food"), AccountId("asset"), AccountId("liability"), 5000L, 4900L,
            ),
            ids("mixed", 3),
        )
    ...
    fun mixedLegValidationRejectsInvalidTotalsAndArithmeticOverflow() {
        val catalog = fixtureCatalog()
        fun create(assetLeg: Long, creditLeg: Long): DomainResult<ImportFormalCommit> =
            MixedPaymentFlowFormalFactory(catalog).create(
                ImportCandidateFormalizationInput(
                    LedgerId("ledger"),
                    source(amountMinor = 99L, precision = 0),
                    ImportConfirmDecisionFields.MixedPaymentFlow(
                        CategoryId("category-food"), AccountId("asset"), AccountId("liability"), assetLeg, creditLeg,
                    ),
                ),
    ...
    fun nonRepresentableAndCurrencyMismatchFailBeforeDomainAssembly() {
        val catalog = fixtureCatalog()
        val nonRepresentable = OrdinaryFlowFormalFactory(catalog).create(
            ImportCandidateFormalizationInput(
                LedgerId("ledger"), source(amountMinor = 1234L, precision = 3),
                ImportConfirmDecisionFields.OrdinaryFlow(CategoryId("category-food"), AccountId("asset")),
            ),
            ids("reject", 2),
        )
        ...
        val mismatch = OrdinaryFlowFormalFactory(catalog).create(
            ImportCandidateFormalizationInput(
                LedgerId("ledger"), source(currencyCode = "USD", amountMinor = 99L, precision = 0),
                ImportConfirmDecisionFields.OrdinaryFlow(CategoryId("category-food"), AccountId("asset")),
            ),
            ids("mismatch", 2),
        )
    ...
    fun genericBindingRejectsAFormallyValidGraphWithAReplacedPostingId() {
        ...
        val canonical = assertIs<DomainResult.Success<ImportFormalCommit>>(
            TransferFlowFormalFactory(catalog, AccountId("asset")).create(input, ids),
        ).value
        val malicious = rebuildTransferGraph(
            base = canonical,
    ...
    fun catalogBindingRejectsAValidGraphUsingTheWrongAccountPrecision() {
        ...
        val wrongPrecisionGraph = rebuildTransferGraph(
            base = canonical,
    ...
    private fun formal(
        ...
        return result.transaction.postingSets.single().postings
    }
    ...
    private fun rebuildTransferGraph(
        ...
        amountMinor: Long,
        sourcePostingId: PostingId = base.transaction.postingSets.single().postings[0].id,
        destinationPostingId: PostingId = base.transaction.postingSets.single().postings[1].id,
    ): ImportFormalCommit {
    ...
    private fun ids(prefix: String, postingCount: Int) = ImportCommitIds(
    ...
    private fun originalExpense(id: TransactionId) = CreditRefundOriginalExpense(
        transactionId = id,
    ...
    private fun fixtureCatalog(): LedgerCatalog = assertIs<DomainResult.Success<LedgerCatalog>>(
