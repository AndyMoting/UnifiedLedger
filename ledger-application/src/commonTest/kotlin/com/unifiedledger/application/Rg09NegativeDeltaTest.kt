...
class Rg09NegativeDeltaTest {
    ...
    fun `negative adjustment uses target decrease and positive explanation reversal`() {
        ...
        val preview = previewSource.copy(
            input = previewSource.input.copy(
        ...
        val adjustment = adjustmentSource.copy(
            input = adjustmentSource.input.copy(
        ...
        val transfer = transferSource.copy(
            input = transferSource.input.copy(
        ...
        val transferRecord = runtime.snapshot().formalTransactions.single {
            it.formalTransaction.transaction.id.value == "transaction-transfer-negative-rg09"
        }
        assertEquals(-2_000L, transferRecord.formalTransaction.currentPostings().single {
        ...
        val explanation = explanationSource.copy(
            input = explanationSource.input.copy(
        ...
        val reversal = after.formalTransactions.single {
            it.formalTransaction.transaction.id.value == "transaction-reversal-negative-rg09"
        }
        assertEquals(2_000L, reversal.formalTransaction.currentPostings().single {
