...
class Rg07OperationsTest {
    ...
    private val case = Rg07AdaptedCase(
        ledger, currency,
    ...
    @Test fun manualExpenseAdapterRejectsNonPositiveAndWrongCurrency() {
        val base = Rg07ManualExpenseInput(
            RequestId("request"), Money.ofMinor(12_000, currency), CategoryId("daily"), AccountId("bank"),
            Instant.parse("2026-01-10T00:00:00Z"), "", true,
        )
        assertEquals(
    ...
    @Test fun statusAdapterRequiresAtLeastOneRecordedTime() {
        val base = Rg07StatusInput(
            RequestId("request"), TransactionId("original"), Money.ofMinor(3_000, currency),
            null, null, null,
        )
        assertEquals(
    ...
    @Test fun statusSourceUsesTypedRefundStatus() {
        val accepted = adaptRg07StatusSource(
            case,
    ...
    @Test fun dualRoleEvidenceRequiresValidRoleSet() {
        val good = Rg07DualRoleEvidenceInput(
            "source", "evidence", "relation", "posting", Instant.parse("2026-02-02T10:00:00Z"),
    ...
    @Test fun originalPaymentEvidencePreservesSignedAssetAmountThroughExecution() {
        ...
        val result = ExecuteRg07Operation { operation ->
            committed = operation
    ...
    @Test fun importConfirmationAdapterChecksCurrencyAndPreservesNullableBindings() {
        val complete = Rg07ImportConfirmationInput(
            "request", "candidate-refund-rg07-import", TransactionId("original"), CategoryId("daily"),
            Money.ofMinor(3_000, currency), AccountId("wallet"), Instant.parse("2026-02-02T07:20:00Z"),
            Instant.parse("2026-02-02T10:00:00Z"), true,
        )
    ...
    @Test fun validateAdapterIsStructuralAndDefersSemanticsToStore() {
        val base = Rg07ValidateInput(
            "attempt", TransactionId("original"), Money.ofMinor(3_000, currency), CategoryId("daily"), AccountId("wallet"),
            true, Money.ofMinor(12_000, currency),
        )
    ...
    @Test fun dualRoleFingerprintCanonicalizesSetOrder() {
        val forward = Rg07Operation.DualRoleEvidence(
            ledger,
