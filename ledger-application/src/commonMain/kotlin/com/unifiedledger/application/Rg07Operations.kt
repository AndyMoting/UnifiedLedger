...
data class Rg07OperationIdentity(val ledgerId: LedgerId, val value: String)

enum class Rg07Action(val code: String) {
...
enum class Rg07RejectionReason(val code: String) {
...
enum class Rg07FieldPath(val value: String) {
...
sealed interface Rg07ExecutionResult {
    ...
    data class Rejected(val reason: Rg07RejectionReason, val fieldPath: Rg07FieldPath) : Rg07ExecutionResult
    data object RequestIdentityConflict : Rg07ExecutionResult
}

sealed interface Rg07ReturnedId {
    data class Confirmation(val id: String) : Rg07ReturnedId
    data class Transaction(val id: String) : Rg07ReturnedId
    data class Source(val id: String) : Rg07ReturnedId
    data class Evidence(val id: String) : Rg07ReturnedId
    data class EvidenceLink(val id: String) : Rg07ReturnedId
    data class Candidate(val id: String) : Rg07ReturnedId
    data class Relation(val id: String) : Rg07ReturnedId
    data class DomainEntity(val id: String) : Rg07ReturnedId
}

fun interface Rg07CommitPort { fun commit(operation: Rg07Operation): Rg07ExecutionResult }
 ...
 */
class ExecuteRg07Operation(private val port: Rg07CommitPort) {
    fun execute(operation: Rg07Operation): Rg07ExecutionResult = when (operation) {
        is Rg07Operation.ManualExpense ->
            if (!operation.input.explicitConfirmation) rejected(Rg07RejectionReason.ARRIVAL_CONFIRMATION_REQUIRED, Rg07FieldPath.ARRIVAL_CONFIRMED)
            else if (operation.input.amount.minorUnits <= 0L) rejected(Rg07RejectionReason.MUST_BE_POSITIVE, Rg07FieldPath.AMOUNT)
            else port.commit(operation)
        is Rg07Operation.ManualReceipt ->
            if (!operation.input.arrivalConfirmed) rejected(Rg07RejectionReason.ARRIVAL_CONFIRMATION_REQUIRED, Rg07FieldPath.ARRIVAL_CONFIRMED)
            else if (operation.input.refundRelationId == null) rejected(Rg07RejectionReason.REFUND_RELATION_NOT_FOUND, Rg07FieldPath.REFUND_RELATION_ID)
            else if (!operation.input.isCompleteAcceptedInput()) rejected(Rg07RejectionReason.INVALID_RECEIPT_MODE, Rg07FieldPath.ARRIVAL_CONFIRMED)
            else port.commit(operation)
        ...
        is Rg07Operation.OriginalPaymentEvidence ->
            if (operation.input.amount.minorUnits >= 0L) rejected(Rg07RejectionReason.MUST_BE_NEGATIVE, Rg07FieldPath.AMOUNT)
            else port.commit(operation)
        is Rg07Operation.DestinationEvidence ->
            if (operation.input.amount.minorUnits <= 0L) rejected(Rg07RejectionReason.MUST_BE_POSITIVE, Rg07FieldPath.AMOUNT)
            else port.commit(operation)
        is Rg07Operation.DualRoleEvidence ->
            if (!isValidRg07DualRoleSet(operation.input.roles)) rejected(Rg07RejectionReason.EVIDENCE_TARGET_MISMATCH, Rg07FieldPath.EVIDENCE_ID)
            else port.commit(operation)
        is Rg07Operation.Allocate ->
            if (operation.input.requestedAllocation.minorUnits <= 0L) rejected(Rg07RejectionReason.MUST_BE_POSITIVE, Rg07FieldPath.REQUESTED_ALLOCATION)
            ...
            else rejected(Rg07RejectionReason.REFUND_AMOUNT_EXCEEDS_REMAINING, Rg07FieldPath.REQUESTED_ALLOCATION)
...
private fun rejected(reason: Rg07RejectionReason, path: Rg07FieldPath): Rg07ExecutionResult = Rg07ExecutionResult.Rejected(reason, path)

fun isValidRg07StatusTransition(previous: Rg07RefundStatus?, next: Rg07RefundStatus): Boolean = when (previous) {
...
fun isValidRg07DualRoleSet(roles: List<String>): Boolean =
    roles.size == 2 && roles.toSet() == setOf("refund_relationship", "destination_asset_posting")

fun Rg07ManualReceiptInput.isCompleteAcceptedInput(): Boolean =
    arrivalConfirmed && refundRelationId != null && originalTransactionId != null && amount != null &&
        categoryId != null && destinationAccountId != null && sourceObservedAt != null && bookingAt != null &&
        valueAt != null && arrivedAt != null && confirmedAt != null &&
        confirmationMode == "explicit_manual_receipt" && observationMode == "manual_account_observation"

/** Stable, closed payload used by persistence and replay conflict detection. */
fun Rg07Operation.fingerprint(): String = buildString {
    ...
    when (this@fingerprint) {
        ...
        is Rg07Operation.StatusSource -> append(input.sourceId).append('|').append(input.observedAt).append('|').append(input.reportedState).append('|').append(input.provesArrival)
...
data class Rg07AdaptedCase(val ledgerId: LedgerId, val currency: CurrencyUnit, val catalog: LedgerCatalog)
sealed interface Rg07AdaptResult {
    data class Success(val operation: Rg07Operation) : Rg07AdaptResult
    data class Invalid(val reason: Rg07RejectionReason, val fieldPath: Rg07FieldPath) : Rg07AdaptResult
}

private fun Rg07AdaptedCase.positiveAmount(amount: Money): Rg07AdaptResult.Invalid? =
    if (amount.currency != currency) Rg07AdaptResult.Invalid(Rg07RejectionReason.SAME_CURRENCY_REQUIRED, Rg07FieldPath.CURRENCY)
    else if (amount.minorUnits <= 0L) Rg07AdaptResult.Invalid(Rg07RejectionReason.MUST_BE_POSITIVE, Rg07FieldPath.AMOUNT)
    else null

fun adaptRg07ManualExpense(case: Rg07AdaptedCase, input: Rg07ManualExpenseInput): Rg07AdaptResult =
    case.positiveAmount(input.amount) ?: Rg07AdaptResult.Success(Rg07Operation.ManualExpense(case.ledgerId, input))

fun adaptRg07Status(case: Rg07AdaptedCase, input: Rg07StatusInput): Rg07AdaptResult =
    if (input.requestedAmount.currency != case.currency) Rg07AdaptResult.Invalid(Rg07RejectionReason.SAME_CURRENCY_REQUIRED, Rg07FieldPath.CURRENCY)
    else if (input.requestedAmount.minorUnits <= 0L) Rg07AdaptResult.Invalid(Rg07RejectionReason.MUST_BE_POSITIVE, Rg07FieldPath.AMOUNT)
    ...
    else Rg07AdaptResult.Success(Rg07Operation.Status(case.ledgerId, input))

fun adaptRg07StatusSource(case: Rg07AdaptedCase, input: Rg07StatusSourceInput): Rg07AdaptResult =
    Rg07AdaptResult.Success(Rg07Operation.StatusSource(case.ledgerId, input))

fun adaptRg07ManualReceipt(case: Rg07AdaptedCase, input: Rg07ManualReceiptInput): Rg07AdaptResult =
    Rg07AdaptResult.Success(Rg07Operation.ManualReceipt(case.ledgerId, input))

fun adaptRg07OriginalPaymentEvidence(case: Rg07AdaptedCase, input: Rg07OriginalPaymentEvidenceInput): Rg07AdaptResult {
...
fun adaptRg07DestinationEvidence(case: Rg07AdaptedCase, input: Rg07DestinationEvidenceInput): Rg07AdaptResult =
    case.positiveAmount(input.amount) ?: Rg07AdaptResult.Success(Rg07Operation.DestinationEvidence(case.ledgerId, input))

fun adaptRg07DualRoleEvidence(case: Rg07AdaptedCase, input: Rg07DualRoleEvidenceInput): Rg07AdaptResult =
    if (isValidRg07DualRoleSet(input.roles)) Rg07AdaptResult.Success(Rg07Operation.DualRoleEvidence(case.ledgerId, input))
    else Rg07AdaptResult.Invalid(Rg07RejectionReason.EVIDENCE_TARGET_MISMATCH, Rg07FieldPath.EVIDENCE_ID)

fun adaptRg07ConfirmReceipt(case: Rg07AdaptedCase, input: Rg07ConfirmReceiptInput): Rg07AdaptResult =
    Rg07AdaptResult.Success(Rg07Operation.ConfirmReceipt(case.ledgerId, input))

fun adaptRg07Allocate(case: Rg07AdaptedCase, input: Rg07AllocateInput): Rg07AdaptResult =
    ...
    else Rg07AdaptResult.Success(Rg07Operation.Allocate(case.ledgerId, input))

fun adaptRg07ImportCredit(case: Rg07AdaptedCase, input: Rg07ImportCreditInput): Rg07AdaptResult =
    case.positiveAmount(input.amount) ?: Rg07AdaptResult.Success(Rg07Operation.ImportCredit(case.ledgerId, input))

fun adaptRg07ImportConfirmation(case: Rg07AdaptedCase, input: Rg07ImportConfirmationInput, identity: Rg07OperationIdentity): Rg07AdaptResult =
    if (input.allocatedAmount?.currency != null && input.allocatedAmount.currency != case.currency)
        Rg07AdaptResult.Invalid(Rg07RejectionReason.SAME_CURRENCY_REQUIRED, Rg07FieldPath.CURRENCY)
    else Rg07AdaptResult.Success(Rg07Operation.ImportConfirm(case.ledgerId, input, identity))

fun adaptRg07Mirror(case: Rg07AdaptedCase, input: Rg07MirrorInput): Rg07AdaptResult =
    case.positiveAmount(input.amount) ?: Rg07AdaptResult.Success(Rg07Operation.Mirror(case.ledgerId, input))

fun adaptRg07Validate(case: Rg07AdaptedCase, input: Rg07ValidateInput): Rg07AdaptResult =
