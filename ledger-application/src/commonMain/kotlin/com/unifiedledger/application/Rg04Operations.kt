...
data class Rg04FundingInput(
    val accountId: Rg04Field<String>,
    val amount: Rg04Field<String>,
    val currency: Rg04Field<String>,
)

data class Rg04SettlementInput(
...
data class Rg04ManualInput(
    val requestId: Rg04Field<String>,
    val occurredAt: Rg04Field<String>,
    val totalAmount: Rg04Field<String>,
    val currency: Rg04Field<String>,
    val categoryId: Rg04Field<String>,
    val funding: List<Rg04FundingInput>,
    val settlement: Rg04SettlementInput? = null,
    val explicitConfirmation: Rg04Field<Boolean> = Rg04Field.Omitted,
)

data class Rg04RepaymentInput(
    val requestId: Rg04Field<String>,
    val occurredAt: Rg04Field<String>,
    val assetAccountId: Rg04Field<String>,
    val liabilityAccountId: Rg04Field<String>,
    val principalAmount: Rg04Field<String>,
    val currency: Rg04Field<String>,
...
data class Rg04OperationSource(
    val locator: String,
...
sealed interface Rg04DecodedOperation {
    ...
    data class Manual(
        ...
        override val action = Rg04Action.MANUAL_MIXED_EXPENSE
    }
...
data class Rg04RawJsonCase(
    ...
    val operations: List<Rg04DecodedOperation>,
    val deferredOperations: List<Rg04DeferredOperation>,
    val manualIds: MixedPaymentExpenseIds,
    val repaymentIds: CreditPrincipalRepaymentIds,
    val relationId: String,
    val relationDisplayName: String,
    val importOperations: List<Rg04DecodedImportOperation> = emptyList(),
)

data class Rg04FundingSnapshot(
    val accountId: AccountId,
    val amount: Money,
)

data class Rg04SettlementSnapshot(
    val original: Money,
    val discount: Money,
    val settled: Money,
)
...
data class Rg04ManualSnapshot(
    ...
    val settlement: Rg04SettlementSnapshot,
...
fun interface Rg04CommitPort {
    fun commit(operation: Rg04PreparedOperation): Rg04ExecutionResult
}

class ExecuteRg04Operation(
    private val port: Rg04CommitPort,
) {
...
sealed interface Rg04AdaptResult {
    data class Success(
        val operation: Rg04PreparedOperation,
    ) : Rg04AdaptResult
...
fun adaptRg04Operation(
    ...
    when (operation) {
        ...
        is Rg04DecodedOperation.Repayment ->
            when (val adapted = adaptRepayment(case, operation.input)) {
                is Rg04AdaptResult.Invalid -> adapted
                is Rg04AdaptResult.Success -> {
...
private fun adaptManual(
    ...
    val category =
        when (input.categoryId) {
            is Rg04Field.Value -> input.categoryId.value
            else -> return invalid("secondary_category_required", "category_id")
        }
    val funding =
        input.funding.mapIndexed { index, item ->
            ...
            val amount =
                money(amountText, CurrencyUnit(currency, case.currency.precision))
...
private fun adaptRepayment(
    case: Rg04RawJsonCase,
    input: Rg04RepaymentInput,
): Rg04AdaptResult {
    fun text(field: Rg04Field<String>): String? = (field as? Rg04Field.Value)?.value
    val request = text(input.requestId) ?: return invalid("required", "request_id")
    val occurred = text(input.occurredAt) ?: return invalid("required", "occurred_at")
...
private fun invalid(
    reason: String,
    field: String,
): Rg04AdaptResult.Invalid = Rg04AdaptResult.Invalid(reason, field)

private fun timestamp(text: String): Instant? =
    try {
        Instant.parse(text)
