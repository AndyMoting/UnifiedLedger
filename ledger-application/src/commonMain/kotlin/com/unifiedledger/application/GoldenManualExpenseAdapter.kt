...
sealed interface Rg01AttemptedExpenseResult {
    data class Mapped(
        val projection: Rg01OutcomeProjection,
    ) : Rg01AttemptedExpenseResult

    data class InvalidContract(
        val error: Rg01ContractError,
    ) : Rg01AttemptedExpenseResult
}

sealed interface Rg01ManualExpenseParseResult {
    data class Success(
        val value: Rg01ParsedManualExpenseInput,
...
fun parseRg01ManualExpenseInput(
    ...
    val originalAmountText =
        when (val field = input.amount) {
    ...
    val amount =
        parseExactMoney(originalAmountText, context.currency)
    ...
    val occurredAt =
        when (val parsed = parseCaseTimestamp(context, occurredAtText)) {
    ...
    val paymentAccountText =
        when (val field = input.paymentAccountId) {
...
fun evaluateRg01AttemptedManualExpense(
    ...
    when (val field = input.occurredAt) {
        ...
        is Rg01JsonField.Value ->
            when (val parsed = parseCaseTimestamp(context, field.value)) {
    ...
    val projection =
        when {
            input.amount !is Rg01JsonField.Value ->
                frozenRejection(
                    "$.attempted_input.amount",
                    "missing_required_field",
                )
            input.paymentAccountId !is Rg01JsonField.Value ->
                frozenRejection(
                    "$.attempted_input.payment_account_id",
                    "missing_required_field",
                )
            input.categoryId !is Rg01JsonField.Value ->
                frozenRejection(
                    "$.attempted_input.category_id",
                    "missing_required_field",
                )
            checkNotNull(amount).minorUnits <= 0L ->
                frozenRejection(
                    "$.attempted_input.amount",
                    "must_be_positive",
                )
            checkNotNull(category).parentId == null ->
                frozenRejection(
                    "$.attempted_input.category_id",
                    "secondary_category_required",
                )
            !checkNotNull(category).active ->
                frozenRejection(
                    "$.attempted_input.category_id",
                    "category_inactive",
                )
...
fun projectRg01ManualExpenseResult(result: ManualExpenseSaveResult): Rg01ProjectionResult =
    when (result) {
        ...
        is ManualExpenseSaveResult.Executed ->
            when (val executed = result.result) {
                is ConfirmedManualExpenseResult.Created ->
                ...
                is ConfirmedManualExpenseResult.NoChange ->
                    Rg01ProjectionResult.Mapped(
                        Rg01OutcomeProjection(
...
private fun <T> Rg01JsonField<T>.valueOrNull(): T? = (this as? Rg01JsonField.Value<T>)?.value
