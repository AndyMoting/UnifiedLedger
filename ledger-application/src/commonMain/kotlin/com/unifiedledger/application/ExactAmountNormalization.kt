...
internal sealed interface ExactAmountNormalization {
    data class Success(val amountMinor: Long) : ExactAmountNormalization
...
internal fun normalizeSourceMinorExact(
    ...
    if (targetPrecision >= sourceScale) {
        val factor = powerOfTenOrNull(targetPrecision - sourceScale)
    ...
    val divisor = powerOfTenOrNull(difference)
        ?: return ExactAmountNormalization.ArithmeticOverflow
...
internal fun normalizeImportAmountExact(
    ...
    return when (
        val result = normalizeSourceMinorExact(
            amountMinor = resolved.amountMinor,
            sourceScale = resolved.currencyPrecision,
            targetPrecision = targetCurrency.precision,
        )
        ...
        ExactAmountNormalization.NotRepresentable -> DomainResult.Failure(
            DomainViolation.AmountNotRepresentableInCurrency(
                amountMinor = resolved.amountMinor,
                sourceScale = resolved.currencyPrecision,
                targetCurrencyCode = targetCurrency.code,
...
internal fun parseImportOccurredAt(value: String): DomainResult<Instant> = try {
...
private fun multiplyExactOrNull(left: Long, right: Long): Long? {
...
fun validateImportFormalBindingAgainstCatalog(
    ...
    val targetCurrencies = when (fields) {
    ...
    fun categoryPostingAccount(categoryId: com.unifiedledger.domain.CategoryId, kind: CategoryKind): AccountId? {
        val category = catalog.categories.firstOrNull { it.id == categoryId && it.ledgerId == input.ledgerId }
            ?: return null
        ...
        val postingAccount = catalog.accounts.firstOrNull {
            it.id == postingAccountId && it.ledgerId == input.ledgerId
        } ?: return null
        if (!category.active || category.kind != kind ||
            parent.parentId != null || parent.kind != kind ||
            postingAccount.kind != when (kind) {
    ...
    fun matchesCategory(index: Int, categoryId: com.unifiedledger.domain.CategoryId, kind: CategoryKind): Boolean {
    ...
    val categoryBindingValid = when (val fields = input.decisionFields) {
        is ImportConfirmDecisionFields.OrdinaryFlow -> when (input.resolved.directionToken) {
            "out" -> selectedAccountMatches(fields.fundingAccountId, AccountKind.ASSET) &&
                matchesCategory(0, fields.categoryId, CategoryKind.EXPENSE)
            "in" -> selectedAccountMatches(fields.fundingAccountId, AccountKind.ASSET) &&
...
internal fun checkedImportFormalCommit(
    ...
    created: ImportFormalCommit,
    catalog: LedgerCatalog,
): DomainResult<ImportFormalCommit> = when (
