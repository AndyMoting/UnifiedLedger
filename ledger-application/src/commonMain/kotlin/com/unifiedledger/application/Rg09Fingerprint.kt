...
object Rg09LedgerFingerprint {
    fun project(
        ...
        val rows = formalTransactions.asSequence()
            .mapNotNull { record ->
    ...
    fun digest(projection: Rg09LedgerFingerprintProjection): String =
        "sha256:${Sha256.digestHex(projection.canonicalJson().encodeToByteArray())}"

    fun digest(formalTransactions: Iterable<Rg09FormalTransactionRecord>, targetAt: Instant): String =
        digest(project(formalTransactions, targetAt))
}

internal fun isRg09ShanghaiTimestamp(text: String, value: Instant): Boolean =
...
private fun formatDecimal(money: Money): String {
    ...
    val magnitude = if (money.minorUnits == Long.MIN_VALUE) {
        "9223372036854775808"
    } else {
...
private fun jcsString(value: String): String = buildString {
    append('"')
    var index = 0
