...
class ImportSpineUseCaseJvmTest {
    ...
    private val commitIds = ImportCommitIds(
    ...
    fun `T-26 intake assembles identity and snapshot and allocates ids only in the port callback`() {
        ...
        val port = RecordingIntakePort { identity, snapshot ->
            observed = ImportIntakeResult.Accepted(
    ...
    fun `T-26 reject wires the status id source and never touches the formal factory`() {
        ...
        val result = RejectImportCandidate(
            port,
            ImportStatusIdSource { statusCalls++; ImportStatusHistoryId("status-b-2") },
        ).execute(
