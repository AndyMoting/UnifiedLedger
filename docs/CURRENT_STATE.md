...
- SQLDelight schema 当前为 v12。v10 以 dedicated normalized owners 保存 RG-06 与 RG-07 operation、relation/lifecycle/installment、source/evidence/candidate 与 reconciliation；v11 以 additive nullable column 保存 RG-06 candidate confirmation provenance；v12 以 additive normalized owners 保存 D-082 批准的 RG-09 operation、source/evidence/candidate、adjustment/allocation、formal metadata 与 posting reconciliation，并继续复用共享正式账本表；迁移 verifier 与 JVM migration tests 已存在。
...
- `RG-08`、`RG-10` 已有冻结 v1、Python 测试与逐路径 mapping，尚无 Kotlin runtime；RG-09 已有 D-082 批准范围内的 domain/application/runtime、schema v12 persistence、50-operation oracle、D-065 proof 与 mapping closure candidate，仍待独立高风险审查。`RG-11`、`RG-12` 已有 approved direct-v2 fixtures 与 Python 语义测试，尚无 Kotlin runtime。
...
- SQLDelight `2.3.2` 与 Android system SQLite 已确定用于当前正式持久化边界；当前 schema 为 v12。UI、导航、依赖注入、同步和更广泛查询方案尚未选择。

## 未完成门槛

- `D-075` 只批准 RG-05 expected，不授权 adapter 实现或 fixture 迁移。
- RG-05 publication 仍未授权，当前没有 `golden/rules-v2` RG-05 工件。
- RG-05 完整状态 oracle、RG-06 publication 与 RG-07 closure 不表示全部黄金场景或正式账务核心已完成；RG-01、RG-02 和 RG-04 仍缺各自声明的完整比较，RG-08 至 RG-12 尚无 Kotlin runtime。
- 其他 RG 的 v1 rewrite、v2 publication 和未完成完整比较仍分别受其现有 gate 约束；RG-09 publication 仍未授权。
