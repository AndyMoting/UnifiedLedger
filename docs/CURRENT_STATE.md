...
- SQLDelight schema 当前为 v11。v10 以 dedicated normalized owners 保存 RG-06 与 RG-07 operation、relation/lifecycle/installment、source/evidence/candidate 与 reconciliation；v11 以 additive nullable column 保存 RG-06 candidate confirmation provenance，并继续复用共享正式账本表；迁移 verifier 与 JVM migration tests 已存在。
...
- SQLDelight `2.3.2` 与 Android system SQLite 已确定用于当前正式持久化边界；当前 schema 为 v11。UI、导航、依赖注入、同步和更广泛查询方案尚未选择。
