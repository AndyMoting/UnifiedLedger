...
- RG-08 无 v2 expected 工件且未发布（publication 不在当前授权范围）；RG-01、RG-02 亦未发布，其 v2 publication 仍未授权（完整比较已按 `D-087` 完成）。
- RG-05 完整状态 oracle、RG-06 publication 与 RG-07 closure 不表示全部黄金场景或正式账务核心已完成；RG-01、RG-02 的完整 state/delta/status 比较已实现（`D-087`，`f9da4b6`）但 v2 publication 未授权；RG-08 的 runtime 与 mapping gate 已按 `D-084` 完成（4 个 gap 已关闭），但仍未生成 v2 expected 工件。
