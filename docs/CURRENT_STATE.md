...
`ledger-domain` 当前 `26` 项 Kotlin 测试通过，`ledger-application` 当前 `6` 项 Kotlin 测试通过；完整 Python 测试 `379` 项及正式文档验证通过。当前实现只覆盖上述有限切片，不代表完整 `RG-01` 黄金契约或正式账务核心已经完成。应用层原子提交只是端口契约，尚无数据库、schema、ORM、并发或持久化实现；报表、对账、导入、UI 和平台客户端运行时也均尚未实现。
...
以测试优先方式实施应用边界的原始稀疏输入验证：在构造字段非空的领域命令之前，让缺少金额、付款账户或分类分别返回类型化输入失败，并映射到 `$.attempted_input.amount`、`$.attempted_input.payment_account_id`、`$.attempted_input.category_id` 与既有 `missing_required_field` 契约。该切片仍不选择数据库，不顺带冻结未知 ID、跨账本错误或用户界面文案。
