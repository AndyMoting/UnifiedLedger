...
- SQLDelight `2.3.2` 与 Android system SQLite 已确定用于当前正式持久化边界；当前 schema 为 v25。UI、导航、依赖注入、同步和更广泛查询方案尚未选择。
