...
`ledger-domain` 当前 `26` 项 Kotlin 测试通过，`ledger-application` 当前 `30` 项 Kotlin 测试通过，`ledger-data` 当前 `15` 项 Kotlin 测试通过；完整 Python 测试 `379` 项及正式文档验证通过。当前实现只覆盖上述有限切片，不代表完整 `RG-01` 黄金契约或正式账务核心已经完成。typed decoded-field adapter 已实现；raw JSON decoder、序列化方案和完整 state/report/reconciliation/delta comparison 尚未实现或授权，报表、对账、导入、UI 和平台客户端运行时也均尚未实现。
...
raw JSON decoder 与序列化库需要独立技术选择和明确授权；完整 state/report/reconciliation/delta comparison 也需要单独验收边界。后续产品切片不能从当前 typed adapter 推定其行为、依赖或实施授权。
