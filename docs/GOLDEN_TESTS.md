...
储值充值实付 `1000.00 CNY`、到账 `1200.00 CNY`，其中赠送权益为 `200.00 CNY`，形成储值资产 `+1200.00 CNY`、银行资产 `-1000.00 CNY` 和赠送权益收入 `-200.00 CNY` 的平衡交易。随后确认储值消费 `300.00 CNY`，只形成费用 `+300.00 CNY` 与储值资产 `-300.00 CNY`，不产生第二条现金流；批次按商户证据或最早到期日、载入时间、稳定批次 ID 的确定性顺序分配。提醒和未确认失效不产生交易，明确确认且有余额证据的到期才形成到期损失，历史批次和消费保持可追溯。
