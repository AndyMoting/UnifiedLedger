...
首批 12 个公开场景和 8 个本地场景均使用前两个正式级别。未来草案另行登记，不能伪装成已经确认的黄金答案。
...
机器可比较答案保存在 [`golden/rules/rg-01.json`](../golden/rules/rg-01.json)。该答案冻结手工创建、备注修改、必填字段失败和同次请求重试四类结果；完整设计见 [`RG-01 普通支出设计`](specs/2026-07-14-rg-01-normal-expense-design.md)。
...
资产账户 A 向资产账户 B 转账，到账金额小于转出金额。验证本金不进入收支统计、手续费独立进入费用、两端分录与费用共同平衡。
...
| `RL-04` | `core_required` | 跨账户资产互转；组合展示、分账户余额和对账证据 |
