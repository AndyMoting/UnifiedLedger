...
状态：approved（2026-09-24 起草；本文已经**两份独立评审**——独立规格评审与独立质量评审，结论均为 APPROVE WITH FINDINGS——并经 **distinct verifier** 对全部承重事实 VERIFIED，全部 findings 已闭合；本修订 draft-3 应用**收口评审**的全部 findings（NEW-1、NEW-2、P3-7、P3-8 及各 nit，见下 Revision）。本文由 `docs/DECISIONS.md` D-174 批准（D-174 即批准依据）。按 `docs/CONTRIBUTING.md:162` 的允许分类标 `approved`。本文冻结 Q13 容器格式字节定义与 Q14 世代/指针设计级方案，并登记 06.0 匿名技术门结果；实施、Git 写操作与最终验收属后续实施批）。

**Revision:** draft-4（2026-09-24）。相对 draft-3 是一次**已批准规格上的事实门状态更新**：§6 两条 OPEN 门项经新证据闭合——(a) **错密码跨端实证**（桌面 JDK 21 `SunJCE` 与 Android 16/API 36 `AndroidOpenSSL` 均得同一 `AEADBadTagException`）；(b) **非 96-bit IV / 非 128-bit tag 的两端拒绝行为**（探针发现**平台分歧**：`SunJCE` 拒绝 tag 64/32、`AndroidOpenSSL` 接受，两端均接受 8 字节 IV）。该分歧**正是** §4.3.1/§4.5 在**格式层**强制 `iv_len == 12`、`tag_len == 16` 而非依赖 provider 拒绝的理由——依赖 provider 会使同一容器在两端行为不同；此发现**支持**已冻结设计，**不修改**它。同步更新 §4.1 的错密码措辞、§6 结论与 §9 的未验证清单。本文仍为 `approved`（依据 `docs/DECISIONS.md` D-174，状态 token 不变）；本次不改动任何冻结字节布局、§4.4 代码块或参数值。draft-3（2026-09-24）相对 draft-2 应用**收口评审** findings：§4.7 结构检查按时机拆为认证前/认证后三类（修正 §4.3.2/§5.4 被误列为解密前短路的矛盾）、§9 状态与头部一致化（两份独立评审与 distinct verifier 已发生，本文现为 approved，依据 D-174）、§4.6/§6 登记 §4.6 AAD 构造尚无跨端向量（OPEN）、§5.3 补 Q14 无法可靠实现时的退回/审批义务、§4.3.1 一行拆为逐条映射、§6 相邻门项重标、§4.8 64 MiB 余量措辞自洽化。draft-2 相对 draft-1 应用评审 findings：冻结密码→`PBEKeySpec` 的字节加宽构造（§4.4）、补齐固定头部类型化拒绝（§4.3）、区分解密前结构检查与统一密码学失败（§4.7/§4.8）、限定错密码实证范围（§4.1/§6）、补恢复侧磁盘预算（§4.8）、登记 OS 自动备份门与新增 OPEN 项（§6）、明确 `db_schema_version` 为非权威提示（§4.3/§5.4）、冻结重启规则为回滚（§5.3）等。draft-1 事实：工作基线 `main` = `5ea7f67`（P7-05 lost-commit 人工复核面，D-173），schema **v31**，迁移链 `1.sqm`～`30.sqm`（30 个文件，v1→v31）。tracked 行号为该基线实读行号；`.local.md` 与 `local/artifacts/` 以主 checkout 为准、只读。示例与向量全部匿名合成；引用不粘贴大段产品代码，不写本机绝对路径。本文**不**新增决定条目、**不**修改任何既有决定。
...
`AndroidStartupController`（`App.kt:217`）与桌面 `DesktopStartupController`（`Main.kt:239`；其调用的 `openDesktopLedger` 见 `Main.kt:759`）都只有「打开或失败」两态，没有跨「导出/恢复」的租约与代际。因此本规格的 Q13（容器格式）可以在 06.0/06.A 内冻结，而 Q14 的**切换机制**只能给设计级方案，落地属 06.1/06.D（§5、§6.3）。
...
- 任何既有决定的修改；Q13/Q14 的最终批准（本文已由 `docs/DECISIONS.md` D-174 批准，批准前经两份独立评审与 distinct verifier）。
...
| 15 | 4 | `db_schema_version` | `u32`；payload 快照的 `PRAGMA user_version`（当前 v31）。**非权威提示**（见下 §4.3.2） |
...
| `salt_len ∉ 16..32` | `P706_KDF_PARAMETERS_UNSUPPORTED` | KDF 参数越界（§4.4） |
| `iv_len != 12` | `P706_AEAD_PARAMETERS_UNSUPPORTED` | AEAD 参数越界（§4.5） |
| `tag_len != 16` | `P706_AEAD_PARAMETERS_UNSUPPORTED` | AEAD 参数越界（§4.5） |
...
- **AAD 向量的现状（不得过度声称）**：§6 门表中引用的 AAD 字符串（`ULBK1|schema=31|kdf=PBKDF2-HMAC-SHA256|iters=600000` 与 `ULBK1-v1-aad-test`）是既有跨端 crypto 向量所用的**测试替身（stand-in）**，**不是**本节冻结的 `header(0..58) || salt` 构造。因此**本节构造本身尚无跨端向量**；`ULBK1-v1-aad-test` 只证明「相同 AAD 字节在两端给出相同 ciphertext+tag」，不证明 §4.6 的拼接规则已被跨端验证。该缺口登记为 §6 的一条 **OPEN** 门项。
...
- **结构检查不属于密码学失败，但必须按其发生时机分三类**。本节的统一失败条款**只**适用于密码学失败；结构拒绝各自给出类型化拒绝码。三类时机互不重叠，且**类 2/类 3 绝不是解密前短路**：

  1. **认证前、公开格式检查（可在任何解密之前短路）**：只读 59 字节固定头部与容器大小即可判定，不依赖密码、不依赖解密，故允许提前短路。范围 = §4.3.1 的全部固定头部拒绝行（`magic`、`container_format_version`、`kdf_id`、`aead_id`、`salt_len`/`iv_len`/`tag_len`、`kdf_iterations`）**加** §4.8 的三项认证前检查（「容器大小 vs `plaintext_len` 自洽」、容器输入上限 `P706_CONTAINER_TOO_LARGE`、明文 `plaintext_len` 上限）。这些检查**不**泄露密码相关信息，**不是**密码 oracle。
  2. **认证后、payload 结构检查（只在标签校验通过之后）**：§4.3.2 对 payload 快照实际执行 `PRAGMA user_version` 并据以给出的 `P706_SCHEMA_VERSION_UNSUPPORTED` 结构拒绝。**该检查必须先解密并打开 payload 才能进行，因此不是认证前的短路**，绝不得在任何解密或 payload 打开之前运行（§4.3.2、§4.9）。
  3. **认证后、身份/账本检查**：§5.4 的身份/账本结构拒绝（ledgerId 固定身份、库中无额外账本）。同属**认证之后**，因为「库中无额外账本」只能通过打开 payload 数据库判定；**不是认证前的短路**，绝不得在认证完成之前运行（§4.9）。

  **类 2 与类 3 绝不允许在认证完成前运行**——它们要求解密并打开 payload，若被当作「解密前短路」会违反 §4.3.2「发生在认证成功之后」与 §4.9「认证完成前绝不打开或迁移数据库」，并把未认证的 payload 内容暴露为可观察差异。此前把「§4.3.1 / §4.3.2 / §4.8 / §5.4 全部」笼统归入「在认证之外且允许提前短路」属表述过宽：只有类 1 可在认证前短路，类 2/类 3 必须等待认证。此外，此前把「头部字段与密文不一致」笼统列入统一失败亦属过宽：头部与密文的大小自洽矛盾由类 1（§4.8）提前拒绝，而受认证的头部内容篡改仍由 GCM 标签失败覆盖（归 `P706_CONTAINER_AUTHENTICATION_FAILED`）。
...
即恢复峰值 ≈ `container + 3 × snapshot + old_generation + 64 MiB`（隔离副本与新代 DB 各自接近快照大小）。**64 MiB 余量相对账本规模很小但并非字面可忽略**（相对 354 MB 级快照不足 20%），故不得把它当作主要余量来源；真正的余量来自「不得在写满磁盘后留下成功标记」的 fail-closed 检查与可回退的旧代保留（§5.5）。
...
- **无法可靠实现时的退回规则（计划 `:146` 的义务）**：若「小指针 + 持久 journal」方案在目标平台**不能可靠实现**（例如平台缺少可证明原子的替换原语或崩溃顺序无法实测），则本设计**必须退回重开**，另行**明确审批**一套持久切换协议，再据此实施；**不得**混搭两种半实现，**不得**静默替换为未经审批的替代协议。即：本节已批准的是**设计级方案**（非实施授权），不是「实现者可就地自由选择」的开放项；落地实现仍属 06.1/06.D。
...
| 错密码跨端实证：错密码 → 统一 FAIL（`AEADBadTagException`），两端一致 | **CLOSED** | `local/artifacts/p7-06-gate/q13-crypto-official-evidence.md`（追加的「错密码跨端实证（已补 Android 侧，闭合该 OPEN 门）」段）：同一容器（右密码 `right` 加密）以错密码 `wrong` 解密，桌面 JDK 21（`SunJCE`）与 Android 16/API 36（`AndroidOpenSSL`）**均得同一 `AEADBadTagException`**，与 RFC 5116 §2.2 统一 FAIL 语义一致。此前仅桌面侧已实证，Android 侧已补，故由 OPEN 转 **CLOSED** |
| 统一 FAIL 的官方标准依据，及官方参数依据（AES-GCM 96-bit IV / 128-bit tag；PBKDF2-HMAC-SHA256 ≥ 600,000 次；salt ≥ 128 bit；AAD 先供给） | **CLOSED** | 同上（NIST SP 800-38D、NIST SP 800-132、OWASP、RFC 5116/8018）。**此行为「统一 FAIL 有标准依据」+「官方参数依据」，与上一行「错密码跨端的实证」是两项独立主张** |
...
| **§4.6 AAD 构造（`header(0..58) || salt`）的跨端向量** | **OPEN** | 既有跨端向量使用的 AAD 字符串（`ULBK1|schema=31|…`、`ULBK1-v1-aad-test`）是**测试替身**，不是 §4.6 冻结的拼接构造；**该构造本身尚无跨端向量**。本规格**不**声称 §4.6 的 AAD 已跨端验证（§4.6） |
| **非 96-bit IV / 非 128-bit tag 的两端拒绝行为** | **CLOSED（结论为平台分歧，见右）** | `local/artifacts/p7-06-gate/q13-crypto-official-evidence.md`（追加的「非 96-bit IV / 非 128-bit tag 的两端拒绝行为（已实证，闭合该 OPEN 门）」段）。探针（32 字节零密钥、`AES/GCM/NoPadding`）实测：tag=128/112/96 **两端均 ACCEPTED**；tag=64/32 **桌面 JDK 21（`SunJCE`）REJECTED**（`InvalidAlgorithmParameterException`）而 **Android 16/API 36（`AndroidOpenSSL`）ACCEPTED**；IV=8 字节 **两端均 ACCEPTED**。即**两端对非标准 tag 长度的接受行为不一致**，且**两端都接受非 96-bit IV**。**设计后果（明确）**：这正是 §4.3.1/§4.5 在**格式层**强制 `iv_len == 12`、`tag_len == 16`（否则类型化拒绝 `P706_AEAD_PARAMETERS_UNSUPPORTED`）而非依赖 provider 拒绝的理由——若依赖 provider，同一容器在 Android 会被接受、在桌面会被拒绝，破坏跨端一致性。该实证**支持**已冻结设计，**不修改**它 |
...
**结论（如实）**：Q13 的**格式字节定义**（§4）与**两端 driver 快照可达性**、**跨端 crypto 字节兼容（ASCII 向量与非 ASCII 密码向量均实证）**、**错密码跨端统一 FAIL**、**官方参数依据**已具备冻结条件；**非 96-bit IV / 非 128-bit tag 的两端拒绝行为**亦已实证闭合，其实测结论为**平台分歧**（桌面拒绝非标准 tag 长度、Android 接受；两端均接受非 96-bit IV），该分歧**正是** §4.3.1/§4.5 采用**格式层强制** `iv_len == 12`/`tag_len == 16` 而非依赖 provider 的理由，**支持**而非修改已冻结设计。但下列项**仍未闭合**，故本规格给出格式定义的同时，把它们列为 06.A 最终冻结前的待补证据（计划 `:460`）：**§4.6 AAD 构造的跨端向量**、**大库峰值内存**、**桌面侧 61k 资源读数**、**OS 自动备份/设备迁移门**、**旧 schema 迁移门**。计划 `:460` 要求 Q13 实施规格冻结前必须补齐「现用 driver 快照实证；官方加密/KDF 依据、精确格式/参数/资源上限及双端验证」——其中「精确格式/参数」由本文给出，「资源上限」的**实测读数**与 §4.6 AAD 构造的跨端验证仍待补。
...
- 本文状态为 **approved**（`docs/CONTRIBUTING.md:162` 允许分类之一），批准依据为 `docs/DECISIONS.md` D-174。本文已经**两份独立评审**（独立规格评审与独立质量评审，结论均为 APPROVE WITH FINDINGS）与 **distinct verifier**（全部承重事实 VERIFIED），全部 findings 已闭合；draft-3 已应用**收口评审**的 findings，本修订 draft-4 为**已批准规格上的事实门状态更新**（§6 两条门项 OPEN→CLOSED，见 Revision；不改动状态 token、冻结字节布局或参数值）。本文已由主代理按用户既有授权批准并冻结 Q13/Q14 的设计级裁决（沿 D-156 规格先例 `docs/DECISIONS.md:2947`）；产品代码实施、Git 写操作与最终验收仍属后续实施批。
- 每项事实主张均带 file:line 或公开 URL 证据；本地证据以主 checkout 的 `local/artifacts/p7-06-gate/` 两文件为准（只读，不粘贴大段原文）。
- **明确标记为未验证/未取读数**的项：61k 峰值内存（设备侧与桌面侧均未取）、桌面侧 61k `VACUUM INTO` 资源读数、**§4.6 AAD 构造（`header(0..58) || salt`）的跨端向量**（既有向量用的是测试替身 AAD）、OS 自动备份/设备迁移门、旧 schema 迁移门、指针切换/重启恢复、06.D 完整机制、两端稳定存储（§6）。**非 ASCII（CJK）密码的跨端向量**、**错密码跨端统一 FAIL**、**非 96-bit IV / 非 128-bit tag 的两端拒绝行为**原列于此，现已实证闭合（§4.4、§4.1、§6），故不再属未验证项；其中 IV/tag 探针的实测结论是**平台分歧**，由 §4.3.1/§4.5 的格式层强制覆盖（§6）。
