...
- **结构检查不属于密码学失败，但必须按其发生时机分三类**。本节的统一失败条款**只**适用于密码学失败；结构拒绝各自给出类型化拒绝码。三类时机互不重叠，且**类 2/类 3 绝不是解密前短路**：
  ...
  1. **认证前、公开格式检查（可在任何解密之前短路）**：只读 59 字节固定头部与容器大小即可判定，不依赖密码、不依赖解密，故允许提前短路。范围 = §4.3.1 的全部固定头部拒绝行（`magic`、`container_format_version`、`kdf_id`、`aead_id`、`salt_len`/`iv_len`/`tag_len`、`kdf_iterations`）**加** §4.8 的「容器大小 vs `plaintext_len` 自洽」检查。这些检查**不**泄露密码相关信息，**不是**密码 oracle。
