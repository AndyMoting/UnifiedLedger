...
- 任何既有决定的修改；Q13/Q14 的最终批准（本文仅为 proposal，已经两份独立规格评审，收口后由主代理裁决）。
...
- **无法可靠实现时的退回规则（计划 `:146` 的义务）**：若「小指针 + 持久 journal」方案在目标平台**不能可靠实现**（例如平台缺少可证明原子的替换原语或崩溃顺序无法实测），则本设计**必须退回重开**，另行**明确审批**一套持久切换协议，再据此实施；**不得**混搭两种半实现，**不得**静默替换为未经审批的替代协议。即：本节的方案是 proposal 级设计，不是「实现者可就地自由选择」的开放项。
...
- 本文状态为 **proposal**（`docs/CONTRIBUTING.md:162` 允许分类之一），**不是 approved**。本文已经**两份独立规格评审**（结论均为 APPROVE WITH FINDINGS），本修订 draft-3 应用**收口评审**的 findings；但**仍为 proposal**，**不构成实施授权**，不冻结 Q13/Q14 的最终裁决。收口评审经主代理裁决并由主代理按流程批准后，本文方可转 `approved`（沿 D-156 规格先例 `docs/DECISIONS.md:2947`）。
