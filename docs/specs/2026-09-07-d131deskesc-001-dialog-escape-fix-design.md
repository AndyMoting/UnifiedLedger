...
**状态：** approved — 本文件为缺陷 D131DESKESC-001 的 R-1 规格增量修复批（D-132 先例）**已批准规格**。修复路线（人工门规程路线 (a)：共享状态驱动 back 禁用 + 确定性对话框侧 Esc 关闭）已经用户批准（2026-09-07「批准」指令，授权整批）；独立规格评审已于 2026-09-07 出具 **APPROVE-with-notes**（无 P0/P1；P2-1 冻结登记措辞并入本翻转、P3-1..P3-4 处置见 docs/DECISIONS.md D-137）；批次决策与冻结哈希登记见 `docs/DECISIONS.md` **D-137**。批准后按既有实施路由执行（独立 worktree、单一 bounded writer、独立评审、distinct verifier、主代理最终验收），并重跑桌面 Esc 人工门复门（§5.3）。§3/§4 为冻结构造条款，与评审时逐字节一致（除本状态行与 §9 登记措辞的批准翻转增量外）。
...
1. 独立规格评审（2026-09-07 已出具 APPROVE-with-notes，无 P0/P1；P2-1 冻结登记措辞并入本翻转，P3-1..P3-4 处置登记于 `docs/DECISIONS.md` D-137）→ 本文件状态行翻转 `approved`（本翻转完成）→ 主代理按常设授权批准并冻结：`docs/DECISIONS.md` **D-137**（内容 = 缺陷 FAIL 证据 + 机制结论 + 修复路线 (a) + 本规格 §3/§4 冻结条款 + 范围 + 验证/复门计划 + 关闭判据 + 冻结 SHA-256 登记；哈希按既有先例（D-116/D-122）登记于 DECISIONS 条目，不在本文件内自引用，避免文件字节变动导致哈希失效）。
