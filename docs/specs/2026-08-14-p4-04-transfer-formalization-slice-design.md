...
- 外部锚点：CORE_ACCEPTANCE_PLAN（.external/requirements/golden-ledger/，只读）RL-03 行 `GL-A3CB7F3D48BC`「微信零钱转入零钱通」= 验证两条资产分录平衡且不进入对外收支。本规格以该 report 语义为转账确认 oracle；零钱通/零钱理财类交易类型不在本批冻结类型集合内（§11 第 6 项）。
...
本批合成 ID 常量（轮 B 的确定性测试 IdSource 依序产出并钉死）：source-t1/evidence-t1/candidate-t1（C1）、source-t2/evidence-t2/candidate-t2（C2）、source-t3/evidence-t3/candidate-t3（C3）、source-t4/evidence-t4/candidate-t4（C4）、source-t6/evidence-t6/candidate-t6（C6）、source-t10/evidence-t10/candidate-t10（C10）；status-t1-1/status-t1-2（C1）、status-t2-1/status-t2-2（C2）、status-t3-1/status-t3-2（C3）、status-t4-1/status-t4-2（C4）、status-t6-1（C6）；confirmation-t1/tx-t1（E-10）与 confirmation-t2/tx-t2（E-15）；失败注入、领域失败与 binding mismatch 的废弃批次（attempt-1 系列，零持久化痕迹）在 E-21~E-25/E-31/E-32/E-34/E-36/E-37 中钉死；E-34 的 setup C1d 使用 batch-p404-c / ordinal 2 与 source-t1d/evidence-t1d/candidate-t1d，十三个恶意 formal 子向量分别使用隔离的 `attempt-binding-b01-*`…`attempt-binding-b13-*` ID 命名空间；E-36 使用 `attempt-scale-s01-*`…`attempt-scale-s09-*`，E-37 使用 `attempt-id-i01-*`…`attempt-id-i07-*`（完整 ID 清单分别随 T-53/T-54 case table 钉死，失败向量任何 ID 均不得持久化）；posting/version 常量随 E-10/E-15 断言钉死。

### 1.3 操作集（P-01…P-12 解析级；E-01…E-37、E-40…E-41 spine 端到端/迁移；R-01…R-03 回归）

`Δ` 表示该操作自身的新增行数（request/source/evidence/candidate/status_history/decision_snapshot/confirmation/receipt 与 formal transaction/version/posting）。`零写入` 表示该操作不留下任何新行（包括 request claim 行）。`零 record` = 该行不产生 normalized record。诊断按冻结集合（multiset）比较，message 不比较（D-097:1459）。transfer 确认的 formal 断言固定为：transaction+1、version+1、posting+2 逐币种平衡，且报表语义 internal transfer = 金额、net worth change = 0、零普通收入/费用/消费/预算/分类效应与零对外现金流流入/流出（docs/ACCOUNTING_RULES.md:227、CORE_ACCEPTANCE_PLAN §3/RL-03）。
...
- E-15 confirm C2 @ req-t2-confirm，TransferFlow(from=account-bank-a, to=account-wallet-wechat)，explicitConfirmedAt=2026-08-14T11:00:00+08:00 → accepted（direction in → 钱包为 TO 腿）。formal：account-bank-a −200.00；account-wallet-wechat +200.00；报表同 E-10 完整形状（internalTransferMinor=20000，其余八个 report 维度全零）。returned：[{confirmation,confirmation-t2},{transaction,tx-t2}]。
...
- E-36 source-scale→账户币种精确归一化（九个独立子向量 S01…S09；目标 from/to 账户均为 CNY/2，每项从独立 pending transfer candidate 执行）：(S01 `scale_0_up`) source=(100,CNY,0) → 10000 CNY/2 accepted；(S02 `scale_1_up`) source=(1000,CNY,1) → 10000 accepted；(S03 `scale_2_equal`) source=(10000,CNY,2) → 10000 accepted；(S04 `scale_3_exact_down`) source=(100000,CNY,3) → 10000 accepted；(S05 `scale_3_remainder`) source=(100001,CNY,3) → rejected（SPINE_DOMAIN_VALIDATION_FAILED，`AmountNotRepresentableInCurrency`）；(S06 `scale_19_remainder`) source=(1,CNY,19) → 同码 rejected，且不得先构造 `CurrencyUnit(CNY,19)`；(S07 `scale_up_overflow`) source=(Long.MAX_VALUE,CNY,0) → rejected（SPINE_DOMAIN_VALIDATION_FAILED，`ArithmeticOverflow`）；(S08 `negative_scale`) source=(10000,CNY,-1) → rejected（SPINE_DOMAIN_VALIDATION_FAILED，`AmountNotRepresentableInCurrency`，§4.3 分支 (a)）；(S09 `scale_gap_over_18`) source=(1,CNY,21) → 同码 rejected（§4.3 分支 (c) 差值 19 > 18，非零 amountMinor 直接判定不可整除，禁止构造溢出的 10^19 power-of-ten）。S01…S04 的 formal/report/canonical shape 与 E-10 相同但本金为换算后的 10000；S05…S09 均回滚 claim、candidate 保持 pending、request identity 可用于修正重试、14 表 delta 全空，attempt IDs 无持久化痕迹。所有换算禁止舍入或二进制浮点。
- E-37 allocated ID binding mismatch（七个独立子向量 I01…I07，共用与 E-34 相同的 canonical input，并由 commit port 记录本次 `allocateIds` 的冻结返回值）：测试专用 factory 收到正确 allocated IDs 后，仅把 created 中一项替换为另一合成 ID，其余经济字段与 graph 均 canonical：(I01 `wrong_confirmation_id`)、(I02 `wrong_status_history_id`)、(I03 `wrong_transaction_id`)、(I04 `wrong_version_id`)、(I05 `wrong_posting_set_id`)、(I06 `wrong_source_posting_id`)、(I07 `wrong_destination_posting_id`)。`validateImportFormalBinding(input, allocatedIds, created)` 必须在任何 formal/decision/status/confirmation INSERT 前逐项拒绝（SPINE_REFERENCE_INTEGRITY_VIOLATION），每项 14 表 delta 全空、candidate 仍 pending、identity 可重试；不得因 callback 返回内部自洽 graph 就接受任意新 ID。
...
- R-03 P4-03 P 系列逐值不变，唯两处解析级冻结修订：(1) W7/P-07 由「拒行 SPINE_WEIXIN_UNSUPPORTED_TX_TYPE」改为「接受 transfer v2 record（本规格 P-12）」；(2) P-14 记录数明确由 8 变 9，诊断 multiset 由 9 条变 8 条（删除 W7 的 UNSUPPORTED_TX_TYPE，新增 W7 record 不新增诊断）。加上 R-02 的 E-12 full-batch intake delta，共计恰好三处跨 P4-03 冻结修订。该修订是 D-099:1539 已登记的 P4-04 类型转移（§11 第 1 项），其余 P 系列断言逐值不变。

### 1.4 Complete canonical state oracle（每个 E 操作必检）

每个 E-01…E-37、E-40…E-41 都冻结一个独立 pre-state 和 complete expected post-state；复合操作另在 setup、失败、retry 每个可观察边界冻结 checkpoint。expected 只能由 §1 fixtures/操作合同在测试侧独立构造，禁止从被测数据库读回后再生成 expected，也禁止只比较 count、delta 或选定字段。
...
- 状态接受子集最小扩展（冻结修订，§11 第 2 项）：`SETTLED_STATUS_TOKENS = {支付成功, 已存入零钱, 已到账, 提现已到账}`——仅新增 `提现已到账`（W7/T1 行为证据形状，P4-03 规格 §1.2）；其余状态 token 维持 raw + unresolved。
...
- `validateImportFormalBinding` 位于 ledger-application commonMain，是 `persistFormal` 前的纯 contract validator。TransferFlow 分支必须遍历并校验 callback 返回的完整 graph，而非只读取 current postings：created.confirmationId == allocatedIds.confirmationId、created.statusHistoryId == allocatedIds.statusHistoryId；transaction.id == allocatedIds.formalIds.transactionId、ledgerId=input.ledgerId、kind=ACCOUNT_TRANSFER；`versions.size == 1`、`postingSets.size == 1`，transaction.currentVersionId 指向 sole version，sole version.id == allocatedIds.formalIds.versionId、transactionId 指向该 transaction、versionNumber=1、postingSetId == allocatedIds.formalIds.postingSetId 且指向 sole posting set；allocatedIds.formalIds.postingIds 必须恰好两项，sole posting set 恰好两条 posting，index 0/1 的 posting.id 分别等于 allocated postingIds[0]/[1]，并分别为 (decision.fromAccountId, -normalized target minor, target CurrencyUnit) 与 (decision.toAccountId, +normalized target minor, 同币种/精度)；sole version 的 occurredAt/statisticsAt/effectiveAt 分别且全部等于 persisted source 的 `resolved.occurredAt`，note=NULL；无额外 version、posting set 或 posting。数据库专属且 callback 不可控的 `ledger_transaction.canonical_kind` 与 `transaction_version.confirmation_id` 继续由 `persistFormal` 固定写为 NULL，并由 §1.4 oracle 验证。上述任一不符必须在首次 formal INSERT 前复用 SPINE_REFERENCE_INTEGRITY_VIOLATION 并回滚 claim，保证 14 表 delta 全空；OrdinaryFlow 维持 P4-02 经济路径，但同样必须绑定其 allocated IDs——该普通路径义务不设本批新向量，其证明依赖 R-01（P4-02 30-op oracle 的冻结 receipt 与 returned IDs 逐值不变）；E-34 B01…B13、E-37 I01…I07、T-44 与 T-54 的 binding 不变量向量只针对 transfer 路径，对相应不变量逐项证明 fail-closed（T-48 的普通/端口级断言不受此收窄影响）。
...
### 4.3 生产 transfer 工厂（ledger-application commonMain；既有转账原语行为不变）
...
- factory 首先要求 `input.decisionFields is TransferFlow`，再从该值局部取得 fromAccountId/toAccountId；构造器不存在 legs 或 IDs，禁止从其他字段或配置重新推导。方向门先于领域原语：`input.resolved.directionToken == "out" ⇒ fromAccountId == walletAccountId`；`"in" ⇒ toAccountId == walletAccountId`；否则返回 DomainResult.Failure。冻结 failure 载体仍为 `DomainViolation.InvalidOrdinaryIncome`（沿用现有 spine E2E factory 对应用层不可解析确认输入的既有映射先例 ImportSpineWechatEndToEndTest.kt:293；spine 不比较 violation 内容——P4-02 规格 §8、SqlDelightImportSpineStore.kt:278-285）。该名称与转账语义不匹配，只用于方向门兼容：轮 B 实现必须在返回点写明这一原因和 §11 第 7 项技术债引用，禁止把它解释为 ordinary income 行为或作为未来转账先例。
- 账户币种与 source scale：factory 在创建 `Money` 前从 catalog 分别取得 from/to 账户，先验证两端属于 input.ledgerId、均为 self-owned real asset，且两端 `CurrencyUnit`（code + precision）完整相等；随后要求 `input.resolved.currencyCode == targetCurrency.code`。`input.resolved.currencyPrecision` 只是 source decimal scale，不得传入 `CurrencyUnit` 构造器。`normalizeSourceMinorExact(amountMinor, sourceScale, targetCurrency.precision)` 的冻结算法为：(a) sourceScale < 0 ⇒ `AmountNotRepresentableInCurrency`；(b) targetPrecision >= sourceScale ⇒ 以 checked power-of-ten 乘法升 scale，任何中间或最终 Long 溢出 ⇒ `ArithmeticOverflow`；(c) targetPrecision < sourceScale ⇒ 只在 amountMinor 可被 10^(sourceScale-targetPrecision) 整除时精确除法，否则 `AmountNotRepresentableInCurrency`；当差值 > 18 时，对非零 Long 可直接判定不可整除，禁止构造溢出的 power-of-ten；(d) 全程禁止舍入、Double/Float 与 locale 格式化。`AmountNotRepresentableInCurrency` 是本批唯一允许新增的通用 DomainViolation，不改变任何会计原语。
- 正式效果：以 targetCurrency 与 normalizedTargetMinor 构造 `Money.ofMinor`，再调用 `createOwnAssetPrincipalTransfer(catalog, OwnAssetPrincipalTransferCommand(input.ledgerId, fromAccountId, toAccountId, normalizedMoney, TransactionTimes.collapsed(Instant.parse(input.resolved.occurredAt))), OwnAssetPrincipalTransferIds(transactionId=ids.formalIds.transactionId, versionId=ids.formalIds.versionId, postingSetId=ids.formalIds.postingSetId, sourcePostingId=ids.formalIds.postingIds[0], destinationPostingId=ids.formalIds.postingIds[1]))`——唯一转账原语。唯一 ledger 来源是 input.ledgerId（即 request identity）；唯一 leg 来源是 input.decisionFields；唯一 formal ID 来源是本次 allocatedIds。OwnAssetPrincipalTransfer.kt:40-128 产生两条资产分录 PRINCIPAL_OUT/PRINCIPAL_IN、internalTransferMinor=归一化金额、netWorthChangeMinor=0、kind ACCOUNT_TRANSFER；统计时间 = 来源支付时间（docs/ACCOUNTING_RULES.md:202-204），confirmed_at 独立于时间字段。
- leg 有效性（同账户 DistinctAccountsRequired、非资产/非自有/非真实 OwnedRealAssetRequired、未知账户 KnownAccountRequired、币种不符 SameCurrencyRequired、非正金额 AmountMustBePositive）由 factory 前置检查或原语的 DomainResult.Failure 承载；source amount 不可精确表示与算术溢出分别由上述两个 violation 承载。全部映射为 SPINE_DOMAIN_VALIDATION_FAILED 并原子回滚。
...
- `ledger-domain` 最小改动（与 §4.3、§7 最小改动声明、§9、轮 B 建议文件布局及 D-100 条款 6 一致）：正式效果复用既有 `createOwnAssetPrincipalTransfer`（OwnAssetPrincipalTransfer.kt:40-128，该原语及会计行为零改动），仅允许在通用 `DomainViolation` 新增 `AmountNotRepresentableInCurrency`。
...
6. 不可变/追加-only 与状态迁移触发器重建（20.sqm:195-227 同形状：上述八张重建表各自的 BEFORE UPDATE/DELETE ABORT 守卫 + `import_candidate_status_history` 的 sequence/transition BEFORE INSERT 守卫）。E-40/T-46 必须逐表、逐类断言，不能以触发器总数替代。
...
- ledger-domain 最小改动声明：本批正式效果完全由既有 `createOwnAssetPrincipalTransfer` 产生，该原语及会计行为零改动；只允许在通用 `DomainViolation` 增加 `AmountNotRepresentableInCurrency`，不得借此修改 transfer kind、posting、report 或账户规则。
...
- T-22…T-40 一一对应 E-10…E-28：逐操作断言 transfer confirm 的 decision_snapshot 四列逐值（category/funding NULL、from/to 非空）、formal 断言（transaction+1/version+1/posting+2 逐币种平衡、posting 账户与金额精确对应 −100.00/+100.00、−200.00/+200.00）、报表断言（internalTransfer=金额，netWorthChange、普通收入/费用/消费/预算/分类效应、对外现金流流入/流出全部为零——docs/ACCOUNTING_RULES.md:227 与 CORE_ACCEPTANCE_PLAN §3/RL-03）、replay/conflict/stale/NOT_PENDING（E-11~E-14）、NOT_CONFIRMABLE（E-16/E-17）、缺腿 reject（E-18/E-19）、incomplete 拒认（E-20）、方向门独立失败向量（E-21）、实际到达原语的四个领域失败向量与修正重试（E-22~E-26，每变体含 claim 回滚、候选仍 pending、identity 可用、attempt 批次钉死）、kind 失配（E-27/E-28）。每项同时执行 §1.4 complete canonical state 比较。
...
- T-48 应用层：ExecuteImportIntake 的 ledger 不一致 require 拒绝（P4-02 先例）；decision snapshot 不再含 ledgerId；commitOnce 只在全部前置校验后调用 allocateIds 恰好一次并把同一实例传给 factory/validator；TransferFlowFormalFactory 方向门单元向量（out/in/未知方向）；`validateImportFormalBinding` 的 table-driven 单元测试必须与 T-44 共用同一十三项 ID/名称 manifest，逐项覆盖 reversed legs、wrong ledger/kind/amount/currency/precision、extra posting、multiple versions + current version_number=2、extra posting set、wrong occurredAt/statisticsAt/effectiveAt、non-null note，并另有 canonical graph 通过向量。每个失败向量断言 validator 在 persistence boundary 前返回同一 binding failure，不得只靠 E-34 端到端测试间接覆盖。
...
- T-53 exact scale normalization（E-36）：case manifest 必须恰为 S01…S09；逐项断言目标账户 `CurrencyUnit` 来源于 catalog、source scale 不构造 CurrencyUnit、0/1/2/3/19/21 与负 scale 的精确换算或 typed rejection、差值 > 18 捷径不构造 10^19、checked multiplication overflow、无舍入/无浮点，以及 accepted/rejected 的 §1.4 complete canonical state。
- T-54 allocated ID binding（E-37）：case manifest 必须恰为 I01…I07；逐项断言实际 allocated confirmation/status/transaction/version/posting-set/source-posting/destination-posting ID 与 created/persisted 值相等，任一替换都在首次 INSERT 前零残留拒绝；另断言 allocated postingIds.size != 2 在 factory 前以 SPINE_REFERENCE_INTEGRITY_VIOLATION 拒绝且 factory 零调用。

T-13…T-47、T-53、T-54 除各自专项断言外全部调用同一 §1.4 canonical comparator；T-50…T-52 继续运行冻结前序 oracle，防止 canonical helper 只覆盖新路径。测试 manifest 必须精确覆盖 P-01…P-12、E-01…E-37、E-40…E-41、R-01…R-03，无遗漏或重复 ID。
...
- 不含新接受类型：红包类 fail-closed（未分配批次）、退款类 fail-closed（→ P4-06）、未知 token fail-closed；零钱通/零钱理财类 token 不在冻结集合 → SPINE_WEIXIN_UNKNOWN_TOKEN（RL-03 anchor 的 report 语义由提现/充值 fixture 验证，§11 第 6 项）。
...
- schema 变更仅限 §7 范围；golden 冻结契约与 `.external/` 零改动；竖井表零改动（仅作 jvmTest 冻结语料）；ledger-domain 仅允许 §4.3 的一个通用 violation token，既有会计原语零改动。
...
- `P404-QUAL-001` BLOCKER disposition：§1.3 E-36、§4.3、T-53 冻结 source scale 与账户 CurrencyUnit 的精确换算边界；source precision 不再直接构造 CurrencyUnit，S01–S04 覆盖 scale 0/1/2/3 的精确换算，S05/S06 覆盖 scale 3 与 19 的余数拒绝，S07 覆盖 Long.MAX_VALUE 升 scale 溢出拒绝，S08/S09 覆盖负 scale 与差值 > 18 捷径的 typed 拒绝，均有独立向量。
- `P404-QUAL-002` MAJOR disposition：§1.3、§1.4、T-22…T-40 与 D-100:14 把 budget effect 和 external cash inflow/outflow 加入每个 checkpoint 的独立 report reducer；收入/费用或 consumption/category 不能替代这三个维度。
- `P404-QUAL-003` MAJOR disposition：§4.2 将 allocateIds 与 factory callback 分离，由 commit port 保存实际 allocatedIds；§4.2 validator、E-37 与 T-54 逐项绑定 confirmation/status/transaction/version/posting-set/两条 posting ID。
- 顺带修正登记：§7 迁移阶段 6 的触发器断言引用由 T-44 更正为 T-46（E-40 对应的成功迁移测试是 T-46；T-44 属 E-34 formal binding mismatch），该交叉引用修正保持不回退。
- 2026-08-16 对原开放问题 1–10 的历史裁决继续作为候选方向，但不绕过上述三项重新评审。只有本修订后的完整文件通过独立 spec review、独立 quality review、distinct verification 和主代理明确重新批准，Status 才能恢复 approved，D-100 才能重新放行轮 B。

## 11. 开放问题（历史裁决保留，重新批准时一并确认）
...
7. 方向门失败载体（§4.3）：本规格冻结沿用 `DomainViolation.InvalidOrdinaryIncome`（ImportSpineWechatEndToEndTest.kt:293 先例；spine 不比较 violation 内容）。备选：(a) 请求改携带 walletAccountId + otherAccountId、由 store 按方向机械派生 from/to（方向违约结构性不可能，但改变 Q4 冻结的 from+to 形状）；(b) 再新增专用方向 violation（超出本次仅增加通用金额可表示性 violation 的最小边界）。推荐维持本规格冻结。
...
- `ledger-domain/src/commonMain/kotlin/com/unifiedledger/domain/DomainResult.kt`（仅增加通用 `AmountNotRepresentableInCurrency` violation；既有会计原语零改动）。
...
- `ledger-application/src/jvmTest/.../import/wechat/WechatBillParserJvmTest.kt`（T-01…T-12、T-49）与 `ledger-data/src/jvmTest/.../ImportSpineTransferEndToEndTest.kt`（T-13…T-48、T-50…T-54；含 `P404CanonicalState`、合成 xlsx 构建器与 P4-02/P4-03 装配复用）。
