...
| **Android 无可跑 `Schema.migrate` 的非创建式 `SqlDriver`** | 本仓 SQLDelight 版本 **2.3.2**（`build.gradle.kts:4`）；Android 侧驱动族为 `AndroidSqliteDriver`（`AndroidLedgerDatabaseHandle.kt:15`，带 create/version 回调） | **06.C 的 Android 交付物依赖新增驱动代码**：需自定义一个 `SqlDriver` 适配器包住 `SQLiteDatabase.openDatabase(..., OPEN_READWRITE)`，或核实 2.3.2 是否存在非创建式打开。**不是**一个参数选择（§8.2、§10 第 8 项） |
...
- **持 operation lease 覆盖整段预检（本规格选定）**：预检在开始时经 `owner.acquireLease()`（`LedgerRuntimeOwner.kt:321`，非阻塞，非 Ready 返回 `RuntimeNotReady`）取得一个 operation lease，**全程持有**，在结束（成功/失败/取消）后于 `finally` 释放（形状同 06.B 导出的 `BackupExport.kt:200-204`）。generation 取该 lease 捕获的值（`LedgerLease.generation`，`:179`）并绑定进 token（§7.1 第 3 点）。
...
- **读写驱动的落地形状（未验证项，且明确需要新代码）**：Android 的 `SQLiteDatabase`（framework 句柄）**不是** SQLDelight `SqlDriver`；而严格迁移需要 `LedgerDatabase.Schema.migrate(driver, from, to)`。本仓 SQLDelight 版本为 **2.3.2**（`build.gradle.kts:4`、`android-app/build.gradle.kts:49`、`ledger-data/build.gradle.kts:56`），其 Android 侧驱动为 `AndroidSqliteDriver`（`AndroidLedgerDatabaseHandle.kt:15`）。**候选路径（实施批择一并实证）**：
  1. **自定义 `SqlDriver` 适配器**（**新代码**）：在 `SQLiteDatabase.openDatabase(path, null, OPEN_READWRITE)`（**不带** `CREATE_IF_NECESSARY`，故不 create；无任何 schema/version 回调，故不自动迁移）之上实现一个最小 `SqlDriver`，使其 `execute`/`executeQuery` 转发到 framework 句柄，从而可喂给 `Schema.migrate`。这是**最可能**的路径，因为 SQLDelight 2.3.2 的 Android 侧驱动族中，`AndroidSqliteDriver` 与 `SupportSQLiteOpenHelper` 路径都带 create/version 逻辑（`AndroidLedgerDatabaseHandle.kt:140-156` 记录了零版本 schema 在构造期即抛的 P0 事实），故**不能**用它们跑隔离副本的严格迁移。
  2. **任何 SQLDelight 支持的「非创建式」打开**（**待核实**）：若 2.3.2 存在一个不触发 create/version 的 Android `SqlDriver` 工厂，则优先使用它；本规格**未**确认其存在，归实施批核实。
  - **明确结论**：**06.C 的 Android 交付物依赖新增驱动代码**（至少候选 1 的自定义适配器），而**不是**一个参数选择。本规格登记该依赖，不把它说成「归实施批的选择」。
...
8. **Android 上「读写、不 create-on-open、不自动迁移、能跑 `Schema.migrate`」的 driver 面**：现状只有 `OPEN_READONLY` 的 `verifyAndroidSnapshotFile`（`AndroidLedgerDatabaseHandle.kt:161`），**无**可迁移的读写面（§8.2）。**06.C 的 Android 交付物依赖新增驱动代码**（自定义 `SqlDriver` 适配器，或已核实的 SQLDelight 非创建式打开）；该依赖与具体实现归实施批。
