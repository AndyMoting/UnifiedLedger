...
- catalog：ledger `ledger-p409-oracle`，账户 = `account-asset-a`（余额宝锚 ASSET）、`account-asset-b`（ASSET）、`account-credit-huabei`（LIABILITY）、`expense-account-food`/`expense-account-clothes`、`income-account-salary`，另于第二 ledger `ledger-p409-other` 挂 `account-asset-other-ledger`（ASSET；§3.4 测试 12 F4 跨账本领域拒绝向量必需）；分类 = food/clothes（EXPENSE 二级，挂费用账户）+ salary（INCOME 二级，挂收入账户）。全部确定性合成 ID；`generatedAt = "2026-08-22T08:00:00Z"`、`confirmedAt = "2026-08-22T10:00:00+08:00"`、`inputRef = "batch-p409-oracle"`。
...
| RL-04 | `transfer_flow_source` v2（余额宝路由输出形状：VALID_COMPLETE 与 VALID_INCOMPLETE 两变体，`AlipayCsvParser.kt:451/:467` 形态） | 2000 | 2026-08-05T12:00:00 | out / settled | `TransferFlow(account-asset-a → account-asset-b)`（余额宝锚为 from 端 `account-asset-a`，与本格冻结表达式一致） | −2000/+2000 两端余额；internalTransfer +2000；组合锚点推进 posting reconciliation |
...
| RL-08 关闭行 | `ordinary_flow_source` v1 | 900 | 2026-08-10T12:00:00 | out / `closed`（raw 保留） | 无（source `VALID_COMPLETE`；funding 门 → candidate `incomplete`，不可确认） | 零资金影响、零正式写入；`ImportFundingState.NO_FUNDS` + rule `source-contract-closed-v1`、duplicate kind `CLOSED_OR_FAILED_NO_FUNDS`（P407 oracle :115-118/:533 冻结形态）；重复导入幂等（与 RL-01 重导入共用机制面） |
...
| RL-08 关闭变体·混合 | `mixed_payment_source` v3 + profile `MIXED_PAYMENT(余额宝, 花呗)` | 1860 | 2026-08-13T12:00:00 | out / `closed`（raw 保留） | 无 | 同上形态（状态 raw 保留 + unresolved + source `VALID_COMPLETE`（funding 门 → candidate `incomplete`），零拒行升级 = slice2 §3.2 状态门纪律） |
