...
| 当前正式持久化边界的数据库与迁移 | 已确定 | `ledger-data` 使用 SQLDelight `2.3.2`；Android 只使用 system SQLite driver；当前 schema v9 及其迁移链均经过验证。该选择不预先决定报表、同步或更广泛查询的存储方案 |
