...
本文件定义目标模块边界和依赖约束。Android、Desktop 与 Kotlin 工程尚未建立，下列模块是后续实现必须遵守的逻辑职责，不代表仓库当前已经包含同名目录。
...
Python 只用于旧账迁移、规则原型、来源解析实验和黄金结果基线。个人配置与真实来源场景保持在本地；生产客户端不能调用个人脚本或依赖本地参考目录。Python 中确认过的通用行为通过匿名黄金测试逐项迁移到共享核心，而不是整套脚本直接嵌入客户端。
