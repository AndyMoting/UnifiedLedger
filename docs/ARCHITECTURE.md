...
本文件定义目标模块边界和依赖约束。仓库当前已经包含两个可构建的 Kotlin Multiplatform 共享库模块，均带 JVM 测试目标。`ledger-domain` 已实现精确最小货币单位、稳定 ID 与目录、逐币种平衡分录集、正式交易当前版本链、资产账户付款的 `RG-01` 普通支出创建与当前分录余额重放，以及只修改备注的版本替代。

`ledger-application` 当前实现 `RG-01` 的最小明确确认用例与端口边界：请求身份和完整快照、只含身份的回执、幂等重放、独立重复记账、类型化冲突与类型化领域拒绝。原子提交只是应用层定义的端口契约；仓库尚无 `ledger-data` 适配器、数据库、schema、ORM、并发或持久化实现。

两个模块的完整目标职责仍以本文为准，当前切片不代表完整 `RG-01` 黄金契约或正式账务核心已经全部实现。下表中除 `ledger-domain` 与 `ledger-application` 之外的模块仍是后续实现必须遵守的逻辑职责，仓库尚未包含对应的构建模块；Android 与 Desktop 应用和平台模块也尚未建立。`ledger-application` 在此指共享 application library，不是可运行的 app/client。
