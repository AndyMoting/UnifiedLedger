...
本文件定义目标模块边界和依赖约束。仓库当前已经包含首个可构建的 Kotlin Multiplatform 模块 `ledger-domain`：它是带 JVM 测试目标的纯 `commonMain` 领域库，已实现精确最小货币单位、稳定 ID 与目录、逐币种平衡分录集、正式交易当前版本链、资产账户付款的 `RG-01` 普通支出创建，以及当前分录余额重放这一有限切片。

`ledger-domain` 的完整目标职责仍以本文为准，当前切片不代表正式账务核心已经全部实现。下表中的其他模块仍是后续实现必须遵守的逻辑职责，仓库尚未包含对应的构建模块；Android 与 Desktop 应用和平台模块也尚未建立。
