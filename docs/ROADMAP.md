...
在阶段 5 稳定基线上完成 Android 视觉和平台适配。包括 SDK/工具链证据门、以 `compileSdk 37` 为目标（minSdk 34 保持）；`targetSdk 37` 只有在官方兼容性证据、行为审计和 Android 14-17 回归全部通过后才升级，未通过则保持 targetSdk 36。阶段 6 再验证稳定主皮库双主题、AndroidLiquidGlass/Backdrop、Material3 回退路径，以及 Android 14-17 的设备、性能和人工验收；阶段 5 不提前修改这些依赖。阶段 6 进入条件已满足（D-133）；D1 工具链批完成（compileSdk 37，targetSdk 36 保持）；D2 主题与组件批完成（D-134）；D3 平台回归批完成 Android 14/15/16 模拟器证据与 targetSdk 37 行为审计（D-135），API 37 模拟器回归环境阻塞登记；阶段 6 收口声明：已于 2026-09-11 按 `D-142` 完成收口——Android 14/15/16/17 回归、视觉（Material3 稳定基线 + 玻璃独立回退层）、平台与性能证据、targetSdk 37 升级与真机确认全部通过，完成条件逐项满足；下一阶段 P7（Android 日常工作流）待用户门禁。
...
实现移动端的日常记录、查询、草稿确认和必要的系统入口。第一组 P7-01（账户与分类目录管理）、P7-02（录入基础）、P7-03（看账）与 P7-04（导入与草稿确认）已按 D-143/D-144/D-145/D-146 交付；P7-05（修错与回收站，D-156）片 1a 基底层已按 D-158 交付至 schema v31，片 1b 产品接线已全量交付并合入 `main`（Piece 1–6，`6f74ee4`/`02af67a`/`cfe0722`/`f814f7a`/`b4d6200`/`5ea7f67`），V-17 Android 人工验收已执行并按 D-181 收口（四项用户可观察期望与 V-16 设备部分 PASS；TalkBack 朗读巡检与再核对入口设备观察两项缺证经用户 2026-09-26 裁决接受延期/替代证据，V-17 判 PASS 含披露；V-19 以会话内人工再核对为满足口径判 PASS，片 1b 验收就此收口）。P7-06（整体备份与恢复）五个设计门已批准——06.0/06.A 备份容器格式（D-174）、06.1 稳定存储与 runtime owner（D-176）、06.2/06.B 备份导出（D-177）、06.3/06.C 恢复预检（D-179）、06.4/06.D 恢复确认与切换（D-182，规格 `docs/specs/2026-09-26-p7-06-restore-confirm-switch-design.md`），其中 06.1 已实施并合入 `main`（`d6cffb6`），但其 Android 启动路径经设备实证存在两项 P0 缺陷（相对库名含路径分隔符致 `StartupError`；主线程阻塞打开致 ANR 并留下 `POINTER_MISSING` 无指针代际）并已修复合入（`0c0b319`，登记 D-178；残余的崩溃窗口归 06.D，06.1 仍非完整 P706-A06 通过）；06.B（备份导出）已实施并合入 `main`（`6c3c415`，跟进修复合并 `a18738f`/`aed0920`；面向用户的取消控件延期，规格 §9 开放项与未并入 PASS 的验收向量见 `CURRENT_STATE.md`）；**06.C（预检）已按 D-179 规格实施并合入 `main`（merge `3d99b48`，登记 D-180；组合根接线归 06.D）**；06.4/06.D（恢复确认与切换）设计门已按 D-182 批准、实施属后续实施批；P7-07～P7-09 尚未开始。当前 `origin/main` 为 `328bee0`（D-181 登记合并；此前 `b69ba7b`（D-180 登记合并）同提交 CI run `36246932378` 三 job 全 success）。
