...
实现移动端的日常记录、查询、草稿确认和必要的系统入口。第一组 P7-01（账户与分类目录管理）、P7-02（录入基础）、P7-03（看账）与 P7-04（导入与草稿确认）已按 D-143/D-144/D-145/D-146 交付；P7-05（修错与回收站，D-156）片 1a 基底层已按 D-158 交付至 schema v31，片 1b 产品接线已全量交付并合入 `main`（Piece 1–6，`6f74ee4`/`02af67a`/`cfe0722`/`f814f7a`/`b4d6200`/`5ea7f67`），V-17 Android 人工验收已执行（四项用户可观察期望与 V-16 设备部分 PASS；TalkBack 朗读巡检与 Piece 6 再核对入口两项缺证，V-17 整体不判 PASS）。P7-06（整体备份与恢复）四个设计门已批准——06.0/06.A 备份容器格式（D-174）、06.1 稳定存储与 runtime owner（D-176）、06.2/06.B 备份导出（D-177）、06.3/06.C 恢复预检（D-179），其中 06.1 已实施并合入 `main`（`d6cffb6`），但其 Android 启动路径经设备实证存在两项 P0 缺陷（相对库名含路径分隔符致 `StartupError`；主线程阻塞打开致 ANR 并留下 `POINTER_MISSING` 无指针代际）并已修复合入（`0c0b319`，登记 D-178；残余的崩溃窗口归 06.D，06.1 仍非完整 P706-A06 通过）；06.B（备份导出）已实施并合入 `main`（`6c3c415`，跟进修复合并 `a18738f`/`aed0920`；面向用户的取消控件延期，规格 §9 开放项与未并入 PASS 的验收向量见 `CURRENT_STATE.md`）；**06.C（预检）设计门已按 D-179 批准（规格 `docs/specs/2026-09-25-p7-06-restore-preflight-design.md`），其实现尚未开始**；06.D 与 P7-07～P7-09 尚未开始。当前 `origin/main` 为 `7199c56`（同提交 CI run `36116010096` 三 job 全 success）。
