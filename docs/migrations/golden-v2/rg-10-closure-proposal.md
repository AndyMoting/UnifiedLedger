...
The frozen v1 reconciliation uses transaction-level keys with `complete`/`partial`/`pending`/`pending_financial_evidence` values. The runtime stores posting-level keys with `pending`/`matched` values. The v2 expected artifact publishes each runtime posting status without widening that posting vocabulary, then derives one transaction-level `reconciliation_summary`: v1 `complete` collapses to v2 `matched`, v1 `partial` remains `partial`, and both v1 `pending` and `pending_financial_evidence` collapse to v2 `pending` because the current v2 derived-status registry has no separate financial-evidence token. Thus this is intentionally a many-to-one compatibility projection; the v2 artifact does not claim that `pending_financial_evidence` remains distinguishable after publication. The summary is synthesized per transaction that owns reconciliation-eligible postings (`matched` when all eligible postings matched, `partial` when some matched, `pending` otherwise). Legacy `not_present`/`not_applicable` values map to record absence (mapping: "Legacy not_present/not_applicable reconciliation maps to no posting_reconciliations record") and are dropped from the projected expected block. The transaction-level pending rule for expiry and activation kinds is an oracle/publication-side inference of the frozen v1 semantics and is annotated in `Rg10FullStateOracleTest.projectReconciliation`; it does not add a posting-level runtime state.

### Other registered projections

- Publication uses exact v2 catalog balance coverage. Therefore a disabled stored-value account is still present in every published `balances` array at its ledger-derived amount (zero for the RG-10 frozen data), even though the frozen v1 state omits that balance. `catalog.accounts[*].stored_value.enabled = false` means the account cannot accept RG-10 stored-value operations; it does not authorize omitting the account from the v2 balance snapshot or suppressing a nonzero ledger-derived balance in a future fixture.
...
- The v2 expected artifact includes one `stored_value_expiry_event` domain entity for the confirmed expiry. This is a deterministic publication-side projection derived from the frozen expiry transaction/lot/amount and required confirmation/evidence binding so the v2 `stored_value_expiry_confirmation` can target the exact event. The current Kotlin runtime snapshot does not own or expose a `stored_value_expiry_event` collection; this projection does not claim otherwise. Reminder-only acceptance publishes no expiry event.
...
Rejected field paths have an authoritative oracle-side mirror (`Rg10FullStateOracleTest.expectedFieldPath`, RG10-SPEC-006); the authoritative runtime source is the `rejectInvalidInput` table in `Rg10Operations.kt` together with the mapping registry's six rejection branches, and this proposal declares that both sides must stay in agreement. In particular, the reviewed W3 anchors are `component_sum_mismatch` -> `$.attempted_input.credited_amount`, `bank_payment_model_and_all_recharge_facts_required` -> `$.attempted_input.explicit_confirmation`, and `spend_category_and_behavior_confirmation_required` -> `$.attempted_input.explicit_confirmation`.
...
- The reconciliation transaction-level collapse (`complete` -> `matched`; `pending_financial_evidence` -> `pending`) is an oracle/publication-side compatibility projection (annotated in `Rg10FullStateOracleTest.projectReconciliation`); the loss of the separate financial-evidence token is explicit and stays projection-layer behavior until a v2 reconciliation authority adds such a token.
