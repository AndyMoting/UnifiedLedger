...
The corrected expected v2 output has been generated, passed independent specification and quality re-review and distinct verification, received explicit user re-approval, and has been published to `golden/rules-v2/rg-04.json`. All 26 raw v1 operations have runtime coverage; the full all-26 state/report/reconciliation/delta comparison has been merged into main via `Rg04FullStateOracleTest` (commit `88c9bfa`, 17 roots / 26 operations / 43 states). The manifest discovery metadata (`golden/rules-v2/manifest.json`) still records the publication-time partial comparison facts; updating that metadata belongs to the publication gate and is not part of this mapping document. V1 fixture rewrite remains closed.
