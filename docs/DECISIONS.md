...
**状态：** 已批准并交付（2026-09-20，§10.3 大数据整链的**单次 instrumented 会话全链**实测：D-162 第 4(a) 条登记的「阶段顺序未在单一连续会话内满足」缺口已由本次实测闭合；本条只登记实测与限定，**§10.3 规模项仍维持「缺证（部分证据在位）+ 承接」，不记 PASS**——剩余限定见第 4 条）。

**决定：**

1. **实测配置**：同一 AVD（`ul_p7_d01`，隔离 adb）；候选 APK sha256 `fa0f96af…`（`840bf67` 工作树本地构建）；测试 APK sha256 `1929529e…`；库由 `local/artifacts/p7-05-scale/ledger-50600-v31-backup.db`（sha256 `92a9b646…`，integrity ok）回灌恢复（v31、50,600 候选、1 交易），**步骤之间从未清库**；会话内由主机侧执行一次产品 SAF 导入（200 行 fixture，致动器在等待期不建立 `UiAutomation`）。

2. **链条顺序在单次会话内满足（承接 (a) 闭合）**：致动器新增 `fullChain` 模式，在一次 instrumented 会话内按计划 §10.3 顺序执行——冷启动→首页→进入导入→（主机侧导入）→接治→收尾重读→列表可操作→**候选详情**（点首行→补齐决策→勾选→返回）→**滚动**→**重复组打开**→**审核/确认**→**详情与月度刷新**（列表末尾 `进入批量确认`→授权→权威回读）。会话结束时**不 finish 活动**，由 instrumentation 结束触发的进程终止充当链条末段的「杀进程」，主机侧随即 `am start` 完成「重开」。**关键设计**：候选详情屏自带勾选与 `进入批量确认` 入口，故选择动作可在深遍历**之前**完成，使列表末尾的批量入口在组卡之后仍可达——从而无需回滚遍历。
...
3. **逐阶段权威读数（DB 前后计数）**：
   ...
   - **详情与月度刷新**：批量入口 `进入批量确认` 位于组卡之后（bounds `[42,1938][525,2043]`），点击后批量屏显示 `确认入账（1 项）`；授权后**权威提交**：`ledger_transaction` 1 → 2、`import_confirmation` 1 → 2、`posting` 2 → 4，**39,426 ms**（逐 2 s 轮询至权威计数增长）。
...
4. **登记限定（不并入 PASS）**：
   ...
   - **数据条件「20k 重复组」仍未构造**：本次组为 1,600 条重复记录。**并登记一项结构性成本观察**：组卡条目**内联渲染在列表末尾的批量入口之前**，组越大则批量入口越远——按本会话实测速率（≈4.4 项/动作、≈0.23 s/动作）推算，20,000 条组的卡页脚需约 4,500 次动作（≈17 min），而 70,000 条组（10k 导入对五份既有来源）需约 16,000 次动作（≈1 h）。故「以 10k 导入构造 20k+ 组并在同链复跑」在现行 UI 结构下成本很高；承接须明确该条件的规模口径，或改用**关闭组卡**的流程（组卡关闭后批量入口回到列表末尾附近）。
   - **「完整计数」观察（更新，维持开放）**：本次遍历起点 `rowCount=50,614`，而**入库前**候选为 50,600——偏移 **+14**，与 D-162 三次运行的 **+11** **不一致**，故「常量偏移」读法**不成立**。正确表述应为：`rowCount` = 列表的**非候选渲染项数 + 候选渲染数**，而本次候选渲染数在会话内导入后仍等于**入库前**候选数（50,600）。**两种读法仍未区分**：列表未随会话内导入刷新 vs `collectionInfo` 为陈旧快照。**D-163 第 4(g) 条的倾向性表述据此收窄修正**：其「常量容器项偏移」的倾向不成立；保留的确定事实仅为「无导入会话的 `rowCount` 也等于候选数 + 非候选项数」。该观察**维持开放**，须专门取证（例如会话内导入后触发一次显式「刷新清单」再读 `rowCount`）。
