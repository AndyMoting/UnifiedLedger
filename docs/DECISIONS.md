...
**批界**：RL-04 维度的来源整合与普通收支 formalization 子切片。本批证明共享 spine 不是首个来源特化（PHASE4_DESIGN_PACKAGE.local.md:84-88）；RL-04 锚点 `GL-A6F5A461E605` 的转账语义切片被负证据阻塞（9/9 真实文件无「余额转入余额宝」token），登记为开放问题。
...
**行为证据补充**（本地参考项目，行为证据 only，不复制代码）：两个本地记账 app 的 Alipay 解析实现显示不同表头检测策略——一个使用固定列索引（时间列 0、金额列 6），另一个使用动态表头检测（查找含"交易时间"、"收/支"、"金额"的行）+ BOM 容错 + 数据区结束标记。本规格按 9/9 真实文件取证冻结（固定第 23 行表头），行为证据仅作格式演化参考，不影响冻结契约。

**RL-04 负证据**：9/9 真实文件无「余额转入余额宝」交易分类 token；最近似 = 转账红包（收入）、账户存取（不计收支）。用户决策选项 A：提供新导出含此类行的样本后另起切片。
