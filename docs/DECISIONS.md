...
4. **计划 §10.3 大数据整链规模项：裁决为「缺证（部分证据在位）+ 承接」，不记 PASS。** 计划原文（`docs/PHASE7_REMAINING_IMPLEMENTATION_PLAN.local.md:188`）：固定匿名数据至少覆盖单次 10k 导入与多次累积 30k 候选、20k 重复组，并保留无重复与部分已处置状态；必须覆盖 冷启动→首页可操作→进入导入→解析/接治→收尾重读→列表可操作→滚动→候选详情→重复组打开→审核/确认→详情与月度刷新→杀进程后重开。**在位部分证据**：D01 门的 10k 接治实测（`local/artifacts/d01-p704/d01-gate-verdict-2026-09-15.md`）；A-PERF 20k 候选库（经产品 SAF 路径重铺）与 20k 级重复组，及 B2/B3/B5/B6 测量——B6 20k 组开卡冷开 2.61s 位于 ≤3s 门槛内，B5 定向详情门槛经更细粒度方法重测决定性 PASS ≤1s（`local/artifacts/a-perf-p704/baseline-report.md`、`retest-report.md`、`aperf-followup-plan.md`；状态更新见 D-148）。**缺证**：所需规模的**单一连续会话**覆盖上述整链全流程端到端、30k 累积候选状态、以及早期 50k 库（已丢失，须重铺）。**承接** = A-PERF 后续排查批（`local/artifacts/a-perf-p704/aperf-followup-plan.md`）：经产品路径重铺 20k/50k 库并执行受影响向量；该规模项在此证据出现前维持非 PASS。
5. **A-05 总状态：部分完成，不整体 PASS。** PASS 项：D01-1～D01-8/D01-10、D02-1、D02-2、D02-3（JVM）、D03-1、D03-4、D04-1、D04-2、D04-4（本条）、D04-5（JVM）、D05（JVM 维度）、D06-1、D06-2、D06-3。裁决登记项：D01-9 权限撤回缺证（PARTIAL 终态，承接 = 平台或环境提供确定性注入点时按计划 §10.2 重开复测）、D03-2 不适用（设计退化）、D03-3 接受延期（JVM 证据接受）、D04-3 接受延期（JVM 接受）、D05 设备维度不适用。剩余开放项：第 4 条的 §10.3 规模项；`A05BATCHITEM-STALE-SELECTION-001`（低，OPEN）——本轮再次实证（批量确认成功后已终结候选仍计入下一批选择集，观测计数与列表实际勾选不一致），且本次注入批次的计数正确（6 项）。按计划 §10.4.5 的规则（仍有必需项非 PASS 时报告部分完成，不得因已推送、CI 全绿、评审通过或仅余设备问题宣布整体完成），**无残余被并入 PASS**。
