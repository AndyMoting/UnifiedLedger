...
1. **实测配置**：AVD `ul_p7_d01`（API 36，`emulator-5680`，隔离 adb）；候选 APK 由 `840bf67` 工作树本地构建（sha256 `fa0f96af…`，与测试 APK 同签名配对——**CI debug APK 每次运行签名不同，`install -r` 必失败**，故本轮全部使用本地配对构建）；测试 APK sha256 `8a3bdb62…`（该绑定**适用于证据文件的末段**，即本节各读数所属的那一次运行；同一证据文件为追加写入，含此前三次尝试的段落，其中前两次使用更早的测试构建）。库为**同一匿名 fixture 的产品 SAF 路径累积导入**（五轮 10k + 本轮三轮 200 行），**步骤之间从未清库**。**读数来源限定**：`am start -W` TotalTime、`dumpsys meminfo` 内存值与 SystemUI 弹窗三项为**运行期直接观察，未入证据工件**（沿 D-159 已确立的登记口径）；其余读数均可由 `local/artifacts/p7-05-scale/instr-group3.log` 与设备内 `files/ul-scale-traversal.log` 复核。

2. **库与数据集状态**：**50,600 候选 / 50,600 来源 / 103,600 重复记录 / 1 交易 / 1 确认 / 2 posting**（本轮确认产生）。重复状态：**DEFERRED 102,200 + CONFIRMED_DUPLICATE 1,400**（即「部分已处置」状态已在位）；无重复候选行亦在位（`重复 无`）。`user_version=31`，`quick_check` ok，库文件 266,014,720 字节。以上六项计数与直方图由独立 verifier 在设备上只读复核一致。
...
3. **四阶段实测结果（同一库、未清库）**：
   ...
   - **(ii) 审核/确认 PASS**：组卡打开后，卡内页脚 `确认整组标记` 位于**卡内容之后**，须再遍历 **341 次动作 / 112,616 ms** 才可达；点击后整组处置完成（**54,690 ms**）。**组卡条目口径**：组内条目为**重复记录**而非候选——该会话 200 个候选各带 7 条重复记录，故组卡为 **1,400 条**（这也解释了卡页脚需长距离滚动才可达）。**权威效果（DB 前后计数）**：`import_duplicate_status_history` **+1,400**，直方图 `DEFERRED 103,600 → 102,200`、`CONFIRMED_DUPLICATE 0 → 1,400`；`import_candidate`/`import_source_record`/`import_duplicate_candidate`/`ledger_transaction` **增量全为 0**——组处置**不产生资金效果**，与该路径语义一致。
...
4. **登记限定（不并入 PASS）**：
   ...
   - **数据条件「20k 重复组」未覆盖（可构造，非结构性不可能）**：单文件接治上限 10,000 **候选**（D-146），但**组内条目按重复记录计**，且一个文件可对既有来源产生多条重复记录（本库实测比例：200 候选 → 1,400 条，7×）；D-157 第 4 条亦已登记过 20,000 条组（A-PERF B6）。故「20k 重复组」**可经产品路径构造**，本轮只是未构造该规模；承接 = 以更大的重复比例或更多既有来源构造并复测，或由裁决明确该条件的读法。
   - **未覆盖向量**：本轮无拒绝/零写入向量被触发（无拒绝发生）；组处置的逐项结果**仅以 DB 直方图为准**（致动器自身的卡内逐项文案统计只覆盖可见窗口，不能作为完整分母——1,400 条组内可见窗口仅显示 4 条）。
   - **致动器自身的两处登记残余（非产品缺陷）**：① 「组卡已打开」判定在 120 s 内报 `group card open=false`（卡标题未进入可见窗口），但卡确实已打开且 `确认整组标记` 随后可达并被点击——检测粒度问题；② 遍历重取容器失败时会**保留上一个节点句柄**并继续最多 25/50 次动作，理论上可能对已回收节点派发滚动（本轮未观察到误滚，且受拒绝守卫与停顿守卫双重限界）——登记为残余，后续改为丢弃句柄。**整模式墙钟时间无总上限**（默认确认等待 1,000 s + 兜底 500 s，叠加约 18 min 遍历），主机侧须自备超时。
