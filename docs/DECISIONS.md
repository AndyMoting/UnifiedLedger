...
2. **方法事实（登记，决定此前尝试的成败）：** (i) 主机侧往返过慢——`uiautomator` dump 单次约 2.5s，主机发出 tap 后 0.4s 计划执行的 kill 落到整批提交完成之后；定时器与 tap 必须在**同一设备 shell**内发出。(ii) 进度触发注入**不可行**：批量提交期间 `run-as … sqlite3` 返回陈旧计数（读取停在提交前的值，而主机侧查询已看到新行；方法登记见 `local/artifacts/d01-p704/a05-execution-checklist.md` §12）。(iii) 提交窗口为**亚秒级**（7 项在 <0.4s 内完成）。(iv) 确认前的决策与勾选为**内存态**（数据库与应用文件均无草稿副本，实测）；back 级联退出应用（叠加穿过详情→列表→桌面）即丢失。
