...
1. **问题（计划 §2 第 44 行）**：`local/artifacts/a01-a05/b5_round3.py` 取点击后的首个 `IntendedVsync`，该帧可能是列表按下/涟漪帧，**不能绑定到「详情屏关键内容可见」**；uiautomator dump 地板（~2.29 s）又粗于 1 s 门槛，故须更换终点。冻结规格 §6 对关键内容的唯一括注为「（金额/状态行）」。
...
4. **口径与限定**：（a）该读数为**上界**，与历史 `b5_round3.py` 的「首帧」读数（**下界**）**不是同一命题**，两者不可直接比较；（b）该库为 **0 交易**，有效谓词在本库上为 no-op（沿 D-159 第 2 条口径）——故该读数**不构成** D-158 §5 所需的「含交易 20k 库上的有效谓词代价读数」；（c）三轮均为热态连续测量，未比较冷/热差异；（d）本条不改变 D-148 已登记的 B5 PASS，只为其测量方法补上有效证据与终点定义。
