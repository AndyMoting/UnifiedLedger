...
4. **登记限定（不并入 PASS）**：
   ...
   - **数据条件「20k 重复组」仍未构造**：本次组为 1,600 条重复记录。**并登记一项结构性成本观察**：组卡条目**内联渲染在列表末尾的批量入口之前**，组越大则批量入口越远——按本会话实测速率（卡页脚 1,600 条 ÷ 389 次动作 **≈4.1 项/动作**、≈0.23 s/动作）推算，20,000 条组的卡页脚需约 **4,900 次动作（≈19 min）**，而 70,000 条组（10k 导入对五份既有来源）需约 17,000 次动作（≈1 h）。**参数注意**：`cardScrollActions` 默认 4,000，**低于该推算所需**；执行 20k 条组复跑时须显式调高该参数，否则会以 FINDING 提前结束。故「以 10k 导入构造 20k+ 组并在同链复跑」在现行 UI 结构下成本很高；承接须明确该条件的规模口径，或改用**关闭组卡**的流程（组卡关闭后批量入口回到列表末尾附近）。
...
5. **裁决口径**：**§10.3 大数据整链规模项维持 D-157/D-159 的「缺证（部分证据在位）+ 承接」，不记 PASS**。本条的增量是：**D-162 第 4(a) 条（单一连续会话链条顺序）已闭合**；剩余缺口收窄为第 4 条的「单次 10k 导入」「20k 重复组（含结构性成本观察）」与「完整计数」观察。**无残余被并入 PASS。**
