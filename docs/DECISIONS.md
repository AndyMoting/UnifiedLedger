...
**实施登记：** 已实施（2026-08-22，规格链 `83b3c25`→`d0a5307`（P406S2-SPEC-001..006 修复）→`f256b11`（批准）、实施 `eabe316`（7 文件 +770/-28）、补测 `edd9a7f`（+35/-3 仅测试）；已合并 main（2026-08-22，merge `df34388`；push 待用户明示））。交付面：解析层 6d 构成门 + 混合激活 + `parseMixedPaymentRow`、6e 显式构成门、`MixedPaymentFlow` 决策形状、`MixedPaymentFlowFormalFactory`（纯复用 `createMixedPaymentExpense`，领域零改动）、确认门七步序（片 1 mixed 防御解除登记）、决策快照腿金额列启用、`mixed_payment_group` 确认事务内腿先头后写入（`group_id` 确定性派生、`generated_at` = 决策 confirmedAt）、replay 等价链五处扩展、`Ledger.sq` 恰 +2 命名查询。零 schema 四条 git 证据（无 `.sqm` 新文件、DDL 零字节改动、版本钉 v25、fresh = migrated 原样通过）。过程事故登记（不隐瞒）：首位实施 writer 环境性静默超时中断，留下部分实施；续作 writer 逐文件盘点核验前任改动与规格完全一致（零修正零补齐）后完成聚焦验证。双评审：规格增量 APPROVE with findings——P406S2-IMPSPEC-001（MINOR，规格 §5.1.2「6e 单元断言」与 §3.1「结构性不可达」自相矛盾；**登记偏差：取 §3.1 冻结语义，测试以 folded 对照为最近似锁定**）、P406S2-IMPSPEC-002（MINOR，null `explicitConfirmedAt` 走异常回滚路径；**登记已知限制：不变量安全、claim 可重试、零残留；类型化门需语义修订，落点已挂 WORK_PLAN backlog，该路径当前仅测试侧可达**）、P406S2-IMPSPEC-003（LOW，合并负路径用例；creditLeg=0 变体已补、同账户残余登记接受）、P406S2-IMPSPEC-004（INFO，零建议断言为间接形式，登记备查）；质量 APPROVE with findings——QUAL-001/003/004（Low）由补测 `edd9a7f` 闭合（领域 `mixedExpenseRejectsTwoDistinctAssetLegsWithAssetAndCreditLiabilityRequired` 直击 `AssetAndCreditLiabilityRequired`、冲突 replay 断言 `SPINE_REQUEST_IDENTITY_CONFLICT` + 零新写入三重锁、creditLeg=0 变体），QUAL-002（Low）=IMPSPEC-002 登记处置；补测后双评审闭环复审均 CLOSURE APPROVE（附带条件 = 本段对 IMPSPEC-001 偏差与 IMPSPEC-002/QUAL-002 已知限制的显式登记，本段即满足）。独立 verifier（冻结 `edd9a7f`，11/11 PASS）：聚焦 `AlipayCsvParserCreditJvmTest` 16/16、`P406CreditFullStateOracleTest` 12/12、`MixedPaymentTest` 4/4；全量 `:ledger-application:jvmTest` 33 类 256、`:ledger-data:jvmTest` 56 类 379/379（29m26s 实执行）、`:ledger-domain:jvmTest` 18 类 111；Android `:ledger-data:compileAndroidMain` PASS；Python 806/806 OK；`project_docs` 通过。非阻塞观察：store :440 冗余 safe-call 编译警告（清理建议，不阻断，留给后续批；与 IMPSPEC-002 类型化门语义修订同挂 WORK_PLAN backlog 观察项）。

**关联决定：** `D-106`（主）、`D-107`、`D-072`、`D-073`、`D-078`、`D-097`、`D-100`、`D-102`、`D-103`、`D-104`、`D-105`

## D-109 P4-09 阶段收口契约

**状态：** 已批准（2026-08-22 用户批准 O-1..O-9 全 A 组合；O-1 仓库外真实来源验证为收口验收必选勾选门）；本决定批准契约，仅授权起草 P4-09 实施规格，不授权实施；实施规格与实施是后续独立批。

**决定提案：** P4-09（M4 Phase Closure）阶段收口契约由 `docs/specs/2026-08-22-p409-phase-closure-contract-design.md`（Status: approved）提出并冻结以下范围与验收，2026-08-22 获用户批准：

1. **现状基线登记**：P4-01~P4-08 各批已交付覆盖逐批登记（测试锚点 + merge commit + schema 版本链至 v25）；收口缺口五项 = 无逐 RL 全状态收口 oracle、无 populated v1→v25 单链数据级迁移测试、无 12 RG 竖井 + 完整 spine v25 + P4-08 表同库共存单一验收、无 RL 维度 × 失败模式统一矩阵、正式状态文档滞后（CURRENT_STATE「push 待用户明示」措辞与 `b19a6c1` 已在 `origin/main` 的事实不符）。
2. **交付面 WHAT 冻结（D1..D5）**：D1 逐 RL-01..RL-08 合成 full-state oracle（WORK_PLAN 矩阵逐维度映射测试锚点表；跨 RL 组合维度归属裁决——RL-05/RL-06 × P4-07 duplicate 判属各批已覆盖为主（`P406CreditFullStateOracleTest.kt` 比较面含 P4-07 表并含信用/混合 duplicate 场景）、RL-03 × P4-08 evidence linkage 判属 P4-09 新增组合锚点）；D2 populated v1→v25 单链数据级迁移 + reopen + fresh 等价；D3 12 RG 竖井 + 完整 spine v25 + P4-08 表同库共存 + reopen 单一验收（竖井不退役）；D4 统一失败矩阵（RL 维度 × 解析 fatal/intake 注入/confirm 注入/领域拒绝/并发输家/迁移 late-stage，逐格锚点映射，既有覆盖引用 + 新增项）；D5 全量受影响回归 + CURRENT_STATE/WORK_PLAN 状态同步。
3. **裁决点 O-1..O-9（2026-08-22 用户批准：全部采用推荐项 A；O-1 仓库外真实来源验证由显式可勾选门升格为收口验收必选勾选门，非可选）**：O-1 仓库内合成逐 RL oracle + 真实来源维持本地边界（GOLDEN_TESTS:164-166）+ 用户侧真实验证为仓库外验收步骤；O-2 零 schema、版本钉 v25、零生产语义变更（缺陷走评审路由、无新功能，schema/语义演进需求即退回重开契约）；O-3 不发布 golden 工件、阶段闭合不自动授权 publication 或 Phase 5；O-4 收口 = ROADMAP:41 阶段 4 完成判据被 D1..D5 证据满足，阶段 5 进入是独立用户门；O-5 P4-08 correction/successor invalidation 维持延期并在失败矩阵登记为已知延期维度；O-6 十四项已登记欠账显式排除并逐条指针（constraint_solved 推断证据、null explicitConfirmedAt 类型化门、store :440 清理、营销腿/非资金标注腿剥离、微信信用负证据与信用卡还款形态、分期 D-049、利息/手续费/逾期费、拆单/代付（含他人代付）、仅资产腿退款与信用借还其余形态、银行 PDF 门、共享负债账户、provider token funding 映射、整文件保留生命周期、产品 ID/Clock→Phase 5）；O-7 RG 竖井不退役（D-092 默认）；O-8 RL-07 按平台侧适用子集收口、银行侧真实镜像维度显式登记延期至银行 parser 门（D-099:1540 仍开），不阻断 ROADMAP:41「支持的标准来源」判据；O-9 分层验证矩阵（聚焦 oracle → 迁移 verifier → 三 JVM 模块全量 → Android 编译 → Python 全套 → project_docs → verify-project full → push 前 trace），release scope 留给真实发布门。
4. **实施授权边界**：本决定不授权实施代码或测试；P4-09 实施规格须在本契约批准后按独立批起草与评审（独立 worktree、单一 writer、独立规格/质量评审、distinct verifier、主代理最终检查、全量受影响套件），实施规格不得变更本契约 D1..D5 语义与已批准 O 组合。

**理由：** 阶段 4 完成判据（ROADMAP:41）要求以可重复验证结果闭合（ROADMAP:73），而 WORK_PLAN:102 与 PHASE4_DESIGN_PACKAGE:109-113 把 P4-09 定义为全 oracle、migration/reopen、RG coexistence、失败恢复与全量回归的收口批，并明确阶段闭合不自动授权 publication 或 Phase 5。各批已交付覆盖存在但按批切片（现状基线表），单链迁移/全竖井共存/统一失败矩阵三项验收形态从未被单一证据承载；RL-07 的银行侧维度受 D-099:1540 开放门限制只能以子集收口。GOLDEN_TESTS:164-166 与 :297-303 隐私边界排除真实来源入仓，故 oracle 以合成匿名代表值交付、真实验证留作仓库外步骤。零 schema 与零生产语义变更沿用 D-108 先例，保持「收口 = 证明」而非「收口 = 追加功能」的批语义；全部已登记欠账显式排除并留指针，符合 D-096:1435 未决不由实现默认冻结的纪律。

**边界：** 不发布 golden 工件；不动 `.external/`；不改 12 套 rgXX 竖井；不实现 O-6 排除清单任何项；不扩权 D-103 matcher 语义；不引入产品 Clock/随机 ID；CURRENT_STATE/WORK_PLAN 同步属收口实施批（D5），本提案批不改正式状态文档。
...
**状态：** 已批准（2026-08-22 依 D-109 §8 批准链：独立规格评审 + 质量评审双闭环后实施授权生效）。
...
**评审闭合：** 双评审（规格 + 质量）均 APPROVE with findings（P409IMPSPEC-001..011 / P409IMPQUAL-001..006），两位评审对规格裁决 4/5/8 与三项 writer 异议均给出同意判断。2 项 MAJOR：P409IMPSPEC-001 = O-5 延期标注落入矩阵基线（§6.2 显式延期登记行，D-103:1639 指针）；P409IMPSPEC-003/P409IMPQUAL-001 = §4.2 触发器计数勘正为 20.sqm..24.sqm 逐名分组（→v21 20 / →v23 18 / →v24 17 / →v25 14，v25 终态去重合计 61）。其余 findings（P409IMPSPEC-002/004/005/006/007/008/010 与 P409IMPQUAL-002..006）为文本精度与遗漏项修复；P409IMPSPEC-011 登记备查无需修复。修复冻结 `42bdd27`（2026-08-23，仅规格文件）；行号仲裁登记 = P408 触发器清单字面量以 writer 复核值 `LedgerDatabaseMigrationTest.kt:655-665`（18 项）为准。修复后双评审闭环复审均 CLOSURE APPROVE，规格 Status 转 approved，本决定实施授权生效。

**实施登记：** 已实施（实施完成、验证进行中；2026-08-23）。实施链与事故登记（不隐瞒）：首位实施 writer 配额中断留下未验证 WIP，冻结 `76b2781`（三新测试类合计 3719 行——oracle 1981 / migration 575 / silo 1161——+ `ImportSpineMigrationCoexistenceTest.kt` 头注释一行指向 D3 新验收，即本授权边界唯一既有测试文件例外）；续作 writer 盘点核验前任改动后修复 14 处测试自身缺陷至聚焦全绿 `97c0339`（+153/-68，仅三新类；oracle 修复后约 2050 行）。交付面：D1 `P409PhaseClosureFullStateOracleTest` 12 测试（逐 RL-01..RL-08 全状态比较、组合锚点 a/b、reopen/replay/并发/领域拒绝腿）；D2 `P409SingleChainMigrationTest` 单链 populated v1→v25 13 步编排 + reopen + 守卫 + 全部共有表逐行数据级 fresh 等价；D3 `P409SiloSpineCoexistenceTest` 12 竖井各一个代表 operation + replay 与 spine 六 kind 完整形态同库、零串音 + reopen（12 竖井零回退，全部经裁决 5 (i) store/adapter 层接入，无 raw SQL 播种回退）；D4 失败矩阵登记基线落 `docs/GOLDEN_TESTS.md` 新节「P4-09 收口失败矩阵」（本次落盘）；D5 文档同步（CURRENT_STATE 既成事实化与本段实施登记，本次落盘；WORK_PLAN 与 PROJECT_STATE 收口验收清单按规格 §7 路由由主代理在主 checkout 执行）。双评审（实施后）均 APPROVE：质量评审 P409IMPL-QUAL-001..005 全 LOW/INFO 登记接受——QUAL-001（Z 形差异未锚定规格条目）登记处置：组合锚点 linking evidence 因生产同形时间门（`P408Matcher.kt:183`/store :296-300/temporalShape）与 spine Z 形持久化时间结构性不可链，改用同刻 Z 形合成镜像 evidence，锚点本体 RL-03 posting 与 posting_reconciliation PENDING→CHECKED 全行保持；QUAL-002（candidateId 硬编码误导）、QUAL-003（D1 脚手架副本缺出处注释）、QUAL-004（D3 头注释 ledger 拓扑不全）、QUAL-005（rg09 replay 形态按先例）登记接受。窄域规格评审 P409IMPL-SPEC-001..003 均为规格文本缺陷、测试为正确侧，已勘正/补记（本次同批落盘）：SPEC-001 = §3.2 RL-08 completeness token 与 P407 冻结语料矛盾（P407 = VALID_COMPLETE + NO_FUNDS，candidate `incomplete` 来自 funding 门，非 source incomplete）；SPEC-002 = §3.2 RL-04 括注「余额宝锚为 to 端」与冻结表达式矛盾（锚在 from 端 `account-asset-a`）；SPEC-003 = oracle catalog 第 7 账户 `account-asset-other-ledger`（挂 `ledger-p409-other`，F4 跨账本向量必需）未入 §3.1 清单。两项 fixture 适配经双评审独立裁定接受：Z 形镜像 evidence（生产 temporalShape 同形门证实唯一合法形态）；早竖井独立 ledger（D-091 全载设计证实）。零 schema 证据：无新 `.sqm`、`Ledger.sq` 零字节改动、生产三模块源码零改动。验证状态：八层矩阵第 1 层聚焦完成（三新类 14/14 两轮全绿，含 `--rerun-tasks` 复跑）；第 2 层起（迁移 verifier、三 JVM 模块全量、Android 编译、Python 全套、project_docs、verify-project full、push 前 trace）待执行——完整验证证据与 O-1 勾选门留痕待 distinct verifier 与主代理补登本段。补登（2026-08-23，独立 verifier + 主代理复跑 + 终稿登记批）：合并与验证闭合。合并：实施链已 `--no-ff` 合入 `main`（merge `e45626b`，2026-08-23，父 `b19a6c1` × 冻结候选 `36e0611`，9 文件 +4380/-3：契约/实施规格两文档、`docs/GOLDEN_TESTS.md` 失败矩阵节、三收口测试类 2033/1181/588 行、`ImportSpineMigrationCoexistenceTest.kt` 头注释一行、CURRENT_STATE/DECISIONS D5 同步）。独立 verifier（冻结候选 `36e0611`）八层验证矩阵第 1-7 层全部 PASS：第 1 层三新类聚焦 14/14；第 2 层迁移 verifier v25 原样通过；第 3 层三 JVM 模块全量零失败（domain 18 类 111 / application 33 类 256 / data 59 类 393，data = 56 基线 + 3 新类）；第 4 层 Android compileAndroidMain PASS；第 5 层 Python 全套 806/806 OK；第 6 层 project_docs 通过；第 7 层 verify-project full `valid:true`。零 schema 四条复核成立：无 `.sqm` 改动、生产源码目录零 diff、无 `25.sqm`、版本钉 v25。主代理已复跑聚焦证明通过。第 8 层（push 前 trace）未执行、收口完成声明未落：依 D-109 O-1（升格必选勾选门）与实施规格 §8.2，用户侧仓库外真实来源验证未执行，不勾选即收口不完成——待用户对本地真实账单在仓库外执行受支持收口形态（导入→确认→正式账目→可解释对账状态→重复导入幂等）并在主 checkout `docs/PROJECT_STATE.local.md` 收口验收清单留痕后，方可在 CURRENT_STATE 声明收口完成；push 待用户明示，push 前完成勾选。非阻断遗留登记：两处基线编译 warning（`ImportSpineLifecycleEndToEndTest.kt:1263`/`:1273`、`SqlDelightImportSpineStore.kt:440`）均不在本批 diff（基线遗留，非本批引入）；P409IMPL-QUAL-002..005 四项 LOW/INFO 已在上文实施登记处置，无需追加动作。终稿补登（2026-08-23，O-1 门闭合 + 阶段 4 收口声明落稿）：O-1 必选勾选门执行通过。用户侧对真实微信账单（8 天 46 行）在仓库外执行完整收口形态验证：44 接受/2 类型化拒绝零写入；2 笔确认建立正式分录（余额 −98.50，报表吻合）与可解释对账状态；转账缺腿保持待确认；同账单重导入零重复账目（44 精确重复候选）。执行与结果留痕于主 checkout `docs/PROJECT_STATE.local.md` 收口验收清单，无真实标识入 tracked 文件；临时驱动器已删除（从未入库）。产品发现经 O-2 评审路由登记：provider 精度逐行不定 vs 账户 CNY(2)，确认装配层需「精确补零重标定」语义，已挂 WORK_PLAN backlog（独立小批或 Phase 5 装配层），不阻断收口。据此依 D-109 O-1 与实施规格 §8.2 落阶段 4 收口完成声明：判据以 ROADMAP:41 原文为唯一基准（支持的标准来源能够从导入走到正式账目与可解释对账状态，重复导入不产生重复账目），证据 = 本批 D1..D5（合成收口 oracle/单链迁移/共存/失败矩阵/全量回归与文档同步）+ O-1 真实来源验证门通过（2026-08-23，用户侧执行）；CURRENT_STATE:41/:45 已同步终态。push 已获用户授权，第 8 层（push 前 trace）复跑后随全部提交一并推送 `main`；Phase 5（双平台壳）进入为独立用户门（ROADMAP:47）。

**关联决定：** `D-109`（主，契约）、`D-096`、`D-097`、`D-098`、`D-099`、`D-100`、`D-101`、`D-102`、`D-103`、`D-104`、`D-105`、`D-106`、`D-107`、`D-108`

## D-111 O-2 确认装配金额精度重标定

**状态：** 已批准（2026-08-23，用户批准独立 O-2 小批；Phase 5 尚未开始）。

**问题：** 真实账单来源金额逐行可能使用 0、1 或 2 位小数，而正式账户使用固定 `CurrencyUnit` 精度；确认装配若直接用来源 precision 构造正式金额，会产生账户表示不一致或隐式有损转换。

**决定：** 来源 `amountMinor`、`currencyCode`、`currencyPrecision`、指纹和 source row 原样保留；正式装配从确认字段引用的显式账户取得目标 `CurrencyUnit`，只允许等精度、精确补零和可整除降精度。禁止浮点、舍入和截断；币种不符或不可表示返回 `AmountNotRepresentableInCurrency`，乘法/分录算术溢出返回 `ArithmeticOverflow`。高 source scale 的零金额精确归一化为零，非零不可精确表示即拒绝。失败沿现有确认事务零正式写入并保持候选可重试。

**范围：** 更新 transfer、credit/refund、mixed 生产工厂；新增 application-level `OrdinaryFlowFormalFactory`，不创建 Phase-5 平台根。mixed 的 `assetLegMinor` / `creditLegMinor` 没有独立 scale，本批冻结为确认字段已按目标账户 minor units 表达；只归一化 source total，并用归一化 total 写入 mixed 持久化投影。不改变 P4-08 matcher/reconciliation precision 规则，不改 schema、parser 或 Phase 5。
...
## D-121 阶段编号重排与 P5-04 基础交互规划

**状态：** 已批准（2026-08-31）。

**决定：** 阶段编号与职责映射冻结如下：P5 = 稳定双端 MVP 与基础交互；P6 = Android 视觉与平台适配；P7 = Android 日常工作流；P8 = Desktop 批量工作台；P9 = 同步与 AI 扩展。

**P5-04 规划授权：** P5-04.1 为三 Tab（首页、账户、分析）与中央新增入口；P5-04.2 为共享状态控制的新增记账全屏编辑页（不引入导航库），系统返回关闭编辑页，确认/取消行为固定；P5-04.3 完善既有手工支出流程，不新增收入、转账或借贷正式类型；P5-04.4 覆盖启动、加载、提交和失败状态并保持 fail-closed，仅 startup error 或 handoff 前可证明 `commitOnce` 零调用的 `InfrastructureFailure` 可恢复重试，`UnknownCommit`、冲突和领域拒绝禁止自动重试；P5-04.5 完成 Android 与现有 Desktop 回归验证并收口 P5。上述内容是 P5-04 的实施规划授权，不代表代码已实施。

**P6 证据门：** `compileSdk 37` 为目标，minSdk 34 保持；`targetSdk 37` 只有在官方兼容性证据、行为审计和 Android 14-17 回归全部通过后才升级，未通过则保持 targetSdk 36。P6 可在视觉功能或稳定回退路径与平台证据通过后收口，不以视觉实现成功作为 P7 的硬前置；核心账务流程不得因视觉失败而阻塞。

**理由：** 先稳定基础交互，再隔离高风险视觉与平台依赖，避免 SDK 和厂商差异阻塞核心账务流程。

**影响：** P5-04 可按规划实施但不接入视觉依赖；P6 必须通过 SDK/平台证据门和稳定回退路径；P7、P8、P9 按新编号执行；P5-03 已交付账务与状态边界不变。
...
**机制摘要（冻结）：** ledger-application 新增纯解析器 `ParseManualExpenseOccurredAt`（sealed Result Valid/Invalid + `OccurredAtFormatError{INVALID_FORMAT,DST_GAP}`，镜像 ParseManualExpenseAmount）；接受格式（a）ISO 8601 instant `YYYY-MM-DDTHH:mm(:ss)?Z`（超集，保 P503IMPL-Q-001）、（b）`YYYY-MM-DD[ 或 T]HH:mm(:ss)`（允许单位数小时）、（c）省年 `MM-DD HH:mm(:ss)`/`MM-DD`（按 LedgerClock 补年）、（d）`YYYY-MM-DD`（当日 00:00 本地）；相对/自然语言表达与一切其他 locale 格式一律 Invalid（用户裁决）；墙钟输入按固定 Asia/Shanghai 走选择器同款 round-trip 换算，DST 空档 Invalid(DST_GAP) fail-closed；parse-on-type 三条分支（空白→既有 missing 路径；有效→派发既有 `UpdateOccurredAt`（选择器同通道，reducer 零改动）；无效非空→不派发 + 内联错误文案冻结「无法识别的时间格式，示例：2026-09-15 08:30 或 2026-09-15T00:30:00Z」）；Continue 门 `occurredAtTextReconciles` 换用同一解析器实例（P503IMPL-Q-001 不变量保持）；显示语义不变（选择器仍写 ISO 串、手输原样保留）；零 schema、零 ledger-domain、零 Esc/back 行为（D-137 不动）、零新坐标（ledger-application 新增对既有已过门坐标 kotlinx-datetime 0.7.1 的声明；对 D-131 §3.2 文本兜底接受集：偏移与小数秒形式由接受改为拒绝，其余保留）。

**实施规格：** `docs/specs/2026-09-08-d138-occurred-at-lenient-manual-input-design.md`（状态 proposal；冻结 SHA-256 占位由主代理在规格冻结/提交时填入）。规格冻结 SHA-256（UTF-8+LF 工作副本字节域）哈希链：评审前冻结 `C289E28D4A5E7EFF470F92010B893B9AF5DE4ED012485F8A82960C654B68C795`（f9c0de2）→ 评审处方闭环（P1-1/P1-2/P2-1..P2-5/P3-1..P3-4）后 `F1141AAE8AB3E8F5E0743F56647BE8D09A7BFF1471ACFCB7FA14D6E98451B563`（fd18523，blob `126ff084…`）→ **批准终值** `BBD5ABB5B6A1E638A25DDBD98A42C084364D0B5CF956DAF3EDF870DE35B86C54`（blob `d6f31c661171ffe32c0141dcf6bb6bf2a01085a6`；终值增量 = 状态行翻 approved + 评审残留 R-A..R-D 折入，§2 机制条款与 §3 变更表自 CLOSURE-APPROVE 后零改动）→ **实施期勘误** `A3BFBE821540C233657818B51471C0603AAECFF708968BA8CA425A436CB7D86B`（blob `be012c0c41939932838d754c7d81700cf43f4dba`；增量 = §3.1 闰年行期望值笔误 02-28→02-29，与机制 +08:00 换算及闭环保留核验数学一致，解析器按机制实现）。

**实施登记（2026-09-08）：** 实施提交见批次 merge（writer 单批：`ParseManualExpenseOccurredAt.kt` 新文件 + facade/校验/编辑屏/app-ui 装配/两组合根注入 + `ledger-application/build.gradle.kts` 增 kotlinx-datetime 0.7.1 + 三测试文件）；本地验证全绿（`:ledger-application:jvmTest` 376、`:app-ui:jvmTest` 66（61+5）、`:desktop-app:jvmTest` 5、`:android-app:testDebugUnitTest` 7、ktlintCheck ledger-application+app-ui 0 违规）。构造默认参数裁定：`P503DraftValidation.parseOccurredAt`/`ledgerClock` 与 `P503LedgerFacade.parseOccurredAt` 带 fail-fast 默认（错误时钟仅在被误用时抛错；产品全部路径注入共享实例，R-4 由构造保证；默认仅保 frozen 零改动行——`P503Reducer.kt` 构造面与 `AndroidStartupControllerTest`——免改编译）；实施旗标 1（§3.1 闰年笔误）以上述勘误闭环；旗标 3（新文件 LF）经 ktlint/仓库归一化无影响。

**批准登记（2026-09-08）：** 用户 2026-09-08 发起并授权批次（手输友好时间诉求 + 范围裁决「不要支持"昨天 20:00"这种相对表达」+ 继续指令）；独立规格评审 REJECT（P1-1/P1-2 + P2-1..P2-5 + P3-1..P3-4）→ 处方闭环 → 同评审 **CLOSURE-APPROVE**（4 项 P3 残留 R-A..R-D 随批准翻转折入）；规格状态 proposal → **approved**。实施批按既有路由执行（独立 worktree、单一 bounded writer、独立评审、distinct verifier、主代理验收）。

**范围：** ledger-application（新解析器文件）+ app-ui（facade/装配/校验/编辑屏接线）+ 两组合根注入；测试 = 新 `ParseManualExpenseOccurredAtTest`（格式矩阵、相对表达拒绝、固定时钟年补齐、ISO superset、DST gap）+ `P503DraftValidationTest`/`P503ReducerTest` 新增；零 schema（v27 不变）、零 reducer 状态机/事件集变更、零新坐标（ledger-application 新增对既有已过门 kotlinx-datetime 0.7.1 的声明）；对 D-131 §3.2 文本兜底接受集：偏移与小数秒形式由接受改为拒绝，其余保留；零 CI、零主题/玻璃、零 Esc/back。
...
1. **实施提交与触达文件：** 实施提交 `4b0763c`（代码：`P503EditScreen.kt`，实测 diff +6/-1、2 hunks = 规格 §2.1 `:98` 去 key + §2.2 选择器确认条件回写；§2.1 处含一行 D-139 spec 2.1 约束注释——规格 §3 仅对 §2.2 明示注释授权，实施评审 P2-1 接受为良性偏差留痕）；登记提交 `f830e4d`（规格冻结 approved + 本条目）；合并提交 `209bd3c`。实施评审 P2-2 留痕：交接摘要「+5/-1」系笔误，以实测 +6/-1 为准。实施评审 P2-3 留痕：登记日期 2026-09-09 为真实日期跨日，非笔误。
2. **独立实施评审：** APPROVE（同评审 delta 复查，无 P0/P1；3 项 P2 记录项如上处理）。
3. **distinct verifier：** 命令集 12 条全 exit 0；A-1 = `:app-ui:jvmTest` 66 / `:desktop-app:jvmTest` 5 / `:android-app:testDebugUnitTest` 7 全绿（`--rerun-tasks` 强制重跑，JUnit XML 汇总计数，零新增零删除）；A-2 = `:app-ui:ktlintCheck` 零告警（报告文件 0 字节）；A-3 = `:android-app:compileDebugKotlin` exit 0；A-4 = `project_docs` exit 0。
4. **merge/push/CI：** `209bd3c` 推送 origin/main；CI run `34262774510` 三 job（Python tests / Kotlin tests / Android compile）全绿（24m55s），同提交聚合门证据成立。
5. **双端人工门逐键向量结果：** 桌面 7/7 PASS（d139-01…17）+ Android 7/7 PASS（d139-a01…a15，APK SHA-256 `04B837F0…`，emulator ul_p6_api35）；向量 1-7 判据 + 向量 8 记录性观察双端照实登记（冲突/拒绝流演示库不可达，现状由源码机制登记，见遗留缺陷登记节）。逐键纪律：每键 ≥350ms、关键键位「5」/「1」后立即截图断言（d139-05/09、d139-a04/a08）；Android 中文向量以 ASCII locale 向量替代（adb 注入限制，中文拒绝向量由 app-ui 66 套件覆盖）。判定与证据：`local/artifacts/d131-desktop-esc/d139-gate-verdict-2026-09-09.md` 及 `d139-*` 截图（本地存证）。
...
**状态：** 已关闭（2026-09-11）。技术栈升级批（CMP 1.12.0 + backdrop 2.0.1 + Kotlin 2.4.20 + AGP 9.3.1 + targetSdk 37 + kotlinx-datetime 0.8.0）已交付并推送（实施 `870de1f`、合入 `c2376a8`、登记 `f07c854`）；CI run `34555111003` 三 job 绿；双端人工门向量 1-9 全 PASS；A-1..A-7 全部满足（见「实施登记」节）。实施规格已冻结（见下方「实施规格」节）。

**批准登记（2026-09-10）：** 原批准记录——先按用户指令「D-141先记录吧」完成 docs-only 登记，批准时用户裁定 kotlinx-datetime 0.8.0 并入升级项、并授权启动规格与实施流程。
...
4. **merge/push/CI 证据：** 已合入本地 main（`c2376a8`，`--no-ff`）并**已推送 origin/main**（用户 2026-09-11 授权「push」；push 前 trace 验证 exit 0，485 提交历史零追踪违规，`39d50a8..f07c854`）。同提交 CI run `34555111003` 三 job 全绿（Kotlin tests 21m2s / Python tests 7m8s / Android compile 6m21s，含 `:android-app:assembleDebug` 与 debug APK artifact 上传）；CI artifact `android-debug-apk-f07c854c2c498c86a59280b1820f478824ebf73c` 已下载，APK SHA-256 `5AC404C12C1D947E5401FFFCB833FAB8AD6A058A112F8430F5D7A6EC113B2AFF`。本机 `:android-app:assembleDebug` 经尝试在 `mergeExtDexDebug` 阶段因 16 GB 主机内存不足中止（生成 729 MB 堆转储，已清理；无产品失败），与 D-118 R-9「assembleDebug 归 CI」既有判定一致——人工门 APK 取 CI artifact。
5. **双端人工门向量结果（规格 §2.7）：** **桌面 1-5 全 PASS + Android 6-9 全 PASS**（证据 `local/artifacts/d141-stack-upgrade/d141-gate-verdict-2026-09-11.md` + `d141-01…22` / `d141-a01…15`）。桌面要点：向量 1 启动正常（左缘裁切文本与 D-140 基线逐像素同形＝批前既有，非 CMP 1.12 回归）；向量 2 DatePickerDialog 按 Esc 仅关对话框、编辑页与 draft 完整（D-137 零回归）；向量 3 窗口缩至 760×620 强制溢出后 Tab 驱动滚动，内容滚动无测量异常/裁剪——**R-1（CMP 1.12 桌面无限约束测量）风险面实测通过**；向量 4 玻璃 Tab 栏正常渲染无崩溃（D-136 零回归）；向量 5 逐键 `2026-09-15 08:30` 原样保留、确认页换算 `2026-09-15T00:30:00Z`、取消返回文本原样保留（D-138/D-139/D-140 语义保持）。Android 要点（用户 2026-09-11 授权「Android 你试试」后在 agent 自启 `ul_p6_api37`（`-port 5680` + `adb -P 5038`）执行，用户 MuMu emulator-5554 全程只读未触碰）：向量 6 安装成功且 `targetSdk=37` 生效；向量 7 冷启动正常；向量 8 手工支出创建流全通过（逐键发生时间文本原样保留无 ISO 改写、确认页 `2026-09-15 08:30（UTC+8）＝ 2026-09-15T00:30:00Z`、取消返回两字段原样保留）；向量 9 选择器打开/回写 `2026-09-19T00:30:00Z`（Asia/Shanghai 换算正确）后继续门放行。
...
## D-146 P7-04 导入与草稿确认（设计门 + Q08/Q09/Q10 裁决）

**状态：** 已批准（设计门冻结，2026-09-14 DELTA CLOSURE APPROVE）；实施登记随实施批闭合后补登。

**授权依据：** 用户常设授权「除不 push 外，其余默认采用推荐方案」+ 2026-09-14 用户指示继续完成阶段 7 计划。主代理在只读取证（阶段计划 §6/§7/§8.1 于基线 `b490bea` 的增量核验、E11/E12/E13、P7-03 交付资产、四 parser 冻结契约、Q08 外部证据只读取证 2026-09-14——POI 官方组件页/POM/源码、AOSP libcore `javax.xml`、Android SAF 官方文档、ECMA-376/ISO-IEC 29500、fastexcel/社区项目证据）后，按推荐方案裁决 Q08/Q09/Q10；其中 R-Q09-1 经主代理治理裁决（2026-09-14）翻转为随机句柄方案；规格经首轮独立规格评审修复（draft-2）与 delta 复验后随本设计门冻结。工作基线 = 分支 `UL-p7-04` @ `b490bea`（P7-03 merge，D-145），schema = v29。

**决定：** 启动并冻结 P7-04「导入与草稿确认」设计契约 `docs/specs/2026-09-14-p7-04-import-draft-confirmation-design.md`（approved，设计门 DELTA CLOSURE APPROVE；**冻结 SHA-256（LF 域）= `72F3B8076BA086DE4F53E960B25C32FDAFCDA0DDE07F35A721DC2BB9C68E10F0`**）；配套回写 `docs/PRODUCT_REQUIREMENTS.md`「导入与草稿确认（P7-04）」小节与 `docs/ARCHITECTURE.md` D-146 段（含 CSV/XLSX 技术选择状态行修订）随门冻结。实施批以冻结规格与「实施批义务」段为契约，任何语义变更即重开评审门。

**Q08 文件端口、格式矩阵与有界限额（裁决全文）：**

1. **文件端口**：app-ui commonMain 定义有界读取端口（拟议 `ImportFilePickPort`：选择 → `PickedImportFile`（display name + size 元数据 + 有界读取闭包 `readBoundedBytes(): BoundedFileRead`——`Bytes(ByteArray)`/`ExceedsLimit(actualBytes)`/`ReadFailed`；因 app-ui commonMain 无 java.io 且无平台 source set，任何 `InputStream` 形态的端口签名不可在 commonMain 声明，端口形状冻结为同语义闭包，后人不得按流式签名在 commonMain 落地），display name 仅用于显示诊断，绝不入身份）；Android 实现 = SAF `ActivityResultContracts.OpenDocument`（MIME 按矩阵过滤）+ `ContentResolver.openInputStream`；Desktop 实现 = Swing `JFileChooser`（JDK 自带，无新依赖）+ FileInputStream。**不调用 takePersistableUriPermission、不保存文件副本、无临时文件生命周期产品**（重解析 = 用户重新选文件）。
2. **有界限额**：读取字节上限冻结 16 MiB、单文件解析候选行数上限冻结 10,000——超限 = 类型化批量失败（携带实收值），**绝不静默截断**；逐行 malformed 诊断沿既有 parser 语义。〔规格注册解读（已经 delta 复验确认，规格 §3.1.2）：两项新限额分别落位「平台读取层 L0」与「接治编排层 L2」；parser 层既有冻结限额（10 MiB 字节、微信 10,000/其余 20,000 物理行）原样保留（四 parser 测试零期望改动）——三层叠加，如 12 MiB 文件在 parser 层得到既有类型化拒绝；「解析候选行数」操作化为将被接治为候选的已接受记录数（被拒行不计入）。〕
3. **格式能力矩阵**（代码声明、双端编译期/运行期判别；UI 只显示矩阵声明可用的格式）：
   - 支付宝 CSV：双端可用（GB18030 运行时 `Charset.isSupported` 探测，缺则类型化 Unavailable）。
   - 招行 CSV：双端可用（严格 UTF-8）。
   - 微信 XLSX：双端可用——**生产读取改用新「有界最小 XLSX 读取器」**（jvmMain 内，`java.util.zip` + `javax.xml.parsers` SAX 流式：处理 xl/sharedStrings.xml、行/列引用、cell type s/str/n/b/e、inline string），POI XSSF **退出生产读取路径**、仅保留 jvmTest 造数（fixtures 继续用 XSSF 构造）；`poi-ooxml` 依赖从 jvmMain implementation 收敛到 jvmTest（`poi` 留 jvmMain 供 CCB HSSF）。**等价判据**：现有 `WechatBillParserJvmTest` 全部 oracle 期望零修改保持绿（换读取器后原样通过 = 等价证明），另增读取器级单测（sharedStrings/inline/数字/缺格/上限）。
   - 建行 XLS：桌面可用；**Android 诚实标记「待设备运行验证」不显示为可用**（HSSF 无官方 Android 背书）——Android 上选择 .xls 得到类型化「该格式在 Android 待运行验证」结果；待 Android 人工门（外部阻塞解除后）按 D01 验证，通过则后续批次翻转矩阵，失败则另提方案，不静默删除或宣布完成。
4. **Android 运行证据门**：四格式（含 CSV 的 GB18030 路径）的「目标 Android 运行证据」整体登记为外部阻塞待办（ALas 停止 + 设备），D01 向量保留为该门判据；本批以官方平台 API 证据 + JVM 双端语义等价测试 + 上述矩阵诚实呈现为设计侧证据。

**Q09 身份、生产 ID 与批次（裁决全文）：**

1. **身份（主代理治理翻转裁决，2026-09-14；取代原推荐方案）**：inputRef = **每次文件选择生成全新不透明 UUIDv7 句柄**（沿仓内 UUIDv7 id source 既有先例；不含 URI、文件名、内容或任何个人标识）；raw identity = `(inputRef, record ordinal)` 组合（D-098 领域 1.1 不变）；`ImportContentFingerprint` 仍按既有角色在 intake 计算（内容诊断哈希，只作交叉校验，不构成身份、不参与去重）。同一请求（同 inputRef）的重试沿 spine 既有 intake 幂等（内容等价 → NoChange 零新写入；不等价 → 身份碰撞 hard reject）——请求级重试安全不变。重选同一文件/改名 = 新 inputRef → 新候选；spine 既有 duplicateIds/`EXACT_BUSINESS_TUPLE` 匹配（`SqlDelightImportSpineStore.kt:208-257`）自动呈现为疑似重复 → 审核界面人工处置（`CONFIRMED_DUPLICATE` 阻断正式化，沿 P4-07）；**不产生第二次余额/报表影响经由此审核阻断达成**（同来源 replay = 请求幂等 NoChange；同文件重选 = 新候选 + 疑似重复审核阻断）。**零新增表（无 29.sqm）**：恢复完全依赖 v29 既有 request/candidate/snapshot/receipt/confirmation 持久化 + 新增只读查询。〔翻转依据：D-098 领域 1 第 2 条明文「规范化内容哈希……只作诊断、不构成身份、不参与去重」（`docs/DECISIONS.md:1486`）、领域 2 第 1 条「任何以该哈希参与 dedup、折叠或身份的行为继续禁止（dedup 留 P4-07）」（`docs/DECISIONS.md:1493`）；计划 §6.2 亦冻结「内容诊断哈希也不能被悄然升级为身份或自动去重键」（`docs/PHASE7_IMPLEMENTATION_PLAN.local.md:154`）。原推荐方案（inputRef 由内容指纹派生，收益为同文件重选零审核幂等）登记为**已否决替代案**，不复活；其登记过的「跨文件 NoChange 去重效应」「同 ordinal 不同内容身份碰撞」后果在随机方案下不存在。〕
2. **生产 ID 端口修订（E12 缺口闭合）**：`ImportIntakeIdSource.next()` 修订为 `next(requiredDuplicateIds: Int)`，由 store 在获胜 claim 事务内、按分支与实际匹配数（`SqlDelightImportSpineStore.kt:208-257` 语义）确定需求后调用；UI 严禁预查数量预分配（竞态禁止）。所有类型化早拒绝先于 ID 分配（沿 E13 不变量扩展到全部早拒绝路径）；拒绝/重试不消耗不应分配的正式 ID。接口默认 fail-loud（沿 P7-03 G6 纪律）。仓内实现方与测试 double 随批更新（现 14 处：ledger-data jvmTest 13 个测试文件 + ledger-application jvmTest `ImportSpineUseCaseJvmTest`；全部 oracle 期望不变）；**rgXX_/golden 竖井零改动**（golden 回放不经该接口，已核验）。〔注册解读：匹配计数查询在事务内、插入新 source 之前执行（SQLite 事务隔离内无竞态）；计数经**新增只读命名查询 `duplicateMatchCountForIntake`**（SELECT-only、按 ledger 限定、无 sourceId 自排除参数——计数先于 ID 分配与新 source 插入，被计数行全部为既有 source），零 DDL；端口形态 = 普通 interface（非 fun interface）+ 单方法默认抛 `UnsupportedOperationException`（沿 D-145 G6 先例）；写入行集与分支语义逐字节等价，既有 `size != existing.size` fail-loud 复核保留为防御性不变量。〕
3. **批次概念**：批次 = UI 会话内对勾选快照的一次明确授权；**逐项原子提交、可见部分成功**（不承诺整批事务）；无批次表——重开恢复 = 从持久化的逐候选 status/decision_snapshot/receipt 重建待确认清单（ledger-scoped，可跨文件）。〔注册解读：每勾选项 = 一次独立 spine confirm 请求（新 requestId UUIDv7/项，沿 P7-02「每个新意图分配新 requestId」）；因每项为单事务，进程重启后无歧义项——恢复清单读即权威；「未知」仅存在于会话内。〕
4. 确认时间 = 用户确认动作时间（注入 LedgerClock），来源记录时间不由处理 Clock 填补（沿 D-043 与既有 spine 语义）。〔注册解读：授权动作以 LedgerClock 取样一次、经 `explicitConfirmedAt` 通道随本授权全部逐项派发复用（同一用户动作）；mixed 候选必填（E13 门），其余类型同值传入（显式 provenance）。〕

**Q10 重复先审、Unknown 与 incomplete 边界（裁决全文）：**

1. **疑似重复先审后勾**：有重复匹配（EXACT_BUSINESS_TUPLE）或 DEFERRED 重复状态的候选，必须先完成明确审核（CONFIRMED_DUPLICATE 阻断 / 合法相似保留）才可勾选确认——这是 UI 层新增门（core 现状只有 CONFIRMED_DUPLICATE 阻断，规格如实声明该门为本批新增呈现层契约，core 状态机零改动）。**批量处置（已冻结裁决；原 Q09 开放问题闭合；P704SPEC-12 组键修订落入）**：批量处置动作 = UI 层对逐项 core `ReviewImportDuplicateCandidate` 的顺序循环，每项独立 requestId/reviewId（claim-gated、可 replay、逐项 receipt，`ImportSpine.kt:580-588` 既有形状）；core 语义零改动、无整组原子性承诺、可见部分成功；批量入口仅限「同一次文件选择句柄 + `EXACT_BUSINESS_TUPLE` + 最新重复状态 `DEFERRED`」组内整组标记 `CONFIRMED_DUPLICATE`——`comparison_fingerprint` 不作查询键（其输入含 `subject_source_id` 每候选唯一，等值分组只能得单例组），比较快照为整组确认页逐项枚举呈现的核验义务（P704SPEC-12 固定裁决）。**UI 审核决定集冻结三值**：`CONFIRMED_DUPLICATE`/`CONFIRMED_DISTINCT`/`DISMISSED_LOOKALIKE`；`REJECTED` 保留 core 值域、UI 不可达。
2. **Unknown 处置**：逐项提交后未知结果 → 暂停后续派发，按完整 snapshot/receipt 回读核对（沿 P7-02 B04 语义），不自动重试、不换 ID。〔注册解读：核对 = 同 requestId + 等价 snapshot 的 replay（spine 既有 `resolveConfirm` 返回原 receipt 判成功/冲突判冲突/仍未知）；核对后用户显式继续派发剩余项（`ResumeImportBatchDispatch`，复用同次 LedgerClock 取样与既有 requestId）或放弃（`AbandonImportBatch`，授权快照解散、剩余项即普通待确认清单项，不引入隐藏中间 UI 态）；进程重启后由持久化状态直接判定，无歧义项。〕
3. **incomplete 补事实边界**：区分「缺用户决策」（分类/账户/混合确认时间等——经 requires_confirmation 通道补决策后可确认）与「来源事实不完整」（转账腿缺失、混合腿不全、来源未解、关闭/失败无资金——**不可确认**，呈现须说明缺什么来源事实）；UI 不得编辑来源事实；未来若允许编辑恢复，须另定义追加历史转换契约，不在本批。〔规格以候选呈现分类矩阵操作化（candidate_kind/completeness/funding_state/候选状态/重复状态 → 六类呈现，纯投影零写入）。〕
4. **镜像**：镜像证据只追加到既有经济事件（P4-08 既有语义），与「确认新交易」在呈现上明确分离，绝不混为同一操作；本批不提供镜像证据追加的操作入口。

**范围冻结：** 入：四来源手动选文件→解析→候选列表/详情→补用户决策→重复审核（UI 决定集三值 + 同次文件选择 `EXACT_BUSINESS_TUPLE`/`DEFERRED` 组整组 `CONFIRMED_DUPLICATE` 批量处置）与继续/放弃批量派发出口→逐项原子批量确认→结果查询（复用 P7-03 读模型与反向血缘）→重开恢复；双端组合根接线；有界读取与限额；生产 ID 端口修订；候选/重复/恢复只读查询端口（fail-loud 默认）。出：PDF/其他银行/信用卡/微信外层加密 ZIP；自动/监听导入；文件留存与持久 URI 生命周期；整批事务承诺（含批量处置的整组原子性）；来源事实编辑；镜像确认操作化；新库引入（fastexcel/poi-android/aalto 等一律不采用）；rgXX_/golden/导入链写入语义改动；schema 迁移（**零 DDL，停留 v29**，只新增 Ledger.sq 只读命名查询——沿 P7-03 `:8741-8745` 先例）。事件/状态机：app-ui 新增导入面（P503Tab.IMPORT 第四 Tab，D-122 三 Tab 契约扩展登记）+ 新事件族按 P7-03 表 6.2a 纪律冻结（既有 ISE 不反转、新事件在设计态生效、其余态吸收）。

**评审拓扑与闭环（全部以仓库现实为准）：** 首轮独立规格评审 **REQUEST-CHANGES**（P1×1 + P2×2 + P3×8 + 评审附注）→ 主代理逐条裁决 → draft-2 单批修复（P704SPEC-01 批量派发继续/放弃出口、P704SPEC-02 架构分层归属（16 MiB 归文件选择与有界读取端口、10,000 归接治编排）、P704SPEC-03 批量重复处置冻结、P704SPEC-04 RFC 9562、P704SPEC-05/06 行号实测校正（back 扩展点 `P503App.kt:1206/:1226`；duplicateIds 子行号 size 复核 :224-226/逐匹配写 :227-240/NO_FUNDS 恰 1 :241-254/其余 0 :255-256；WechatBillParser `ZipInputStream` import 实测 :21，评审记 :22、以实测为准）、P704SPEC-07 端口形态普通 interface + 默认抛、P704SPEC-08 `duplicateMatchCountForIntake` 命名查询 + candidate→source join 粒度注记、P704SPEC-09 UI 审核决定集三值、P704SPEC-10 接治耗时披露（规格 §8 R-14）、P704SPEC-11 关联决定补 D-111、评审附注 Q08-1 裁决正文移除流式字面签名改闭包形状）→ delta 复验 **CLOSURE APPROVE**（全部修复逐项确认 CLOSED、无回归），并附不阻断新 finding **P704SPEC-12**（P3，批量处置组键修订）按评审人固定裁决落入冻结版：组键 = `import_duplicate_candidate.kind='EXACT_BUSINESS_TUPLE'` AND subject source 的 `import_source_record.input_ref` = 本次接治会话文件选择句柄 AND 最新重复状态 `DEFERRED`——「比较快照等价」不可作查询键（`comparison_fingerprint` 输入含 `subject_source_id`，每候选唯一，等值分组只能得单例组），操作化为整组确认页逐项枚举呈现隐私安全比较快照的核验义务；读取面经 `input_ref` 参数化组查询或于投影暴露不透明句柄（零 DDL、D06 安全——句柄为随机不含个人标识）；D03 族测试钉死组边界（同次选择入组、异次选择/异 kind/非 DEFERRED 出组）；规格 §3.3.1/§3.2.1/§4.5/§7 D03/§8 R-8/Appendix A 与 ARCHITECTURE/PRODUCT_REQUIREMENTS/D-146 同步。规格随本设计门冻结（approved，LF 域 SHA-256 见上）。

**实施批义务（实施批必须逐项闭合并在实施登记中报告；规格 §9 只引用不重复）：**
1. P704SPEC-12 分组键照裁决落法 + D03 组边界测试（同次选择入组；异次选择/异 kind/非 DEFERRED 出组）+ 整组确认页逐项枚举呈现比较快照。
2. `duplicateMatchCountForIntake` 与 `selectDuplicateMatches` 的 D05 族等价回归（0/1/多匹配与 statusToken-null 变体逐点相等）。
3. `AbandonImportBatch` 不引入隐藏中间 UI 态（授权快照解散，剩余项即普通待确认清单项，`docs/PRODUCT_REQUIREMENTS.md` 措辞为权威读法）。
4. R-14 接治耗时（10,000 接治上限对应最多 10,000 次独立 spine intake 事务）在 D01 Android 门内实测量化并披露（不预设阈值）。

**契约披露：** 既有锚点零改动即绿（期望值不得修改）：四 parser 测试（`WechatBillParserJvmTest`/`AlipayCsvParserJvmTest`/`AlipayCsvParserCreditJvmTest`/`AlipayCsvParserYuebaoTransferJvmTest`/`CcbBillParserJvmTest`/`CmbBillParserJvmTest`）、`ImportSpineLifecycleEndToEndTest`、`P407DuplicateClosedFullStateOracleTest`、`O2PrecisionRescaleDataTest`（:643 E13 回归）、P7-03 的 22 锚点与四只读查询；允许改动面仅限：wechat 读取器内部实现（期望不变）、id-source double 签名随端口修订（oracle 期望不变）、新增文件。金额 Long minor units checked 运算禁浮点（沿 D-145）；候选/正式分离原则（解析匹配只产候选，确认才入正式）沿 PHASE7_SCOPE_DISCUSSION:139 与 AGENTS 不变量。隐私 D06：display name/诊断不落库个人标识（沿既有 D-098/D-099 脱敏边界）；input_ref 句柄为随机值不含个人标识，经投影暴露安全。微信 XLSX 生产读取技术路径修订构成 D-099 第 2 条的修订（格式契约不变）——治理登记随本条（对齐外部证据结论：POI OOXML 路径硬依赖 `javax.xml.stream`，AOSP libcore 无该包，官方 desugaring 不含；该结论为源码级推论+官方文档混合证据，规格 §2.2 X-1..X-9 如实分级）。

**验证路由：** 高风险路由（单 bounded writer 于隔离 worktree + 独立规格评审 + 独立质量评审 + distinct verifier + 主代理关键 diff 复核与验收复跑；根 AGENTS 变更路由）；本机聚焦测试（受影响模块 jvmTest：四 parser 测试、spine E2E/oracle 锚点类、app-ui reducer/coordinator、desktop/android 组合根）+ `:ledger-data:verifyCommonMainLedgerDatabaseMigration` PASS 佐证零 DDL + `ktlintCheck` + `project_docs`；聚合门（完整 check、Android/KMP 编译、APK、Desktop build、Python 全套）以精确提交 CI 为权威证据；Android 人工门（D01 运行证据）遵守隔离 adb 协议，ALas 停止 + 设备后执行——该门为外部阻塞待办，不以设计侧证据替代。
...
11. **D01 人工门闭合与 FOUND-P704-D01-01 修复登记（2026-09-15/16）：** 第 9 项所余 ① D01 Android 运行证据门：**已闭合**——设备 AVD `ul_p7_d01`（API 36，隔离 adb server 5038，`emu avd name` 归属核实，MuMu/ALas 零触碰，收尾双 server 零残留）；固定 APK（`android-debug-apk-10ef6b0c3827b5861677ff0860cbbb1bf4e6aa4b`，SHA-256 `6DD572…1161`）；格式矩阵 11 向量 PASS + 1 PARTIAL（权限撤回 typed 文案 AOSP 不可确定性注入，如实披露，语义由 JVM 等价钉死）；义务④（10,000 全接受接治）墙钟 **192 s / 196 s** 两次独立实测（≈51 tx/s；恰好 10,000 条原子落库、quick_check 通过、正式账零写入、同内容重选 10 条 EXACT_BUSINESS_TUPLE/DEFERRED）；SAF 实机观察完成（roots 抽屉/内部存储/MIME 过滤/一次性授权/重启无再授权/查询实时回收）；裁决文书 `local/artifacts/d01-p704/d01-gate-verdict-2026-09-15.md`（本地工件）。建行 XLS Android 列维持平台门 typed 呈现（实机判读为待运行验证，未自动翻转，如实登记）。
   同门发现缺陷 **FOUND-P704-D01-01**（实机四次复现）：cap 规模（10,000/10,039 候选）进「导入」Tab 即 Compose FATAL OOM——`P503ImportScreen` 外层 `Column+verticalScroll` 急式全量组合（基线 `P503ImportReview.kt:357`/`:426`）；R-14 末句「结果摘要呈现进度」与 P7-04.E 列表环节 cap 规模不可达（接治数据层完整原子，记录零放弃，D02 级零正式效果保持）。
   修复（实施分支 `UL-p7-fix2`，基线 `10ef6b0`，提交 `feee3cb`，3 files +831/−97）：概览改为 `LazyColumn` 消费纯渲染模型 `importReviewRenderItems`（19 子型、驻 `P503ImportReviewPresentation.kt` 零 Compose API、唯一 `stableKey` 与按形状 `contentType`）；候选清单零截断（无新显示上限；唯一登记截断先例 200 条披露行原样复用随行）；批量结果保留段逐行展开同享窗口化；全部文案/冻结顺序/条件门/无障碍标签逐字等价（C1–C6：六类冻结顺序与空组省略、组头计数、notice 位置与压制、双空态、P704SPEC-12 会话句柄门、勾选非空批量确认门、处置卡条件、Unknown 核对入口、「待设备运行验证」诚实呈现）。新增 14 个 commonTest 纯断言（`P503ImportReviewRenderItemsTest`）钉住映射：双空态、notice 位置、六类冻结顺序+空组省略+组头计数、勾选传播、会话摘要 200 披露（251→201 行含「其余 51 条」原文复用）、批量逐条展开+Unknown 核对项、key 唯一/确定性、cap 10,000 形状钉（10,010 条目、首末次序、零丢失零截断）。
   验收拓扑（高风险路由）：单 bounded writer（零 Git 写）→ 独立规格评审 **APPROVE**（S1–S6 全 PASS、无阻断，SPEC-01～04 咨询级）→ 独立质量评审 **APPROVE**（Q1–Q7 全 PASS；强制条件 = 残余项登记 + distinct verifier + 人工门保留）→ distinct verifier **V1–V8 全部 PROVEN**（`:app-ui:jvmTest` XML 20 套件/332 用例 0 失败（318 既有+14 新增双通道互证）、`:app-ui:ktlintCheck` 三报告 0 字节、`project_docs` exit 0、presentation 零 Compose import 复证、`Ledger.sq:7685` `PRIMARY KEY (ledger_id, candidate_id)` 旁证（质量评审坐实点引用 :7684 行号偏 1，以实测 :7685 修正）、Gradle daemon 收敛）→ 主代理临界 diff 亲检（容器转换、条件矩阵逐门对基线、文案抽查逐字）与受影响全套亲跑（332/332、ktlint、project_docs 复证）。
   **登记残余项（不得静默关账）**：整组处置卡（`ImportDuplicateGroupDispositionCard`）保持单 item 内急式组合——非回归（修复前同类路径因概览先 OOM 不可达）、不在四次复现路径；cap 规模疑似重复组打开整组确认页时同类 OOM 理论可达（组规模 ≤ intake cap 10,000）；处置 = 后续批窗口化处置卡枚举、或 D01 族人工门增补 cap 组开卡向量；随本登记挂起，见 `docs/WORK_PLAN.local.md` 未决事项。器件级 cap 规模列表渲染可达性复测（含上述残余向量候选）归 D01 族人工门；聚合编译（Android/KMP/Desktop）与同提交 CI 由后续 push 经用户授权后以 CI 为权威。本修复改变量仅限：呈现层渲染结构；零 reducer/零 application/零 data/零 schema/零构建文件/零依赖变化。

12. **账本 selector 缺位裁决（2026-09-16，短程补钉登记）：** B 侦察（独立只读探索）确认 ground truth = 代码层与设计层均**不存在** ledger selector/切换/active-ledger 持久化——`P503AppState.kt:279-284` `P503Tab` 四枚无 ledger 项、`P503TabShell.kt:70-100` 四 NavigationBarItem 无任何 selector/picker、`P503StartupScreen.kt` Starting/Ready/StartupError 三态无「选择账本」分支、`P503App.kt` Ready 态直接落 P503TabShell；两级组合根硬编码常量 `android-app/.../App.kt:283` 与 `desktop-app/.../Main.kt:318` 均为 `LedgerId("ledger-local-test")`、Desktop 端 `Main.kt:132` 文件名常量 `ledger-local-test.db`。D-025（多账本领域权能，DECISIONS.md:295-305）与 D-144/D-145 范围冻结（D-144「不做…多账本」:2606、D-145「不做多账本、不做多币种汇总…」:2646）之间的空隙是事实缺口而非实现缺位——呈现义务在 P5/P7 范围冻结里全部明文排除。B 侦察更正（与 B 证言补记一致）：此前 D01 会话把「在 P7-01 新建一级分类对话框里 exit/cancel 后 device 无法返回 main」**误记为「P7-01 新建账本对话框**；真实对话框 = 「新建一级分类」（`P503CatalogManagementScreen.kt:200-202/:340`）；20k 候选污染的 ground truth = 换烤 input_ref（UUIDv7 新句柄）后 fresh `Accepted(records=10000,newCandidateIds=10000)` 对主 10k 叠加——intake 语义**本身正确**，不是 bug。
...
**B5 状态更新（2026-09-19，A-DOC 批补登）：** 本段登记项已由 A-PERF 后续排查批按本段处置要求以更细粒度方法重测并**决定性通过**：20k 库（产品 SAF 路径铺设，`sqlite_stat1` 导入统计满足层0）、当前 main 候选 APK 绑定下，以设备侧 `/proc/uptime` 括号（单 shell up0→tap→up1，括号 ~70ms）+ `dumpsys gfxinfo framestats` 首帧计时（InputEventId 判别 tap 消费帧）三轮读数 0.049/0.049/0.056s（max 0.056s）通过 ≤1s 门槛；历史 4.92s/~2.4s 读数确认为 uiautomator dump 地板（~2.29s）伪影。测值为上界（up0 读取与 tap 派发开销 ≈0.3–0.4s 含于测值内）。本段「未获证实」状态就此解除；原登记文字不重写，本段为状态更新。
...
## D-151 P7-02 A-02 设备验收缺陷修复批（置顶状态接线 + 确认页类型化呈现）

**状态：** 已批准（2026-09-18，按 A-02 设备验收的 OPEN 缺陷清单分批修复；本条为 `defect-A02PIN-001`、`defect-A02PIN-002`、`defect-A02CONFIRM-001` 三个缺陷的修复批登记）。规格：`docs/specs/2026-09-18-p7-02-a02-defect-fix-design.md`（approved）。

**决定（纯 UI 状态接线与呈现，零账务、零 schema、零迁移、零依赖）：**

1. **置顶状态接线（A02PIN-001 / A02PIN-002）**：`TogglePin` 事件由「只带 target」扩展为携带 store 的权威结果 `pinned`，reducer 由「自行翻转」改为「设为期望值」——渲染副本与持久化状态因此不可能反向分叉（幂等：目标已在集合中再置顶不变，已不在集合中再取消也不变）。5 处编辑器流 `Back` 重建 `OverviewEmpty` 时显式携带 `pinnedTargets`，流状态相应新增该字段并随流贯穿（含冲突/拒绝屏的字段编辑、`AbandonConflict`、提交失败分支与 `UnknownCommit` 的冲突解析），`SelectTab` 的集合保留由本批测试覆盖固定（基点提交已通过 `state.copy(...)` 保留），READ 重试刷新在事件缺省时新增回退到保留的月度概览（`state.monthlyOverview?.pinnedTargets`）。置顶成功后宿主以同一权威结果重新派生管理快照并随事件安装，使账户/分类管理列表排序即时生效（A02PIN-001）；采用「事件携带快照」而非复用 `CatalogSnapshotRefreshed`，因为后者的分支只清空 `catalogNotice`，会意外抹掉已打开的管理横幅（该分支不改动 `catalogDialog`）。普通 `RefreshResult` 的 `pinnedTargets` 改为可空缺省（缺省保持现值、提供则采用），刷新因此可自愈分叉。**store 的翻转语义、`EntryPinOrdering` 排序派生语义与 `entry_pin` 表结构零改动。**
2. **确认页类型化呈现（A02CONFIRM-001）**：确认页标题与字段按类型渲染（确认支出 / 确认收入 / 确认转账 / 确认借出 / 确认收回），不再对转账、借出、收回套用支出模板。字段集合按类型：支出（支付账户、费用分类、金额）、收入（收款账户、收入分类、金额）、转账（转出账户、**转入账户**、到账本金、手续费、手续费分类仅手续费 > 0、**转出总额**）、借出（往来对象、出资账户、借出金额）、收回（往来对象、到账账户、本金、利息、利息分类仅利息 > 0、**实收总额**）；**往来对象与手续费/利息分类一律不标注为「费用分类」**。
3. **金额消歧（关键产品口径）**：转账**同时**呈现「到账本金」与「转出总额 = 到账本金 + 手续费」，收回**同时**呈现「本金」「利息」与「实收总额」，**不做单值取舍**。依据：`PRODUCT_REQUIREMENTS.md:11` 要求内部转账与对外收支在用户界面中明确区分，`ACCOUNTING_RULES.md:76` 要求确认须含精确组成金额；并列呈现两值可同时满足两条且不丢失任何信息——单一金额在转账上会被误读为扣款额（到账本金不等于实际扣款），在收回上会隐藏本金/利息构成。金额一律精确十进制（复用共享 minor-unit 解析与渲染，**禁二进制浮点**）：转账总额为精确加法，收回总额直接渲染草稿已携带的 `totalReceived` 而不重算。呈现逻辑落为纯函数（`app-ui` 的 `P503ConfirmationPresentation.kt`，零 Compose API），以 commonTest 覆盖，**不引入 Compose UI 测试基建**。
...
**与 D-152 的关系：** 复用 FIX-MONTH-1 的同一 arm/consume 机制——`onImportBatchConfirmed()` 触发权威刷新并置位同一 `pendingMonthlyReRequestAfterRefresh` 挂起标记，由同一落地跳消费：成功落地 `consumeMonthlyReRequestAfterRefresh`（恰一次无条件重请求，落月盖章抑制随后的 (a)/(d) 守卫；与 `P503CurrentStateLoadCoordinator` 合并重跑兼容——标记存活到首个成功落地）、失败落地 `dropMonthlyReRequestAfterFailedRefresh`（清标记不请求）。触发点有二，共用同一共享置位门 `shouldArmImportBatchConfirmed`（落地态为带保留摘要的 `OverviewEmpty` 且摘要无 Unknown → 置位）：`runImportBatchDispatch` 的 finally 主线程跳 `completed == true` 分支，与 `checkImportUnknownItem` 的核对结果落地跳。该门保证一次批量运行跨全部顺序恰置位一次（审查 F-1 及其证伪顺序的修正）：完成且全部项终态（自动离开）→ completed 分支置位；完成但仍有 Unknown（状态仍 `ImportBatchSubmitting`，Unknown 阻断自动离开）→ 不置位，核对补完时由落地跳置位；批中核对 → 不置位，后续 completed 分支置位；核对使全部项终态 → 落地跳置位；StillUnknown/被吸收/摘要仍有 Unknown/无保留摘要 → 不置位。暂停不触发。(e) 分支、consume/drop、(a)/(d) 守卫与 §6.2 吸收表零改动；(a)–(e) 语义逐字保留，仅增补 (f)。
...
## D-154 P7-04 A05IMPORT-INCOME-FACE-001 决策面裁决：导入候选决策面按 face×方向渲染分类源
...
**决定（FIX-INCOME-FACE-1，决策面分类源按 face×方向匹配）：** 导入候选决策面（P503 候选详情「补齐决策」表单）的「分类」区按 face 与候选结构化 token（候选 detail 行 `candidateKind`/`directionToken`，与 commit 路径读取同字段）渲染匹配的分类源。四个 requiresCategory 面的选项源矩阵：仅 `ordinary_flow` 面按方向渲染——token "in" → 收入分类选项（入口收入流同源的叶 INCOME 分类，满足 `SecondaryCategoryRequired` 校验；`OrdinaryFlowFormalFactory` in → 收入入账 commit），"out"/null → 支出分类选项（现状不变）；`credit_expense`（直付与 `credit_expense_refund` profile）与 `mixed_payment` 面恒渲染支出分类选项——这些面的 commit 要求 EXPENSE 分类，其中退款 commit（`createCreditRefundReceipt`）要求原支出的精确二级 EXPENSE 分类（`RefundReceipt` 校验 kind != EXPENSE → `InvalidRefundReceipt`），退款行方向 token 硬编码 "in" 不得翻转其选项源（评审 F1）；`transfer_flow` 面（转出/转入）无「分类」区。理由：仅 ordinary_flow 的 commit 按方向分派，而决策表单此前只渲染支出选项，in 方向 ordinary 候选无法获得合法决策，批量提交被 spine 域校验类型化拒绝（`IncomeCategoryRequired`，零写入）——收入方向导入候选在产品路径上永远不可确认。校验行为本身正确，缺陷在决策面不提供合法选项，故本条为纯呈现面修复：决策草稿/校验/commit 路径零改动（`ImportDecisionFieldUpdate.Category` 事件同型）。

**验证证据：** 独立 verifier（2026-09-19，候选 `2381b6e`）ACCEPT：V1 决策面按 face×方向渲染分类源的纯选择器与调用点接线（`P503ImportReview.kt`、`P503App.kt`）；V2 commit/校验/草稿路径零改动；V3 变更集恰 5 文件（`git diff --name-status ba029cf 2381b6e`）；V4 测试真实非恒真，`:app-ui:jvmTest` 实测 402/0/0/0（= 基线 397 + 5 向量），`:app-ui:ktlintCheck`、`:android-app:compileDebugKotlin`、`:desktop-app:compileKotlinJvm` 全部 exit 0（`--no-daemon --max-workers=1 --rerun-tasks`，BUILD SUCCESSFUL 5m59s）；V5 变更代码与构建面隐私零命中；独立 verifier 的唯一 finding 为规格文档内的工具/角色名（低），已随本批修正。设备回归（2026-09-19，AVD `ul_p7_d01` API 36，设备内 APK 的 base.apk sha256 `397fa05774611f8fd398b839866d2e5423a71e1680b46eb359f923123068662b` 与 `2381b6e` 构建完全一致）：导入 CCB 样本 13 候选（12 可确认 + 1 来源不完整）→ in 方向 ordinary 两项（10.00/8.80）决策面「分类」区实际渲染收入分类 `A02FIX-SAL-C1` 且无支出分类泄漏（逐项 uiautomator dump 留证）→ 两项按 `A02FIX-SAL-C1` + `默认支付账户` 补全并勾选 → 批量确认 12/12 入账（`import_confirmation` 12、`ledger_transaction` 12，零拒绝）→ DB 出现两条 INCOME posting（`默认支付账户` +10.00/+8.80 ↔ `A02FIX-SAL-C1` −10.00/−8.80，逐笔平衡）→ 未重启应用时首页当次会话已反映 12 笔（触发 (f)，D-153）→ 2026-08 月卡「交易 12 笔；CNY：普通收入 18.80，净支出 53.81，结余 −35.01」与 DB 逐值一致。设备回归证据按隐私边界归档于本地证据目录，不入仓。
...
1. **正式状态同步 D-148～D-154**：`CURRENT_STATE.md` 登记此前缺失的七条决定及其批次内容——D-148（A-PERF 导入候选读治理批：层0 统计刷新/层1 定向读/层2 主线程 catalog 读治理；B5 定向详情 ≤1s 门槛曾未获证实、后经 A-PERF 后续排查批重测决定性 PASS，状态更新见 D-148 条目；OBS-APERF-INPUT-FREEZE 与 50k 冷读两条范围外观察项）、D-149（建行 XLS Android instrumented 验证、能力矩阵翻转与产品路径设备验收——四格式双平台可用性由此以设备证据确立）、D-150（A-DOC 批与 CCB fixture 内嵌 WriteAccess 隐私清理）、D-151（P7-02 置顶状态接线与确认页类型化呈现）、D-152（P7-02 提交后刷新链缺陷修复）、D-153（P7-03 触发集 (f) 增补）、D-154（P7-04 决策面按 face×方向渲染分类源）。各批同提交 CI 三 job 全 success（本条自带 CI 证据链）：D-148 run `35280369455`、D-149 run `35289045109`/`35301225076`、D-150 run `35334540478`、D-151 run `35383765551`、D-152 run `35400302039`、D-153 run `35419996777`、D-154 run `35436021944`。本条闭合评审发现 D154CLO-007（`CURRENT_STATE.md` 尚无 D-148～D-154 记录，A-05 收口时必须同步）。
2. **A-05 事实登记（部分完成，不整体 PASS）**：A-05 导入全链验收补链按 `docs/PHASE7_REMAINING_IMPLEMENTATION_PLAN.local.md` §10.1 逐项登记——PASS 项：D01-1～D01-8/D01-10、D02-1～D02-3、D03-1、D03-4、D04-1、D04-2、D04-5（已成功保持一次，JVM）、D05（JVM 维度）、D06-1～D06-3；裁决登记项：D01-9 权限撤回缺证（PARTIAL 保留，计划 §2 A-05 行「权限撤回原 PARTIAL 保留，补可控平台测试」；承接=平台或环境提供确定性注入点时按 §10.2 重开复测）、D03-2 不适用、D03-3 接受延期（JVM）、D04-3 接受延期（JVM）、D05 设备维度不适用；开放项：D04-4 批量中段进程中断注入待补证、§10.3 的 10k 级流水规模项待明确裁决、`A05BATCHITEM-STALE-SELECTION-001`（低，OPEN）。
3. **状态口径修正**：`CURRENT_STATE.md` 的阶段 7 head 声明由 `c93e18a` 更新为 `23d3bd2`（D-150～D-154 各批及其文档批均已合入本地 `main` 并推送 `origin/main`）；A-PERF B5 定向详情门槛由「未获证实」更新为重测决定性 PASS（历史读数与测值上界口径保留，D-148 条目附状态更新）；D01 权限撤回向量按计划 §2 A-05 行「权限撤回原 PARTIAL 保留，补可控平台测试」修订（原表述「归 D01 族后续设备窗口」不再成立：AOSP 无确定性注入点、可控平台测试在位，分母保留、不记 PASS；承接=平台或环境提供确定性注入点时按 §10.2 重开复测）；四格式可用性与 D01 披露式闭合继续表述为两个不同命题（披露式闭合只证成诚实披露义务，可用性由 D-149 的设备证据另行确立）。
4. **A-01～A-03 设备验收状态登记（正式状态文档此前未记录）**：A-01（P7-01 账户与分类目录管理）全部向量 PASS（设备路径 + 权威 DB 核对；另登记 UX 观察 `OBS-A01-SILENT-REJECT`）；A-02（P7-02 五类型录入与恢复）计划行整体 PASS（写入侧五类型与计划指定借贷向量逐值成立；呈现侧五缺陷经 D-151/D-152 与设备回归全部 CLOSED；计划点名的 B06 金额算式设备向量已执行——`12.5+8`/`2*(3+4)`/`10/4` 结果与「应用到金额」写入 PASS，`1/3` 与 `1-2` 按 E-3 类型化拒绝，全向量零正式写入）；A-03（P7-03 看账）部分完成——A03-1～A03-6 与 A03-8a PASS、A03-7 长列表向量与 A-05 窗口合并后 PASS，TB1～TB7 TalkBack 朗读巡检接受延期（用户 2026-09-19 裁决）与 A03-8b 缺证按 §10.1 单列披露（不记 PASS、分母保留，承接见 CURRENT_STATE「未完成门槛」）。
...
**状态：** 已批准（2026-09-19，主代理按常设授权「除不 push 外默认采用推荐方案」、经独立设计取证评审后裁决 Q11/Q12 与规格 §6 决定点 DP-1..DP-13）。设计门规格 = `docs/specs/2026-09-19-p7-05-correction-void-recycle-design.md`，**状态：approved**（独立规格评审与独立质量评审终局 APPROVE WITH CONDITIONS，P3 残余已随 draft-4 修复；DP-9 澄清与 DP-10 收窄见下）；本条只登记设计裁决，实施按高风险路由另行建立。
...
**决定（P7-03 读模型重冻结，2026-09-19 规格评审 P705QUAL-004）：** 有效谓词必须施加到**全部**交易派生读面（规格 §3.6 枚举 14 面）；本批**重开并重冻结** D-145 的读模型语义——「current-version 即有效」重冻结为「current-version **且未被作废**」。受影响锚点：`currentVersionRowsForLedger`（`Ledger.sq:8713-8740`，HOME 账户余额与「当前交易」回退）、`QueryLedgerCurrentState.transactions`/`balances`（`QueryLedgerCurrentState.kt:64-92`）、`SummarizeLedgerActivity` 输入语义（`SummarizeLedgerActivity.kt:48-99`，Analysis Tab）、`ledgerEntryRowsForLedger`（`Ledger.sq:8748`，月度/流水/详情）。D-145 登记的 22 个既有测试锚（`SummarizeLedgerActivityTest` 7 / `QueryLedgerCurrentStateTest` 9 / `SqlDelightLedgerCurrentStateReadAdapterTest` 6）在无作废事实的账本上逐值不变（谓词为 no-op），故不重写既有期望、只新增向量；`ACCOUNTING_RULES.md:299` 的读模型口径、D-145 规格表述与 `ledger-application/.../LedgerEntryReadModels.kt:18` 的 "Existing [CurrentVersionRow] loads are untouched" 注释（本批后不再成立）随实施批同步重述（A-DOC 同步清单）。

**边界（本批）：** 本条为**规格批**：只写设计门规格与决定登记两个文件，零产品代码、零测试、零 schema/迁移、零版本号预占、零依赖；计划列为「可复用」的四个机制（`TransactionNoteReplacement.kt`、`ConfirmedTransactionNoteUpdate.kt`、版本/CAS 对、`P408CorrectionCommitPort.kt`）**当前均不可由产品路径到达**，本批不改变该事实（DP-10 收窄后首切片不使用该 port；其落地属后续实施批）；首切片必须新增 schema 边（作废/恢复事实 owner），版本号在实施时分配。不改 `rgXX_` 竖井/golden，不改对账 owner、借贷历史与导入 owner 的写入语义；不改 P7-01/P7-02/P7-04 冻结面，P7-03 读模型按上段重冻结（受影响锚点如上，其余冻结面不变）。实施前须先闭环独立规格评审（本轮修复后为评审候选），并按高风险路由实施（隔离 worktree、单一 bounded writer、独立规格/质量评审、distinct verifier、主代理最终验收）。

**关联决定：** D-047、D-048、D-098、D-113（UQ-1 跨层集成延后至转账切片，见 DP-10 修订；其余边界不变）、D-144、D-145（读模型按本条目重冻结）、D-146、D-154；阶段计划 §5（P7-05）与 Q11/Q12。

## D-157 A-05 导入全链验收收口裁决（D04-4 补证与 §10.3 规模项）

**状态：** 已批准（2026-09-19，A-05 导入全链验收收口裁决：D04-4 批量中段进程中断由「缺证/PARTIAL」转为 PASS、A-02 金额算式（B06）设备向量 PASS、计划 §10.3 大数据整链规模项裁定为「缺证（部分证据在位）+ 承接」；本条为验收裁决登记，不引入产品行为变更，不改动既有批次裁决——零产品代码、零测试、零 schema/迁移/依赖，schema 维持 v30）。

**决定：**

1. **D04-4 批量中段进程中断：PASS（由「缺证/PARTIAL」转为 PASS）。** 证据与 D-154 设备回归同一候选绑定（`2381b6e` 构建，设备内 `base.apk` 哈希逐字节一致，见 D-154 验证证据段）。批次 = 第二次产品路径导入 CCB 样本（「新增 13」）产生的重复候选组：6 项逐项处置「不是重复」（CONFIRMED_DISTINCT）、完成决策并勾选，确认屏呈现「确认入账（6 项）」。注入采用**设备内定时器与确认 tap 同一 shell**（`(sleep 0.15; …; am force-stop <应用包名>) & input tap <确认入账>`），应用在 **k=2/6** 处被杀死。重开核对（恰一次）：`ledger_transaction` 34→36（+2 = 已提交前缀 k）；`import_confirmation` 34→36（receipts = claims，1:1）；`posting` 68→72（2 笔交易 × 2 腿）；`pending_confirmation` 13→11（恰消费 2 项，其余 4 项未动）；36 个 posting set 全部恰 2 腿且逐笔平衡至零；`import_confirmation` 无重复 candidate（0）；`PRAGMA quick_check` = ok。结论：**无半提交（逐项原子：前缀 k=2 已入账、余 4 项未动）、无重复经济效果、无部分写入**。
2. **方法事实（登记，决定此前尝试的成败）：** (i) 主机侧往返过慢——`uiautomator` dump 单次约 2.5s，主机发出 tap 后 0.4s 计划执行的 kill 落到整批提交完成之后；定时器与 tap 必须在**同一设备 shell**内发出。(ii) 进度触发注入**不可行**：批量提交期间 `run-as … sqlite3` 返回陈旧计数（工件登记见 `local/artifacts/d01-p704/a05-execution-checklist.md` §12）；轮内读取停在提交前的值、而主机侧查询已看到新行，该对照为主代理本轮运行期间的直接观察（未入工件）。(iii) 提交窗口为**亚秒级**（7 项在 <0.4s 内完成）。(iv) 确认前的决策与勾选为**内存态**（数据库与应用文件均无草稿副本，实测）；back 级联退出应用（叠加穿过详情→列表→桌面）即丢失。
3. **A-02 金额算式（B06）设备向量：PASS。** 入口页「金额算式（可选：+ - × ÷ 与括号）」+「计算」渲染 `= <值>` 结果行与「应用到金额」：`12.5+8` → `= 20.50`（应用后「金额」字段 `20.50`）、`2*(3+4)` → `= 14.00`、`10/4` → `= 2.50`；`1/3` 与 `1-2` 按类型化拒绝（文案分别为「算式结果无法用该币种精度精确表示（如 1/3 或 1/8）」与「算式结果为负」）。全向量期间正式账零写入（`ledger_transaction` 36 / `import_confirmation` 36 / `posting` 72 不变，`quick_check` ok），入口页未提交关闭。与 D-155 第 4 条登记一致。
4. **计划 §10.3 大数据整链规模项：裁决为「缺证（部分证据在位）+ 承接」，不记 PASS。** 计划原文摘引（`docs/PHASE7_REMAINING_IMPLEMENTATION_PLAN.local.md:188`）：固定匿名数据至少覆盖单次 10k 导入与多次累积 30k 候选、20k 重复组，并保留无重复与部分已处置状态；必须覆盖 冷启动→首页可操作→进入导入→解析/接治→收尾重读→列表可操作→滚动→候选详情→重复组打开→审核/确认→详情与月度刷新→杀进程后重开；同处续句要求**不得每测一步就清库**（以免隐藏累积规模与状态交互问题）。同节 `:190` 另有逐阶段核对义务（首/末项可达、完整计数与稳定排序、勾选/组处置范围、正式效果次数、拒绝零写入、计时、峰值内存、崩溃/OOM/ANR 记录；合法规模下 ANR/OOM 为未通过），`:192` 要求 P7-05～09 后按影响复验累积数据上的查询、预算/标签商家统计与备份恢复。**在位部分证据**：D01 门的 10k 接治实测（`local/artifacts/d01-p704/d01-gate-verdict-2026-09-15.md`）；A-PERF 的 20k 级库与修复后 B2/B3/B5/B6 测量——B6 的 20,000 条组冷开 2.61s 位于 ≤3s 门槛内，测于累积至 **30,039 候选 / 30,010 重复候选**的库（`local/artifacts/a-perf-p704/retest-report.md` 会话 4），不是后续经产品 SAF 路径重铺的 20,000 候选 / 10,000 重复候选库（该 20k 级库修复前 B6 组开卡为 ANR 基线，见 `baseline-report.md`）；B5 定向详情门槛经更细粒度方法在经产品 SAF 路径重铺的 20k 库上重测决定性 PASS ≤1s（`local/artifacts/a-perf-p704/aperf-followup-plan.md` §5；状态更新见 D-148）。**候选差异说明（计划 §10.2）：** D01 门 10k 接治实测绑 `10ef6b0`（当时设备库 `user_version=29`），与当前候选存在 schema（D-147 升至 v30）与候选读路径（D-148）差异；解析与接治语义对 10k 接治未变，差异以差异说明 + 轻量重钉三场景承接（`local/artifacts/d01-p704/a05-execution-checklist.md` §4），不以旧 APK 的成功替代当前版本；A-05 设备窗口候选 `2381b6e` 与当前 head `76034be` 的差异经核实为**纯文档**（`git diff --name-only 2381b6e 76034be` 仅触及 `README.md` 与 `docs/`，无产品代码），故该窗口的设备证据在当前候选上以差异说明承载有效、不需重跑。**缺证**：所需规模的**单一连续会话**覆盖上述整链全流程端到端；30k 累积候选状态（上述 30k 库读数只覆盖 B6 单向量，该累积库状态未保留，也未在该规模执行整链）；早期 50k 库（已丢失，须重铺）；`:190` 的逐阶段核对义务与 `:192` 的 P7-05～09 后累积数据复验义务在所需规模上均无证据。**承接** = A-PERF 后续排查批（`local/artifacts/a-perf-p704/aperf-followup-plan.md`）：经产品路径重铺 20k/50k 库、执行受影响向量并覆盖 `:190` 的对应核对；`:192` 的复验按计划在 P7-05～09 各批验收时履行；该规模项在此证据出现前维持非 PASS。
5. **A-05 总状态：部分完成，不整体 PASS。** PASS 项：D01-1～D01-8/D01-10、D02-1、D02-2、D02-3（JVM）、D03-1、D03-4、D04-1、D04-2、D04-4（本条）、D04-5（JVM）、D05（JVM 维度）、D06-1、D06-2、D06-3。裁决登记项：D01-9 权限撤回缺证（PARTIAL 保留——本条不改变其非 PASS 状态；承接 = 平台或环境提供确定性注入点时按计划 §10.2 重开复测）、D03-2 不适用（设计退化）、D03-3 接受延期（JVM 证据接受）、D04-3 接受延期（JVM 接受）、D05 设备维度不适用。剩余开放项：第 4 条的 §10.3 规模项；`A05BATCHITEM-STALE-SELECTION-001`（低，OPEN）——本轮再次实证（批量确认成功后已终结候选仍计入下一批选择集，观测计数与列表实际勾选不一致），且本次注入批次的计数正确（6 项）。按计划 §10.4.5 的规则（仍有必需项非 PASS 时报告部分完成，不得因已推送、CI 全绿、评审通过或仅余设备问题宣布整体完成），**无残余被并入 PASS**。
...
## D-159 A-05 §10.3 规模项承接复核裁决（50k 遍历受 INPUT-FREEZE 阻断与程序化滚动要求）

**状态：** 已批准（2026-09-20，A-05 导入全链验收 §10.3 大数据整链规模项承接复核裁决：登记 50k 库上的就地证据增补、剩余缺证的实测成因与承接细化；本条为验收证据与裁决登记，不引入产品行为变更、不改动 D-157 的非 PASS 裁决——零产品代码、零测试、零 schema/迁移/依赖/清单，schema 维持 v31）。

**决定：**

1. **50k 就地证据（步骤之间不清库）**：在 50,000 候选库（同一 fixture 经五次产品 SAF 路径导入铺就，`sqlite_stat1` 统计在位）上，按计划 §10.3（`docs/PHASE7_REMAINING_IMPLEMENTATION_PLAN.local.md:188`）的顺序执行且**不在步骤之间清库**，下列阶段通过：冷启动→首页可操作（`am start -W` TotalTime 2,813–3,324 ms；首页首帧 Δ 2.010–4.020 s；**同一命令的同期 50k 冷读分解另有 10,332 ms 读数**——该读数取自另一次运行、库处于已沉降（较暖）状态，两个读数并列登记、不取其一）、进入导入→列表可操作（Δ 1.060–1.274 s）、候选清单可见、候选详情打开（首行候选详情 Δ 2.924 s）；**另一条独立观察**：重复候选详情的「重复比较」段完整渲染。**同期分项测量**（同一证据周期内独立执行，作为该规模项的上下文）：(i) (f)/FIX-MONTH-1 的 20k 向量三轮，取最大值 **tap→commit 9.970 s、tap→lastframe 12.474 s**（**限定**：三轮非同质——r1 的 2.870 s 明显快于 r2/r3 的 ~9.5–10 s，且 r3 复现 `A05BATCHITEM-STALE-SELECTION-001`）；(ii) INPUT-FREEZE 阈值——20k **人速 5/5 不触发**、**机械风暴第 6 次触发**，50k **第 2 次触发**（阈值随规模显著提前）；(iii) 50k 冷读分解**未复现 r7 记录的 31.62 s 量级**，且 20k 基线为单次样本、其库随后已被 50k 覆盖，该对比属**基线参照而非同质对照**。证据与测量方法见 `local/artifacts/a01-a05/`（`aperf_scale_session.py`/`aperf_freeze.py`/`aperf_coldread.py`/`aperf_f_run.py`/`aperf_f_round.py`/`saf_import.py`）。**数据条件覆盖（计划 §10.3）**：本条证据覆盖「多次累积 30k 候选」（50k 库、五次产品 SAF 导入、步骤之间不清库）；**未覆盖**：「单次 10k 导入」（仅见 `docs/PROGRESS_LOG.local.md` 的 D01 门记录，不在本证据集内）、「20k 重复组」数据集（本条出现的「20k」为计时向量与冻结阈值，不是重复组数据集）与「保留无重复与部分已处置状态」两项状态条件——验收分母据此显式登记。

2. **候选绑定与 §10.2 差异说明**：本条设备证据绑定候选 `70a8c71` 构建的 APK（sha256 `117c7b9137996e8ed4b2e57ed4fdf098dac0e50f68003181e49e3cedd1f1c2b8`；证据目录 `local/artifacts/d01-p704/apk-70a8c71/`），当前 main = `4588aaa`（schema v31），按计划 §10.2 复用较早候选的设备证据须以差异说明承载。D-158 第 5 条记录：v31 有效谓词为 `currentVersionRowsForLedger` 与 `ledgerEntryRowsForLedger` 各增一次相关 join——正是「冷启动→首页」与「详情与月度刷新」阶段背后的读查询（后者仍属未完成的剩余阶段）。差异评估按「库事实状态 → 谓词 no-op → 阶段不受影响」展开：该 50k 库为 **0 交易、0 作废事实**，有效谓词的 join 返回与变更前相同的行集，故 v30→v31 的读路径变更对**在本库上测量的阶段**不具实质影响，证据以本差异说明承接、无须重跑。**该结论仅适用于空交易库上取得的证据，不适用于任何在含交易库上测量的阶段。**

3. **剩余四阶段的实测成因（机械滑动遍历触发 INPUT-FREEZE）**：以每探针 10 次快速 fling 推进 50k 列表，约 **50 次 fling 后停止推进**（可见行在连续探针之间钉在 `4.07 CNY`；123 s 内可见行净推进约 300 行，折合约 2.4 行/s、约 6 行/fling）。**速率口径登记**：该 300 行是整段探针序列中可见行的**净推进**（含探针之间的读取开销），不是单次手势的瞬时速率；与已记录的快速 fling 约 **134 行/次**（`docs/PROJECT_STATE.local.md` 工具教训④）相差约一个数量级，现有工件不足以区分「净推进口径差异」与「冻结前推进已退化」两种读法——后者若成立即为待查发现，由程序化滚动承接一并表征；本条不把约 6 行/fling 当作稳态列表速率。冻结态与 `OBS-APERF-INPUT-FREEZE` 逐项吻合——列表忽略 200 ms 慢速滑动、**BACK 仍响应**、`top` 显示 **CPU 0.0%**、`dumpsys input` 显示 **`PendingEvent: <none>`**、force-stop 后恢复正常。该冻结**阻断的正是 §10.3 的遍历本身**，而非仅探针粒度问题；此前把失败归因于粗/细探针取舍的记录**由本测量取代**。**替代输入通道在该规模不可用**：键盘滚动（`KEYCODE_PAGE_DOWN`）每次约推进六行，`KEYCODE_MOVE_END` 与 `KEYCODE_DPAD_DOWN` 无有效推进，轨迹球 `input roll` 完全不滚动列表——**该三条读数为主代理本轮运行期间的直接观察（未入工件）**，证据目录内无对应脚本或记录，故仅作定性登记、不作定量断言。**后果**：定位列表后段的单项入口（组处置入口与批量确认入口，二者均在整个候选清单之后发出）在该规模**必须依赖程序化滚动**（Compose semantics 滚动动作或 instrumented 驱动滚动）——这是**必需使能项而非优化**；「以机械 fling 遍历大列表」在该规模**不是可靠的测量方法**，与 20k 时遍历可行形成**规模相关差异**，一并登记为观察。

4. **裁决与承接**：**计划 §10.3 大数据整链规模项维持 D-157 的「缺证（部分证据在位）+ 承接」，不记 PASS**；本条增补就地证据与实测、可执行的缺证成因，但不闭合该项。所需证据在种类上不变：**所需规模上的单一连续会话覆盖所列全流程端到端**。承接 = (a) 为大规模库遍历实现程序化滚动；(b) 随后完成剩余四个阶段（重复组打开 → 审核/确认 → 详情与月度刷新 → 杀进程后重开）；(c) 再按计划 §10.1/§10.4 的格式重新评估。(a) 触及**测试/自动化基础设施**而非产品代码；新增测试基础设施本身是一项变更，须**先行登记**再实施。

5. **§10.4.5 纪律与归属保留**：按计划 §10.4.5，仍有必需项非 PASS 时报告部分完成——**无残余被并入 PASS**。`OBS-APERF-INPUT-FREEZE` 的归属依据为 D-148 的**范围外观察登记**（长连发合成 fling 后列表指针输入冻结、两次独立复现、疑似 Compose pointerInput 在合成 fling 风暴下状态卡滞，**真实用户手势能否触发未验证**）与本条第 1(ii) 项的**阈值实验**（20k 人速 5/5 不触发、机械风暴触发；50k 第 2 次触发）：据此归因于自动化输入模式，非已证实的用户可见缺陷，**真实手势未验证的限定保持**。其**新测后果**——阻断大规模库遍历——登记为**验收方法上的真实约束**（见第 3 条）。

**边界：** 入：本条在 `docs/DECISIONS.md` 的追加登记（证据增补与承接细化）。出：零产品代码、零测试、零 schema/迁移/依赖/清单、零 Golden/fixture；不改 D-157 的裁决与其 PASS 项，不改 D-148～D-158 的任何裁决；`OBS-APERF-INPUT-FREEZE` 的既有归属不变（本条只增其后果测量与归属依据引用）；本条的证据复用按计划 §10.2 以差异说明承载（第 2 条）；程序化滚动属测试/自动化基础设施，须另行登记后实施；正式状态文档的同步不在本条范围。

**关联决定：** D-157（§10.3 规模项「缺证（部分证据在位）+ 承接」裁决的来源与本条复核对象——本条不改其任何裁决）、D-148（A-PERF 读治理与 B5 门槛状态更新；`OBS-APERF-INPUT-FREEZE` 与 50k 冷读观察的登记来源）、D-147（A-PERF 证据纪律——规模向量与测量方法依据）、D-153（(f)/FIX-MONTH-1 触发集来源）、D-155（A-DOC 批 A-05 事实登记先例）、D-158（第 5 条 v31 有效谓词相关 join 记录——本条 §10.2 差异说明的依据）、阶段计划 §10.1/§10.2/§10.3/§10.4.5。

## D-160 A-DOC 批登记（2026-09-20）：schema v31 正式状态同步与 D-156～D-159 事实

**状态：** 已批准并交付（2026-09-20，A-DOC 批次：schema v31 正式状态同步与 D-156～D-159 事实登记；本条为登记，不引入新裁决，不改变产品行为、schema、迁移与依赖——本批为零 DDL 的文档批，schema 维持 v31）。

**决定：**

1. **版本漂移闭合（范围大于 D-158 第 6 条的登记）**：D-158 第 6 条只登记了 `docs/ARCHITECTURE.md` 与 `docs/CURRENT_STATE.md` 的 v30 漂移；本批核实后确认同一「当前版本」错误声明还存在于 `README.md`（schema 行与推送 head 行）与 `docs/PROJECT_MAP.md`（schema 行与推送 head 行），并且 `docs/CURRENT_STATE.md` 的阶段 7 head 声明也滞后。四处均已按当前现实同步：当前 schema **v31**、迁移链 `1.sqm`~`30.sqm`（30 个文件，v1→v31）；v31 边的描述与 D-158 第 1 条一致（`30.sqm`，纯加性、零回填、`PRAGMA defer_foreign_keys = 1` 由调用方外层事务包裹，11 个新 DDL 对象，与 fresh `Ledger.sq` 终态逐字节一致）。**登记可以落后于能力，绝不可超前**——本条把实际已发生的同步补登。历史性陈述（v30 的逐版本条目、D-147 批次描述、D-146 修订注记、D01 证据绑定注记、v30→v31 读路径差异说明）一律保留原样，只改「当前版本」声明。

2. **阶段与批次状态同步**：`docs/CURRENT_STATE.md` 的阶段 7 表述由「其余组（P7-05～09）待按批裁决推进」更新为 D-156/D-158 的现实——P7-05 设计门已批准（D-156，规格 `docs/specs/2026-09-19-p7-05-correction-void-recycle-design.md`）、首切片（片 1a 基底层）已交付至 schema v31（D-158）、片 1b（产品接线）与 P7-06～09 尚未交付。推送与 CI 声明同步为 head `1b36629`、末三次合并 `c997e51`（P7-05 片 1a，D-158）→ `4588aaa`（ktlint 修复）→ `1b36629`（D-159 登记），同提交 CI run `35511493892` 三 job（Kotlin tests / Android compile / Python tests）全 success。

3. **状态不升级（显式保留）**：本批**不改动任何 PASS / PARTIAL / 缺证 / 接受延期 判定**。A-05 维持**部分完成、不整体 PASS**（D-157）；计划 §10.3 大数据整链规模项维持**缺证（部分证据在位）+ 承接**，不记 PASS（D-157/D-159）；D-159 的 50k 就地证据、未覆盖数据条件、剩余四阶段成因（INPUT-FREEZE 阻断遍历）与「程序化滚动属测试/自动化基础设施、须先行登记后实施」的承接一并登记；`OBS-APERF-INPUT-FREEZE` 的「真实用户手势能否触发未验证」限定逐字保留；替代输入通道三条读数按其原始口径登记为**未入证据工件的定性观察、不作定量断言**；D-159 同期另三项（(f)/FIX-MONTH-1 20k 计时向量及其三轮非同质限定、INPUT-FREEZE 阈值实验作为归属依据、50k 冷读分解未复现 r7 的 31.62 s 量级属基线参照而非同质对照）一并登记，避免选择性引用。

4. **评审与验证**：本批变更集为四个 tracked 文档（`README.md`、`docs/ARCHITECTURE.md`、`docs/CURRENT_STATE.md`、`docs/PROJECT_MAP.md`）。独立合并规格与质量评审终局 **APPROVE**（零 P1/P2；四项 P3 残余中「替代输入通道读数的来源限定」一项与其余措辞精度项均在同批修复）。验证：`project_docs` 通过（exit 0）、文档测试模块 15 项通过、受影响完整 Python 套件 809 项通过；漂移 grep 零剩余「当前版本为 v30」声明（本条引用该串不计）；隐私 grep 零命中。聚合门（完整 `check`、Android/KMP 编译、Debug APK、Desktop build、完整 Python 与 migration verifier）按根 `AGENTS.md` 验证分工由同提交 CI 承担。
...
## D-162 A-05 §10.3 规模项四阶段实测登记（程序化遍历打通后）

**状态：** 已批准并交付（2026-09-20，A-05 §10.3 大数据整链规模项的实测登记：在 D-161 的程序化滚动致动器可用后，于 50,411 项候选列表上取得**剩余四阶段的直接设备证据**；本条只登记实测与限定，**不自行改判** D-157/D-159 的「缺证（部分证据在位）+ 承接」结论——§10.3 规模项**仍不记 PASS**，剩余限定见第 4 条）。零产品代码、零测试、零 schema/迁移/依赖；schema 维持 v31。

**决定：**

1. **实测配置**：AVD `ul_p7_d01`（API 36，`emulator-5680`，隔离 adb）；候选 APK 由 `840bf67` 工作树本地构建（sha256 `fa0f96af…`，与测试 APK 同签名配对——**CI debug APK 每次运行签名不同，`install -r` 必失败**，故本轮全部使用本地配对构建）；测试 APK sha256 `8a3bdb62…`（该绑定**适用于证据文件的末段**，即本节各读数所属的那一次运行；同一证据文件为追加写入，该末段之前还有四次尝试的段落，其中更早的构建不含本轮新增的等待与 tab 解析修正）。库为**同一匿名 fixture 的产品 SAF 路径累积导入**（五轮 10k + 本轮三轮 200 行），**步骤之间从未清库**。**读数来源限定**：`am start -W` TotalTime、`dumpsys meminfo` 内存值、SystemUI 弹窗、以及第 2 条的设备计数复核四项为**运行期直接观察，未入证据工件**（沿 D-159 已确立的登记口径）；第 3(i)(ii) 条读数可由设备内 `files/ul-scale-traversal.log` 复核，第 2 条与第 3(iii)(iv) 条的界面读数可由 `local/artifacts/p7-05-scale/` 下的 `dump-import.xml`/`dump-detail.xml`/`dump-detail2.xml`/`dump-batchconfirm.xml`/`dump-batchresult.xml`/`dump-batchresult2.xml`/`dump-home.xml`/`dump-home2.xml`/`dump-home3.xml` 复核；`local/artifacts/p7-05-scale/instr-group3.log` **仅含运行器摘要**（`OK (1 test)` / `Time: 1,445.568`），不含读数。

2. **库与数据集状态**：**50,600 候选 / 50,600 来源 / 103,600 重复记录 / 1 交易 / 1 确认 / 2 posting**（本轮确认产生）。重复状态：**DEFERRED 102,200 + CONFIRMED_DUPLICATE 1,400**（即「部分已处置」状态已在位）；无重复候选行亦在位（`重复 无`）。`user_version=31`，`quick_check` ok；库文件在组阶段为 266,010,624 字节，批量确认写入后为 **266,014,720 字节**。以上六项计数、直方图、版本与完整性检查由独立 verifier 在设备上只读复核一致（该复核为运行期观察，未入工件）。

3. **四阶段实测结果（同一库、未清库）**：

   - **(i) 重复组打开 PASS（附计数观察）**：致动器以 `ACTION_SCROLL_FORWARD` 程序化遍历，**4,575 次动作 / 1,052,039 ms（≈17.5 min，≈4.3 动作/s，≈48 行/s）**后 `整组标记为重复` 进入可见窗口（bounds `[42,1938][423,2043]`）并被点击（accepted）。**全程未触发 INPUT-FREEZE**——D-159 第 3 条记录的机械 fling 冻结在无障碍动作通道**不复现**。**同时登记**：`ACTION_SCROLL_TO_POSITION` **不被该列表提供**（运行时确认），故回退路径即真实路径；`settleMillis=0` 时动作被接受但列表零推进（动画驱动）。**计数观察（开放）**：遍历起点读到的 `collectionInfo rowCount=50411`，与同期库计数 50,600 **不一致**；三次运行呈**系统性偏移**（库 50,000→`rowCount` 50,011；50,200→50,211；50,400→50,411，均为「入库前候选数 + 11」），即**渲染项计数未包含本次会话新导入的 200 个候选**。两种读法（列表确实未随会话内导入刷新 vs `collectionInfo` 计数为陈旧快照）**现有证据不足以区分**；可确定的是组枚举**确实包含**该 200 个候选（见第 (ii) 条 1,400 条记录 = 200 × 7），故该不一致位于「渲染项计数」与「库计数」之间。**该不一致使计划 §10.3「完整计数」向量在本轮未获证实，登记为开放观察，须另行取证**（不据此判定产品缺陷）。
   - **(ii) 审核/确认 PASS**：组卡打开后，卡内页脚 `确认整组标记` 位于**卡内容之后**，须再遍历 **341 次动作 / 112,616 ms** 才可达；点击后整组处置完成（**54,690 ms**）。**组卡条目口径**：组内条目为**重复记录**而非候选——该会话 200 个候选各带 7 条重复记录，与权威增量 1,400 一致（**卡自身计数不可读**：设备日志为 `group card itemCount=unavailable`，故 1,400 是 DB 权威增量而非卡面读数；这也解释了卡页脚需长距离滚动才可达）。**权威效果（DB 前后计数）**：`import_duplicate_status_history` **+1,400**，直方图 `DEFERRED 103,600 → 102,200`、`CONFIRMED_DUPLICATE 0 → 1,400`；`import_candidate`/`import_source_record`/`import_duplicate_candidate`/`ledger_transaction` **增量全为 0**——组处置**不产生资金效果**，与该路径语义一致。
   - **(iii) 详情与月度刷新 PASS**：候选详情屏渲染完整（`候选详情` + 金额 + `类型 …；状态 settled；重复 无` + 决策表单 + `勾选候选`）；补齐决策（分类 `早餐`、资金账户 `默认支付账户`）后 `进入批量确认` 出现；授权后 **`ledger_transaction` 0 → 1、`import_confirmation` 0 → 1、`posting` = 2（两腿平衡，`+100`/`-100` CNY 精度 2）**，待确认分类计数 `10000 → 9999`。月度刷新：HOME `月度小结` 切至 2026-08 显示 **`2026-08 · 交易 1 笔`、`CNY：普通收入 0.00，净支出 1.00，结余 -1.00`**；`账户余额` `默认支付账户 -1.00` / `隐藏费用过账账户 1.00`；交易详情显示 `类型：EXPENSE`、`创建入口：导入创建`、`金额明细` 两腿（`分类：早餐`）。
   - **(iv) 杀进程后重开 PASS**：`am force-stop` 后 `am start -W` TotalTime **1,377 ms**（运行期直接观察，未入工件）；重开后 schema v31、`CONFIRMED_DUPLICATE=1,400`、`DEFERRED=102,200`、候选 50,600、交易 1 全部保持，`quick_check` ok，应用正常渲染。

4. **登记限定（不并入 PASS）**：

   - **阶段顺序未在单一连续会话内满足**：四阶段证据分属**两个应用进程周期**——instrumented 会话覆盖「冷启动→首页→进入导入→接治→收尾重读→列表可操作→滚动→候选详情入口→重复组打开→组确认」，其后该会话结束（`am instrument` 结束即 force-stop，即计划链条末段的「杀进程」），**候选详情、批量确认与月度刷新是在重开后的新会话中测得的**。§10.3 链条顺序为「…详情与月度刷新 → 杀进程后重开」，故**链条顺序的一致性仍是缺口**；承接 = 在单次 instrumented 会话内按链条顺序补跑一次（遍历后回滚至候选行 → 详情 → 批量确认 → 月度核对 → 由会话结束充当杀进程）。
   - **数据条件「单次 10k 导入」未在本证据集内实测**：本轮会话内的导入为 200 行；50,000 候选基数来自此前会话的五轮 10k 导入（不在本证据集）。
   - **数据条件「20k 重复组」未覆盖（可构造，非结构性不可能）**：单文件接治上限 10,000 **候选**（D-146），但**组内条目按重复记录计**，且一个文件可对既有来源产生多条重复记录（**该会话**实测 200 候选 → 1,400 条 = 7×；同库上一会话为 6×，比例随既有来源重叠度变化）；D-157 第 4 条亦已登记过 20,000 条组（A-PERF B6）。故「20k 重复组」**可经产品路径构造**，本轮只是未构造该规模；承接 = 以更大的重复比例或更多既有来源构造并复测，或由裁决明确该条件的读法。
   - **未覆盖向量**：本轮无拒绝/零写入向量被触发（无拒绝发生）；组处置的逐项结果**仅以 DB 直方图为准**（致动器自身的卡内逐项文案统计只覆盖可见窗口，不能作为完整分母——1,400 条组内可见窗口仅显示 4 条）。
   - **致动器自身的两处登记残余（非产品缺陷）**：① 「组卡已打开」判定在 120 s 内报 `group card open=false`（卡标题未进入可见窗口），但卡确实已打开且 `确认整组标记` 随后可达并被点击——检测粒度问题；② 遍历重取容器失败时会**保留上一个节点句柄**并继续最多 25/50 次动作，理论上可能对已回收节点派发滚动（本轮未观察到误滚，且受拒绝守卫与停顿守卫双重限界）——登记为残余，后续改为丢弃句柄。**整模式墙钟时间无总上限**（源码默认确认等待 3,600 s + 兜底 1,800 s，叠加约 18 min 遍历），主机侧须自备超时。
   - **环境噪声**：运行期出现一次 SystemUI 无响应对话框覆盖应用窗口，人工 `Wait` 后恢复（属模拟器环境问题，非应用缺陷；该观察未入工件）。**应用侧 ANR/FATAL/OOM 记录：0 次**（logcat 与 `dumpsys dropbox` 双查，运行窗口内无记录）；应用内存为运行期点读值（TOTAL PSS 304,189 KB / RSS 421,220 KB，未入工件；重开后新进程读数为 142,050 KB / 259,272 KB，两者属不同进程周期）。

5. **裁决口径**：**§10.3 大数据整链规模项维持 D-157/D-159 的「缺证（部分证据在位）+ 承接」，不记 PASS**。本条的增量是：**D-159 第 3 条登记的四阶段缺证已由直接设备证据取代**，且其成因（INPUT-FREEZE 阻断遍历）**已由程序化滚动使能项解除**；剩余缺口收窄为第 4 条的「单一连续会话链条顺序」「单次 10k 导入数据集」「20k 重复组（可构造，未构造）」与第 3(i) 条的「完整计数」观察。**无残余被并入 PASS。**

**边界：** 入：本条在 `docs/DECISIONS.md` 的追加登记（实测结果与限定）。出：零产品代码、零测试、零 schema/迁移/依赖/清单、零 Golden/fixture；不改 D-157/D-159 的任何裁决与其 PASS 项；不改 D-161 的基础设施边界；正式状态文档的同步随 A-DOC 承接。

**关联决定：** D-161（本条实测所依赖的程序化滚动致动器——本条不改其边界）、D-159（四阶段缺证与 INPUT-FREEZE 成因的来源，本条第 3 条取代其缺证表述）、D-157（§10.3「缺证 + 承接」裁决与本条维持结论；其第 4 条 20,000 条组为第 4 条「可构造」的依据）、D-146（单文件 10,000 候选接治上限——第 4 条「按重复记录计」的依据）、D-153（触发 (f) 权威刷新链——第 3(iii) 条月度刷新的机制来源）、D-147/D-148（A-PERF 证据纪律）、阶段计划 §10.1/§10.2/§10.3/§10.4.5。

## D-163 A-PERF B5 测量方法复核裁决（终点绑定详情关键内容，20k 库三轮）

**状态：** 已批准并交付（2026-09-20，A-PERF B5 门槛的测量方法复核：按阶段计划 §2 第 44 行要求，复核既有 B5 PASS 的测量终点「点击后首个 `IntendedVsync`」不能绑定详情关键内容呈现，并以新终点在 20k 库上取得有效证据。本条只登记测量方法与读数，**不改 D-148 的 B5 状态**，也不改变 §10.3 的任何裁决）。
...
3. **读数（20k 库：产品 SAF 路径 2×10k 铺设，20,000 候选 / 10,000 重复记录 / 0 交易，`sqlite_stat1` 23 行；候选 APK sha256 `fa0f96af…` 由 `840bf67` 工作树构建；测试 APK sha256 `98cd6e7a…`）**：三轮全部有效（`valid=3/3`），区间 `[249,295]` / `[177,217]` / `[158,202]` ms，**最大值上界 295 ms**；三轮 `statusLinePresent=true`。门槛 ≤1 s → **通过**。

4. **口径与限定**：（a）该读数为**上界**，与历史 `b5_round3.py` 的「首帧」读数（**下界**）**不是同一命题**，两者不可直接比较；（b）该库为 **0 交易**，有效谓词在本库上为 no-op（沿 D-159 第 2 条口径）——故该读数**不构成** D-158 §5 所需的「含交易 20k 库上的有效谓词代价读数」；（c）三轮均为热态连续测量，未比较冷/热差异；（d）本条不改变 D-148 已登记的 B5 PASS，只为其测量方法补上有效证据与终点定义；（e）**门槛判定只取「标题 + 金额」两项，`statusLinePresent` 仅记录、不参与判定**（本轮三轮该值均为 `true`，故已登记读数成立；若后续出现 `false` 的运行，须按规格括注把状态行纳入判定）；（f）计划第 44 行同时要求「核对 `aperf-followup-plan.md` 的终点与开销口径」——该文件 §1 的下一步测量建议为「logcat tap 事件 → 详情首帧渲染事件，不在计时路径用 uiautomator dump」，与本条一致（计时路径不使用 dump），本条进一步把终点从「首帧」收紧到**关键内容可见**，并把开销口径（上界含点击派发、轮询粒度与树读取代价，中位 7–11 ms、最大 77–110 ms）显式登记于第 2 项；（g）**对 D-162 第 3(i) 条开放观察的增量证据**：本次 `b5Detail` 在同一 20k 库上进行，**该会话没有任何会话内导入**，而列表仍报 `collectionInfo rowCount=20011`（= 20,000 + 11）——与 D-162 三次运行观察到的「入库前候选数 + 11」偏移一致。这**倾向于**「常量容器项偏移」读法（11 为列表自身的非候选项，而非列表未随会话内导入刷新），但**不足以单独定论**（未在无导入库上重复多轮、也未排除陈旧快照）；D-162 第 3(i) 条的开放观察据此**收窄但维持开放**，须由 §10.3 承接的专门取证闭合。**同时披露**：本次为铺 20k 库执行了清库，D-162 的 50,600 候选库已不在设备上（其备份见检查点登记），故 D-162 的剩余承接取证须先恢复该库。

5. **裁决**：计划 §2 第 44 行对 B5 测量方法的复核要求**已履行**（终点绑定关键内容、口径与开销明确、20k 库三轮上界读数 ≤1 s）。B5 门槛维持 PASS；**不据此改动任何其他门槛或状态**。

**边界：** 入：本条登记与 `docs/CURRENT_STATE.md` 的同步。出：零产品代码、零 schema/迁移/依赖/清单；不改 D-148 的 B5 状态与 D-157/D-159/D-162 的任何裁决；`b5Detail` 模式属 D-161 已登记的测试基础设施范围内的功能扩展（同一文件、同一零依赖边界、CI 仍维持零 `connectedAndroidTest`），不另立基础设施授权。

**关联决定：** D-148（B5 门槛状态与其历史读数）、D-147（A-PERF 证据纪律）、D-159（空交易库差异说明口径——第 4(b) 条依据）、D-161（本条读数所依赖的 instrumented 基础设施）、D-158（§5 有效谓词代价读数——本条明确不构成其证据）、阶段计划 §2 第 44 行与 §10.1/§10.2。

## D-164 A-05 §10.3 单次会话全链实测登记（承接 (a) 闭合）

**状态：** 已批准并交付（2026-09-21，§10.3 大数据整链的**单次 instrumented 会话全链**实测：D-162 第 4(a) 条登记的「阶段顺序未在单一连续会话内满足」缺口已由本次实测闭合；本条只登记实测与限定，**§10.3 规模项仍维持「缺证（部分证据在位）+ 承接」，不记 PASS**——剩余限定见第 4 条）。

**决定：**

1. **实测配置**：同一 AVD（`ul_p7_d01`，隔离 adb）；候选 APK sha256 `fa0f96af…`（`840bf67` 工作树本地构建）；测试 APK sha256 `1929529e…`；库由 `local/artifacts/p7-05-scale/ledger-50600-v31-backup.db`（sha256 `92a9b646…`，integrity ok）回灌恢复（v31、50,600 候选、1 交易），**步骤之间从未清库**；会话内由主机侧执行一次产品 SAF 导入（200 行 fixture，致动器在等待期不建立 `UiAutomation`）。**读数来源限定**：`am start -W` TotalTime 与重开后的 `quick_check` 与计数核对为**主机侧运行期直接观察，未入证据工件**（沿 D-159/D-162 已确立口径）；其余读数可由设备内 `files/ul-scale-traversal.log` 复核。

2. **链条在单次会话内完成（承接 (a) 闭合）**：致动器新增 `fullChain` 模式，在一次 instrumented 会话内执行——冷启动→首页→进入导入→（主机侧导入）→接治→收尾重读→列表可操作→**候选详情**（点首行→补齐决策→勾选→返回）→**滚动**→**重复组打开**→**审核/确认**→**详情与月度刷新**（列表末尾 `进入批量确认`→授权→权威回读）。**顺序偏差（显式登记）**：计划 §10.3 的箭头顺序为「滚动→候选详情」，本次把**候选详情置于滚动之前**——理由是详情屏自带勾选与 `进入批量确认` 入口，先完成选择可使列表末尾的批量入口在组卡之后仍可达；除该一对**相邻**阶段外，其余阶段的相对顺序与计划一致，且**全部阶段在同一会话内完成**。会话结束时**不 finish 活动**，由 instrumentation 结束触发的进程终止充当链条末段的「杀进程」，主机侧随即 `am start` 完成「重开」。**关键设计**：候选详情屏自带勾选与 `进入批量确认` 入口，故选择动作可在深遍历**之前**完成，使列表末尾的批量入口在组卡之后仍可达——从而无需回滚遍历。

3. **逐阶段权威读数（DB 前后计数）**：

   - **接治**：`import_candidate` 50,600 → 50,800（**+200**）；`import_duplicate_status_history` 105,000 → 106,600（**+1,600**，即该 200 行对既有来源产生 8× 重复记录）。
   - **候选详情**：`ledger_transaction` 1 → 1、`import_candidate` 与重复状态历史均无变化——**补齐决策与勾选零经济效果**；详情屏文本与决策标记 `决策已补全。` 均观察到位（分类选项为既有 `A02FIX-SAL-C1`、资金账户 `默认支付账户`），勾选后 `进入批量确认` 入口出现。
   - **滚动**：遍历起点 `collectionInfo rowCount=50614`；**4,600 次 `ACTION_SCROLL_FORWARD` / 1,052,923 ms（≈17.5 min，≈4.4 动作/s）**后 `整组标记为重复` 进入可见窗口并点击（accepted）；**全程不触发 INPUT-FREEZE**。
   - **重复组打开 + 审核/确认**：卡页脚 `确认整组标记` 需再 **389 次动作 / 146,164 ms**；整组处置 **76,687 ms**；重复状态历史 106,600 → 108,200（**+1,600**）、`CONFIRMED_DUPLICATE` **+1,600**；`import_candidate`/`import_source_record`/`import_duplicate_candidate`/`ledger_transaction` **增量全 0**（组处置无资金效果）。
   - **详情与月度刷新**：批量入口 `进入批量确认` 位于组卡之后（bounds `[42,1938][525,2043]`），点击后批量屏显示 `确认入账（1 项）`；授权后**权威提交**：`ledger_transaction` 1 → 2、`import_confirmation` 1 → 2、`posting` 2 → 4，**39,426 ms**（逐 2 s 轮询至权威计数增长）。**该阶段的会话内覆盖口径（限定）**：本次会话内测得的是**批量确认入口可达 + 授权 + 权威 DB 回读**（`ledger_transaction`/`import_confirmation`/`posting` 三项权威计数在会话内增长）；**HOME 月度小结的界面读数未在本会话内取得**（D-162 第 3(iii) 条的月度界面读数属另一进程周期），故「详情与月度刷新」的**界面刷新面在本会话内未独立取证**，仅由权威计数与 D-162 既有读数承接。
   - **杀进程后重开**：会话结束后进程终止；`am start -W` TotalTime **1,587 ms**；v31、`quick_check` ok、`CONFIRMED_DUPLICATE=3,000` / `DEFERRED=102,200`、50,800 候选 / 2 交易 / 2 确认 / 4 posting 全部保持。

4. **登记限定（不并入 PASS）**：

   - **数据条件「单次 10k 导入」仍未实测**：本次会话内导入为 200 行（为控制组卡规模）；50,000 基数来自此前会话的五轮 10k 导入。
   - **数据条件「20k 重复组」仍未构造**：本次组为 1,600 条重复记录。**并登记一项结构性成本观察**：组卡条目**内联渲染在列表末尾的批量入口之前**，组越大则批量入口越远——按本会话实测速率（卡页脚 1,600 条 ÷ 389 次动作 **≈4.1 项/动作**、**≈0.376 s/动作**）推算，20,000 条组的卡页脚需约 **4,900 次动作（≈31 min）**，而 70,000 条组（10k 导入对五份既有来源）需约 17,000 次动作（≈1.8 h）。**参数注意**：`cardScrollActions` 默认 4,000，**低于该推算所需**；执行 20k 条组复跑时须显式调高该参数，否则会以 FINDING 提前结束。故「以 10k 导入构造 20k+ 组并在同链复跑」在现行 UI 结构下成本很高；承接须明确该条件的规模口径，或改用**关闭组卡**的流程（组卡关闭后批量入口回到列表末尾附近）。
   - **「完整计数」观察（更新，维持开放）**：本次遍历起点 `rowCount=50,614`，而**入库前**候选为 50,600——偏移 **+14**，与 D-162 三次运行的 **+11** **不一致**，故「常量偏移」读法**不成立**。可确定的表述应为：**`rowCount=50,614` 小于接治后的库计数 50,800**，即该 200 个会话内候选**未被反映在 `rowCount` 中**；而 `rowCount` 相对**入库前** 50,600 的偏移由 D-162 的 +11 变为本次的 +14，**该差异本身未有解释**（可能与列表状态有关——本次列表另携带一项勾选与已就绪的批量入口）。**两种读法仍未区分**：列表未随会话内导入刷新 vs `collectionInfo` 为陈旧快照。**D-163 第 4(g) 条的倾向性表述据此收窄修正**：其「常量容器项偏移」的倾向不成立；保留的确定事实仅为「无导入会话的 `rowCount` 也等于候选数 + 非候选项数」。该观察**维持开放**，须专门取证（例如会话内导入后触发一次显式「刷新清单」再读 `rowCount`）。
   - **未覆盖向量**：本轮无拒绝/零写入向量被触发；组处置的逐项结果仍仅以 DB 直方图为准（卡内文案统计只覆盖可见窗口）；致动器自身残余（组卡打开判定粒度、容器句柄保留）与「整模式无总墙钟上限」不变；本轮应用侧 ANR/FATAL/OOM 记录 **0 次**。

5. **裁决口径**：**§10.3 大数据整链规模项维持 D-157/D-159 的「缺证（部分证据在位）+ 承接」，不记 PASS**。本条的增量是：**D-162 第 4(a) 条（单一连续会话链条连续性）已闭合**；剩余缺口收窄为第 4 条的「单次 10k 导入」「20k 重复组（含结构性成本观察）」与「完整计数」观察。**无残余被并入 PASS。**

**边界：** 入：本条登记与 `docs/CURRENT_STATE.md` 的同步。出：零产品代码、零 schema/迁移/依赖/清单；不改 D-157/D-159/D-162 的任何裁决（D-163 第 4(g) 条的倾向性表述由本条**收窄修正**并显式登记）；`fullChain` 模式属 D-161 已登记的测试基础设施范围内的功能扩展（同一文件、零依赖边界）。

**关联决定：** D-162（第 4(a) 条缺口与本条闭合对象；其第 3(i) 条观察由本条更新）、D-163（第 4(g) 条由本条收窄修正）、D-161（基础设施边界）、D-159/D-157（§10.3 非 PASS 裁决与本条维持结论）、D-146（单文件 10,000 候选接治上限）、阶段计划 §10.1/§10.2/§10.3/§10.4.5。

## D-165 A-05 §10.3 数据条件「单次 10k 导入」与「≥20k 重复组」实测登记

**状态：** 已批准并交付（2026-09-21，§10.3 数据条件的实测登记：在 D-164 结束状态的库上执行**一次**产品 SAF 路径的 10k 导入，覆盖「单次 10k 导入」数据集条件，并使库中出现 ≥20k 条重复组；本条只登记实测与限定，**§10.3 规模项仍维持「缺证（部分证据在位）+ 承接」，不记 PASS**——剩余限定见第 4 条）。

**决定：**

1. **实测配置**：同一 AVD（`ul_p7_d01`，隔离 adb）；库起点 = D-164 结束状态（**50,800 候选 / 105,200 重复候选 / 2 交易**，`import_duplicate_status_history` 最新态直方图 `CONFIRMED_DUPLICATE=3,000 + DEFERRED=102,200`）；导入文件为既有匿名 10k fixture（`alipay-d01-10000.csv`），经产品 SAF 路径（主机侧选择与派发）。

2. **「单次 10k 导入」数据条件（覆盖）**：应用自身的权威接治结果行为 **`接治完成：新增 10000，等价重放 0，解析拒绝 0，接治拒绝 0。`**（`最近导入：alipay-d01-10000.csv`）；DB 权威增量 = `import_candidate` 50,800 → **60,800**（**+10,000**）、`import_source_record` +10,000、`import_duplicate_candidate` 105,200 → **156,000**（**+50,800**，即 10,000 个候选对既有来源产生 5.08× 重复关系）。**一次导入、无等价重放、无解析/接治拒绝**。

3. **「≥20k 重复组」数据条件（覆盖）**：上述导入所属会话的重复组规模为 **50,800 条重复记录**（≥20k），即库中现存在一个 ≥20k 条的会话级重复组；库最新态直方图 `CONFIRMED_DUPLICATE=3,000 + DEFERRED=153,000`（共 156,000）。**口径说明（重要）**：`import_duplicate_candidate` 每行是一条**重复关系**（其 `candidate_id` 与 `import_candidate.candidate_id` 属不同命名空间，本库 156,000 行、156,000 个互异 id），故本条与 D-162/D-164 所称「重复记录」即该表的行数，语义一致。

4. **登记限定（不并入 PASS）**：

   - **该组规模的「卡页脚 / 批量入口」实测在本轮不可行**（沿 D-164 第 4(b) 条结构性成本观察）：组卡条目内联渲染在列表末尾的批量入口之前，50,800 条组的卡页脚按实测速率（≈4.1 项/动作、≈0.376 s/动作）推算需 ≈12,400 次动作 ≈1.3 h，超出致动器默认边界（`cardScrollActions` 默认 4,000）。故本轮只覆盖**数据条件**，不覆盖该规模上的**卡页脚可达性**；承接 = 由裁决明确该条件的读法（数据条件 vs 可达性条件），或改用关闭组卡的流程。**受影响的同规模链条步骤须一并命名**：在 ≥20k 条组的规模上，除卡页脚可达性外，**重复组打开 → 审核/确认 → 批量确认**（以及其后的批量入口可达）同样受该可达性约束，故该裁决同时决定同规模链条复跑是否可行，不能只读作界面可达性问题。
   - **单次 10k 导入的接治耗时观察（新增，非门；读数来源限定：该耗时为主机侧运行期直接观察、未入证据工件，沿 D-159/D-162/D-164 已确立口径）**：主机侧 SAF 选择与派发耗时 34 s（运行期观察）；接治稳定耗时以**留存工件可界定的口径**为准——新批次的 `generated_at`（`2026-09-21T01:06:25.640335Z`）到结果 dump 落盘（`dump-import-10k.xml`）之间的窗口为 **≈11.2 min**，本轮轮询在 30 s 粒度下于该窗口内观察到 +10,000 稳定（其间多次遇到数据库写锁，读侧需重试；该轮询日志未入工件）。该耗时随库规模增长（同一 200 行导入在 50,600 候选库上约 111 s 稳定），登记为**规模相关的接治耗时观察**，不构成门槛判定，也不据此判定缺陷。
   - **「完整计数」观察仍未闭合**（D-162 第 3(i) 条、D-164 第 4(c) 条）：本轮未做专门取证（需在会话内导入后触发「刷新清单」再读 `rowCount`）；维持开放。
   - 本轮无拒绝/零写入向量被触发（应用自报「解析拒绝 0、接治拒绝 0」）。

5. **裁决口径**：**§10.3 大数据整链规模项维持 D-157/D-159 的「缺证（部分证据在位）+ 承接」，不记 PASS**。本条的增量：**D-164 第 4 条的「单次 10k 导入」与「20k 重复组」两项数据条件已覆盖**；剩余缺口收窄为「该组规模上的卡页脚/批量入口可达性（或其读法裁决）」与「完整计数」观察。**无残余被并入 PASS。**
...
## D-167 D-158 §5 有效谓词代价读数登记（片 1b 合并阻断项，20k 库宿主侧读数）

**状态：** 已批准并交付（2026-09-22，D-158 第 5 条所要求的「热读增本先取实测读数」的读数登记：对 v31 有效谓词为两个最热读查询各增的相关 join，在 20k 交易库上取得宿主侧读数；本条只登记读数、方法与限定，**不改 D-158 的任何裁决**，也不改变 P7-05 片 1b 的既有边界——零产品代码、零 schema/迁移/依赖，schema 维持 v31）。测试证据：`ledger-data/src/jvmTest/kotlin/com/unifiedledger/data/P705EffectivePredicateCostTest.kt` 与 `P705Harness.kt`（test-only，共 +946 行 / 2 文件，修订候选 `887caf6`，merge `ce8d995`）。

**决定：**

1. **读数登记（D-158 §5 承接）**：D-158 第 5 条要求有效谓词的成本「片 1b 验收前须在 20k 库上取得该项读数；读数显示回归的，须以索引边或另立决定处置，不得静默通过」。本条登记该读数。**结论：在本样本规模下未检出回归**——D-158 第 5 条「读数显示回归的，须以索引边或另立决定处置」的条件**未被触发**。该结论是「未检出」，**不是**「已证明无回归」（见第 3、4 条）。

2. **测量对象与方法**：对象 = 两个生产查询 `currentVersionRowsForLedger` 与 `ledgerEntryRowsForLedger` 在 v31 有效谓词下的成本；库 = **20,000 交易 / 40,000 posting** 的**文件型** DB（非内存库），夹具为单版本双腿、**零作废事实**。两种口径各测：`pure`（仅游标步进，查询自身成本）与 `materialized`（读满并存储全部列，产品实际支付的成本）。方法：计时前先 `ANALYZE`（经 `driverExecute`，非 `executeQuery`；无计划器统计时该 join 为计划器伪影）；丢弃 1 轮预热；**4 轮实测**，轮内谓词侧/消融侧顺序**2:2 交替**（谓词先行 = 第 1、3 轮，消融先行 = 第 2、4 轮），使任一侧不系统性承担对侧的缓存预热成本；**先取每轮配对差值、再取 min/max**，故报告的 delta 上界结构性不小于下界。

3. **读数（写者单次运行；一律以区间引用，不取单点）**：

   - current-version `pure` delta ≈ **+10.7..+25.5 ms**（**+10.0%..+22.7%**）；
   - current-version `materialized` delta ≈ **+10.7..+33.9 ms**（**+3.5%..+10.9%**）；
   - entry `pure` delta ≈ **+12.6..+36.8 ms**（**+11.6%..+30.1%**）；
   - entry `materialized` delta ≈ **−22.5..+47.7 ms**（**−6.7%..+12.3%**）。

   **披露**：以上四条区间均出自**写者的单次运行**（无留存工件可复现该组具体数值），独立复跑得到**不同但相互重叠**的区间（见第 5 条），故**具体数值不具跨运行可复现性**；此处仅作**区间引用**，不取任何单点。

   四条序列中**三条为一致的小幅正值**，entry 的 `materialized` 序列**跨零**。**判定口径（显式陈述）**：仅当 delta 相对产品实际支付的 `materialized` 读**大到改变其数量级**时，才按回归处置；在本样本规模下观测到的 delta 位于**运行间包络之内**。本条**不**声称该读数是「决定性的」，也**不**声称它「证明」无回归——它只表明：以该口径衡量、在该样本规模下，未达到 D-158 第 5 条触发处置的门限。

4. **环境与限定（必须披露）**：(a) 该读数为**宿主侧 JVM SQLite 3.51.3**（sqlite-jdbc `3.51.3.0`）读数，**不是**设备系统 SQLite（3.44.3）读数；**设备侧的 D-158 §5 读数仍是 P7-05 片 1b 的开放前置条件，本读数不解除它**。(b) 夹具为**同质**（单版本双腿、**零作废事实**），故谓词的**过滤**成本（丢弃已作废交易的工作）与 `transaction_effective_state` 中对**已填充** `transaction_void_fact` 的相关 `MAX(sequence)` 子查询**均未测量**。(c) **样本规模限定**：结论仅在 20k 交易 / 本包络下成立。(d) 打印的 `EXPLAIN QUERY PLAN` 行**随宿主版本脆弱**，**不得**作为证据引用；可引用的只有计时序列。(e) 夹具中手抄的生产 SQL 与 `Ledger.sq` 之间仅有行为等价守卫，故每当两个热查询任一被编辑时，须重新核对抄本——否则读数会在不知不觉中测量产品已不再运行的查询。

5. **独立证据**：该读数经独立评审 **APPROVE**，并经 distinct verifier 复核——全部具名断言 **VERIFIED**，聚焦测试在 verifier 自身复跑下 **3/3 通过**。**跨运行不可复现性（必须披露）**：第 3 条四条区间出自**写者的单次运行**（无留存工件可复现该组具体数值）；独立复跑（评审者与 verifier 各一次）得到与第 3 条**不同但相互重叠**的区间，且各自包络**明显宽于**第 3 条（各口径均出现更宽或跨零的区间）。故**第 3 条的具体百分比不具跨运行可复现性**，只可作**区间与包络**引用，本条**不以任何数值作点估计**。**如实披露**：写者自报的完整 `:ledger-data:jvmTest`（580 tests）**未经独立复跑**；聚合检查的权威是同提交 CI。

6. **范围边界**：本批**纯测试**，+946 行 / 2 文件；**零生产代码、零 schema/DDL、零依赖变更**（测试源集内新增 test-only 计时装置）。

7. **对片 1b 的影响**：仅就该读数而言，D-158 第 5 条「在 20k 库上取得该项读数」的**字面要求在宿主侧已满足**；但**片 1b 尚未开始**，且**设备侧的 D-158 §5 读数仍是片 1b 的开放前置条件**（第 4(a) 条），本读数不替代之。

**边界：** 入：上述两个 test-only 测试文件与本条登记。出：零产品代码、零 schema/迁移/依赖/清单、零 Golden/fixture；不改 D-158 的任何裁决与其第 4 条残余登记；不改 D-156/D-157/D-159～D-166 的任何裁决；正式状态文档的同步随本批承接。

**关联决定：** D-158（第 5 条为本条登记对象——本条不改其裁决，其第 4 条残余不受影响）、D-156（v31 有效谓词与视图 `transaction_effective_state` 的来源）、D-147/D-148（A-PERF 证据纪律——热读增本读数的方法依据）、D-163（宿主侧/设备侧口径区分与「上界/下界不可直接比较」先例）、D-166（同一片 1b 前置条件链上的最新登记）。

## D-168 P7-04 导入审核列表 OOM 修复设计批准与范围取代登记

**状态：** 已批准（2026-09-22，P7-04 导入审核列表 61k 规模 OOM 缺陷的修复**设计**批准登记：`docs/specs/2026-09-21-p7-04-import-review-list-oom-fix-design.md` 由 `proposal` 升为 **`approved`**（修订版 v0.2），并登记其对 A-PERF approved 规格**唯一一条非目标**的显式取代。本条**只登记批准与取代**，**不实施任何产品代码**——零产品代码、零 schema/迁移/依赖；schema 维持 v31。实施、独立评审与 distinct verifier、设备复验与最终验收属后续批次）。

**决定：**

1. **对象与来源**：本规格修复的是 D-166 第 3 条登记的缺陷——61k+ 候选库冷启动/刷新触发 `loadImportReviewRows` 时 **OutOfMemoryError**（`SqlDelightImportReviewReadAdapter.kt:46` 的 `.executeAsList()` 整表物化点；192 MB 堆；进程死亡两次）。规格现为 **`approved`**。

2. **机制（一段，不重新设计）**：修复为**候选边界 keyset 分批读**——在 `Ledger.sq` 新增只读命名查询 `importReviewRowsForLedgerPage`，按 `import_candidate` 主键 `(ledger_id, candidate_id)` 的候选边界取「严格大于游标、按 id 升序至多 pageSize 个候选」的全部行，并在**单个事务**内做**批内折叠**（批内链与现状逐字节相同）。输出语义、端口签名与 UI 可见结果**均不变**（完整列表、永不截断）。内存收益来自**①②③ 整表结构不再与 ④ 同时存活**这一结构变化，**不是**来自「1/7」或「一个数量级」的量级断言（该两项因未获证明已被撤回）。

3. **范围取代（治理，计划 §10.4.1 五要素）**：本规格**恰取代** A-PERF approved 规格 `docs/specs/2026-09-17-p7-04-a-perf-import-read-governance-design.md` 的**一条**非目标——其 §2.2 第 3 点与 §3 的「本批不改分页」/「不做分页」。取代的授权是阶段计划 §2 A-PERF 第 42 行「分页若采用，必须另冻结排序、快照一致性、跨页勾选、总数及组枚举完整性」——该行本身即**预留了分页路径**，其许可条件是五项分别冻结，五项均已在规格 §3 逐项冻结。A-PERF 规格**其余全部非目标继续有效**（不做截断、不隐藏待确认项、不改组审核范围、不做投影裁剪、D-147 会话批量读语义、19 子型渲染模型、披露行先例）。A-PERF 规格本体**不改写**，只在其**下次实质修改时**同步该非目标的解除记录。

4. **批准依据**：独立规格评审首轮结论 **REVISE**（机制经确认 **sound**）→ 修订版 v0.2 → 独立闭包复评 **APPROVE**（条件 C1–C4 全部闭合、未引入新 P1/P2），其中含一项独立复核：从 `runtime-jvm-2.3.2.jar` 确认 `Transacter.transactionWithResult` 的参数名为 **`noEnclosing`**（默认 `false`；嵌套抛 `IllegalStateException("Already in a transaction")`），**不是** `readOnly`。主代理据此批准本规格。

5. **须存活至实施的登记条件（不得豁免）**：(a) `LIMIT :page_size` 是本仓库**首次使用**——实施批必须以编译产物核对生成的 Kotlin 签名（`page_size: Long`），不得假定 `Int`；(b) `ACC-D-CONN-01`（连接持有读数）为**必测设备向量**——设计在非 WAL 的单连接 Android 连接池上把逐批折叠保留在单个 `BEGIN IMMEDIATE` 事务内，故连接/锁持有窗口严格长于现状单语句读；(c) 规格的数值阈值（峰值堆 ≤160 MB、列表可操作 ≤3 s、零 ANR/OOM/FATAL）是**由设备读数裁决的冻结阈值**，**不是**已达成结果。

6. **状态与边界**：本条**只登记批准**——**尚无任何产品代码被实施**；实施、独立评审 + distinct verifier、设备复验与最终验收属**后续批次**。D-166 第 5 条登记的缺口（「61k 规模完整计数受 OOM 阻断」）**在修复被实施并经设备验证前维持开放**；**无任何项被记为 PASS**。

7. **规格同批冻结的其他边界**：零 DDL（既有主键覆盖索引 + bloom filter 足够，无新索引需求）、schema 维持 v31、零迁移、零依赖变更；不做投影裁剪、不做流式物化、不改 UI/端口/用例层。
...
## D-170 P7-04 OOM 修复残余设备向量读数登记（ACC-D-CONN-01 与 ACC-D-20K-01）

**状态：** 已批准并交付（2026-09-23，D-169 第 6 条留作**开放（未豁免）**的两条设备向量——`ACC-D-CONN-01`（连接持有 / D-148 ANR 链）与 `ACC-D-20K-01`（20k B2/B3/B6 回归）——的设备读数**登记**。本条**只登记读数**：**零产品代码改动**，**不改** D-169 的任何裁决，也**不**闭合 D-169 登记的 61k 规模可操作性缺陷（该项**维持开放**）。设备与 APK 绑定与 D-169 一致：AVD `ul_p7_d01`、隔离 adb 端口 5038、APK sha256 `63b2897f629bce769b0082f82be4045e9c051b7b0dbcf6ead374e4ebd27d1b8d`。原始读数工件落于主 checkout 的只读设备目录 `local/artifacts/p7-05-scale/oomfix-evidence/`，不入 tracked 文件）。

**决定：**

1. **对象与性质**：本条对象为 D-169 第 6 条明确列为**开放、不得豁免**的两条设备向量。本条**只登记读数**：它**不**改变任何产品行为，**不**改 D-169 的任何裁决，**不**闭合 D-169 第 5 条登记的 61k 规模可操作性缺陷，也**不**把 OOM 修复的冻结阈值整体记为已裁决。

2. **ACC-D-CONN-01 读数（连接持有 / D-148 ANR 链）**：程序 = 冷启动 → 点「导入」（在单条非 WAL Android 连接上的单个 `BEGIN IMMEDIATE` 内启动 7 批分页读）→ 2 s 后点 HOME Tab（在连接持有仍存活时强制主线程组合工作）。结果：HOME 正常渲染（月度小结 / 首页 / 导入均在位）；logcat 扫描 `ANR in` / `Input dispatching timed out` / `Application Not Responding` **零匹配**，零 `ANR in com.unifiedledger`。结论：更长持有窗口（7 批 + 批内折叠 vs 修复前单语句）在本设备 / 本库上**未触发** D-148 ANR 链形态。**如实陈述（不得升格）**：这是**单次运行观察**，且持有窗口本身**未被仪器化**，故本条记录的是「**未观察到 ANR**」，**不是**实测的持有时长。工件：`local/artifacts/p7-05-scale/oomfix-evidence/acc-d-conn-01-reading.txt`。

3. **ACC-D-20K-01 读数（20k 回归，仅 B2/B3）**：库 `ul20k-measure.db`（20,000 候选 / 10,000 重复关系 / `sqlite_stat1` 在位）；**schema 未经验证**——保留的宿主工件 `local/artifacts/a-perf-p704/ul20k-measure.db` 为 `user_version=30`，应用打开该库时可能迁移至 v31，但无证据工件记录该迁移，故本条不主张该库为 v31。APK sha256 同第 2 条状态行。方法：截图尺寸轮询（>250 KB 即判列表渲染，约 0.15–0.4 s 粒度），计时路径内**不使用** uiautomator dump。读数：B2（冷进入，点导入 → 列表渲染）**2708 / 2608 / 3204 ms**（3 轮）；B3（热刷新，点「刷新清单」→ 列表重渲染）**1303 / 1459 / 1351 ms**（3 轮）；故障（OOM/FATAL/ANR）**0**。**覆盖范围**：本条只覆盖 D-169 第 6 条向量定义中的 **B2/B3** 两个组件；**B6（组卡打开）组件未测量、维持开放**，故 `ACC-D-20K-01` 仅为**部分覆盖**。对比：D-148 **修复后（OOM 修复前）**的原始读数——B2 **4.93 s**（10,039 候选）、B3 **5.03 s**（20,039 候选），地板校准 ≤ ~2.7 s；D-148 的**修复前**基线是同场景 **300+s 卡死**；OOM 修复后原始读数（B2 2.6–3.2 s、B3 1.3–1.5 s）**无回归**，位于历史原始带之内或之下。**口径声明**：本条是**回归核对**，**不是** A-PERF 门槛的重新冻结——门槛是冻结的 B2 首帧方法，20k 冷进入对照按该方法测得 **0.035 / 0.035 / 0.041 s**。**读数披露**：本条的 B2/B3 三轮与 D-169 第 5 条登记的 20k 冷渲染 2621 / 3159 / 2613 ms 为**同一向量的两次独立运行**，两者均为单次不可复现读数，该分歧属预期、不构成矛盾。工件：`local/artifacts/p7-05-scale/oomfix-evidence/acc-d-20k-01-reading.txt`。

4. **读数披露（沿 D-167 第 3/5 条口径）**：第 2、3 条读数为**写者的单次运行值**，由宿主侧脚本打印到 stdout；第 2、3 条引用的原始工件即留存证据；本条**不声称跨运行可复现性**，也不取任何数值作点估计。

5. **状态与边界**：D-169 留作开放的两条向量现为**已覆盖**（附第 2、4 条的单次运行限定）——其中 `ACC-D-CONN-01` 按第 2 条记录「未观察到 ANR」；`ACC-D-20K-01` 的覆盖**仅限 B2/B3**，其 **B6（组卡打开）组件未测量、维持开放**，故该向量仅为**部分覆盖**；OOM 修复的其余开放项**不变**——**61k 规模可操作性缺陷（D-169 第 5 条）维持开放**，且 OOM 修复的冻结阈值仍**只获部分裁决**（≤160 MB 与零 ANR/OOM 通过；≤3 s 在 61k 未达）。**凡未获证据者不记 PASS。** 本条**零产品代码改动**。

**边界：** 入：本条登记与 `docs/CURRENT_STATE.md` 的同步。出：零产品代码、零测试、零 schema/迁移/依赖/清单、零 Golden/fixture；不改 D-169 的任何裁决、不改 D-168 的批准内容、不改 D-166 条目本身的缺陷与缺口状态、不改 D-147/D-148 的读治理裁决、不改 D-157/D-159/D-162～D-167 的任何裁决与其 PASS 项；不把任何残余并入 PASS。

**关联决定：** D-169（第 6 条两条残余设备向量为本条登记对象——本条不改其任何裁决，其第 5 条 61k 可操作性缺陷维持开放）、D-168（OOM 修复设计批准，其第 5(b) 条将 `ACC-D-CONN-01` 列为必测设备向量）、D-148（连接持有 / ANR 链背景依据；其 A-PERF 20k B2/B3 原始读数为本条第 3 条对比基线）、D-163（测量方法先例——截图轮询口径与冻结 B2 首帧口径区分的依据）、D-166（第 3 条 OOM 缺陷登记链）、阶段计划 §10.1/§10.3/§10.4.5。

## D-171 P7-04 导入审核 61k 可操作性缺陷的宿主侧归因登记与 D-169 定位更正

**状态：** 已批准并交付（2026-09-22，D-169 第 5 条登记的 61k 规模可操作性缺陷的**宿主侧分量归因读数登记**，并**更正** D-169 第 5 条的代码定位推理。本条**只登记读数与更正推理**：**零产品代码改动**、零 schema/迁移/依赖；**不改** D-169 登记的缺陷、其开放状态与其「须仪器化 / 无已证明根因」的既有对冲；61k 规模可操作性缺陷**维持开放**。测试证据：`ledger-data/src/jvmTest/kotlin/com/unifiedledger/data/ImportReview61kReadAttributionTest.kt`（side A，随 `bab7080` 合入）与 `app-ui/src/commonTest/kotlin/com/unifiedledger/ui/P503ImportReviewRenderModelAttributionTest.kt`（side B，同批），两者均为 test-only，merge `cfab272`。）

**决定：**

1. **对象与性质**：本条对象为 D-169 第 5 条所登记的 61k 规模可操作性缺陷（冷首次进入列表 ≈11 s，超出冻结 ≤3 s）的**宿主侧分量归因**。两条 test-only 计时测试分别测量该缺陷的两个宿主侧分量——(A) 分页读 + 折叠、(B) 急切渲染模型构建——以判定二者在 ≈11 s 中的占比。本条**只登记读数并更正 D-169 的定位推理**：它**不**改变任何产品行为，**不**改 D-169 登记的缺陷，**不**闭合该缺陷，也**不**把任何阈值记为 PASS。

2. **读数（宿主侧 JVM 单次运行；一律以区间引用，不取单点）**：

   - **Side A（分页读 + 折叠）**：`SqlDelightImportReviewReadAdapter.loadImportReviewRows` 在**真实 61k 夹具副本**上 ≈ **2.0–2.4 s/轮**（写者 ≈2.0–2.1 s；评审者 ≈2.2–2.3 s；verifier ≈2.27–2.43 s）。测试 `ImportReview61kReadAttributionTest`，打印前缀 `D169ATTR: side=A ...`；夹具经环境变量 `UL_D169_FIXTURE_DB` 指向（未设置或不可读时打印 skip 行并返回，无绝对路径嵌入）。
   - **Side B（急切渲染模型构建）**：`importReviewRenderItems` 在 61,000 行列表上 ≈ **10–50 ms/轮**（verifier 11.9–40.1 ms），渲染模型条目数 **61,011**。测试 `P503ImportReviewRenderModelAttributionTest`，打印前缀 `D169ATTR: side=B ...`。
   - **合计 ≈ 2.4 s**，即 D-169 登记的 ≈11 s 中约 2.4 s 由这两个宿主侧分量解释。

   **披露（沿 D-167 第 3/5 条口径）**：以上读数为**宿主 JVM 单次运行值**（非设备读数），**不具跨运行可复现性**、**无留存原始工件**；两条测试可重跑（side A 经 `UL_D169_FIXTURE_DB`，side B 直接运行），故本条**不取任何数值作点估计**，只作区间引用。

3. **结论（如实，不过度声称）**：残余 ≈ **8.6 s 既不能由分页读解释，也不能由急切渲染模型构建解释**（side B 的 10–50 ms 相对 ≈11 s 可忽略）。该残余**最可能**是设备侧约 61k 条目列表的 UI 组合/重组（或 GC）成本——**这两条宿主侧测试均不测量设备侧**。故本条**不主张任何已证明的设备侧根因**；**下一步是设备侧测量**（主线程帧 / 组合墙钟），而非据此进入修复批或规模口径裁决。

4. **对 D-169 定位的更正**：D-169 第 5 条的代码定位——「成本在 UI 层：导入屏在组合线程上急切构建整个渲染模型」——被 **side B 的 10–50 ms 读数证伪**：急切渲染模型构建**不构成** ≈11 s 的成本。**但** D-169 登记的缺陷本身、其开放状态、以及其「残余等待不能由分页读解释、须先仪器化归因」的**既有对冲措辞**均**不变**（D-169 未主张已证明根因）。**本条更正的是推理（定位），不是缺陷登记**；D-169 条目原文不改。

5. **同批的测试文件文档更正（prose-only）**：`P503ImportReviewRenderModelAttributionTest.kt` 的一处类注释原称合成 61,000 行构成「**单一**组（待确认——缺用户决策）」，与同文件自身「**TWO group headers**」条目数注释及代码相矛盾（`classifyImportCandidate` 将 10,000 个 null 行归入 `PENDING_USER_DECISION`、51,000 个 DEFERRED 行归入 `SUSPECTED_DUPLICATE_PENDING_REVIEW`，为**两个非空组**）。该句已更正为「合成 61,000 行构成**两个**分类组（10,000 待确认 + 51,000 疑似重复）」，使 `importCandidateClassGroups` 仍在全部 61,000 行上运行其六个全列表过滤。**仅改散文，不动任何断言或测量行为。**

6. **状态与边界**：D-169 第 5 条登记的 61k 规模可操作性缺陷**维持开放**；OOM 修复的冻结阈值仍**只获部分裁决**——**≤160 MB** 与**零 ANR/OOM** 仍**通过**、**≤3 s 在 61k 仍未达**；**凡未获证据者不记 PASS**。本条**零产品代码改动**。

**边界：** 入：上述两个 test-only 测试文件、其 P3 文档更正、本条登记与 `docs/CURRENT_STATE.md` 的同步。出：零产品代码、零 schema/迁移/依赖/清单、零 Golden/fixture；不改 D-169 的任何裁决与其缺陷/开放状态、不改 D-168 的批准内容、不改 D-166 条目本身的缺陷与缺口状态、不改 D-170 的读数登记、不改 D-147/D-148 的读治理裁决、不改 D-157/D-159/D-162～D-167 的任何裁决与其 PASS 项；不把任何残余并入 PASS。

**关联决定：** D-169（第 5 条 61k 可操作性缺陷为本条登记对象，其代码定位为本条第 4 条更正对象——本条不改其缺陷与开放状态）、D-168（OOM 修复设计批准——其冻结阈值 ≤160 MB/≤3 s/零 ANR-OOM 为本条第 6 条状态依据）、D-166（第 3 条 OOM 缺陷登记链）、D-167（宿主侧读数的方法与披露先例——本条区间引用与不可复现性披露的依据）、D-163（宿主侧/设备侧口径区分先例）、D-170（同缺陷链上的残余设备向量读数登记）、阶段计划 §10.1/§10.3/§10.4.5。

## D-172 D-158 §5 有效谓词代价的设备侧读数登记（P7-05 片 1b 开放前置条件解除）

**状态：** 已批准并交付（2026-09-23，D-167 第 4(a) 条留作 P7-05 片 1b **开放前置条件**的「设备侧 D-158 §5 有效谓词代价读数」的登记：在受管 AVD 上以设备系统 SQLite 取得该读数；本条**只登记读数、方法与限定**，**不改 D-158/D-167 的任何裁决**，也不实施片 1b——零产品代码、零测试、零 schema/迁移/依赖，schema 维持 v31）。原始读数工件落于主 checkout 的只读设备目录 `local/artifacts/p7-05-scale/d158s5-device-reading.txt`（gitignored，不入 tracked 文件）。

**决定：**

1. **对象与性质**：本条对象为 D-167 第 4(a) 条明确留作**开放前置条件**的**设备侧** D-158 §5 读数（D-167 第 4(a) 条原文：设备侧的 D-158 §5 读数仍是 P7-05 片 1b 的开放前置条件，本读数不解除它）。本条**只登记该读数**：它**不**改变任何产品行为，**不**改 D-158 的任何裁决，**不**改 D-167 的宿主侧读数与其限定，也**不**实施片 1b。

2. **设备与程序**：设备 = AVD `ul_p7_d01`、隔离 adb 端口 5038、API 36、系统 SQLite 3.44.3（**设备引擎**，非宿主 JDBC）。库 = **20,002 交易 / 40,004 posting**，修复为干净 v30 后由应用在打开时迁移至 v31；计时前先执行 `ANALYZE`（生产 intake 完成语义，`sqlite_stat1` 在位）。方法 = 设备侧 `sqlite3 .timer on` 围绕两个生产查询的**精确 SELECT 形状**，**带** `transaction_effective_state` 有效谓词 join（生产）与**不带**（消融）各 3 轮；因 sqlite3 墙钟含进程启动，故以**消融侧与谓词侧的 DELTA** 为信号（两侧承担相同启动成本）。

3. **读数（一律以区间引用，不取单点）**：

   - `currentVersionRowsForLedger`：带谓词 **0.061 / 0.059 / 0.059 s**；不带（消融）**0.041 / 0.041 / 0.042 s**；delta **+20 / +18 / +17 ms（~+44%）**。
   - `ledgerEntryRowsForLedger`：带谓词 **0.060 / 0.060 / 0.064 s**；不带（消融）**0.041 / 0.041 / 0.042 s**；delta **+19 / +19 / +22 ms（~+46%）**。

   即两查询的 delta 约 **+17–22 ms（相对约 +44–46%）**，消融侧约 **41 ms**。

4. **结论（如实，不过度声称）**：在 20k 交易 / 40k posting 的设备规模上，有效谓词增加的是一项**小且一致的绝对成本**（约 **+18–23 ms**，消融侧约 41 ms）——**远低于**任何用户可感知门限，且**不是** D-158 §5 处置条件（索引边或另立决定）所针对的**数量级回归**。故 **D-158 §5 的设备前置条件已满足；其处置条件未被触发**。该结果与宿主侧 D-167 读数（约 +12–21 ms）**方向一致**，但本条提供的是 D-167 明确未提供的**设备**读数。

5. **限定（必须披露）**：(a) sqlite3 墙钟含约进程启动，故绝对数值是**上界**（信号是 delta）；(b) **单设备会话、3 轮**；(c) 该库为**同质**（单版本、**零作废事实**），故谓词的**过滤**成本未被行使——含作废事实的库会使谓词**过滤**而非**直通**，本读数**不测量该路径**。

6. **效果**：本条满足 D-167 §4(a) / D-158 §5 关于 P7-05 片 1b 的**设备前置条件**，片 1b **现可推进**。片 1b **尚未开始**、仍待实施；除本读数外**无任何项被记为 PASS**。D-167 自身的宿主侧读数与其披露的限定**不变**。

7. **读数披露（沿 D-167 第 3/5 条口径）**：以上为**写者的单次运行值**，由设备 shell 打印到 stdout；第 2 条引用的原始工件即留存证据；本条**不声称跨运行可复现性**，也不取任何数值作点估计。
...
3. **V-19 交互缺口登记（显式承接，不得静默丢弃）**：V-19 的「重开后可再次核对（沿 P7-02 S-3）」在片 1b 只满足能力层——快照感知 resolver（`ResolveTransactionCorrectionCommitStatus`/`ResolveTransactionVoidCommitStatus`）为 DB 支撑，其命中/冲突/缺失三种解析均已有测试（`P705EffectiveSurfaceTest.kt` 的 restore 半族向量，且这些测试在同一连接上进行、不关闭重开）。跨重开可解析性是对请求/回执行持久化（二者是同一数据库文件中的普通已提交行）的合理推断，**但并非**由「重开后再跑 resolver」的测试直接证实：V-16 重开测试并不读取修正/作废的请求或回执表（其断言仅及有效分录行、作废行、`transaction_version` 计数、余额、月度计数与回收站），故该可解析性**保留为残余**，不得记为已直接验证。但 P7-05 的修正/作废/恢复**没有任何在会话内的人工再核对入口**（P7-02 有 D-126 R4 的「重新核对」按钮）；丢失提交后仅保留 `submitting` 标记且宿主状态为 `remember`（非 `rememberSaveable`），重组/进程死亡后该面消失。因此 V-19 **不得判 PASS**，登记为「已实现待验收/缺证」，缺口为「P7-05 未知提交的人工再核对面」。
...
## D-176 P7-06 06.1 稳定存储与 LedgerRuntimeOwner 设计门批准

**状态：** 已批准（2026-09-24，P7-06 切片 06.1 / 06.D 前置设计规格的批准登记；规格 `docs/specs/2026-09-24-p7-06-stable-storage-runtime-owner-design.md` 经独立规格评审（首轮 REJECT，1 项 P1 + P2/P3；修订后闭包复评 APPROVE，全部 findings 已闭合）与 distinct verifier（承重事实 VERIFIED，含两处行号核对与一处依赖措辞更正）后，由主代理按用户既有授权批准）。

**决定：**

1. **交付与批准**：新增规格 `docs/specs/2026-09-24-p7-06-stable-storage-runtime-owner-design.md`，冻结 06.1 / 06.D 前置的设计级方案：两端稳定存储、旧路径升级、`LedgerRuntimeOwner`（单活动图、generation、operation lease、quiesce→close→reopen）。规格状态由 `proposal` 转为 `approved`（本决定即批准依据）。本规格承接已批准容器规格（D-174）§5/§6 中登记为 OPEN 的「稳定存储」与「文件指针切换/重启恢复」两项，**不重开**其任何冻结决定。

2. **关键设计裁决**：

   - **旧路径升级为非破坏式有序序列**：Android 既有 `databases/ledger.db`（含 `-wal`/`-shm`）首次接入代际目录时，**复制（非移动）→ 新代三文件 fsync → 打开并权威读回 → 发布原子指针（含目录 fsync）→ 仅在此后移除旧文件**；fsync 是**发布指针的前置**而非删除旧副本的前置。
   - **静默空库禁令**：全新安装**仅**在「无代际目录且无旧位置」时允许；**代际目录存在但指针缺失/无效 → fail-closed**（绝不回退为全新安装）；打开前守卫为**非空/有效 SQLite 头**检查，非仅存在性；非全新路径**不得**调用 create-on-open 工厂。
   - **owner 契约**：`quiesce()`（拒绝新租约、等待在飞）、`closeActiveGraph()`（internal，受在飞谓词守卫）、`reopen()`（在飞 > 0 返回类型化 `QuiesceBlocked`/`RuntimeNotReady`）；单活动约束由 owner 内部 `kotlinx.coroutines.sync.Mutex` + 原子状态承载（不依赖文件锁）。转换义务 = **每一个 `facade.*` 入口点**（异步 Default 启动、主线程同步读写、普通 `scope.launch` 目录命令三类）。
   - **启动顺序**：先处理未完成 journal（06.1 对存在 journal 一律 fail-closed）→ 再选择活动代 → 最后才允许正常业务。

3. **评审承接（已闭合）**：首轮 REJECT 的 P1（旧路径升级在指针持久前破坏性移动，且规则 3 可静默建空库）已按上述非破坏式序列与收紧规则闭合；P2（close/reopen 缺在飞谓词、租约清单不全、journal 行为未定、静默空库机制未述、未命名同步原语）与 P3 及 verifier 的两处过度归因均已修正。闭包复评新增的 P2（fsync 锚定于删除而非发布）与 P3（状态机路由、枚举不全、引用微调）亦已闭合。

4. **边界**：入本规格与 `docs/DECISIONS.md` 本条；零产品代码、零测试、零 schema/迁移、零依赖；不改 D-174 的容器格式冻结决定与 §6 门表状态、不改 D-156/D-158/D-175 及 P7-01～P7-05 既有冻结面；不把任何验收向量并入 PASS（P706-A03/A04/A06/A07/A10 仍为待实现/待验收，各自归 06.B/C/D 切片）；不声称 OS 自动备份/设备迁移已关闭（归隐私规格）。

5. **承接**：06.1 的实施（稳定存储、owner、旧路径升级、FOUND-001 回归）为后续实施批；06.B/C/D 的导出/预检/切换属更后切片。

**关联决定：** D-174（上游容器格式规格与门表——本条承接其 §5/§6 的 OPEN 项，不改其决定）、D-156/D-158（规格与登记先例）、D-175（前序最高 id）。

## D-177 P7-06 06.2/06.B 备份导出设计门批准

**状态：** 已批准（2026-09-24，P7-06 切片 06.2/06.B（一致快照 + 认证加密导出 + 取消/空间不足）设计规格的批准登记；规格 `docs/specs/2026-09-24-p7-06-backup-export-design.md` 经独立规格评审（首轮 APPROVE WITH FINDINGS，含 1 项 P2 写序矛盾 + P2/P3；修订后闭包复评 APPROVE WITH FINDINGS，全部条目已闭合）后，由主代理按用户既有授权批准）。

**决定：**

1. **交付与批准**：新增规格 `docs/specs/2026-09-24-p7-06-backup-export-design.md`，冻结 06.B 的设计级方案：取得 operation lease → `VACUUM INTO` 一致快照 → 关闭并 `PRAGMA integrity_check` 验证 → 有界流式认证加密写到用户目标 → 仅在认证尾部完成且流正常关闭后报成功 → 清理私有暂存。规格状态由 `proposal` 转为 `approved`（本决定即批准依据）。本规格**逐字节实现**已批准容器规格（D-174）§4 的冻结格式，**不重开**任何冻结字节/参数/失败码；并复用 06.1（D-176）的 runtime owner/lease。

2. **关键设计裁决**：

   - **`payload_sha256` 写序**：必须先以一遍 64 KiB 有界流式读**完整算完** `payload_sha256`，再组装固定头部（该值位于偏移 27，且固定头部是加密前供给的 AAD），然后第二遍有界流式读做 GCM 加密；**不存在「哈希与加密同一遍」的可行顺序**；两遍都不得整读入内存。
   - **写侧 1 GiB 明文上限为绑定上限**：快照前（源大小）与快照后（实际快照文件大小）两处门，均在用户目标创建前拒绝（`P706_EXPORT_PLAINTEXT_TOO_LARGE`）；因 `container_size = plaintext_len + 59 + salt_len + iv_len + tag_len`，明文 ≤ 1 GiB 时容器 ≤ 1 GiB + 103 字节，远低于 2 GiB 容器上限。**快照大小不保证等于源大小**（`VACUUM INTO` 重写页面布局），故必须两处门。
   - **投递原语**：容器投递**必须**使用 06.B 新增的**有界流式写端口**；**禁止**使用现有 `LedgerFileSystem.writeAtomic(path, bytes: ByteArray)`（整文件入内存，`LedgerStableStorage.kt:81-84`）；仅沿用其原子 rename **语义**（临时文件 + 原子替换）。
   - **资源与线程**：2 GiB 容器 / 1 GiB 明文 / 64 KiB 流式缓冲；禁止无界 `readBytes`；导出与加密**必须**在后台线程（61k 库 `VACUUM INTO` 实测 ~65.8 s / 354 MiB）。
   - **失败不伪成功**：失败的导出绝不报成功、不留下声称成功的部分产物；私有暂存按取消/会话结束/下次启动清理；OS 自动备份/设备迁移排除归隐私规格（不声称已关闭）。

3. **评审承接（已闭合）**：首轮 P2-1（`payload_sha256` 写序自相矛盾）已按上述两遍顺序闭合；P2-2（写侧 1 GiB 未门）、P2-3（投递误用整文件原语）、P3-1（`integrity_check` 执行面未登记）、P3-2（A02 的 WAL 证据口径夸大）、P3-3～P3-5（引用精度）均已修正；闭包复评新增的两项数字/单位更正（容器开销 103 字节、快照与源的大小方向与单位）亦已闭合。

4. **边界**：入本规格与 `docs/DECISIONS.md` 本条；零产品代码、零测试、零 schema/迁移、零依赖（除平台自带 crypto）；不改 D-174 的容器格式冻结决定、不改 D-176 的 06.1 决定、不改 P7-01～P7-05 既有冻结面；不把任何验收向量并入 PASS（P706-A01/A02/A08/A10 仍为待实现/待验收，06.C/06.D 部分归后续切片）。
...
## D-181 P7-05 片 1b 验收裁决登记（V-17 判 PASS 含披露与 V-19 会话内口径，片 1b 验收收口）

**状态：** 已批准（2026-09-26，用户对 P7-05 片 1b 验收残余的三项裁决登记：V-17 两项缺证分别接受延期与替代证据、V-19 的重开（应用重启）子句以会话内人工再核对为满足口径；V-17 判 PASS（含披露）、V-19 判 PASS（会话内口径），P7-05 片 1b 验收就此收口。本条为 docs-only 登记，零产品行为变更）。

**决定：**

1. **V-17 TalkBack 朗读巡检缺证 → 接受延期**：用户确认 TalkBack 属无障碍功能，不影响主要软件功能，沿 A-03 先例（TB1～TB7 TalkBack 朗读巡检接受延期，用户 2026-09-19 裁决）接受该项缺证的延期处置；缺证披露保留——headless AVD 不输出 speech 日志，设备朗读验证未执行，将来有真机/配置 TTS 的环境可补做；不阻塞后续切片。V-17 的其余项此前已 PASS（四项用户可观察期望与 V-16 设备部分 PASS，2026-09-24 执行，见 CURRENT_STATE 既有登记）。

2. **V-17 再核对入口设备观察缺证 → 接受现有证据**：Piece 6（`5ea7f67`）交付的入口行为已由 JVM 测试与结构证据覆盖（状态保持型再核对事件、确认时保留请求快照、三面宿主再核对入口 + `liveRegion` 状态行）；设备观察登记为环境限制、不可达——再核对入口仅在丢失提交后渲染，产品无入口可制造丢失提交状态，force-stop 连宿主状态一并清除。

3. **V-19 重开（应用重启）子句 → 以会话内人工再核对为满足口径**：V-19（`docs/specs/2026-09-19-p7-05-correction-void-recycle-design.md:252`，「重开后可再次核对（沿 P7-02 S-3）」）中的「重开（应用重启）后可再次核对」子句，由 Piece 6（`5ea7f67`）交付的**会话内**人工再核对面满足——本裁决即满足 V-19 验收；D-173 第 3 条登记的 V-19 交互缺口承接就此关闭；跨重启持久化未决提交**另立需求**——本裁决不排期、不自动立项，若将来立项须新决定。

4. **验收效果（显式登记）**：**V-17 判 PASS**（含两项缺证按本裁决接受的披露——设备朗读验证未执行、设备观察未执行，均为用户裁决接受的延期/替代证据；披露语义沿 A-03，缺证事实如实保留在案、不抹除）；**V-19 判 PASS**（会话内口径，重启持久化子句按第 3 条另立需求）。P7-05 片 1b 验收就此收口。
