...
4. **登记限定（不并入 PASS）**：
   ...
   - **该组规模的「卡页脚 / 批量入口」实测在本轮不可行**（沿 D-164 第 4(b) 条结构性成本观察）：组卡条目内联渲染在列表末尾的批量入口之前，50,800 条组的卡页脚按实测速率（≈4.1 项/动作、≈0.376 s/动作）推算需 ≈12,400 次动作 ≈1.3 h，超出致动器默认边界（`cardScrollActions` 默认 4,000）。故本轮只覆盖**数据条件**，不覆盖该规模上的**卡页脚可达性**；承接 = 由裁决明确该条件的读法（数据条件 vs 可达性条件），或改用关闭组卡的流程。
   - **单次 10k 导入的接治耗时观察（新增，非门）**：主机侧 SAF 选择与派发耗时 34 s；接治在 30 s 粒度轮询中约 **13 min** 后稳定到 +10,000（其间多次遇到数据库写锁，读侧需重试）。该耗时随库规模增长（同一 200 行导入在 50,600 候选库上约 111 s 稳定），登记为**规模相关的接治耗时观察**，不构成门槛判定，也不据此判定缺陷。
...
**关联决定：** D-164（第 4(b)(c)(d) 条为本条对象；其结构性成本观察为本条第 4 条依据）、D-162（第 3(i) 条观察维持开放）、D-159/D-157（§10.3 非 PASS 裁决与本条维持结论）、D-146（单文件 10,000 候选接治上限——本条第 2 条的边界依据）、阶段计划 §10.1/§10.2/§10.3/§10.4.5。
