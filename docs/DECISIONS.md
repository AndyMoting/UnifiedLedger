...
**与 D-152 的关系：** 复用 FIX-MONTH-1 的同一 arm/consume 机制——`onImportBatchConfirmed()` 触发权威刷新并置位同一 `pendingMonthlyReRequestAfterRefresh` 挂起标记，由同一落地跳消费：成功落地 `consumeMonthlyReRequestAfterRefresh`（恰一次无条件重请求，落月盖章抑制随后的 (a)/(d) 守卫；与 `P503CurrentStateLoadCoordinator` 合并重跑兼容——标记存活到首个成功落地）、失败落地 `dropMonthlyReRequestAfterFailedRefresh`（清标记不请求）。触发点有二、互斥且对一次运行恰置位一次：`runImportBatchDispatch` 的 finally 主线程跳 `completed == true` 分支；以及 Unknown 核对裁决使运行到达全部项终态时的核对结果落地跳（纯判定 `shouldArmImportBatchConfirmedAfterUnknownCheckResolution`：状态已离开 `ImportBatchSubmitting` 且保留摘要无剩余 Unknown——覆盖 reducer 自动离开与摘要已保留两条路径；批中裁决与 StillUnknown 不置位，运行继续时由 completed 分支置位）。暂停不触发。(e) 分支、consume/drop、(a)/(d) 守卫与 §6.2 吸收表零改动；(a)–(e) 语义逐字保留，仅增补 (f)。

**边界：** 入：`app-ui` 的宿主协调器/宿主与 commonTest，以及本条登记。出：零账务语义、零 `ledger-domain`/`ledger-application`/`ledger-data`、零 schema/迁移/依赖、零 Golden/fixture。自动验证：`:app-ui:jvmTest` 397/0/0/0（较基线 393 新增 4 向量：(f) 臂标记→落地恰一次消费 1、失败落地丢弃 1、与 (e) 同标记恰一次 1、核对全终态纯判定 1）、`:app-ui:ktlintCheck`、`:android-app:compileDebugKotlin` + `:desktop-app:compileKotlinJvm` 全绿。接线（`runImportBatchDispatch` completed 分支与 `checkImportUnknownItem` 核对落地跳两个触发点）位于 `@Composable` 宿主内、无既有 JVM 断言基建；纯判定以协调器向量覆盖，接线本身以设备回归覆盖并如实登记。设备回归（导入 CCB 样本 → 批量确认入账 → 首页月卡与流水当次会话立即反映确认结果、无需重启 → 月份切换正常（SelectMonth 域含新交易月）→ 重启持久）由合并后新 APK 的设备窗口（隔离 adb 5038）承担。技术注记：无。
