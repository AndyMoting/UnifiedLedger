...
1. **50k 就地证据（步骤之间不清库）**：在 50,000 候选库（同一 fixture 经五次产品 SAF 路径导入铺就，`sqlite_stat1` 统计在位）上，按计划 §10.3（`docs/PHASE7_REMAINING_IMPLEMENTATION_PLAN.local.md:188`）的顺序执行且**不在步骤之间清库**，下列阶段通过：冷启动→首页可操作（`am start -W` TotalTime 2,813–3,324 ms；首页首帧 Δ 2.010–4.020 s）、进入导入→列表可操作（Δ 1.060–1.274 s）、候选清单可见、候选详情打开（Δ 2.924 s，含重复候选的「重复比较」段完整渲染）。**同期分项测量**（同一证据周期内独立执行，作为该规模项的上下文）：(i) (f)/FIX-MONTH-1 的 20k 向量三轮，取最大值 **tap→commit 9.970 s、tap→lastframe 12.474 s**；(ii) INPUT-FREEZE 阈值——20k **人速 5/5 不触发**、**机械风暴第 6 次触发**，50k **第 2 次触发**（阈值随规模显著提前）；(iii) 50k 冷读分解**未复现 r7 记录的 31.62 s 量级**，且 20k 基线为单次样本、其库随后已被 50k 覆盖，该对比属**基线参照而非同质对照**。证据与测量方法见 `local/artifacts/a01-a05/`（`aperf_scale_session.py`/`aperf_freeze.py`/`aperf_coldread.py`/`aperf_f_run.py`/`aperf_f_round.py`/`saf_import.py`）。

2. **剩余四阶段的实测成因（机械滑动遍历触发 INPUT-FREEZE）**：以每探针 10 次快速 fling 推进 50k 列表，约 **50 次 fling 后停止推进**（可见行在连续探针之间钉在 `4.07 CNY`；123 s 内约推进 300 行）；冻结态与 `OBS-APERF-INPUT-FREEZE` 逐项吻合——列表忽略 200 ms 慢速滑动、**BACK 仍响应**、`top` 显示 **CPU 0.0%**、`dumpsys input` 显示 **`PendingEvent: <none>`**、force-stop 后恢复正常。该冻结**阻断的正是 §10.3 的遍历本身**，而非仅探针粒度问题；此前把失败归因于粗/细探针取舍的记录**由本测量取代**。**替代输入通道在该规模不可用**：键盘滚动（`KEYCODE_PAGE_DOWN`）每次约推进六行，`KEYCODE_MOVE_END` 与 `KEYCODE_DPAD_DOWN` 无有效推进，轨迹球 `input roll` 完全不滚动列表。**后果**：定位列表后段的单项入口（组处置入口与批量确认入口，二者均在整个候选清单之后发出）在该规模**必须依赖程序化滚动**（Compose semantics 滚动动作或 instrumented 驱动滚动）——这是**必需使能项而非优化**；「以机械 fling 遍历大列表」在该规模**不是可靠的测量方法**，与 20k 时遍历可行形成**规模相关差异**，一并登记为观察。

3. **裁决与承接**：**计划 §10.3 大数据整链规模项维持 D-157 的「缺证（部分证据在位）+ 承接」，不记 PASS**；本条增补就地证据与实测、可执行的缺证成因，但不闭合该项。所需证据在种类上不变：**所需规模上的单一连续会话覆盖所列全流程端到端**。承接 = (a) 为大规模库遍历实现程序化滚动；(b) 随后完成剩余四个阶段（重复组打开 → 审核/确认 → 详情与月度刷新 → 杀进程后重开）；(c) 再按计划 §10.1/§10.4 的格式重新评估。(a) 触及**测试/自动化基础设施**而非产品代码；新增测试基础设施本身是一项变更，须**先行登记**再实施。

4. **§10.4.5 纪律与归属保留**：按计划 §10.4.5，仍有必需项非 PASS 时报告部分完成——**无残余被并入 PASS**。`OBS-APERF-INPUT-FREEZE` 的归属维持既有裁决（属自动化输入模式，非已证实的用户可见缺陷），其**新测后果**——阻断大规模库遍历——登记为**验收方法上的真实约束**（见第 2 条）。

**边界：** 入：本条在 `docs/DECISIONS.md` 的追加登记（证据增补与承接细化）。出：零产品代码、零测试、零 schema/迁移/依赖/清单、零 Golden/fixture；不改 D-157 的裁决与其 PASS 项，不改 D-148～D-158 的任何裁决；`OBS-APERF-INPUT-FREEZE` 的既有归属不变（本条只增其后果测量）；程序化滚动属测试/自动化基础设施，须另行登记后实施；正式状态文档的同步不在本条范围。

**关联决定：** D-157（§10.3 规模项「缺证（部分证据在位）+ 承接」裁决的来源与本条复核对象——本条不改其任何裁决）、D-148（A-PERF 读治理与 B5 门槛状态更新；`OBS-APERF-INPUT-FREEZE` 与 50k 冷读观察的来源）、D-147（A-PERF 证据纪律——规模向量与测量方法依据）、D-153（(f)/FIX-MONTH-1 触发集来源）、D-155（A-DOC 批 A-05 事实登记先例）、阶段计划 §10.1/§10.2/§10.3/§10.4.5。
