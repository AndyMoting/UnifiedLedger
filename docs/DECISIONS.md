...
1. **置顶状态接线（A02PIN-001 / A02PIN-002）**：`TogglePin` 事件由「只带 target」扩展为携带 store 的权威结果 `pinned`，reducer 由「自行翻转」改为「设为期望值」——渲染副本与持久化状态因此不可能反向分叉（幂等：目标已在集合中再置顶不变，已不在集合中再取消也不变）。5 处编辑器流 `Back` 重建 `OverviewEmpty` 时显式携带 `pinnedTargets`，流状态相应新增该字段并随流贯穿（含冲突/拒绝屏的字段编辑、`AbandonConflict`、提交失败分支与 `UnknownCommit` 的冲突解析），`SelectTab` 与 READ 重试的集合保留同样显式化。置顶成功后宿主以同一权威结果重新派生管理快照并随事件安装，使账户/分类管理列表排序即时生效（A02PIN-001）；采用「事件携带快照」而非复用 `CatalogSnapshotRefreshed`，因为后者会清空 `catalogNotice`，会意外抹掉已打开的管理横幅与表单。普通 `RefreshResult` 的 `pinnedTargets` 改为可空缺省（缺省保持现值、提供则采用），刷新因此可自愈分叉。**store 的翻转语义、`EntryPinOrdering` 排序派生语义与 `entry_pin` 表结构零改动。**
