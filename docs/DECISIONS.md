...
**行为证据补充**（XXJZ/JiZhang 项目，行为证据 only）：XXJZ 使用固定列索引（时间列 0、金额列 6、备注列 2/4）；JiZhang 使用动态表头检测（查找含"交易时间"、"收/支"、"金额"的行）+ BOM 容错 + 数据区结束于 `---` 行。本规格按 9/9 真实文件取证冻结（固定第 23 行表头），行为证据仅作格式演化参考。
