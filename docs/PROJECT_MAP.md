...
P5-03 演示面 B 已在两端组合根接通：`app-ui` 提供启动、总览、编辑、确认和结果屏幕，以及纯 reducer 驱动的异步状态转场；Desktop 的鼠标流程和键盘/焦点流程、Android 的模拟器启动/重开/持久化流程与 TalkBack 流程均已完成人工验证。P5-04.2（D-125）将编辑流状态机扩展 `overview`/`originTab` 穿线与 `Back` 事件：编辑流各态系统返回关闭回来源 Tab，`app-ui` 暴露 `backHandler` 平台钩子，Android 组合根以 activity-compose `BackHandler` 接入（零依赖）。Android 调试 APK 由 CI artifact 提供，当前版本的数据库 schema 为 v30。

阶段路线与当前检查点分别见[路线图](ROADMAP.md)和[当前状态](CURRENT_STATE.md)；阶段 6（Android 视觉与平台适配）已于 2026-09-11 按 D-142 收口（D-133…D-141 全部批次交付并关闭）；阶段 7（Android 日常工作流）第一组 P7-01～P7-04（账户与分类目录管理、录入基础、看账、导入与草稿确认）已按 D-143/D-144/D-145/D-146 交付、合入并推送（push `a710c6c..12e3057`），其后 D-147 开卡枚举性能批（v30 覆盖索引）、A-PERF 导入读治理批与 A-04 建行 XLS Android 验证及能力翻转批（四格式双平台全可用）亦已交付并推送，其后 D-150～D-154（正式状态同步与 CCB fixture 隐私清理、A-02 缺陷修复与刷新链修复、导入批量确认完成触发刷新、导入决策面方向化分类源）亦已交付并推送（当前推送 head `23d3bd2`）；阶段 7 尚未完成，剩余 A-03 未竟项（TalkBack 朗读巡检接受延期、A03-8b 缺证）与 A-05 收口项以及 P7-05～09 批次。
