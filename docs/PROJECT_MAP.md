...
P5-03 演示面 B 已在两端组合根接通：`app-ui` 提供启动、总览、编辑、确认和结果屏幕，以及纯 reducer 驱动的异步状态转场；Desktop 的鼠标流程和键盘/焦点流程、Android 的模拟器启动/重开/持久化流程与 TalkBack 流程均已完成人工验证。P5-04.2（D-125）将编辑流状态机扩展 `overview`/`originTab` 穿线与 `Back` 事件：编辑流各态系统返回关闭回来源 Tab，`app-ui` 暴露 `backHandler` 平台钩子，Android 组合根以 activity-compose `BackHandler` 接入（零依赖）。Android 调试 APK 由 CI artifact 提供，当前版本的数据库 schema 为 v31。

阶段路线与当前检查点分别见[路线图](ROADMAP.md)和[当前状态](CURRENT_STATE.md)；阶段 6（Android 视觉与平台适配）已于 2026-09-11 按 D-142 收口（D-133…D-141 全部批次交付并关闭）；阶段 7（Android 日常工作流）第一组 P7-01～P7-04（账户与分类目录管理、录入基础、看账、导入与草稿确认）已按 D-143/D-144/D-145/D-146 交付、合入并推送（push `a710c6c..12e3057`），其后 D-147 开卡枚举性能批（v30 覆盖索引）、A-PERF 导入读治理批与 A-04 建行 XLS Android 验证及能力翻转批（四格式双平台全可用）亦已交付并推送，其后 D-150～D-154（正式状态同步与 CCB fixture 隐私清理、A-02 缺陷修复与刷新链修复、导入批量确认完成触发刷新、导入决策面方向化分类源）亦已交付并推送，其后 P7-05 设计门（D-156）、A-05 收口裁决（D-157）、P7-05 首切片（片 1a 基底层，D-158，schema v31）、ktlint 修复与 §10.3 规模项承接复核（D-159）亦已登记并推送，其后 A-DOC 批正式状态同步（D-160，schema v31）与 §10.3 规模证据批（D-161～D-166：程序化滚动致动器与四阶段证据、B5 测量方法复核、单次会话全链实测、数据条件实测、postScroll 决定性取证与 61k 规模 OOM 登记）亦已登记并推送（当前推送 head `aed0920`）；阶段 7 尚未完成，剩余 A-03 未竟项（TalkBack 朗读巡检接受延期、A03-8b 缺证）与 A-05 收口项，以及 P7-06 06.C/06.D 与 P7-07～09 批次（P7-05 片 1b 与 P7-06 06.1/06.B 已交付并合入 `main`；当前 head 同提交 CI 尚在运行/未决）。
