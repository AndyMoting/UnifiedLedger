...
| P4-06 片 1 信用生命周期（RL-05） | [实施规格](specs/2026-08-22-p4-06-slice1-credit-implementation-design.md)（approved；D-107） | schema v25（`24.sqm`）：多腿 decision snapshot 扩展、信用/混合 candidate 与 `import_candidate_payment_profile`、`mixed_payment` 关联组两表（建而不用）；`CreditFlowFormalFactory`、`AlipayCsvParser` 信用腿路由、`SqlDelightImportSpineStore` v3 kind | `P406CreditFullStateOracleTest`、`AlipayCsvParserCreditJvmTest`、`LedgerDatabaseMigrationTest` |
