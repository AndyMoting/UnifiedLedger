...
class SqlDelightRg09Store private constructor(
    ...
    fun snapshot(ledgerId: LedgerId): Rg09Snapshot = Rg09Runtime(catalog, loadPersistedSnapshot(ledgerId)).snapshot()

    private fun operationFingerprint(operation: Rg09Operation): String = Rg09Runtime(catalog, emptyList()).operationFingerprint(operation)

    private fun operationClass(operation: Rg09Operation): String =
        when (operation) {
    ...
    private fun restoreReturnedId(row: com.unifiedledger.data.db.SelectRg09ReturnedIds): Rg09ReturnedId =
        when (row.id_kind) {
    ...
    private fun restoreDiagnostics(row: com.unifiedledger.data.db.Rg09_operation): Rg09StaleDiagnostics? {
        if (row.preview_fingerprint == null ||
            row.current_fingerprint == null ||
            row.recomputed_replay_minor == null ||
            row.recomputed_delta_minor == null ||
            row.currency_code == null ||
            row.currency_precision == null
        ) {
            return null
        }
    ...
    private fun Rg09ReturnedId.stored(): Pair<String, String> =
        when (this) {
    ...
    private fun loadPersistedSnapshot(ledgerId: LedgerId): Rg09Snapshot {
        ...
        val sources =
            q.selectRg09AllSources(ledger).executeAsList().map { row ->
                val currency = CurrencyUnit(row.currency_code, row.currency_precision.toInt())
                Rg09SourceRecord(
        ...
        val observations =
            q.selectRg09AllObservations(ledger).executeAsList().map { row ->
                val currency = CurrencyUnit(row.currency_code, row.currency_precision.toInt())
                Rg09Observation(
        ...
        val evidence =
            q.selectRg09AllEvidence(ledger).executeAsList().map { row ->
                Rg09Evidence(
        ...
        val evidenceLinks =
            q.selectRg09AllEvidenceLinks(ledger).executeAsList().map { row ->
                Rg09EvidenceLink(
        ...
        val candidates =
            q.selectRg09AllCandidates(ledger).executeAsList().map { row ->
                val currency = CurrencyUnit(row.currency_code, row.currency_precision.toInt())
                Rg09Candidate(
        ...
        val confirmations =
            q.selectRg09AllConfirmations(ledger).executeAsList().map { row ->
                Rg09Confirmation(
        ...
        val adjustmentHistory =
            q
                .selectRg09AllAdjustmentHistory(ledger)
                .executeAsList()
        ...
        val adjustments =
            q.selectRg09AllAdjustments(ledger).executeAsList().map { row ->
                val currency = CurrencyUnit(row.currency_code, row.currency_precision.toInt())
                ...
                val allocated =
                    allocationsByAdjustment[row.adjustment_id]
                        .orEmpty()
        ...
        val auditLinks =
            q.selectRg09AllAuditLinks(ledger).executeAsList().map { row ->
                Rg09AuditLink(
        ...
        val reconciliation =
            q
                .selectRg09AllPostingReconciliations(ledger)
                .executeAsList()
    ...
    private fun loadFormalTransactions(ledgerId: LedgerId): List<Rg09FormalTransactionRecord> {
        ...
        val sourceMap =
            database.ledgerQueries
                .selectRg09FormalTransactionSources(ledger)
                .executeAsList()
        ...
        return database.ledgerQueries.selectRg09FormalTransactions(ledger).executeAsList().map { row ->
            val versions =
                database.ledgerQueries
                    .selectRg09FormalVersions(ledger, row.transaction_id)
                    .executeAsList()
                    .map { version ->
                        TransactionVersion(
            ...
            val postingSets =
                versions.map { it.postingSetId }.distinct().map { postingSetId ->
                    val postings =
                        database.ledgerQueries
                            .selectRg09FormalPostings(ledger, postingSetId.value)
                            .executeAsList()
    ...
    private fun persistDelta(
        ...
        after.sourceRecords.filter { it.id.value !in beforeSourceIds }.forEach { source ->
            database.ledgerQueries.insertRg09Source(
                ...
                source.amount.currency.precision
                    .toLong(),
        ...
        after.observations.filter { it.id.value !in beforeObservationIds }.forEach { observation ->
            database.ledgerQueries.insertRg09Observation(
                ...
                observation.targetAmount.currency.precision
                    .toLong(),
    ...
    private fun persistCandidateDelta(
        ...
        val existingHistory =
            database.ledgerQueries
                .selectRg09AllCandidateHistory(operation.ledgerId.value)
                .executeAsList()
        ...
        after.candidates.forEach { candidate ->
            ...
            if (old == null) {
                database.ledgerQueries.insertRg09Candidate(
                    ...
                    candidate.delta.currency.precision
                        .toLong(),
    ...
    private fun persistNewFormalSemantics(
        ...
        newFormal.forEach { record ->
            ...
            when (formal.transaction.kind) {
                ...
                -> {
                    val targetRole =
                        if (formal.transaction.kind == TransactionKind.BALANCE_ADJUSTMENT) {
                            "ADJUSTMENT_TARGET"
                        } else {
                            "REVERSAL_TARGET"
                        }
    ...
    private fun persistReconciliationTransitions(
        ...
        val histories =
            database.ledgerQueries
                .selectRg09AllReconciliationHistory(ledgerId.value)
                .executeAsList()
        ...
        after.reconciliation.forEach { (postingId, currentStatus) ->
            ...
            val link =
                newLinks.firstOrNull { it.targetId == postingId }
    ...
    private fun operationOccurredAt(operation: Rg09Operation): Instant =
        when (operation) {
