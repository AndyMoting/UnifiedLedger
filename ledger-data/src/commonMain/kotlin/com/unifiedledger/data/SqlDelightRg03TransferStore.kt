...
import com.unifiedledger.application.Rg03CandidateRecoveryPort
...
import com.unifiedledger.application.Rg03MirrorBindingPort
...
import com.unifiedledger.application.Rg03MirrorTarget
...
import com.unifiedledger.application.Rg03PreparedOperationCommitPort
...
import com.unifiedledger.domain.validateIncompleteOwnAssetTransferSource
...
data class Rg03MirrorCommitIds(
...
internal fun interface Rg03FailureInjector {
    fun failAt(point: Rg03FailurePoint)
}
...
class SqlDelightRg03TransferStore private constructor(
...
) : Rg03PreparedOperationCommitPort,
    Rg03CandidateRecoveryPort,
    Rg03MirrorBindingPort {
    constructor(
    ...
    override fun commit(operation: Rg03PreparedOperation): Rg03ExecutionResult =
    ...
    override fun resolve(
        ...
        val targets =
            ...
            database.ledgerQueries
                .selectRg03MirrorBindings(ledgerId.value, scope.candidateId.value) {
                    candidateId,
                    transactionId,
                    postingId,
                    destinationAccountId,
    ...
    private fun commitManual(snapshot: Rg03ManualTransferSnapshot): Rg03ExecutionResult {
        return database.transactionWithResult {
            database.ledgerQueries.claimRg03OperationRequest(
                ...
                Rg03ActionType.MANUAL_ACCOUNT_TRANSFER.name,
            )
            ...
            database.ledgerQueries.insertRg03ManualTransferSnapshot(
                snapshot.ledgerId.value,
                snapshot.requestId.value,
                snapshot.originalOccurredAtText,
                snapshot.sourceAccountId.value,
                snapshot.destinationAccountId.value,
                snapshot.sourceDebit.minorUnits,
                snapshot.destinationCredit.minorUnits,
                snapshot.fee.minorUnits,
                snapshot.sourceDebit.currency.code,
                snapshot.sourceDebit.currency.precision
                    .toLong(),
                    ...
                    val created =
                        ...
                        createOwnAssetAccountTransfer(
                            ...
                            OwnAssetAccountTransferCommand(
                                ...
                                snapshot.destinationAccountId,
                                snapshot.sourceDebit,
                                snapshot.destinationCredit,
                                snapshot.fee,
            ...
            database.ledgerQueries.insertRg03Confirmation(
                ...
                snapshot.requestId.value,
                null,
                transfer.formalTransaction.transaction.id.value,
                "MANUAL_TRANSFER",
            )
            ...
            database.ledgerQueries.insertRg03PostingReconciliation(
                snapshot.ledgerId.value,
                ids.sourceReconciliationId,
                ids.transferIds.sourcePostingId.value,
                "PENDING",
            )

            database.ledgerQueries.insertRg03PostingReconciliation(
                snapshot.ledgerId.value,
                ids.destinationReconciliationId,
                ids.transferIds.destinationPostingId.value,
                "PENDING",
            )
    ...
    private fun commitSource(snapshot: Rg03SourceSnapshot): Rg03ExecutionResult {
        return database.transactionWithResult {
            database.ledgerQueries.claimRg03OperationRequest(
                snapshot.ledgerId.value,
                snapshot.requestId.value,
                Rg03ActionType.IMPORT_SOURCE_RECORD.name,
            )
            ...
            database.ledgerQueries.insertRg03SourceIntakeRequestSnapshot(
                ...
                snapshot.originalObservedAtText,
                snapshot.sourceAccountId.value,
                snapshot.sourceDebit.minorUnits,
                snapshot.sourceDebit.currency.code,
                snapshot.sourceDebit.currency.precision
    ...
    private fun commitCandidate(
        ...
        return database.transactionWithResult {
            database.ledgerQueries.claimRg03OperationRequest(
                ...
                requestId.value,
                Rg03ActionType.EXPLICIT_CANDIDATE_CONFIRMATION.name,
            )
                    ...
                    val created =
                        ...
                        createOwnAssetAccountTransfer(
                            ...
                            OwnAssetAccountTransferCommand(
                                ...
                                destination,
                                candidate.sourceDebit,
                                destinationCredit,
                                fee,
                                candidate.feeCategoryId,
                                TransactionTimes.collapsed(candidate.observedAt),
                            ),
                            ids.transferIds,
                        )
            ...
            database.ledgerQueries.updateRg03CandidateStatus(
                candidate.ledgerId.value,
                candidate.candidateId.value,
                checkNotNull(ids.candidateStatusId),
                "CONFIRMED",
                requestId.value,
            )
            ...
            database.ledgerQueries.insertRg03OperationReceipt(
                ...
                candidate.evidenceId.value,
            )

            database.ledgerQueries.insertRg03EvidenceLink(
                candidate.ledgerId.value,
                checkNotNull(ids.sourceEvidenceLinkId),
                candidate.evidenceId.value,
                ids.transferIds.sourcePostingId.value,
                "POSTING",
    ...
    private fun commitMirror(
        ...
        database.transactionWithResult {
            ...
            database.ledgerQueries.insertRg03MirrorSnapshot(
                ...
                snapshot.credit.minorUnits,
                snapshot.credit.currency.code,
                snapshot.credit.currency.precision
                    .toLong(),
            )
            ...
            database.ledgerQueries.insertRg03EvidenceLink(
                ...
                linkId,
                snapshot.evidenceId.value,
                persistedTarget.destinationPostingId.value,
                "POSTING",
            ...
            database.ledgerQueries.insertRg03OperationReceipt(
                ...
                "ACCEPTED",
                null,
                persistedTarget.candidateId.value,
                persistedTarget.transactionId.value,
                snapshot.sourceId.value,
                snapshot.evidenceId.value,
            )
            ...
            database.ledgerQueries
                .selectRg03SourceCommit(
                    ...
                    StoredSource(
                        ...
                        destinationCreditMinor,
                        feeMinor,
                        checkNotNull(candidateId),
                        checkNotNull(receiptSourceId),
                        checkNotNull(receiptEvidenceId),
                    )
                }.executeAsOneOrNull() ?: return Rg03ExecutionResult.RequestIdentityConflict
            ...
            database.ledgerQueries
                .selectRg03ConfirmationCommit(
                    ...
                    StoredConfirmation(
                        ...
                        storedCandidateId,
                        confirmed,
                        checkNotNull(confirmationId),
                        checkNotNull(receiptCandidateId),
    ...
    private fun persistFormalTransaction(
        ...
        database.ledgerQueries.insertTransactionCurrentVersion(
            ...
            value.transaction.ledgerId.value,
            value.transaction.currentVersionId.value,
        )
    ...
    private fun persistTransferSemantics(
        ledgerId: LedgerId,
...
private data class StoredManual(
    ...
    fun matches(snapshot: Rg03ManualTransferSnapshot): Boolean =
        ...
        action == Rg03ActionType.MANUAL_ACCOUNT_TRANSFER.name &&
            ...
            occurredAt == snapshot.originalOccurredAtText &&

            Instant.parse(occurredAt) == snapshot.occurredAt &&
...
private data class StoredSource(
    ...
    fun matches(snapshot: Rg03SourceSnapshot): Boolean =
        ...
        action == Rg03ActionType.IMPORT_SOURCE_RECORD.name &&
            ...
            observedAt == snapshot.originalObservedAtText &&

            Instant.parse(observedAt) == snapshot.observedAt &&
...
private fun DomainViolation.toRg03Rejected(): Rg03ExecutionResult.Rejected =
    ...
    when (this) {
        ...
        AccountTransferViolation.AmountsMustBalance ->
        ...
        AccountTransferViolation.SameCurrencyRequired ->
        ...
        AccountTransferViolation.FeeMustNotBeNegative ->
        ...
        AccountTransferViolation.InvalidFeeCategory ->
...
private fun AccountTransferField.inputField(): String =
...
private val SOURCE_VALIDATION_IDS =
