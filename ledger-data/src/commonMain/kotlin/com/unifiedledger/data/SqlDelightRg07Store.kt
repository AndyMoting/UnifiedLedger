...
interface Rg07IdentitySource {
    ...
    fun domainEntity(operation: Rg07Operation, relationId: String): String
    fun formal(operation: Rg07Operation): Rg07FormalIds
    fun receipt(operation: Rg07Operation, relationId: String, assetPostingId: String): Rg07ReceiptCommitIds
}
...
class SqlDelightRg07Store(
    ...
    init { configureSqliteConnection(driver) }

    override fun commit(operation: Rg07Operation): Rg07ExecutionResult {
        database.ledgerQueries.selectRg07Operation(operation.ledgerId.value, operation.identity.value)
    ...
    private fun execute(operation: Rg07Operation): Rg07ExecutionResult = when (operation) {
    ...
    private fun executeManualExpense(operation: Rg07Operation.ManualExpense): Rg07ExecutionResult {
        ...
        val category = categoryWithPostingAccount(input.categoryId)
    ...
    private fun executeStatus(operation: Rg07Operation.Status): Rg07ExecutionResult {
        ...
        val original = originalExpense(operation.ledgerId, input.originalTransactionId)
            ?: return rejected(Rg07RejectionReason.EFFECTIVE_CONFIRMED_ORIGINAL_EXPENSE_REQUIRED, Rg07FieldPath.ORIGINAL_TRANSACTION_ID)
        if (input.requestedAmount.currency.code != original.currencyCode || input.requestedAmount.currency.precision.toLong() != original.currencyPrecision) {
    ...
    private fun executeImportCredit(operation: Rg07Operation.ImportCredit): Rg07ExecutionResult {
        val input = operation.input
        if (input.amount.minorUnits <= 0L) return rejected(Rg07RejectionReason.MUST_BE_POSITIVE, Rg07FieldPath.AMOUNT)
        val account = catalog.accounts.singleOrNull { it.id == input.accountId }
            ?: return rejected(Rg07RejectionReason.KNOWN_DESTINATION_ACCOUNT_REQUIRED, Rg07FieldPath.DESTINATION_ACCOUNT_ID)
    ...
    private fun executeStatusSource(operation: Rg07Operation.StatusSource): Rg07ExecutionResult {
        ...
        if (database.ledgerQueries.selectRg07EntityByRelation(
    ...
    private fun executeManualReceipt(operation: Rg07Operation.ManualReceipt): Rg07ExecutionResult {
        ...
        val relation = database.ledgerQueries.selectRg07EntityByRelation(operation.ledgerId.value, relationId).executeAsOneOrNull()
            ?: return rejected(Rg07RejectionReason.REFUND_RELATION_NOT_FOUND, Rg07FieldPath.REFUND_RELATION_ID)
        val originalTransactionId = input.originalTransactionId
    ...
    private fun executeImportConfirm(operation: Rg07Operation.ImportConfirm): Rg07ExecutionResult {
        ...
        val destinationAccountId = input.destinationAccountId
        ...
        val candidate = database.ledgerQueries.selectRg07Candidate(operation.ledgerId.value, input.candidateId).executeAsOneOrNull()
    ...
    private fun executeOriginalPaymentEvidence(operation: Rg07Operation.OriginalPaymentEvidence): Rg07ExecutionResult {
        ...
        val posting = database.ledgerQueries.selectRg07AssetPosting(operation.ledgerId.value, input.paymentAssetPostingId).executeAsOneOrNull()
            ?: return rejected(Rg07RejectionReason.EVIDENCE_TARGET_MISMATCH, Rg07FieldPath.ORIGINAL_TRANSACTION_ID)
        if (posting.amount_minor != input.amount.minorUnits) {
            return rejected(Rg07RejectionReason.EVIDENCE_TARGET_MISMATCH, Rg07FieldPath.ORIGINAL_TRANSACTION_ID)
        }
        if (posting.currency_code != input.amount.currency.code || posting.currency_precision != input.amount.currency.precision.toLong()) {
            return rejected(Rg07RejectionReason.EVIDENCE_TARGET_MISMATCH, Rg07FieldPath.ORIGINAL_TRANSACTION_ID)
        }
        return accepted(operation)
    }
    ...
    private fun executeDualRoleEvidence(operation: Rg07Operation.DualRoleEvidence): Rg07ExecutionResult {
        val input = operation.input
        val entity = database.ledgerQueries.selectRg07EntityByRelation(operation.ledgerId.value, input.refundRelationId).executeAsOneOrNull()
            ?: return rejected(Rg07RejectionReason.EVIDENCE_TARGET_MISMATCH, Rg07FieldPath.EVIDENCE_ID)
    ...
    private fun executeMirror(operation: Rg07Operation.Mirror): Rg07ExecutionResult {
        val input = operation.input
        if (input.amount.minorUnits <= 0L) return rejected(Rg07RejectionReason.MUST_BE_POSITIVE, Rg07FieldPath.AMOUNT)
        val originalSource = database.ledgerQueries.selectRg07Source(operation.ledgerId.value, mirrorOfSourceId(input.sourceId)).executeAsOneOrNull()
            ?: return rejected(Rg07RejectionReason.EVIDENCE_TARGET_MISMATCH, Rg07FieldPath.EVIDENCE_ID)
        if (input.amount.minorUnits != originalSource.amount_minor ||
            input.amount.currency.code != originalSource.currency_code ||
            input.amount.currency.precision.toLong() != originalSource.currency_precision
        ) {
            return rejected(Rg07RejectionReason.EVIDENCE_TARGET_MISMATCH, Rg07FieldPath.EVIDENCE_ID)
        }
        return accepted(operation)
    ...
    private fun executeValidate(operation: Rg07Operation.Validate): Rg07ExecutionResult {
        ...
        val original = originalExpense(operation.ledgerId, originalId)
            ?: return rejected(Rg07RejectionReason.EFFECTIVE_CONFIRMED_ORIGINAL_EXPENSE_REQUIRED, Rg07FieldPath.ORIGINAL_TRANSACTION_ID)
        if (input.amount.currency.code != original.currencyCode || input.amount.currency.precision.toLong() != original.currencyPrecision) {
            return rejected(Rg07RejectionReason.SAME_CURRENCY_REQUIRED, Rg07FieldPath.CURRENCY)
        }
        val destinationId = input.destinationAccountId
            ?: return rejected(Rg07RejectionReason.KNOWN_DESTINATION_ACCOUNT_REQUIRED, Rg07FieldPath.DESTINATION_ACCOUNT_ID)
    ...
    private fun validateReceipt(ledgerId: LedgerId, proposal: ReceiptProposal): Rg07ExecutionResult.Rejected? {
        if (proposal.amount.minorUnits <= 0L) return rejected(Rg07RejectionReason.MUST_BE_POSITIVE, Rg07FieldPath.AMOUNT)
        val original = originalExpense(ledgerId, proposal.originalTransactionId)
            ?: return rejected(Rg07RejectionReason.EFFECTIVE_CONFIRMED_ORIGINAL_EXPENSE_REQUIRED, Rg07FieldPath.ORIGINAL_TRANSACTION_ID)
        if (proposal.amount.currency.code != original.currencyCode || proposal.amount.currency.precision.toLong() != original.currencyPrecision) {
            return rejected(Rg07RejectionReason.SAME_CURRENCY_REQUIRED, Rg07FieldPath.CURRENCY)
        }
    ...
    private fun persistManualExpense(operation: Rg07Operation.ManualExpense) {
        ...
        check(createAssetPaidOrdinaryExpense(
            catalog,
        ...
        database.ledgerQueries.insertPosting(expensePostingId, postingSetId, ledgerId.value, 0L, expenseAccount, input.amount.minorUnits, input.amount.currency.code, input.amount.currency.precision.toLong())
        database.ledgerQueries.insertPosting(assetPostingId, postingSetId, ledgerId.value, 1L, input.paymentAccountId.value, -input.amount.minorUnits, input.amount.currency.code, input.amount.currency.precision.toLong())
        ...
        database.ledgerQueries.insertRg07OperationConfirmation(
            ledgerId.value, commitFacts.confirmationId, identity.operationId(operation), identity.operationId(operation),
            commitFacts.confirmedAt.toString(), transactionId,
        )
    ...
    private fun persistStatus(operation: Rg07Operation.Status) {
        ...
        if (existing == null) {
            ...
            database.ledgerQueries.insertRg07RefundRelationship(
                ledgerId.value, entityId, relationId, input.originalTransactionId.value, null,
                original.categoryId, input.requestedAmount.minorUnits, 0,
                input.requestedAmount.currency.code, input.requestedAmount.currency.precision.toLong(), null,
                input.requestedAt?.toString(), input.approvedAt?.toString(), input.processorReportedAt?.toString(),
                null, null, null, null, null,
            )
            var sequence = 1
            input.requestedAt?.let {
                database.ledgerQueries.insertRg07RefundRelationshipHistory(
                    ledgerId.value, entityId, sequence.toLong(), historyId(relationId, Rg07RefundStatus.REQUESTED),
                    operation.identity.value, "REQUESTED", it.toString(), null, 0L,
                )
                sequence++
            }
            input.approvedAt?.let {
                database.ledgerQueries.insertRg07RefundRelationshipHistory(
                    ledgerId.value, entityId, sequence.toLong(), historyId(relationId, Rg07RefundStatus.APPROVED),
                    operation.identity.value, "APPROVED", it.toString(), null, 0L,
                )
                sequence++
            }
            input.processorReportedAt?.let {
                database.ledgerQueries.insertRg07RefundRelationshipHistory(
                    ledgerId.value, entityId, sequence.toLong(), historyId(relationId, Rg07RefundStatus.PROCESSING),
                    operation.identity.value, "PROCESSING", it.toString(), null, 0L,
                )
            }
        } else {
            ...
            val next = input.processorReportedAt?.let { Rg07RefundStatus.PROCESSING }
            ...
            if (isValidRg07StatusTransition(previous, next)) {
                ...
                database.ledgerQueries.insertRg07RefundRelationshipHistory(
                    ledgerId.value, existing.entity_id, nextSequence, historyId(relationId, next),
                    operation.identity.value, next.name, occurredAt.toString(), null, 0L,
                )
            }
        }
    }
    ...
    private fun persistStatusSource(operation: Rg07Operation.StatusSource) {
        ...
        database.ledgerQueries.insertRg07Source(
            ledgerId.value, input.sourceId, "merchant_refund_notice",
            input.sourceId, "evidence-$sourceSuffix", "sha256:${input.sourceId}",
            input.observedAt.toString(), input.observedAt.toString(),
            null, null, null, null,
            input.reportedState.name, if (input.provesArrival) 1 else 0, null, null, null, null, null, null,
        )
        database.ledgerQueries.insertRg07Evidence(
            ledgerId.value, "evidence-$sourceSuffix", input.sourceId,
            "refund_notice", input.observedAt.toString(), input.observedAt.toString(), null, null,
        )
        database.ledgerQueries.insertRg07EvidenceLink(
            ledgerId.value, "evidence-link-$sourceSuffix", "evidence-$sourceSuffix",
            "relation", relationId, "refund_relationship",
        )
        ...
        if (isValidRg07StatusTransition(previous, next)) {
            ...
            database.ledgerQueries.insertRg07RefundRelationshipHistory(
                ledgerId.value, entity.entity_id, nextSequence, historyId(relationId, next),
                operation.identity.value, next.name, input.observedAt.toString(), null, 0L,
            )
        }
    }
    ...
    private fun persistManualReceipt(operation: Rg07Operation.ManualReceipt) {
        val input = operation.input
        val proposal = receiptProposal(
            input.originalTransactionId, input.amount, input.categoryId, input.destinationAccountId, input.arrivedAt, input.confirmedAt,
        ) ?: return
    ...
    private fun persistImportConfirmation(operation: Rg07Operation.ImportConfirm) {
        val input = operation.input
        val proposal = receiptProposal(
            input.originalTransactionId, input.allocatedAmount, input.categoryId, input.destinationAccountId, input.arrivedAt, input.confirmedAt,
        ) ?: return
        val original = checkNotNull(originalExpense(operation.ledgerId, proposal.originalTransactionId))
        ...
        database.ledgerQueries.insertRg07CandidateStatus(
            operation.ledgerId.value, input.candidateId, 2, "history-rg07-import-confirmed", "CONFIRMED",
            proposal.confirmedAt.toString(), 1,
        )
        val assetPostingId = "posting-refund-asset-${input.candidateId.removePrefix("candidate-refund-")}"
        database.ledgerQueries.insertRg07EvidenceLink(
            operation.ledgerId.value, evidenceLinkId, candidate.evidence_id,
            "posting", assetPostingId, "destination_asset_posting",
        )
    }
    ...
    private fun persistReceipt(
        ...
        val pendingEntity = (relationId?.let { requestedRelationId ->
        ...
        database.ledgerQueries.insertRg07Confirmation(
            ledgerId.value, commitIds.confirmationId, identity.operationId(operation), entityRelationId,
            proposal.confirmedAt.toString(), proposal.originalTransactionId.value,
        )

        if (pendingEntity != null) {
            database.ledgerQueries.updateRg07RefundRelationshipReceipt(
                transactionId, amount.minorUnits, proposal.destinationAccountId.value,
                sourceObservedAt?.toString(), bookingAt?.toString(), valueAt?.toString(),
                proposal.confirmedAt.toString(), proposal.arrivedAt.toString(),
                ledgerId.value, pendingEntity.entity_id,
            )
            ...
            database.ledgerQueries.insertRg07RefundRelationshipHistory(
                ledgerId.value, pendingEntity.entity_id, nextSequence, historyId(entityRelationId, Rg07RefundStatus.RECEIVED),
                operation.identity.value, "RECEIVED", proposal.confirmedAt.toString(), transactionId, 1L,
            )
        } else {
            val entityId = commitIds.domainEntityId
            database.ledgerQueries.insertRg07RefundRelationship(
                ledgerId.value, entityId, entityRelationId, proposal.originalTransactionId.value, transactionId,
                proposal.categoryId.value, amount.minorUnits, amount.minorUnits,
                amount.currency.code, amount.currency.precision.toLong(), proposal.destinationAccountId.value,
                null, null, processorReportedAt?.toString(),
                sourceObservedAt?.toString(), bookingAt?.toString(), valueAt?.toString(),
                proposal.confirmedAt.toString(), proposal.arrivedAt.toString(),
            )
            database.ledgerQueries.insertRg07RefundRelationshipHistory(
                ledgerId.value, entityId, 1, historyId(entityRelationId, Rg07RefundStatus.RECEIVED),
                operation.identity.value, "RECEIVED", proposal.confirmedAt.toString(), transactionId, 1L,
            )
        }
        ...
        database.ledgerQueries.insertRg07ReconciliationHistory(
            ledgerId.value, commitIds.reconciliationId, 1, if (matched) "MATCHED" else "PENDING", evidenceLinkId,
        )
    }
    ...
    private fun persistOriginalPaymentEvidence(operation: Rg07Operation.OriginalPaymentEvidence) {
        ...
        database.ledgerQueries.insertRg07Source(
            ledgerId.value, input.sourceId, "bank_debit",
            input.sourceId.replaceFirst("source-", "source-record-"), input.evidenceId, input.immutablePayloadHash,
            input.observedAt.toString(), input.observedAt.toString(),
            input.amount.minorUnits, input.amount.currency.code, input.amount.currency.precision.toLong(),
            null, null, null, null, input.observedAt.toString(), input.bookingAt.toString(), input.valueAt.toString(),
            null, null,
        )
        database.ledgerQueries.insertRg07Evidence(
            ledgerId.value, input.evidenceId, input.sourceId,
            "asset_debit", input.observedAt.toString(), input.observedAt.toString(), null, null,
        )
        database.ledgerQueries.insertRg07EvidenceLink(
            ledgerId.value, linkId, input.evidenceId,
            "posting", input.paymentAssetPostingId, "payment_asset_posting",
        )
    ...
    private fun persistDestinationEvidence(operation: Rg07Operation.DestinationEvidence) {
        ...
        database.ledgerQueries.insertRg07Source(
            ledgerId.value, input.sourceId, "wallet_credit",
            input.sourceId, input.evidenceId, "sha256:${input.sourceId}",
            input.bookingAt.toString(), input.bookingAt.toString(),
            input.amount.minorUnits, input.amount.currency.code, input.amount.currency.precision.toLong(),
            input.accountId.value, null, null, null, null, input.bookingAt.toString(), input.valueAt.toString(),
            "sha256:$sourceSuffix", null,
        )
        database.ledgerQueries.insertRg07Evidence(
            ledgerId.value, input.evidenceId, input.sourceId,
            "asset_credit", input.bookingAt.toString(), input.bookingAt.toString(), null, null,
        )
        database.ledgerQueries.insertRg07EvidenceLink(
            ledgerId.value, linkId, input.evidenceId,
            "posting", postingId, "destination_asset_posting",
        )
    ...
    private fun persistDualRoleEvidence(operation: Rg07Operation.DualRoleEvidence) {
        ...
        database.ledgerQueries.insertRg07Source(
            ledgerId.value, input.sourceId, "combined_refund_statement",
            input.sourceId, input.evidenceId, "sha256:${input.sourceId}",
        ...
        database.ledgerQueries.insertRg07Evidence(
            ledgerId.value, input.evidenceId, input.sourceId,
            "combined_refund_statement", observedAt, observedAt, null, null,
        )
        database.ledgerQueries.insertRg07EvidenceLink(
            ledgerId.value, "evidence-link-$sourceSuffix-relation", input.evidenceId,
            "relation", relationId, "refund_relationship",
        )
        database.ledgerQueries.insertRg07EvidenceLink(
            ledgerId.value, "evidence-link-$sourceSuffix-posting", input.evidenceId,
            "posting", postingId, "destination_asset_posting",
        )
    }
    ...
    private fun persistImportCredit(operation: Rg07Operation.ImportCredit) {
        ...
        database.ledgerQueries.insertRg07Source(
            ledgerId.value, input.sourceId, "wallet_credit",
            input.sourceRecordId, evidenceId, "sha256:${input.sourceId}",
            input.sourceObservedAt.toString(), input.sourceObservedAt.toString(),
            input.amount.minorUnits, input.amount.currency.code, input.amount.currency.precision.toLong(),
            input.accountId.value, null, null,
            input.processorReportedAt.toString(), input.sourceObservedAt.toString(),
            input.bookingAt.toString(), input.valueAt.toString(),
            input.originalSourcePayloadHash, null,
        )
        database.ledgerQueries.insertRg07Evidence(
            ledgerId.value, evidenceId, input.sourceId,
            "asset_credit", input.sourceObservedAt.toString(), input.sourceObservedAt.toString(), null, null,
        )
        database.ledgerQueries.insertRg07Candidate(
            ledgerId.value, candidateId, input.sourceId, evidenceId, "0.97",
            input.amount.minorUnits, input.amount.currency.code, input.amount.currency.precision.toLong(),
            null, null, input.accountId.value, input.bookingAt.toString(),
            input.originalSourcePayloadHash, 1,
        )
        database.ledgerQueries.insertRg07CandidateStatus(
            ledgerId.value, candidateId, 1, "history-rg07-import-pending", "PENDING_CONFIRMATION",
            input.sourceObservedAt.toString(), 0,
        )
    }
    ...
    private fun persistMirror(operation: Rg07Operation.Mirror) {
        ...
        val originalSource = database.ledgerQueries.selectRg07Source(ledgerId.value, originalSourceId).executeAsOneOrNull()
            ?: return
        val originalLink = database.ledgerQueries.selectRg07EvidenceLinkForRole(
            ledgerId.value, originalSource.evidence_id, "destination_asset_posting",
        ).executeAsOneOrNull() ?: return
        val sourceSuffix = input.sourceId.removePrefix("source-")
        database.ledgerQueries.insertRg07Source(
            ledgerId.value, input.sourceId, "wallet_credit_mirror",
            input.sourceId, input.evidenceId, "sha256:${input.sourceId}",
            input.observedAt.toString(), input.observedAt.toString(),
            input.amount.minorUnits, input.amount.currency.code, input.amount.currency.precision.toLong(),
            originalSource.account_id, null, null, null, null,
            input.observedAt.toString(), input.observedAt.toString(),
            originalSource.original_source_payload_hash, originalSourceId,
        )
        database.ledgerQueries.insertRg07EvidenceLink(
            ledgerId.value, "evidence-link-$sourceSuffix", input.evidenceId,
            "posting", originalLink.target_id, "destination_asset_posting",
        )
        database.ledgerQueries.insertRg07Evidence(
            ledgerId.value, input.evidenceId, input.sourceId,
            "asset_credit_mirror", input.observedAt.toString(), input.observedAt.toString(),
            originalSource.evidence_id, originalLink.link_id,
        )
    }

    private fun persistRejected(operation: Rg07Operation, result: Rg07ExecutionResult.Rejected) {
        database.ledgerQueries.insertRg07Rejection(
            operation.ledgerId.value, operation.identity.value, result.reason.code, result.fieldPath.value,
        )
    ...
    private fun matchReconciliation(ledgerId: LedgerId, postingId: String, evidenceLinkId: String) {
        val reconciliation = database.ledgerQueries.selectRg07ReconciliationForPosting(ledgerId.value, postingId).executeAsOneOrNull()
            ?: return
        ...
        if (latest == null || latest.status == "PENDING") {
            database.ledgerQueries.insertRg07ReconciliationHistory(
                ledgerId.value, reconciliation.reconciliation_id, (latest?.status_sequence ?: 0L) + 1, "MATCHED", evidenceLinkId,
            )
        }
    }
    ...
    private fun replay(operation: Rg07Operation, saved: com.unifiedledger.data.db.Rg07_operation): Rg07ExecutionResult {
        ...
        return when (saved.outcome) {
            "ACCEPTED", "NO_CHANGE" -> accepted(operation).let {
    ...
    private fun claim(operation: Rg07Operation) {
        database.ledgerQueries.claimRg07Operation(
            operation.ledgerId.value, operation.identity.value, operation.action.code,
            operation.operationClass.name, operation.fingerprint(), "ACCEPTED", null, null,
        )
    }

    private fun accepted(operation: Rg07Operation): Rg07ExecutionResult.Accepted = when (operation) {
        is Rg07Operation.ManualExpense -> identity.manual(operation).let { commitFacts ->
            val formalIds = identity.formal(operation)
            Rg07ExecutionResult.Accepted(
                TransactionId(formalIds.transactionId), null,
                listOf(
        ...
        is Rg07Operation.ImportConfirm -> {
            ...
            Rg07ExecutionResult.Accepted(
                TransactionId(transactionId), relationId,
                listOf(
        ...
        is Rg07Operation.OriginalPaymentEvidence -> Rg07ExecutionResult.Accepted(
            null, null,
            listOf(
        ...
        is Rg07Operation.DualRoleEvidence -> {
            val suffix = operation.input.sourceId.removePrefix("source-").removeSuffix("-role")
            Rg07ExecutionResult.Accepted(
                null, null,
                listOf(
                    Rg07ReturnedId.Source(operation.input.sourceId),
        ...
        is Rg07Operation.ImportCredit -> Rg07ExecutionResult.Accepted(
            null, null,
            listOf(
                Rg07ReturnedId.Source(operation.input.sourceId),
    ...
    private fun sourceEvidenceReturned(sourceId: String, evidenceId: String): Rg07ExecutionResult.Accepted =
        Rg07ExecutionResult.Accepted(
            null, null,
            listOf(
    ...
    private fun receiptReturned(operation: Rg07Operation): Rg07ExecutionResult.Accepted {
        ...
        val confirmationId = identity.receipt(operation, relationId, formalIds.firstPostingId).confirmationId
        val returnedIds = mutableListOf<Rg07ReturnedId>(
            Rg07ReturnedId.Confirmation(confirmationId),
            Rg07ReturnedId.Transaction(transactionId),
        )
    ...
    private fun refundRelationForReceipt(operation: Rg07Operation): String? {
        val originalTransactionId = when (operation) {
        ...
        return database.ledgerQueries.selectRg07EntitiesForOriginal(operation.ledgerId.value, originalTransactionId).executeAsList()
            .firstOrNull { it.refund_transaction_id == null }
            ?.relation_id
    }

    private fun rejected(reason: Rg07RejectionReason, path: Rg07FieldPath): Rg07ExecutionResult.Rejected =
        Rg07ExecutionResult.Rejected(reason, path)

    private fun parseStatus(state: String): Rg07RefundStatus? =
        runCatching { Rg07RefundStatus.valueOf(state) }.getOrNull()
    ...
    private fun receiptProposal(
        ...
        originalTransactionId != null && amount != null && categoryId != null && destinationAccountId != null &&
        arrivedAt != null && confirmedAt != null
    ) {
    ...
    private fun originalExpense(ledgerId: LedgerId, transactionId: TransactionId): OriginalExpenseFact? =
    ...
    private fun remainingRefundable(ledgerId: LedgerId, originalTransactionId: TransactionId, original: OriginalExpenseFact): Long {
        var received = 0L
    ...
    private fun refundRelation(ledgerId: LedgerId): String? =
        database.ledgerQueries.selectRg07RefundRelation(ledgerId.value).executeAsOneOrNull()
    ...
    private fun destinationPostingMatches(
        ...
        accountId: AccountId,
        amount: Money,
    ): Boolean = database.ledgerQueries.selectRg07ExactRefundAssetPosting(
        ledgerId.value,
        relationId,
        postingId,
        accountId.value,
    ...
    private fun refundAssetPosting(ledgerId: LedgerId, relationId: String): String? =
        database.ledgerQueries.selectRg07RefundAssetPosting(ledgerId.value, relationId).executeAsOneOrNull()

    private fun categoryWithPostingAccount(categoryId: CategoryId): Category? =
        catalog.categories.singleOrNull { it.id == categoryId && it.active && it.postingAccountId != null }
    ...
    private fun receiptSuffix(operation: Rg07Operation): String = when (operation) {
    ...
    private fun historyId(relationId: String, status: Rg07RefundStatus): String {
...
private class Rg07Rollback(val result: Rg07ExecutionResult) : RuntimeException()
