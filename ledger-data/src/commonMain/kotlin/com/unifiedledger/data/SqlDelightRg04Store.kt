...
interface Rg04IdentitySource { fun manual(requestId: RequestId): Rg04ManualCommitIds; fun repayment(requestId: RequestId): Rg04RepaymentCommitIds }
internal enum class Rg04FailurePoint { AFTER_FORMAL, AFTER_RELATION, AFTER_RECONCILIATION, BEFORE_RECEIPT }
internal fun interface Rg04FailureInjector { fun failAt(point: Rg04FailurePoint) }
...
class SqlDelightRg04Store private constructor(
    ...
    constructor(database: LedgerDatabase, driver: SqlDriver, catalog: LedgerCatalog, identity: Rg04IdentitySource) : this(database, catalog, identity, NO_FAILURE) { configureSqliteConnection(driver) }
    internal constructor(database: LedgerDatabase, driver: SqlDriver, catalog: LedgerCatalog, identity: Rg04IdentitySource, failure: Rg04FailureInjector) : this(database, catalog, identity, failure) { configureSqliteConnection(driver) }
    ...
    private fun manual(operation: Rg04PreparedOperation.Manual): Rg04ExecutionResult {
        ...
        return database.transactionWithResult {
            ...
            val aggregate = when (created) { is DomainResult.Success -> created.value; is DomainResult.Failure -> { database.ledgerQueries.deleteRg04Request(snapshot.ledgerId.value, snapshot.requestId.value); return@transactionWithResult created.violation.rejected() } }
            ...
            snapshot.funding.forEachIndexed { i, item -> database.ledgerQueries.insertRg04MixedSnapshotComponent(snapshot.ledgerId.value, snapshot.requestId.value, i.toLong(), item.accountId.value, item.amount.minorUnits, item.amount.currency.code, item.amount.currency.precision.toLong()) }
            ...
            database.ledgerQueries.insertRg04MixedComposition(snapshot.ledgerId.value, relationId, formalIds.transactionId.value, operation.relationDisplayName, snapshot.total.minorUnits, snapshot.total.currency.code, snapshot.total.currency.precision.toLong())
            aggregate.postings.filter { it.role != MixedPaymentPostingRole.EXPENSE }.forEachIndexed { i, typed -> val positive = snapshot.funding.first { it.accountId == typed.posting.accountId }.amount; database.ledgerQueries.insertRg04MixedCompositionComponent(snapshot.ledgerId.value, relationId, i.toLong(), typed.posting.id.value, typed.posting.accountId.value, positive.minorUnits, positive.currency.code, positive.currency.precision.toLong()) }
            database.ledgerQueries.insertRg04Settlement(snapshot.ledgerId.value, formalIds.transactionId.value, snapshot.settlement.original.minorUnits, snapshot.settlement.discount.minorUnits, snapshot.settlement.settled.minorUnits, snapshot.total.currency.code, snapshot.total.currency.precision.toLong())
    ...
    private fun repayment(operation: Rg04PreparedOperation.Repayment): Rg04ExecutionResult {
        ...
        return database.transactionWithResult {
            ...
            persistFormal(aggregate.formalTransaction, snapshot.occurredAtText); persistSemantics(snapshot.ledgerId, aggregate.postings)
            failure.failAt(Rg04FailurePoint.AFTER_FORMAL)
    ...
    private fun resolveManual(snapshot: Rg04ManualSnapshot): Rg04ExecutionResult {
        ...
        val matches = stored.confirmation_marker == "explicit_manual_save" && snapshot.confirmed && stored.occurred_at == snapshot.occurredAtText && stored.total_minor == snapshot.total.minorUnits && stored.currency_code == snapshot.currency.code && stored.currency_precision == snapshot.currency.precision.toLong() && stored.category_id == snapshot.categoryId.value && stored.original_minor == snapshot.settlement.original.minorUnits && stored.discount_minor == snapshot.settlement.discount.minorUnits && stored.settled_minor == snapshot.settlement.settled.minorUnits && components.size == snapshot.funding.size && components.zip(snapshot.funding).all { (a, b) -> a.account_id == b.accountId.value && a.amount_minor == b.amount.minorUnits && a.currency_code == b.amount.currency.code && a.currency_precision == b.amount.currency.precision.toLong() }
    ...
    private fun persistFormal(value: FormalTransaction, time: String) {
        ...
        value.postingSets.forEach { set -> set.postings.forEachIndexed { i, p -> database.ledgerQueries.insertPosting(p.id.value, set.id.value, value.transaction.ledgerId.value, i.toLong(), p.accountId.value, p.amount.minorUnits, p.amount.currency.code, p.amount.currency.precision.toLong()) } }
    }
    private fun persistSemantics(ledger: LedgerId, postings: List<MixedPaymentPosting>) = postings.forEach {
...
private fun DomainViolation.rejected(): Rg04ExecutionResult.Rejected = when (this) {
    ...
    is CategoryRenameViolation -> Rg04ExecutionResult.Rejected(Rg04ExecutionError.INTERNAL_DOMAIN_VIOLATION, "operation")
}
