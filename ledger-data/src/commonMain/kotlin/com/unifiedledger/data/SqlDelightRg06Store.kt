...
import com.unifiedledger.application.RG06_CONFIRMATION_REQUIREMENTS
import com.unifiedledger.application.Rg06CandidateId
import com.unifiedledger.application.Rg06CommitPort
import com.unifiedledger.application.Rg06ConfirmationId
import com.unifiedledger.application.Rg06EvidenceId
import com.unifiedledger.application.Rg06EvidenceLinkId
import com.unifiedledger.application.Rg06ExecutionResult
import com.unifiedledger.application.Rg06FieldPath
import com.unifiedledger.application.Rg06ManualBankObservation
import com.unifiedledger.application.Rg06ManualInstallmentCommitIds
import com.unifiedledger.application.Rg06Operation
import com.unifiedledger.application.Rg06RecordStagedPaymentInstallmentInput
import com.unifiedledger.application.Rg06RejectionReason
import com.unifiedledger.application.Rg06ReturnedId
import com.unifiedledger.application.Rg06SourceId
import com.unifiedledger.application.Rg06SourcePaymentAt
import com.unifiedledger.application.Rg06StagedPaymentCandidatePayload
...
import com.unifiedledger.domain.StagedPaymentEvent
import com.unifiedledger.domain.StagedPaymentFormalTransactionSnapshot
import com.unifiedledger.domain.StagedPaymentFulfillment
import com.unifiedledger.domain.StagedPaymentHistoryEntry
import com.unifiedledger.domain.StagedPaymentHistoryId
import com.unifiedledger.domain.StagedPaymentInstallmentIds
import com.unifiedledger.domain.StagedPaymentInstallmentSnapshot
import com.unifiedledger.domain.StagedPaymentLifecycleId
import com.unifiedledger.domain.StagedPaymentLifecycleSnapshot
import com.unifiedledger.domain.StagedPaymentMemberRef
import com.unifiedledger.domain.StagedPaymentPostingSetSnapshot
import com.unifiedledger.domain.StagedPaymentPostingSnapshot
import com.unifiedledger.domain.StagedPaymentProgress
import com.unifiedledger.domain.StagedPaymentRelationId
import com.unifiedledger.domain.StagedPaymentRelationSnapshot
import com.unifiedledger.domain.StagedPaymentResult
import com.unifiedledger.domain.StagedPaymentRole
import com.unifiedledger.domain.StagedPaymentSnapshot
import com.unifiedledger.domain.StagedPaymentSourceTime
import com.unifiedledger.domain.StagedPaymentTransactionSnapshot
import com.unifiedledger.domain.StagedPaymentTransactionVersionSnapshot
...
import com.unifiedledger.domain.rehydrateStagedPayment
...
internal fun interface Rg06FailureInjector {
    fun failAt(point: Rg06FailurePoint)
}

private val NO_RG06_FAILURE = Rg06FailureInjector { }

internal class Rg06PersistenceIntegrityException(
    message: String,
...
class SqlDelightRg06Store private constructor(
    ...
    override fun commit(operation: Rg06Operation): Rg06ExecutionResult {
        ...
        return try {
            database.transactionWithResult {
                ...
                generatedKeys(operation)
                    .firstOrNull { (kind, value) ->
    ...
    private fun execute(operation: Rg06Operation): ValidatedOperation =
    ...
    private fun validateCreate(op: Rg06Operation.CreateStagedPayment): Rg06ExecutionResult {
        ...
        val category =
            ...
            catalog.categories.singleOrNull { it.id == categoryId }

                ?: return rejected(Rg06RejectionReason.SECONDARY_CATEGORY_REQUIRED, Rg06FieldPath.ATTEMPTED_CATEGORY_ID)

        val parent =

            category.parentId?.let { id -> catalog.categories.singleOrNull { it.id == id } }
        ...
        val result =
            ...
            createStagedPayment(
                catalog,
                CreateStagedPaymentCommand(op.ledgerId, input.totalAmount, categoryId, input.createdAt),
                op.ids,
            )
        ...
        return when (
            ...
            aggregate.recordInstallment(
                ...
                RecordStagedPaymentInstallmentCommand(
                    ...
                    op.input.paymentAmount,
                    op.input.fundingAccountId,
                    op.input.actualPaymentAt,
                ),
                op.ids.paymentIds,
            )
    ...
    private fun validateLink(op: Rg06Operation.LinkStagedPaymentEvidence): ValidatedOperation {
        ...
        val payment =
            ...
            database.ledgerQueries.selectRg06Payment(op.input.paymentId.value).executeAsOneOrNull()
        ...
        val observation =
            ...
            manualObservations.find(op.input.sourceId, op.input.evidenceId)
    ...
    private fun evidenceTargetAlreadyBound(op: Rg06Operation.LinkStagedPaymentEvidence): Boolean =
        ...
        database.ledgerQueries
            .selectRg06EvidenceLinkForPayment(
                op.ledgerId.value,
                op.input.paymentId.value,
                op.input.postingId.value,
    ...
    private fun validateCandidateConfirmation(op: Rg06Operation.ConfirmStagedPaymentCandidate): Rg06ExecutionResult {
        ...
        val candidate =
            ...
            database.ledgerQueries.selectRg06Candidate(op.ledgerId.value, op.input.candidateId.value).executeAsOneOrNull()
        ...
        val evidence =
            ...
            database.ledgerQueries.selectRg06Evidence(candidate.evidence_id).executeAsOneOrNull()
        ...
        val input =
            ...
            Rg06RecordStagedPaymentInstallmentInput(
                ...
                op.input.fundingAccountId,
                Instant.parse(candidate.source_payment_at),
            )
        ...
        val sourceTime =
            ...
            (restoredTime as? StagedPaymentResult.Success)?.value
    ...
    private fun validateMirror(op: Rg06Operation.MergeStagedPaymentMirrorEvidence): Rg06ExecutionResult {
        ...
        val link =
            ...
            database.ledgerQueries.selectRg06EvidenceLinkForPayment(op.ledgerId.value, payment.payment_id, payment.asset_posting_id).executeAsOneOrNull()

                ?: return rejected(Rg06RejectionReason.MIRROR_TARGET_NOT_FOUND, Rg06FieldPath.INPUT_POSTING_ID)

        val originalEvidence =

            database.ledgerQueries.selectRg06Evidence(link.evidence_id).executeAsOneOrNull()

                ?: return rejected(Rg06RejectionReason.MIRROR_TARGET_NOT_FOUND, Rg06FieldPath.INPUT_POSTING_ID)

        val original =

            database.ledgerQueries.selectRg06Source(originalEvidence.source_id).executeAsOneOrNull()

                ?: return rejected(Rg06RejectionReason.MIRROR_TARGET_NOT_FOUND, Rg06FieldPath.INPUT_POSTING_ID)

        if (original.amount_minor == Long.MIN_VALUE ||

            op.input.amount.minorUnits != -original.amount_minor ||

            op.input.amount.currency.code != original.currency_code ||

            op.input.amount.currency.precision
                .toLong() != original.currency_precision
    ...
    private fun persist(
        operation: Rg06Operation,
        manualObservation: Rg06ManualBankObservation?,
    ) {
        when (operation) {
    ...
    private fun persistCreate(op: Rg06Operation.CreateStagedPayment) {
        ...
        database.ledgerQueries.insertRg06Lifecycle(
            op.ledgerId.value,
            op.ids.lifecycleId.value,
            op.ids.relationId.value,
            op.input.totalAmount.minorUnits,
            0,
            op.input.totalAmount.minorUnits,
            op.input.totalAmount.currency.code,
            op.input.totalAmount.currency.precision
                .toLong(),
            categoryId.value,
            1,
        )
    ...
    private fun persistRecord(
        ...
        database.ledgerQueries.updateRg06Lifecycle(
            updated.lifecycle.paidAmount.minorUnits,
            updated.lifecycle.dueAmount.minorUnits,
            history.sequence.toLong(),
            op.ledgerId.value,
            updated.lifecycle.id.value,
            aggregate.lifecycle.stateHistory.size
                .toLong(),
        )
        ...
        database.ledgerQueries.insertRg06ReconciliationHistory(
            op.ledgerId.value,
            op.ids.reconciliationId.value,
            1,
            status,
            reconciliationEvidenceLinkId,
        )
    }
    ...
    private fun persistLatestHistory(
        ledgerId: LedgerId,
        operationIdentity: String,
        previous: StagedPayment,
        updated: StagedPayment,
    ) {
        val history = updated.lifecycle.stateHistory.last()
        ...
        database.ledgerQueries.updateRg06Lifecycle(
            ...
            ledgerId.value,
            updated.lifecycle.id.value,
            previous.lifecycle.stateHistory.size
                .toLong(),
        )
    ...
    private fun persistLink(
        op: Rg06Operation.LinkStagedPaymentEvidence,
        ...
        database.ledgerQueries.insertRg06Source(
            op.ledgerId.value,
            op.input.sourceId.value,
            "MANUAL",
            observation.amount.minorUnits,
            observation.amount.currency.code,
            observation.amount.currency.precision
                .toLong(),
            observation.observedAt.instant.toString(),
            observation.observedAt.text,
            "OBSERVED_AT",
            null,
        )
    ...
    private fun persistIngest(op: Rg06Operation.IngestStagedPaymentBankFact) {
        ...
        database.ledgerQueries.insertRg06Source(
            ...
            op.input.sourceId.value,
            "IMPORTED",
            op.input.amount.minorUnits,
            op.input.amount.currency.code,
            op.input.amount.currency.precision
                .toLong(),
    ...
    private fun persistFormal(formal: FormalTransaction) {
        ...
        formal.postingSets.forEach { set ->
    ...
    private fun loadOwnedAggregate(
        ledgerId: LedgerId,
    ...
    private fun loadPersistedAggregate(
        ...
        val lifecycle =
            ...
            database.ledgerQueries
                .selectRg06AggregateLifecycle(ledgerId.value, relationId.value)
                .executeAsOneOrNull()
        ...
        val members =
            ...
            database.ledgerQueries.selectRg06RelationMembers(ledgerId.value, relationId.value).executeAsList().map {
        ...
        val history =
            ...
            database.ledgerQueries.selectRg06History(ledgerId.value, lifecycle.lifecycle_id).executeAsList().map {
    ...
    private fun loadFormal(
        ...
        val tx =
            ...
            database.ledgerQueries
                .selectRg06FormalTransaction(ledgerId.value, transactionId.value)
                .executeAsOneOrNull()
        ...
        val versions =
            ...
            database.ledgerQueries.selectRg06FormalVersions(ledgerId.value, transactionId.value).executeAsList().map {
        ...
        val currentVersion =
            ...
            versions.singleOrNull { it.id == expectedCurrentVersionId }
        ...
        val postingSets =
            ...
            database.ledgerQueries.selectRg06FormalPostingSets(ledgerId.value, transactionId.value).executeAsList().map { row ->

                StagedPaymentPostingSetSnapshot(
                    PostingSetId(row),
                    database.ledgerQueries.selectRg06FormalPostings(ledgerId.value, row).executeAsList().map {
    ...
    private fun replay(
        op: Rg06Operation,
        saved: com.unifiedledger.data.db.Rg06_operation,
    ): Rg06ExecutionResult {
        val fields = canonicalFields(op)
        ...
        val ids =
            ...
            database.ledgerQueries
                .selectRg06Receipts(op.ledgerId.value, op.identity.value)
                .executeAsList()
                .map { it.restored() }
    ...
    private fun canonicalFields(op: Rg06Operation): CanonicalFields =

        when (op) {
            is Rg06Operation.CreateStagedPayment ->

                CanonicalFields(
                    requestId = op.input.requestId.value,
                    categoryId = op.input.categoryId?.value,
                    amountMinor = op.input.totalAmount.minorUnits,
                    currencyCode = op.input.totalAmount.currency.code,
                    currencyPrecision =

                        op.input.totalAmount.currency.precision
                            .toLong(),
                    occurredAt = op.input.createdAt.toString(),
                )

            is Rg06Operation.RecordStagedPaymentInstallment ->

                CanonicalFields(
                    requestId = op.input.requestId.value,
                    relationId = op.input.relationId.value,
                    paymentRole = op.input.paymentRole.name,
                    fundingAccountId = op.input.fundingAccountId.value,
                    amountMinor = op.input.paymentAmount.minorUnits,
                    currencyCode = op.input.paymentAmount.currency.code,
                    currencyPrecision =

                        op.input.paymentAmount.currency.precision
                            .toLong(),
                    occurredAt = op.input.actualPaymentAt.toString(),
                )
            ...
            is Rg06Operation.IngestStagedPaymentBankFact ->
                ...
                CanonicalFields(
                    sourceId = op.input.sourceId.value,
                    evidenceId = op.input.evidenceId.value,
                    amountMinor = op.input.amount.minorUnits,
                    currencyCode = op.input.amount.currency.code,
                    currencyPrecision =

                        op.input.amount.currency.precision
                            .toLong(),
                    occurredAt = op.input.sourcePaymentAt.toString(),
                    occurredAtText = op.input.sourcePaymentAtText,
            ...
            is Rg06Operation.MergeStagedPaymentMirrorEvidence ->
                ...
                CanonicalFields(
                    sourceId = op.input.sourceId.value,
                    evidenceId = op.input.evidenceId.value,
                    paymentId = op.input.paymentId.value,
                    postingId = op.input.postingId.value,
                    amountMinor = op.input.amount.minorUnits,
                    currencyCode = op.input.amount.currency.code,
                    currencyPrecision =
    ...
    private fun relationFailure(
        ledgerId: LedgerId,
        relationId: StagedPaymentRelationId,
    ) = if (database.ledgerQueries.selectRg06RelationOwner(relationId.value).executeAsOneOrNull() == null) rejected(Rg06RejectionReason.RELATION_NOT_FOUND, Rg06FieldPath.INPUT_RELATION_ID) else rejected(Rg06RejectionReason.CROSS_LEDGER_REFERENCE, Rg06FieldPath.INPUT_RELATION_ID)
...
private fun Rg06ReturnedId.persisted(): Pair<String, String> =
...
private fun com.unifiedledger.data.db.SelectRg06Receipts.restored(): Rg06ReturnedId =
...
private fun returnedIds(op: Rg06Operation): List<Rg06ReturnedId> =
    ...
    when (op) {
        ...
        -> error("Lifecycle-only returned IDs are resolved from persisted state")
