...
internal fun interface ImportSpineFailureInjector {
    fun failAt(point: ImportSpineFailurePoint)
}
...
class SqlDelightImportSpineStore private constructor(
    ...
    ImportDuplicateReviewCommitPort {
                ...
                database.transactionWithResult {
                    ...
                    if (snapshot.completeness == ImportCompleteness.VALID_COMPLETE && snapshot.facts.fundingState == com.unifiedledger.application.ImportFundingState.SETTLED) {
                        ...
                        existing.zip(ids.duplicateIds).forEach { (existingSourceId, duplicateIds) ->
                            ...
                            val tupleSnapshot =
                                com.unifiedledger.application
                                    .ImportDuplicateComparisonFingerprint()
                        ...
                        val noFundsTuple =
                            com.unifiedledger.application
                                .ImportDuplicateComparisonFingerprint()
                                .canonicalJson(noFundsProjection)
    ...
    override fun commitReviewOnce(request: ImportDuplicateReviewRequest): ImportDuplicateReviewResult {
        require(
            request.identity.ledgerId.value
                .isNotEmpty(),
        )
        val fingerprint =
            com.unifiedledger.application
                .ImportDuplicateReviewFingerprint()
                .digest(request)
        return rollbackTypedRejection {
            database.transactionWithResult {
                ...
                if (database.ledgerQueries.lastStatementChangedRowCount().executeAsOne() != 1L) {
                    ...
                    val receipt =
                        database.ledgerQueries.selectDuplicateReviewReceipt(request.identity.ledgerId.value, request.identity.requestId.value).executeAsOneOrNull()
                ...
                val candidate =
                    database.ledgerQueries.selectDuplicateCandidate(request.identity.ledgerId.value, request.candidateId.value).executeAsOneOrNull()
        ...
        return rollbackTypedRejection {
            database.transactionWithResult {
                ...
                if (candidateKind == "credit_expense") {
                    val profile =
                        database.ledgerQueries
                            .selectImportCandidatePaymentProfile(
                                identity.ledgerId.value,
                                snapshot.candidateId.value,
                            ).executeAsOneOrNull()
                    val fieldsMatch =
                        when (profile?.variant) {
                ...
                when {
                    ...
                    candidateKind == "ordinary_flow" && decisionFields !is ImportConfirmDecisionFields.OrdinaryFlow ->
                        abortImportSpine(
                            ImportCandidateDecisionResult.Rejected(
                                SpineDiagnostics.decisionKindMismatch(snapshot.candidateId),
                            ),
                        )
                    candidateKind == "transfer_flow" && decisionFields !is ImportConfirmDecisionFields.TransferFlow ->
                        abortImportSpine(
                            ImportCandidateDecisionResult.Rejected(
                                SpineDiagnostics.decisionKindMismatch(snapshot.candidateId),
                            ),
                        )
                    candidateKind == "credit_repayment" && decisionFields !is ImportConfirmDecisionFields.CreditRepaymentFlow ->
                        abortImportSpine(
                            ImportCandidateDecisionResult.Rejected(
                                SpineDiagnostics.decisionKindMismatch(snapshot.candidateId),
                            ),
                        )
                    candidateKind == "mixed_payment" && decisionFields !is ImportConfirmDecisionFields.MixedPaymentFlow ->
                        abortImportSpine(
                            ImportCandidateDecisionResult.Rejected(
                                SpineDiagnostics.decisionKindMismatch(snapshot.candidateId),
                            ),
                ...
                if (mixedFields != null) {
                    ...
                    val normalizedTotalMinor =
                        created.transaction.postingSets
                            .singleOrNull()
                            ?.postings
                            ?.firstOrNull()
                            ?.amount
                    ...
                    database.ledgerQueries.insertMixedPaymentGroup(
                        ...
                        generated_at =
                            mixedConfirmedAt ?: abortImportSpine(
                                ImportCandidateDecisionResult.Rejected(
                                    SpineDiagnostics.candidateIncomplete(snapshot.candidateId),
                                ),
                            ),
                ...
                val creditLiabilityAccountId: String? =
                    when (decisionFields) {
    ...
    private fun materializeEvidenceProjectionWithinConfirm(
        ...
        val targetPosting =
            postings.firstOrNull { it.accountId == decisionAccountId }
                ?: abortImportSpine(
                    ImportCandidateDecisionResult.Rejected(
        ...
        val request =
            com.unifiedledger.application.P408MaterializationRequest(
    ...
    private fun resolveIntake(
        ...
        val receipt =
            readReceipt(identity.ledgerId.value, identity.requestId.value)
                ?: return ImportIntakeResult.Rejected(SpineDiagnostics.requestIdentityConflict(identity.requestId))
    ...
    private fun resolveConfirm(
        ...
        val equivalent =
            stored.decision == "confirm" &&
                ...
                stored.asset_account_id == (
                    (snapshot.confirmDecisionFields as? ImportConfirmDecisionFields.CreditRepaymentFlow)?.assetAccountId?.value
    ...
    private fun intakeReturnedIds(
        ledgerId: String,
        sourceId: String,
    ): List<ImportReturnedId> {
    ...
    private fun creditLiabilityDecisionValue(fields: ImportConfirmDecisionFields?): String? =
        when (fields) {
    ...
    private fun categoryDecisionValue(fields: ImportConfirmDecisionFields?): String? =
        when (fields) {
    ...
    private fun confidenceFor(completeness: ImportCompleteness): String =
        when (completeness) {
    ...
    private fun persistFormal(value: FormalTransaction) {
        ...
        value.versions.forEach {
            database.ledgerQueries.insertTransactionVersion(
                ...
                it.postingSetId.value,
                it.times.occurredAt.toString(),
                it.times.statisticsAt.toString(),
                it.times.effectiveAt.toString(),
                it.note,
            )
        }
...
private class ImportSpineTypedRollback(
    val result: Any,
) : RuntimeException()

private fun abortImportSpine(result: Any): Nothing = throw ImportSpineTypedRollback(result)

private inline fun <T> rollbackTypedRejection(block: () -> T): T =
    try {
        block()
