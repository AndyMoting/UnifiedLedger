...
import com.unifiedledger.application.Rg05Action
import com.unifiedledger.application.Rg05CommitPort
...
internal fun interface Rg05FailureInjector {
    fun failAt(point: Rg05FailurePoint)
}
...
class SqlDelightRg05Store private constructor(
    ...
    internal constructor(database: LedgerDatabase, driver: SqlDriver, catalog: LedgerCatalog, identity: Rg05IdentitySource, failure: Rg05FailureInjector) :
        ...
        this(database, catalog, identity, failure) {
    ...
    private fun resolveManual(snapshot: Rg05ManualSnapshot): Rg05ExecutionResult {
        val action =
            ...
            database.ledgerQueries.selectRg05Action(snapshot.ledgerId.value, snapshot.requestId.value).executeAsOneOrNull()
        ...
        val stored =
            ...
            database.ledgerQueries.selectRg05Commit(snapshot.ledgerId.value, snapshot.requestId.value).executeAsOneOrNull()
        ...
        val matches =
            ...
            stored.occurred_at == snapshot.paymentAtText &&
    ...
    private fun commitIngest(snapshot: Rg05IngestSnapshot): Rg05ExecutionResult {
        ...
        return database.transactionWithResult {
            ...
            database.ledgerQueries.insertRg05Source(
                ...
                bank.evidenceId,
                null,
                Rg05EvidenceKind.BANK_PAYMENT.name,
                bank.observedAtText,
                bank.details,
                bank.amount.minorUnits,
                bank.amount.currency.code,
                bank.amount.currency.precision
                    .toLong(),
                null,
                "COMPLETE",
            )
    ...
    private fun validateIngest(snapshot: Rg05IngestSnapshot): Rg05ExecutionResult.Rejected? {
        ...
        if (snapshot.itemFacts
            ...
            return Rg05ExecutionResult.Rejected(Rg05ExecutionError.DUPLICATE_ITEM_ID, "item_facts")
        }
    ...
    private fun resolveIngest(snapshot: Rg05IngestSnapshot): Rg05ExecutionResult {
        ...
        val candidate =
            ...
            database.ledgerQueries.selectRg05Candidate(snapshot.ledgerId.value, snapshot.candidateId).executeAsOneOrNull()
        ...
        val matches =
            ...
            bank?.source_type == "BANK_FACT" &&

                bank.evidence_id == snapshot.bankFact.evidenceId &&

                bank.evidence_kind == Rg05EvidenceKind.BANK_PAYMENT.name &&

                bank.observed_at == snapshot.bankFact.observedAtText &&

                bank.details == snapshot.bankFact.details &&

                bank.amount_minor == snapshot.bankFact.amount.minorUnits &&

                bank.currency_code == snapshot.bankFact.amount.currency.code &&

                bank.currency_precision ==

                snapshot.bankFact.amount.currency.precision
                    .toLong() &&

                bank.completeness == "COMPLETE" &&

                candidate.bank_source_id == snapshot.bankFact.sourceId &&

                candidate.payment_total_minor == -snapshot.bankFact.amount.minorUnits &&

                candidate.currency_code == snapshot.bankFact.amount.currency.code &&

                candidate.currency_precision ==
                ...
                items.zip(snapshot.itemFacts).all { (saved, expected) ->
                    ...
                    saved?.source_id == expected.sourceId &&

                        saved.source_type == "ITEM_FACT" &&

                        saved.evidence_id == expected.evidenceId &&

                        saved.item_id == expected.itemId &&

                        saved.evidence_kind == expected.evidenceKind.name &&

                        saved.observed_at == expected.observedAtText &&

                        saved.details == expected.details &&

                        saved.amount_minor == expected.amount.minorUnits &&

                        saved.currency_code == expected.amount.currency.code &&

                        saved.currency_precision ==

                        expected.amount.currency.precision
                            .toLong() &&

                        saved.suggested_category_id == expected.suggestedCategoryId.value &&

                        saved.completeness == if (expected.evidenceKind == Rg05EvidenceKind.ITEM_RECEIPT) "COMPLETE" else "SUMMARY_ONLY"
                } &&

                candidateItems.size == snapshot.itemFacts.size &&

                candidateItems.zip(snapshot.itemFacts).all { (saved, expected) ->
    ...
    private fun commitConfirm(operation: Rg05PreparedOperation.Confirm): Rg05ExecutionResult {
        ...
        database.ledgerQueries
            .selectRg05Action(snapshot.ledgerId.value, snapshot.requestId.value)
            .executeAsOneOrNull()

            ?.let { return resolveConfirm(operation) }
        ...
        if (snapshot.allocations.size != 2 ||
            ...
            return Rg05ExecutionResult.Rejected(Rg05ExecutionError.DUPLICATE_ITEM_ID, "items")
        }
        ...
        if (snapshot.allocations.any {
                it.amount.currency.code != candidate.currency_code ||

                    it.amount.currency.precision
                        .toLong() != candidate.currency_precision
            }

        ) {
            return Rg05ExecutionResult.Rejected(Rg05ExecutionError.SINGLE_CURRENCY_REQUIRED, "items")
        }
        ...
        return database.transactionWithResult {
            ...
            snapshot.allocations.forEachIndexed { index, allocation ->

                database.ledgerQueries.insertRg05ConfirmationAllocation(
                    snapshot.ledgerId.value,
                    snapshot.requestId.value,
                    index.toLong(),
                    allocation.itemId,
            ...
            aggregate.consumptions.forEachIndexed { index, item ->
                ...
                val row = candidateItems.first { it.item_id == item.itemId }
            ...
            aggregate.allocations.forEachIndexed { index, allocation ->
                ...
                val candidateItem = candidateItems.first { it.item_id == allocation.itemId }

                val kind =

                    database.ledgerQueries
                        .selectRg05Source(snapshot.ledgerId.value, candidateItem.source_id)
                        .executeAsOne()
                        .evidence_kind

                database.ledgerQueries.insertRg05RelationItem(snapshot.ledgerId.value, operation.relationId, index.toLong(), allocation.itemId, allocation.expensePostingId.value, allocation.amount.minorUnits, allocation.categoryId.value, if (kind == "ITEM_RECEIPT") "COMPLETE" else "NONE")
            }
            ...
            database.ledgerQueries.insertRg05EvidenceLink(
                snapshot.ledgerId.value,
                operation.bankEvidenceLinkId,
                database.ledgerQueries
                    .selectRg05Source(snapshot.ledgerId.value, candidate.bank_source_id)
                    .executeAsOne()
                    .evidence_id,
                "POSTING",
                operation.formalIds.paymentAssetPostingId.value,
            ...
            candidateItems.forEach { item ->
                ...
                val source = database.ledgerQueries.selectRg05Source(snapshot.ledgerId.value, item.source_id).executeAsOne()

                if (source.evidence_kind == "ITEM_RECEIPT") database.ledgerQueries.insertRg05EvidenceLink(snapshot.ledgerId.value, operation.itemEvidenceLinkIds.getValue(item.item_id), source.evidence_id, "ITEM_ALLOCATION", operation.allocationIds.getValue(item.item_id), "ITEM_ALLOCATION_FACT")
            }
    ...
    private fun commitReceipt(snapshot: Rg05ReceiptSnapshot): Rg05ExecutionResult {
        database.ledgerQueries
            ...
            ?.let { return resolveReceipt(snapshot) }

        val target =

            database.ledgerQueries.selectRg05AllocationTarget(snapshot.ledgerId.value, snapshot.allocationId).executeAsOneOrNull()

                ?: return Rg05ExecutionResult.Rejected(Rg05ExecutionError.ALLOCATION_TARGET_NOT_FOUND, "item_allocation_id")

        if (snapshot.amount.minorUnits <= 0 ||

            snapshot.amount.minorUnits != target.amount_minor ||

            snapshot.amount.currency.code != target.currency_code ||

            snapshot.amount.currency.precision
                .toLong() != target.currency_precision

        ) {
            return Rg05ExecutionResult.Rejected(Rg05ExecutionError.ALLOCATION_TARGET_MISMATCH, "amount")
        }
        ...
        return database.transactionWithResult {
            ...
            database.ledgerQueries.insertRg05ReceiptEvidence(
                snapshot.ledgerId.value,
                snapshot.requestId.value,
                target.relation_id,
                snapshot.sourceId,
...
private fun DomainViolation.toRg05Rejected(): Rg05ExecutionResult.Rejected =
