...
class SqlDelightRg05Store private constructor(
    ...
    private fun commitManual(operation: Rg05PreparedOperation.Manual): Rg05ExecutionResult {
        ...
        return database.transactionWithResult {
            ...
            persistFormal(aggregate.formalTransaction, snapshot.paymentAtText)
    ...
    private fun commitConfirm(operation: Rg05PreparedOperation.Confirm): Rg05ExecutionResult {
        ...
        return database.transactionWithResult {
            ...
            aggregate.consumptions.forEachIndexed { index, item -> val row = candidateItems.first { it.item_id == item.itemId }; val source = database.ledgerQueries.selectRg05Source(snapshot.ledgerId.value, row.source_id).executeAsOne(); database.ledgerQueries.insertRg05ItemSnapshot(snapshot.ledgerId.value, snapshot.requestId.value, index.toLong(), item.itemId, item.amount.minorUnits, item.amount.currency.code, item.amount.currency.precision.toLong(), item.categoryId.value, item.details, item.sourceObservedAt.toString(), operation.consumptionIds[item.itemId] ?: "consumption-${item.itemId}", operation.allocationIds[item.itemId] ?: "allocation-${item.itemId}", row.source_id, row.evidence_id, item.expensePostingId.value) }
            ...
            aggregate.allocations.forEachIndexed { index, allocation -> val source = candidateItems.first { it.item_id == allocation.itemId }; val kind = database.ledgerQueries.selectRg05Source(snapshot.ledgerId.value, source.source_id).executeAsOne().evidence_kind; database.ledgerQueries.insertRg05RelationItem(snapshot.ledgerId.value, operation.relationId, index.toLong(), allocation.itemId, allocation.expensePostingId.value, allocation.amount.minorUnits, allocation.categoryId.value, if (kind == "ITEM_RECEIPT") "COMPLETE" else "NONE") }
            ...
            database.ledgerQueries.insertRg05EvidenceLink(snapshot.ledgerId.value, operation.bankEvidenceLinkId, candidateItems.first().let { database.ledgerQueries.selectRg05Source(snapshot.ledgerId.value, candidate.bank_source_id).executeAsOne().evidence_id }, "POSTING", operation.formalIds.paymentAssetPostingId.value, "PAYMENT_ASSET_POSTING")
            candidateItems.forEach { item -> val source = database.ledgerQueries.selectRg05Source(snapshot.ledgerId.value, item.source_id).executeAsOne(); if (source.evidence_kind == "ITEM_RECEIPT") database.ledgerQueries.insertRg05EvidenceLink(snapshot.ledgerId.value, operation.itemEvidenceLinkIds[item.item_id] ?: "link-${item.item_id}", source.evidence_id, "ITEM_ALLOCATION", operation.allocationIds[item.item_id] ?: "allocation-${item.item_id}", "ITEM_ALLOCATION_FACT") }
...
private fun DomainViolation.toRg05Rejected(): Rg05ExecutionResult.Rejected = when (this) {
    ...
    else -> Rg05ExecutionResult.Rejected(Rg05ExecutionError.ALLOCATION_TOTAL_MUST_EQUAL_PAYMENT, "operation")
}
