...
internal fun interface Rg04ImportFailureInjector { fun failAt(point: Rg04ImportFailurePoint) }
...
class SqlDelightRg04ImportStore private constructor(
    ...
    constructor(database: LedgerDatabase, driver: SqlDriver, catalog: LedgerCatalog) :
        this(database, catalog, NO_RG04_IMPORT_FAILURE) { configureSqliteConnection(driver) }
    ...
    ) : this(database, catalog, failure) { configureSqliteConnection(driver) }

    override fun commit(operation: Rg04PreparedImportOperation): Rg04ImportExecutionResult = when (operation) {
        is Rg04PreparedImportOperation.StoreSource -> source(operation.snapshot)
        is Rg04PreparedImportOperation.ConfirmCandidate -> if (!operation.snapshot.confirmed) {
    ...
    private fun source(snapshot: Rg04ImportSourceSnapshot): Rg04ImportExecutionResult = rollbackTypedRejection { database.transactionWithResult {
        ...
        database.ledgerQueries.insertRg04ImportSourceSnapshot(
            snapshot.ledgerId.value, snapshot.requestId.value, snapshot.sourceId.value, snapshot.evidenceId.value,
            snapshot.observedAtText, snapshot.total.minorUnits, snapshot.total.currency.code,
            snapshot.total.currency.precision.toLong(), snapshot.suggestedCategoryId?.value,
            snapshot.completeness.name, snapshot.confidence, snapshot.candidateKind,
            snapshot.candidateId.value, snapshot.candidateStatusId,
        )
        snapshot.funding.forEachIndexed { index, component ->
            database.ledgerQueries.insertRg04ImportSourceComponentSnapshot(
                snapshot.ledgerId.value, snapshot.requestId.value, index.toLong(), component.accountId?.value,
                component.amount.minorUnits, component.amount.currency.code,
                component.amount.currency.precision.toLong(), if (component.evidenceAvailable) 1 else 0,
            )
        }
        database.ledgerQueries.insertRg04ImportSource(
            snapshot.ledgerId.value, snapshot.sourceId.value, snapshot.requestId.value, snapshot.evidenceId.value,
            snapshot.observedAtText, if (snapshot.completeness == Rg04ImportCompleteness.COMPLETE) "COMPLETE_MIXED_PAYMENT" else "INCOMPLETE_MIXED_PAYMENT",
        )
        ...
        database.ledgerQueries.insertRg04ImportCandidate(
            snapshot.ledgerId.value, snapshot.candidateId.value, snapshot.sourceId.value,
            snapshot.candidateKind, snapshot.confidence,
        ...
        database.ledgerQueries.insertRg04ImportCandidateStatus(
            snapshot.ledgerId.value, snapshot.candidateId.value, 1, snapshot.candidateStatusId,
        ...
        database.ledgerQueries.insertRg04ImportReceipt(
            ...
            snapshot.evidenceId.value, snapshot.candidateId.value, null, null, null,
        )
        Rg04ImportExecutionResult.Accepted(sourceIds(snapshot))
    } }

    private fun confirmation(snapshot: Rg04ImportConfirmationSnapshot): Rg04ImportExecutionResult = if (!snapshot.confirmed) {
        Rg04ImportExecutionResult.Rejected(Rg04ImportExecutionError.EXPLICIT_CONFIRMATION_REQUIRED, "confirmed")
    } else rollbackTypedRejection { database.transactionWithResult {
        ...
        val matchesSource = source.completeness == "COMPLETE" && source.suggested_category_id == snapshot.categoryId.value &&
            snapshot.funding.size == 2 && sourceComponents.size == 2 && sourceComponents.zip(snapshot.funding).all { (stored, confirmed) ->
                stored.account_id == confirmed.accountId.value && stored.amount_minor == confirmed.amount.minorUnits &&
                    stored.currency_code == confirmed.amount.currency.code && stored.currency_precision == confirmed.amount.currency.precision.toLong()
        }
        if (!matchesSource) {
        ...
        val created = createMixedPaymentExpense(
            catalog,
            MixedPaymentExpenseCommand(
                snapshot.ledgerId, total, snapshot.categoryId,
        ...
        database.ledgerQueries.insertRg04ImportConfirmationSnapshot(
            snapshot.ledgerId.value, snapshot.requestId.value, snapshot.candidateId.value,
            snapshot.categoryId.value, 1, snapshot.relationId, snapshot.formalIds.versionId.value,
            snapshot.formalIds.postingSetId.value, snapshot.formalIds.expensePostingId.value,
            snapshot.formalIds.fundingPostingIds[0].value, snapshot.formalIds.fundingPostingIds[1].value,
            snapshot.confirmedStatusId, snapshot.relationDisplayName, snapshot.assetEvidenceLinkId,
            snapshot.assetReconciliationId, snapshot.liabilityReconciliationId,
        )
        snapshot.funding.forEachIndexed { index, item ->
            database.ledgerQueries.insertRg04ImportConfirmationComponentSnapshot(
                snapshot.ledgerId.value, snapshot.requestId.value, index.toLong(), item.accountId.value,
                item.amount.minorUnits, item.amount.currency.code, item.amount.currency.precision.toLong(),
            )
        }
        ...
        database.ledgerQueries.insertRg04ImportCandidateStatus(
            snapshot.ledgerId.value, snapshot.candidateId.value, 2, snapshot.confirmedStatusId,
            "CONFIRMED", snapshot.requestId.value,
        )
        database.ledgerQueries.insertRg04ImportConfirmation(
            snapshot.ledgerId.value, snapshot.confirmationId, snapshot.requestId.value,
            snapshot.candidateId.value, snapshot.formalIds.transactionId.value,
        )
        val assetPosting = aggregate.postings.single { it.role == MixedPaymentPostingRole.MIXED_EXPENSE_ASSET_FUNDING }.posting.id
        val liabilityPosting = aggregate.postings.single { it.role == MixedPaymentPostingRole.MIXED_EXPENSE_CREDIT_FUNDING }.posting.id
        ...
        database.ledgerQueries.insertRg04ImportEvidenceMatch(
            snapshot.ledgerId.value, snapshot.assetEvidenceLinkId, source.evidence_id, assetPosting.value, "ASSET_SOURCE",
        )
        database.ledgerQueries.insertRg04ReconciliationTransition(
            snapshot.ledgerId.value, snapshot.requestId.value, snapshot.assetReconciliationId, assetPosting.value,
        )
        ...
        database.ledgerQueries.deleteRg04ReconciliationTransition(
            snapshot.ledgerId.value, snapshot.requestId.value, snapshot.assetReconciliationId,
        )
        database.ledgerQueries.insertRg04ImportReceipt(
            snapshot.ledgerId.value, snapshot.requestId.value, null, source.evidence_id,
            snapshot.candidateId.value, snapshot.confirmationId, snapshot.formalIds.transactionId.value,
            snapshot.assetEvidenceLinkId,
        )
        Rg04ImportExecutionResult.Accepted(confirmationIds(snapshot))
    } }

    private fun mirror(snapshot: Rg04ImportMirrorSnapshot): Rg04ImportExecutionResult = rollbackTypedRejection { database.transactionWithResult {
        ...
        val matches = allTargets.filter {
            it.account_id == snapshot.accountId.value && it.amount_minor == snapshot.amount.minorUnits &&
                it.currency_code == snapshot.amount.currency.code && it.currency_precision == snapshot.amount.currency.precision.toLong() &&
        ...
        database.ledgerQueries.insertRg04ImportMirrorSnapshot(
            snapshot.ledgerId.value, snapshot.requestId.value, snapshot.sourceId.value, snapshot.evidenceId.value,
            snapshot.observedAtText, snapshot.accountId.value, snapshot.amount.minorUnits,
            snapshot.amount.currency.code, snapshot.amount.currency.precision.toLong(), snapshot.evidenceLinkId,
        )
        database.ledgerQueries.insertRg04ImportSource(
            snapshot.ledgerId.value, snapshot.sourceId.value, snapshot.requestId.value, snapshot.evidenceId.value,
            snapshot.observedAtText, "LIABILITY_MIRROR",
        )
        database.ledgerQueries.insertRg04ImportEvidence(
            snapshot.ledgerId.value, snapshot.evidenceId.value, snapshot.sourceId.value,
            "LIABILITY_MIRROR", snapshot.observedAtText,
        )
        database.ledgerQueries.insertRg04ImportEvidenceMatch(
            snapshot.ledgerId.value, snapshot.evidenceLinkId, snapshot.evidenceId.value,
            target.posting_id, "LIABILITY_MIRROR",
        )
        val liabilityReconciliation = database.ledgerQueries.selectRg04ImportConfirmationByCandidate(snapshot.ledgerId.value, target.candidate_id)
            .executeAsOneOrNull()
        if (liabilityReconciliation == null) {
            abortRg04Import(Rg04ImportExecutionResult.Rejected(Rg04ImportExecutionError.RECONCILIATION_PRECONDITION_FAILED))
        }
        database.ledgerQueries.insertRg04ReconciliationTransition(
            snapshot.ledgerId.value, snapshot.requestId.value, liabilityReconciliation, target.posting_id,
        )
        ...
        database.ledgerQueries.deleteRg04ReconciliationTransition(
            snapshot.ledgerId.value, snapshot.requestId.value, liabilityReconciliation,
        )
        failure.failAt(Rg04ImportFailurePoint.MIRROR_AFTER_MATCH)
        database.ledgerQueries.insertRg04ImportReceipt(
            snapshot.ledgerId.value, snapshot.requestId.value, snapshot.sourceId.value,
            snapshot.evidenceId.value, target.candidate_id, null, target.transaction_id, snapshot.evidenceLinkId,
        )
        Rg04ImportExecutionResult.Accepted(mirrorIds(snapshot))
    } }

    private fun validSource(snapshot: Rg04ImportSourceSnapshot): Boolean =
        snapshot.funding.size == 2 && snapshot.funding.all { it.amount.minorUnits > 0 && it.amount.currency == snapshot.total.currency } &&
    ...
    private fun resolveSource(snapshot: Rg04ImportSourceSnapshot): Rg04ImportExecutionResult {
        val stored = database.ledgerQueries.selectRg04ImportSourceCommit(snapshot.ledgerId.value, snapshot.requestId.value).executeAsOneOrNull()
        ...
        val matches = stored.action_type == "IMPORT_SOURCE" && stored.source_id == snapshot.sourceId.value &&
            stored.evidence_id == snapshot.evidenceId.value && stored.observed_at == snapshot.observedAtText &&
            stored.total_minor == snapshot.total.minorUnits && stored.currency_code == snapshot.total.currency.code &&
            stored.currency_precision == snapshot.total.currency.precision.toLong() && stored.suggested_category_id == snapshot.suggestedCategoryId?.value &&
            stored.completeness == snapshot.completeness.name && stored.confidence == snapshot.confidence && stored.candidate_kind == snapshot.candidateKind &&
            stored.candidate_id == snapshot.candidateId.value && stored.candidate_status_id == snapshot.candidateStatusId &&
            components.size == snapshot.funding.size && components.zip(snapshot.funding).all { (a, b) ->
                a.account_id == b.accountId?.value && a.amount_minor == b.amount.minorUnits && a.currency_code == b.amount.currency.code &&
                    a.currency_precision == b.amount.currency.precision.toLong() && a.evidence_available == if (b.evidenceAvailable) 1L else 0L
            }
    ...
    private fun resolveConfirmation(snapshot: Rg04ImportConfirmationSnapshot): Rg04ImportExecutionResult {
        val stored = database.ledgerQueries.selectRg04ImportConfirmationCommit(snapshot.ledgerId.value, snapshot.requestId.value).executeAsOneOrNull()
            ?: return Rg04ImportExecutionResult.RequestIdentityConflict
        ...
        val matches = stored.action_type == "CONFIRM_CANDIDATE" && stored.candidate_id == snapshot.candidateId.value &&
            stored.category_id == snapshot.categoryId.value && stored.confirmed == 1L && stored.relation_id == snapshot.relationId &&
            stored.version_id == snapshot.formalIds.versionId.value && stored.posting_set_id == snapshot.formalIds.postingSetId.value &&
            ...
            stored.confirmed_status_id == snapshot.confirmedStatusId && stored.relation_display_name == snapshot.relationDisplayName &&
            ...
            stored.confirmation_id == snapshot.confirmationId && stored.transaction_id == snapshot.formalIds.transactionId.value &&
            components.size == snapshot.funding.size && components.zip(snapshot.funding).all { (a, b) ->
                a.account_id == b.accountId.value && a.amount_minor == b.amount.minorUnits && a.currency_code == b.amount.currency.code &&
                    a.currency_precision == b.amount.currency.precision.toLong()
            }
    ...
    private fun resolveMirror(snapshot: Rg04ImportMirrorSnapshot): Rg04ImportExecutionResult {
        val stored = database.ledgerQueries.selectRg04ImportMirrorCommit(snapshot.ledgerId.value, snapshot.requestId.value).executeAsOneOrNull()
            ?: return Rg04ImportExecutionResult.RequestIdentityConflict
        val matches = stored.action_type == "MERGE_MIRROR" && stored.source_id == snapshot.sourceId.value &&
            stored.evidence_id == snapshot.evidenceId.value && stored.observed_at == snapshot.observedAtText &&
            stored.account_id == snapshot.accountId.value && stored.amount_minor == snapshot.amount.minorUnits &&
            stored.currency_code == snapshot.amount.currency.code && stored.currency_precision == snapshot.amount.currency.precision.toLong() &&
    ...
    private fun persistFormal(value: FormalTransaction, time: String) {
        ...
        value.postingSets.forEach { set -> set.postings.forEachIndexed { index, posting -> database.ledgerQueries.insertPosting(posting.id.value, set.id.value, value.transaction.ledgerId.value, index.toLong(), posting.accountId.value, posting.amount.minorUnits, posting.amount.currency.code, posting.amount.currency.precision.toLong()) } }
    }

    private fun persistSemantics(ledgerId: LedgerId, postings: List<MixedPaymentPosting>) = postings.forEach {
    ...
    private fun persistComposition(snapshot: Rg04ImportConfirmationSnapshot, postings: List<MixedPaymentPosting>, total: Money) {
        ...
        postings.filter { it.role != MixedPaymentPostingRole.EXPENSE }.forEachIndexed { index, typed ->
            ...
            database.ledgerQueries.insertRg04MixedCompositionComponent(snapshot.ledgerId.value, snapshot.relationId, index.toLong(), typed.posting.id.value, typed.posting.accountId.value, funding.amount.minorUnits, funding.amount.currency.code, funding.amount.currency.precision.toLong())
        }
    }
}

private class Rg04ImportTypedRollback(val result: Rg04ImportExecutionResult) : RuntimeException()
private fun abortRg04Import(result: Rg04ImportExecutionResult): Nothing = throw Rg04ImportTypedRollback(result)
private inline fun rollbackTypedRejection(block: () -> Rg04ImportExecutionResult): Rg04ImportExecutionResult = try {
    block()
} catch (failure: Rg04ImportTypedRollback) {
    failure.result
}

private fun sourceIds(snapshot: Rg04ImportSourceSnapshot) = listOf(
...
private fun confirmationIds(snapshot: Rg04ImportConfirmationSnapshot) = listOf(
...
private fun mirrorIds(snapshot: Rg04ImportMirrorSnapshot) = listOf(
    Rg04ImportReturnedId(Rg04ImportReturnedIdKind.SOURCE, snapshot.sourceId.value),
    Rg04ImportReturnedId(Rg04ImportReturnedIdKind.EVIDENCE, snapshot.evidenceId.value),
