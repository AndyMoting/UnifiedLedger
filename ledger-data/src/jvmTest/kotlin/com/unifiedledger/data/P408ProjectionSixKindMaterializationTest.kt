...
class P408ProjectionSixKindMaterializationTest {
    ...
    private class CountingIntakeIds(private val prefix: String) : ImportIntakeIdSource {
        private var n = 0
    ...
    private class CountingCommitIds(private val prefix: String, private val postingCount: Int = 2) : ImportIdSource {
        private var n = 0
        override fun next(): com.unifiedledger.application.ImportCommitIds {
            ...
            return com.unifiedledger.application.ImportCommitIds(
                ...
                formalIds = com.unifiedledger.application.ImportFormalIds(
    ...
    private fun intake(
        ...
        val result = ExecuteImportIntake(store, CountingIntakeIds(prefix), fingerprint()).execute(
            ImportIntakeRequest(
    ...
    private fun confirm(
        ...
        val result = ConfirmImportCandidate(store, CountingCommitIds(prefix, postingCount), factory, catalog()).execute(
    ...
    private fun creditFacts(amount: Long, direction: String): ImportSourceFacts = ImportSourceFacts(
        amount, "CNY", 2, "2026-08-20T12:00:00+08:00", direction, "settled",
    ...
    fun tp08CreditExpenseMaterializesSameTransaction() {
        ...
        val accepted = assertIs<ImportCandidateDecisionResult.Accepted>(
            confirm(
                db, d, "req-k-ce-confirm", candidate, kind, facts,
                ImportConfirmDecisionFields.CreditExpenseFlow(CategoryId("category-food"), AccountId("account-credit-lia")),
                CreditFlowFormalFactory(catalog(), noOriginalExpense()), "k-ce",
    ...
    fun tp08CreditRefundMaterializesOnLiabilityAccountAfterOriginal() {
        ...
        val accepted = assertIs<ImportCandidateDecisionResult.Accepted>(
            confirm(
                db, d, "req-k-re-confirm", candidate, kind, facts,
                ImportConfirmDecisionFields.CreditExpenseFlow(CategoryId("category-food"), AccountId("account-credit-lia")),
                CreditFlowFormalFactory(catalog(), noOriginalExpense()), "k-re",
            paymentProfile = ImportPaymentProfile(ImportPaymentVariant.CREDIT_EXPENSE_DIRECT, null, "花呗"),
            ),
        )
        ...
        assertIs<ImportCandidateDecisionResult.Accepted>(
            confirm(
                db, d, "req-k-rf-confirm", refundCandidate, refundKind, refundFacts,
                ...
                CreditFlowFormalFactory(catalog(), refundProvider(db, d, originalTx)), "k-rf",
    ...
    fun tp09MixedThreePostingsPrecisionAndLegSumProjection() {
        ...
        assertIs<ImportCandidateDecisionResult.Accepted>(
            confirm(
                db, d, "req-k-mx-confirm", candidate, kind, facts,
                ...
                MixedPaymentFlowFormalFactory(catalog()), "k-mx",
    ...
    fun tp08TransferSpineMaterializesDirectionMatchedLeg() {
        ...
        assertIs<ImportCandidateDecisionResult.Accepted>(
            confirm(
                db, d, "req-k-tf-confirm", candidate, kind, facts,
    ...
    fun tpQual001SpineMaterializeThenConfirmLinkAcceptsThroughAuditDivergence() {
        ...
        assertIs<ImportCandidateDecisionResult.Accepted>(
            confirm(
                db, d, "req-k-q1-confirm", candidate, kind, facts,
                ImportConfirmDecisionFields.TransferFlow(
                    fromAccountId = AccountId("account-asset-a"),
                    toAccountId = AccountId("account-bank-b"),
                ),
        ...
        val integrity = db.ledgerQueries
            .selectP408PostingIntegrity("ledger-k", liabilityPosting.posting_id)
            .executeAsOne()
        val linkOutcome = SqlDelightP408ReconciliationStore(db, d).confirmLink(
    ...
    fun tp10ProjectionMaterializeInjectionRollsBackAndCorrectedRetryAccepts() {
        ...
        assertFailsWith<ImportSpineUnreachable> {
            confirm(
                db, d, "req-k-t10-confirm", candidate, kind, facts,
                ImportConfirmDecisionFields.CreditExpenseFlow(CategoryId("category-food"), AccountId("account-credit-lia")),
                CreditFlowFormalFactory(catalog(), noOriginalExpense()), "k-t10",
                paymentProfile = ImportPaymentProfile(ImportPaymentVariant.CREDIT_EXPENSE_DIRECT, null, "花呗"),
                failure = ImportSpineFailureInjector { point ->
    ...
    fun r3AuthorityAbsentWhenNothingToMaterialize() {
        val (db, d, _) = newWorld()
        val outcome = SqlDelightEvidenceProjectionStore.createShared(db).ensureReadyWithinTransaction(
    ...
    fun r3RejectedAuthorityBlocksMatchingWithNotReady() {
        ...
        val refused = assertIs<com.unifiedledger.application.P408MaterializeResult.Rejected>(
            store.materialize(
                P408MaterializationRequest(
                    ledgerId = "ledger-k",
                    requestId = "req-r3",
        ...
        val request = com.unifiedledger.application.P408ConfirmLinkRequest(
            ledgerId = "ledger-k",
        ...
        val rejected = assertIs<com.unifiedledger.application.P408ReconciliationResult.Rejected>(
    ...
    fun tp01Combined99Scale0To9900Scale2ConfirmLinkAccepted() {
        ...
        val accepted = assertIs<com.unifiedledger.application.P408ReconciliationResult.Accepted>(
            SqlDelightP408ReconciliationStore(db, d).confirmLink(request),
        )
        assertEquals("ACCEPTED", accepted.receipt.outcome)
    ...
    private fun refundProvider(
        ...
        d: JdbcSqliteDriver,
        originalTx: String,
    ): (TransactionId) -> CreditRefundOriginalExpense? = provider@{ requested ->
    ...
    private fun seedScaleZeroSource(db: LedgerDatabase, amount: Long) {
    ...
    private fun seedScaleTwoSource(db: LedgerDatabase, requestId: String, sourceId: String, evidenceId: String, amount: Long) {
    ...
    private fun seedPostingForMatch(db: LedgerDatabase, amountMinor: Long) {
        db.transaction {
            driver!!.execute(
                null,
    ...
    private fun assertProjectionRow(db: LedgerDatabase, evidenceId: String, targetAccountId: String, normalized: Long) {
