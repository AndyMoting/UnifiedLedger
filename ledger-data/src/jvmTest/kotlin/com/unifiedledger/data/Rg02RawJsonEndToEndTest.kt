...
class Rg02RawJsonEndToEndTest {
    @Test fun `tracked raw fixture executes income create retry variants and rejects sparse invalid attempts`() {
        ...
        val v1DrivenOutcomes = buildList {
        ...
        assertEquals(8, decoded.invalidInputs.size)
            decoded.invalidInputs.forEach { invalid ->
                ...
                v1DrivenOutcomes += runInvalid(invalid, decoded.create.input.occurredAt.required(), decoded.metadata, decoded.catalog)
        }
        val unsupportedRename = assertIs<Rg02CategoryRenameProjection.Unsupported>(
    ...
    @Test fun `raw request mutations return typed contract errors without commit or residue`() {
        ...
        val frozenCaseMutations = listOf(
        ...
        driver.use {
            ...
            var commitCalls = 0
            val execute = ExecuteManualIncomeSave(
    ...
    private fun runAccepted(operation: Rg02DecodedOperation, metadata: Rg02DecodedCaseMetadata, catalog: LedgerCatalog, retryRequestId: String? = null): List<Rg02OperationProjection> {
        ...
        driver.use {
            ...
            var factoryCalls = 0
            val execute = ExecuteConfirmedManualIncome(
                port,
                ConfirmedManualIncomeIdSource {
                    ConfirmedManualIncomeCommitIds(
                        ...
                        AssetReceivedOrdinaryIncomeIds(
                            TransactionId(checkNotNull(expected.transactionId)), TransactionVersionId(checkNotNull(expected.versionId)), PostingSetId(checkNotNull(expected.postingSetId)),
                            PostingId(expected.postingIds[0]), PostingId(expected.postingIds[1]),
                        ),
                    )
                },
                ConfirmedIncomeTransactionFactory { snapshot, ids ->
                    ...
                    createAssetReceivedOrdinaryIncome(catalog, AssetReceivedOrdinaryIncomeCommand(snapshot.ledgerId, snapshot.amount, snapshot.categoryId, snapshot.receivingAccountId, TransactionTimes.collapsed(snapshot.occurredAt)), ids.incomeIds).let {
                        when (it) { is DomainResult.Success -> DomainResult.Success(ConfirmedManualIncomeCommit(ids.confirmationId, it.value)); is DomainResult.Failure -> it }
                    }
                },
            )
            val first = assertIs<ManualIncomeSaveResult.Executed>(
            ...
            assertEquals(expected.transactionId, database.ledgerQueries.selectPersistedTransaction().executeAsOne().transaction_id)
            assertEquals("INCOME", database.ledgerQueries.selectPersistedTransaction().executeAsOne().kind)
            val version = database.ledgerQueries.selectPersistedVersions().executeAsList().single()
            ...
            expected.balances.forEach { (accountId, balanceText) ->
                ...
                val openingBalance = if (operation.source == "$.create") {
            ...
            if (retryRequestId != null) {
                ...
                val replay = assertIs<ManualIncomeSaveResult.Executed>(
    ...
    private fun runInvalid(operation: Rg02DecodedInvalidOperation, fallbackOccurredAt: String, metadata: Rg02DecodedCaseMetadata, catalog: LedgerCatalog): Rg02OperationProjection {
        val driver = JdbcSqliteDriver(JdbcSqliteDriver.IN_MEMORY, sqliteProperties())
        LedgerDatabase.Schema.create(driver)
        driver.use {
            val database = LedgerDatabase(driver)
            ...
            val executeConfirmed = ExecuteConfirmedManualIncome(
                ConfirmedManualIncomeCommitPort { identity, snapshot, callback -> commitPortCalls++; sqlPort.commitOnce(identity, snapshot, callback) },
                ...
                ConfirmedIncomeTransactionFactory { snapshot, ids ->
                    ...
                    createAssetReceivedOrdinaryIncome(checkNotNull(context.catalog), AssetReceivedOrdinaryIncomeCommand(snapshot.ledgerId, snapshot.amount, snapshot.categoryId, snapshot.receivingAccountId, TransactionTimes.collapsed(snapshot.occurredAt)), ids.incomeIds).let { created ->
                        when (created) { is DomainResult.Success -> DomainResult.Success(ConfirmedManualIncomeCommit(ids.confirmationId, created.value)); is DomainResult.Failure -> created }
                    }
                },
            )
...
private data class Rg02ReturnedId(val kind: String, val value: String)
private data class Rg02OperationProjection(val requestId: String, val status: String, val reasonCode: String?, val fieldPath: String?, val returnedIds: Set<Rg02ReturnedId>)

private class ApprovedRg02Outcomes private constructor(private val values: List<Rg02OperationProjection>) {
    fun byRequest(requestId: String, status: String): Rg02OperationProjection = values.single { it.requestId == requestId && it.status == status }
    companion object {
        fun decode(raw: String): ApprovedRg02Outcomes {
            val root = Json.parseToJsonElement(raw).jsonObject
            assertEquals("approved", root.getValue("case").jsonObject.getValue("approval_status").jsonPrimitive.content)
            val values = root.getValue("operations").jsonArray.mapNotNull { element ->
                ...
                Rg02OperationProjection(
                    ...
                    operation.getValue("returned_ids").jsonArray.map { returned ->
            ...
            ExecuteConfirmedManualIncome(
                ...
                ConfirmedManualIncomeIdSource {
                    ConfirmedManualIncomeCommitIds(
                        ...
                        AssetReceivedOrdinaryIncomeIds(
                            TransactionId("tx-catalog-mutation"), TransactionVersionId("version-catalog-mutation"),
                            PostingSetId("set-catalog-mutation"), PostingId("receiving-catalog-mutation"), PostingId("income-catalog-mutation"),
                        ),
                    )
                },
                ConfirmedIncomeTransactionFactory { snapshot, ids ->
...
private fun repoFile(relative: String): Path {
    ...
    repeat(6) { if (Files.isRegularFile(candidate.resolve("settings.gradle.kts"))) return candidate.resolve(relative); candidate = candidate.parent ?: error("repository root not found") }
