...
import com.unifiedledger.application.Rg06Action
import com.unifiedledger.application.Rg06CandidateConfirmationCommitIds
import com.unifiedledger.application.Rg06CandidateId
import com.unifiedledger.application.Rg06CandidateStatusId
import com.unifiedledger.application.Rg06ChangeStagedPaymentFulfillmentInput
import com.unifiedledger.application.Rg06ConfirmStagedPaymentCandidateInput
import com.unifiedledger.application.Rg06ConfirmStagedPaymentCompletionInput
...
import com.unifiedledger.application.Rg06IngestCommitIds
import com.unifiedledger.application.Rg06IngestStagedPaymentBankFactInput
import com.unifiedledger.application.Rg06LinkStagedPaymentEvidenceInput
...
import com.unifiedledger.application.Rg06MergeStagedPaymentMirrorEvidenceInput
import com.unifiedledger.application.Rg06ObservedAt
import com.unifiedledger.application.Rg06Operation
import com.unifiedledger.application.Rg06ReconciliationId
...
class SqlDelightRg06StoreTest {
    ...
    fun manualEightActionLifecyclePersistsFormalGraphAndExactEvidence() {
        ...
        try {
            ...
            val store =
                SqlDelightRg06Store(database, driver, catalog(), "+08:00") { source, evidence ->
            ...
            assertEquals(
                ...
                database.ledgerQueries
                    .selectRg06ReconciliationForPosting("ledger-a", deposit.ids.paymentIds.expenseIds.paymentPostingId.value)
                    .executeAsOne()
                    .status,
            )
        } finally {
    ...
    fun manualEvidenceUsesOneImmutableObservationForValidationAndPersistence() {
        ...
        try {
            ...
            var calls = 0
            val store =
                SqlDelightRg06Store(database, driver, catalog(), "+08:00") { _, _ ->
                    calls += 1
            ...
            assertEquals(
                -8_000L,
                database.ledgerQueries
                    .selectRg06Source("manual-source")
                    .executeAsOne()
                    .amount_minor,
            )
        } finally {
    ...
    fun sequentialSecondManualLinkReturnsTypedRejectionAndIdentityCanBeReused() {
        ...
        try {
            ...
            val database = LedgerDatabase(driver)
            val store =
                SqlDelightRg06Store(database, driver, catalog(), "+08:00") { sourceId, _ ->
            ...
            val duplicate =
                linkDepositEvidence().copy(
                    input =
                        linkDepositEvidence().input.copy(
    ...
    fun finalReceiptRejectsCanonicalGraphMismatchForEveryAction() {
        ...
        cases.forEach { case ->
            ...
            try {
                ...
                val failure =
                    assertFailsWith<SQLException>(case.name) {
                        driver.execute(
                            null,
    ...
    fun importConfirmAndMirrorUseStoredFactsAndMatchedReconciliation() {
        ...
        try {
            ...
            assertEquals(
                "MATCHED",
                database.ledgerQueries
                    .selectRg06ReconciliationForPosting("ledger-a", confirmation.ids.paymentIds.expenseIds.paymentPostingId.value)
                    .executeAsOne()
                    .status,
            )
            val mirror =
    ...
    fun semanticFailurePrecedesSharedFormalCollisionAndCollisionRollsBackClaim() {
        ...
        try {
            ...
            val collision =
                record("collision", StagedPaymentRole.DEPOSIT, 8_000, Instant.parse("2026-04-28T10:00:00+08:00")).let { operation ->
                    operation.copy(
                        ids =
                            operation.ids.copy(
                                paymentIds =
                                    operation.ids.paymentIds.copy(
                                        expenseIds =
                                            operation.ids.paymentIds.expenseIds
                                                .copy(transactionId = TransactionId("shared-transaction")),
                                    ),
                            ),
                    )
                }
    ...
    fun storePortsFrozenEighteenRejectionsWithZeroWriteAndIdentityReuse() {
        ...
        fun depositCase(
            name: String,
            amount: Money,
            account: AccountId = AccountId("asset-bank"),
            reason: Rg06RejectionReason,
            path: Rg06FieldPath = Rg06FieldPath.ATTEMPTED_PAYMENT_AMOUNT,
        ) = Case(
            name,
            Rg06ExecutionResult.Rejected(reason, path),
            { store -> assertIs<Rg06ExecutionResult.Accepted>(store.commit(createOperation())) },
        ...
        fun finalCase(
            ...
            name,
            Rg06ExecutionResult.Rejected(reason, Rg06FieldPath.ATTEMPTED_PAYMENT_AMOUNT),
            createThenDeposit,
    ...
    fun malformedOwnedSnapshotIsNotReportedAsMissingOrCrossLedgerRelation() {
        ...
        try {
            ...
            val malformed =
                record("malformed", StagedPaymentRole.DEPOSIT, 8_000, Instant.parse("2026-04-28T10:00:00+08:00")).copy(
                    input =
                        record("malformed", StagedPaymentRole.DEPOSIT, 8_000, Instant.parse("2026-04-28T10:00:00+08:00")).input.copy(
        ...
        try {
            ...
            JdbcSqliteDriver(url, properties).use { driver ->
                ...
                assertEquals(
                    "MATCHED",
                    database.ledgerQueries
                        .selectRg06ReconciliationForPosting("ledger-a", "posting-deposit-asset")
                        .executeAsOne()
                        .status,
                )
            }
    ...
    private fun fulfill() =
        Rg06Operation.ChangeStagedPaymentFulfillment(
            LedgerId("ledger-a"),
            Rg06ChangeStagedPaymentFulfillmentInput(RequestId("request-fulfill"), StagedPaymentRelationId("relation"), StagedPaymentFulfillment.FULFILLED, Instant.parse("2026-04-29T10:00:00+08:00")),
            StagedPaymentHistoryId("history-fulfilled"),
        )

    private fun complete() =
        Rg06Operation.ConfirmStagedPaymentCompletion(
            LedgerId("ledger-a"),
            Rg06ConfirmStagedPaymentCompletionInput(RequestId("request-complete"), StagedPaymentRelationId("relation"), true, Instant.parse("2026-05-04T10:00:00+08:00")),
            StagedPaymentHistoryId("history-completed"),
        )
    ...
    private fun linkDepositEvidence() =
        Rg06Operation.LinkStagedPaymentEvidence(
            LedgerId("ledger-a"),
    ...
    private fun mergeDepositMirror() =
        Rg06Operation.MergeStagedPaymentMirrorEvidence(
            LedgerId("ledger-a"),
    ...
    private fun paymentIds(label: String) =
        StagedPaymentInstallmentIds(
            InstallmentPaymentId("payment-$label"),
            StagedPaymentHistoryId("history-$label"),
    ...
    private fun seedLegacyConfirmation(
    ...
    private fun assertRetryAndConflict(
        store: SqlDelightRg06Store,
        operation: Rg06Operation,
        changed: Rg06Operation,
    ) {
        val accepted = assertIs<Rg06ExecutionResult.Accepted>(store.commit(operation))
        assertEquals(Rg06ExecutionResult.NoChange(accepted.returnedIds), store.commit(operation))
...
private fun storeSnapshot(database: LedgerDatabase) =
    Rg06StoreSnapshot(
