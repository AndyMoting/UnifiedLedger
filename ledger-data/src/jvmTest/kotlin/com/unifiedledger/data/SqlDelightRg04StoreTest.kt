...
class SqlDelightRg04StoreTest {
    @Test fun storeRejectsUnconfirmedOperationsBeforeClaimOrReplay() {
        ...
        val manualRejected = assertIs<Rg04ExecutionResult.Rejected>(
        ...
        val replayRejected = assertIs<Rg04ExecutionResult.Rejected>(
            fixture.store.commit(fixture.operation(fixture.snapshot.copy(confirmed = false))),
        )
        ...
        val source = object : Rg04IdentitySource {
            override fun manual(requestId: RequestId): Rg04ManualCommitIds = error("unused")
            override fun repayment(requestId: RequestId) =
                Rg04RepaymentCommitIds("repayment-confirmation", listOf("repayment-rec-asset", "repayment-rec-liability"))
        }
        val repaymentStore = SqlDelightRg04Store(repaymentFixture.database, repaymentFixture.driver, catalog(), source)
        val repayment = Rg04PreparedOperation.Repayment(
            Rg04RepaymentSnapshot(
                ledgerId, RequestId("repayment-request"), Instant.DISTANT_PAST, "1970-01-01T00:00:00Z",
                AccountId("asset"), AccountId("liability"), Money.ofMinor(5_000, currency), false,
            ),
            CreditPrincipalRepaymentIds(
                TransactionId("repayment-tx"), TransactionVersionId("repayment-version"),
                PostingSetId("repayment-set"), PostingId("repayment-asset-post"), PostingId("repayment-liability-post"),
            ),
        )
    ...
    @Test fun concurrentEquivalentRequestCreatesExactlyOnce() {
        ...
        fun open(): Fixture {
            val driver = JdbcSqliteDriver(url, Properties().apply { setProperty("foreign_keys", "true"); setProperty("busy_timeout", "5000") })
            ...
            val source = object : Rg04IdentitySource { override fun manual(requestId: RequestId) = Rg04ManualCommitIds("confirmation", listOf("rec-a", "rec-l")); override fun repayment(requestId: RequestId): Rg04RepaymentCommitIds = error("unused") }
        ...
        open().also { LedgerDatabase.Schema.create(it.driver); it.driver.close() }
        val pool = Executors.newFixedThreadPool(2)
        try {
            val futures = List(2) { pool.submit<Rg04ExecutionResult> { open().let { f -> try { f.store.commit(f.operation()) } finally { f.driver.close() } } } }
            ...
            open().let { assertEquals(listOf(1L, 1L, 1L, 3L, 2L), it.counts()); it.driver.close() }
        } finally { pool.shutdownNow(); Files.deleteIfExists(path) }
    }
    ...
    @Test fun replayConflictAndEveryFailurePointAreAtomic() {
        ...
        assertEquals(
            ...
            fixture.database.ledgerQueries.selectRg04PostingSemantics().executeAsList().associate { it.posting_id to it.role },
        )
        fixture.driver.close()
    }
    ...
    @Test fun incompleteAndExcessReconciliationIdentitiesFailBeforePersistence() {
        listOf(1, 3).forEach { count ->
            val fixture = fixture(null) { requestId ->
        ...
        listOf(1, 3).forEach { count ->
            ...
            val source = object : Rg04IdentitySource {
                override fun manual(requestId: RequestId): Rg04ManualCommitIds = error("unused")
                override fun repayment(requestId: RequestId) =
                    Rg04RepaymentCommitIds("repayment-confirmation", List(count) { "repayment-rec-$it" })
            }
            val store = SqlDelightRg04Store(fixture.database, fixture.driver, catalog(), source)
            val operation = Rg04PreparedOperation.Repayment(
                Rg04RepaymentSnapshot(
                    ledgerId,
    ...
    private fun fixture(
        ...
        LedgerDatabase.Schema.create(driver); val database = LedgerDatabase(driver)
        val catalog = catalog()
        val ids = MixedPaymentExpenseIds(TransactionId("tx"), TransactionVersionId("v"), PostingSetId("set"), PostingId("expense-post"), listOf(PostingId("asset-post"), PostingId("liability-post")))
        val source = object : Rg04IdentitySource { override fun manual(requestId: RequestId) = manualIdentity(requestId); override fun repayment(requestId: RequestId): Rg04RepaymentCommitIds = error("unused") }
    ...
    private data class Fixture(val driver: JdbcSqliteDriver, val database: LedgerDatabase, val store: SqlDelightRg04Store, val snapshot: Rg04ManualSnapshot, val ids: MixedPaymentExpenseIds) {
    ...
    companion object { val currency = CurrencyUnit("CNY", 2); val ledgerId = LedgerId("ledger-a") }
}
