...
private data class Rg07Tx(val id: String, val kind: String, val currentVersionId: String)
private data class Rg07Version(val id: String, val txId: String, val number: Long, val setId: String, val occurred: String, val statistics: String, val effective: String, val confirmationId: String?, val confirmedAt: String?, val note: String?)
private data class Rg07Posting(val id: String, val setId: String, val accountId: String, val minor: Long, val currency: String, val precision: Long, val role: String?, val categoryId: String?, val eligible: Boolean)
private data class Rg07Source(val id: String, val type: String, val recordId: String, val evidenceId: String, val hash: String, val observed: String, val amount: Long?, val currency: String?, val precision: Long?, val accountId: String?, val reported: String?, val proves: Boolean?, val processor: String?, val sourceObserved: String?, val booking: String?, val value: String?, val originalHash: String?, val mirrorOf: String?)
private data class Rg07Candidate(val id: String, val sourceId: String, val evidenceId: String, val confidence: String, val amount: Long, val currency: String, val precision: Long, val proposedOriginal: String?, val proposedCategory: String?, val proposedDestination: String?, val arrived: String?, val originalHash: String?, val ruleVersion: Long)
private data class Rg07CandidateStatus(val candidateId: String, val sequence: Long, val id: String, val status: String, val occurred: String, val formalEffects: Long)
private data class Rg07Confirmation(val id: String, val operationId: String, val kind: String, val subjectId: String, val confirmedAt: String?, val originalTx: String?)
private data class Rg07Evidence(val id: String, val sourceId: String, val type: String, val observed: String, val mirrorOf: String?, val mergedInto: String?)
private data class Rg07Link(val id: String, val evidenceId: String, val targetKind: String, val targetId: String, val role: String)
private data class Rg07Relation(val id: String, val type: String, val members: List<String>)
private data class Rg07Entity(val id: String, val relationId: String, val originalTx: String, val refundTx: String?, val categoryId: String, val requested: Long, val received: Long, val currency: String, val precision: Long, val destination: String?, val requestedAt: String?, val approvedAt: String?, val processorAt: String?, val sourceObservedAt: String?, val bookingAt: String?, val valueAt: String?, val confirmedAt: String?, val arrivedAt: String?)
private data class Rg07History(val entityId: String, val sequence: Long, val id: String, val operationIdentity: String, val state: String, val occurred: String, val txId: String?, val formalEffects: Long)
private data class Rg07Reconciliation(val id: String, val postingId: String, val status: String)
...
internal class Rg07StateProjector(
    ...
    private fun projectCatalog(): JsonObject = obj(
    ...
    private fun projectSource(source: Rg07Source): JsonObject {
        val payload = linkedMapOf<String, JsonElement>(
            "source_record_id" to JsonPrimitive(source.recordId), "evidence_id" to JsonPrimitive(source.evidenceId), "immutable_payload_hash" to JsonPrimitive(source.hash), "kind" to JsonPrimitive(source.type), "observed_at" to JsonPrimitive(caseTime(source.observed)),
        )
    ...
    private fun projectCandidate(candidate: Rg07Candidate, history: List<Rg07CandidateStatus>): JsonObject = obj(
        "id" to JsonPrimitive(candidate.id), "type" to JsonPrimitive("refund_credit"), "source_ids" to JsonArray(listOf(JsonPrimitive(candidate.sourceId))), "confidence" to JsonPrimitive(candidate.confidence),
    ...
    private fun projectConfirmation(item: Rg07Confirmation): JsonObject = obj(
        "id" to JsonPrimitive(item.id), "type" to JsonPrimitive(item.kind), "operation_id" to JsonPrimitive(item.operationId), "subject" to obj("kind" to JsonPrimitive(if (item.kind == "explicit_manual_save") "operation" else "relation"), "id" to JsonPrimitive(item.subjectId)), "confirmed_at" to (item.confirmedAt?.let { JsonPrimitive(caseTime(it)) } ?: JsonNull), "payload" to (if (item.kind == "explicit_manual_save") obj() else item.originalTx?.let { obj("original_transaction_id" to JsonPrimitive(it)) } ?: obj()),
    )

    private fun projectEvidence(item: Rg07Evidence): JsonObject = obj(
        "id" to JsonPrimitive(item.id), "type" to JsonPrimitive(item.type), "source_ids" to JsonArray(listOf(JsonPrimitive(item.sourceId))), "payload" to obj("observed_at" to JsonPrimitive(caseTime(item.observed)), "mirror_of_evidence_id" to item.mirrorOf?.let(::JsonPrimitive), "merged_into_evidence_link_id" to item.mergedInto?.let(::JsonPrimitive)),
    )

    private fun projectEntity(entity: Rg07Entity, history: List<Rg07History>): JsonObject = obj(
        "id" to JsonPrimitive(entity.id), "type" to JsonPrimitive("refund_relationship"), "payload" to obj("relation_id" to JsonPrimitive(entity.relationId), "original_transaction_id" to JsonPrimitive(entity.originalTx), "refund_transaction_id" to (entity.refundTx?.let(::JsonPrimitive) ?: JsonNull), "category_id" to JsonPrimitive(entity.categoryId), "requested_amount" to JsonPrimitive(amount(entity.requested, entity.precision)), "received_amount" to JsonPrimitive(amount(entity.received, entity.precision)), "currency" to JsonPrimitive(entity.currency), "destination_account_id" to (entity.destination?.let(::JsonPrimitive) ?: JsonNull), "times" to obj("requested_at" to entity.requestedAt?.let { JsonPrimitive(caseTime(it)) }, "approved_at" to entity.approvedAt?.let { JsonPrimitive(caseTime(it)) }, "processor_reported_at" to entity.processorAt?.let { JsonPrimitive(caseTime(it)) }, "source_observed_at" to entity.sourceObservedAt?.let { JsonPrimitive(caseTime(it)) }, "booking_at" to entity.bookingAt?.let { JsonPrimitive(caseTime(it)) }, "value_at" to entity.valueAt?.let { JsonPrimitive(caseTime(it)) }, "confirmed_at" to entity.confirmedAt?.let { JsonPrimitive(caseTime(it)) }, "arrived_at" to entity.arrivedAt?.let { JsonPrimitive(caseTime(it)) }), "state_history" to JsonArray(history.sortedBy { it.sequence }.map { h -> obj("id" to JsonPrimitive(h.id), "state" to JsonPrimitive(h.state.lowercase()), "occurred_at" to JsonPrimitive(caseTime(h.occurred)), "transaction_id" to (h.txId?.let(::JsonPrimitive) ?: JsonNull), "formal_effect_count" to JsonPrimitive(h.formalEffects), "sequence" to JsonPrimitive(h.sequence)) })),
    )

    private fun projectBalances(txs: List<Rg07Tx>, versions: List<Rg07Version>, postings: List<Rg07Posting>, sets: Map<String, List<String>>): JsonArray {
    ...
    private fun projectReports(txs: List<Rg07Tx>, versions: List<Rg07Version>, postings: List<Rg07Posting>, sets: Map<String, List<String>>): JsonArray {
        ...
        return JsonArray(periods.map { (type, period) ->
            val selectedSets = txs.filter { it.kind == "expense" || it.kind == "refund_receipt" }.filter { tx ->
                val date = versions.single { it.id == tx.currentVersionId }.statistics.substring(0, 10)
                type == "cumulative" || date.startsWith(period)
            }.map { versions.single { v -> v.id == it.currentVersionId }.setId }.toSet()
    ...
    private fun projectStatuses(txs: List<Rg07Tx>, versions: List<Rg07Version>, postings: List<Rg07Posting>, entities: List<Rg07Entity>, histories: List<Rg07History>, statuses: List<Rg07CandidateStatus>, reconciliations: List<Rg07Reconciliation>): JsonArray {
        ...
        txs.filter { it.kind == "expense" || it.kind == "refund_receipt" }.forEach { tx ->
            ...
            if (eligible.isNotEmpty()) {
                ...
                val value = when { values.size == eligible.size && values.all { it == "MATCHED" } -> "matched"; values.any { it == "MATCHED" } -> "partial"; else -> "pending" }
    ...
    private fun transactions(): List<Rg07Tx> = rows("SELECT tx.transaction_id, COALESCE(tx.canonical_kind, tx.kind), current.current_version_id FROM ledger_transaction tx JOIN ledger_transaction_current_version current ON current.ledger_id=tx.ledger_id AND current.transaction_id=tx.transaction_id WHERE tx.ledger_id=? ORDER BY tx.rowid", ledgerId) { cursor ->
        val kind = when (cursor.string(1)) {
    ...
    private fun <T> rows(sql: String, vararg params: String, mapper: (SqlCursor) -> T): List<T> = driver.executeQuery(null, sql, { cursor -> val result = mutableListOf<T>(); while (cursor.next().value) result += mapper(cursor); QueryResult.Value(result) }, params.size) {
    ...
    private fun amount(minor: Long, precision: Long): String = BigDecimal.valueOf(minor, precision.toInt()).setScale(precision.toInt()).toPlainString()
