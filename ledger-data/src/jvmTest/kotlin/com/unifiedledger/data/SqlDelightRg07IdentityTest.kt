        ...
        try {
            ...
            fun expense(
                ...
                Rg07ManualExpenseInput(
                    RequestId(request),
                    Money.ofMinor(1_000, currency),
                    CategoryId("${ledger.value}-daily"),
                    AccountId("${ledger.value}-asset"),
...
private object StableRg07IdentitySource : Rg07IdentitySource {
    ...
    ): Rg07ReceiptCommitIds = Rg07ReceiptCommitIds("confirmation-${owner(operation)}", "reconciliation-${owner(operation)}", "entity-${owner(operation)}")
}
