"""RG-12 direct-v2 expected artifact: frozen-contract copy and publication-shape regression tests.

Authorities
-----------
- ``golden/rules/rg-12.json`` (frozen direct-v2 contract, contract_version
  2.0.0, approved at commit ``efbb13a`` per D-085): the 12 operation
  documents across 3 roots.  RG-12 is a direct-v2 scenario with no v1
  mapping and no adapter by design (D-085), so the expected artifact is the
  frozen contract itself, byte for byte.
- ``docs/migrations/golden-v2/rg-12-expected.json``: the expected artifact
...
- ``tools/python/golden_cases/v2.py``: ``validate_golden_case_v2``, the
  validation gate; RG-12 is in ``supported_transaction_types``.
- ``schemas/golden-case-v2.schema.json``: contract 2.0.0 (read-only).
- ``tools/python/golden_cases/rg12_publication.py``: the publication tool
...
from golden_cases.rg12_publication import publish_rg12

ROOT = Path(__file__).resolve().parents[2]
CONTRACT_PATH = ROOT / "golden" / "rules" / "rg-12.json"
EXPECTED_PATH = ROOT / "docs" / "migrations" / "golden-v2" / "rg-12-expected.json"
MANIFEST_PATH = ROOT / "golden" / "rules-v2" / "manifest.json"

# Counted directly from the frozen contract (D-085:1107 and the RG-12
# closure proposal raw-operation registry agree).
EXPECTED_OBJECT_COUNTS = {"operations": 12, "roots": 3, "states": 15}
EXPECTED_STATUS_COUNTS = {"accepted": 1, "no_change": 1, "rejected": 10}
...
class Rg12V2ExpectedTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_expected_contract_identity_and_approval(self):
        ...
        self.assertEqual(self.expected["case"]["id"], "RG-12")
    ...
    def test_publication_shape_registers_source_equal_expected_byte_hash(self):
        ...
        with tempfile.TemporaryDirectory() as name:
            ...
            entry = next(
                item for item in published["cases"] if item["case"] == "RG-12"
            )
