...
from golden_cases.rg05_publication import (
    PublicationRecoveryError,
    publish_rg05,
    recover_rg05_publication,
)


ROOT = Path(__file__).resolve().parents[2]
SOURCE_PATH = ROOT / "golden" / "rules" / "rg-05.json"
...
class Rg05PublicationTests(unittest.TestCase):
    def _copy_inputs(self, directory: Path) -> tuple[Path, Path, Path]:
        source = directory / "v1-rg-05.json"
        expected = directory / "expected-rg-05.json"
        manifest = directory / "manifest.json"
    ...
    def test_publication_hashes_is_idempotent_and_preserves_source(self):
        with tempfile.TemporaryDirectory() as name:
            ...
            output = Path(name) / "rg-05.json"
            source_before = source.read_bytes()
            expected_bytes = expected.read_bytes()

            first = publish_rg05(source, expected, output, manifest)
            ...
            self.assertEqual(
                {"operations": 25, "roots": 17, "states": 42},
                first.object_counts,
            )
            self.assertEqual(
                {"accepted": 4, "no_change": 4, "rejected": 17},
                first.operation_status_counts,
            )

            output_after_first = output.read_bytes()
            manifest_after_first = manifest.read_bytes()
            second = publish_rg05(source, expected, output, manifest)
            ...
            self.assertIn("RG-05", entries)
            self.assertEqual(first.canonical_sha256, entries["RG-05"]["canonical_sha256"])
            self.assertEqual(first.expected_sha256, entries["RG-05"]["expected_byte_sha256"])
            self.assertEqual(first.output_sha256, entries["RG-05"]["hashes"]["output_sha256"])
            ...
            # the discovery record must carry the 25-operation full comparison
            # (D-086 manifest registration format).
            self.assertEqual(first.source_sha256, entries["RG-05"]["source_byte_sha256"])
            self.assertEqual(first.source_sha256, entries["RG-05"]["source_sha256"])
            self.assertEqual(first.source_sha256, entries["RG-05"]["hashes"]["source_sha256"])
            self.assertEqual(first.expected_sha256, entries["RG-05"]["hashes"]["expected_sha256"])
            self.assertEqual(first.canonical_sha256, entries["RG-05"]["hashes"]["canonical_sha256"])
            self.assertEqual(
                {"operations": 25, "roots": 17, "states": 42},
                entries["RG-05"]["object_counts"],
            )
            self.assertEqual(
                {"accepted": 4, "no_change": 4, "rejected": 17},
                entries["RG-05"]["operation_status_counts"],
            )
            self.assertEqual(
                "25-operation full comparison",
                entries["RG-05"]["discovery"]["comparison"],
            )
            self.assertEqual(
                "approved", entries["RG-05"]["approval_status"]
            )
            self.assertEqual("published", entries["RG-05"]["publication_status"])

    def test_output_swap_failure_rolls_back_and_isolates_other_cases(self):
        with tempfile.TemporaryDirectory() as name:
            source, expected, manifest = self._copy_inputs(Path(name))
            output = Path(name) / "rg-05.json"
            original_output = b"old RG-05 output\n"
            ...
            self.assertFalse((Path(name) / ".rg-05-publication.journal.json").exists())
    ...
    def test_output_backup_failure_keeps_original_files_and_allows_retry(self):
        with tempfile.TemporaryDirectory() as name:
            ...
            retried = publish_rg05(source, expected, output, manifest)
    ...
    def test_output_backup_moved_journal_recovery_restores_missing_original(self):
        with tempfile.TemporaryDirectory() as name:
            ...
            output = directory / "rg-05.json"
            original_output = b"old RG-05 output\n"
            output.write_bytes(original_output)
            original_manifest = manifest.read_bytes()
            token = "a" * 32
            output_temp = directory / f".rg-05.json.{token}.tmp"
            manifest_temp = directory / f".manifest.json.{token}.tmp"
            output_backup = directory / f".rg-05.json.{token}.bak"
            ...
            output.unlink()
            journal_path = _craft_journal(directory, "RG-05", "output_backup")

            self.assertTrue(recover_rg05_publication(manifest))
            self.assertEqual(original_output, output.read_bytes())
            self.assertEqual(original_manifest, manifest.read_bytes())
    ...
    def test_prepared_journal_recovery_keeps_original_files(self):
        with tempfile.TemporaryDirectory() as name:
            ...
            source, expected, manifest = self._copy_inputs(directory)
            output = directory / "rg-05.json"
            output.write_bytes(b"old RG-05 output\n")
            ...
            output_temp.write_bytes(b"incomplete output")
            manifest_temp.write_bytes(b"incomplete manifest")
            _craft_journal(directory, "RG-05", "prepared")

            self.assertTrue(recover_rg05_publication(manifest))
            self.assertEqual(original_output, output.read_bytes())
            self.assertEqual(original_manifest, manifest.read_bytes())
            self.assertFalse((directory / ".rg-05-publication.journal.json").exists())
            self.assertFalse(output_temp.exists())
            self.assertFalse(manifest_temp.exists())
        ...
        for flag in flags:
            with self.subTest(flag=flag), tempfile.TemporaryDirectory() as name:
                ...
                with self.assertRaisesRegex(
                    ...
                    publish_rg05(source, expected, output, manifest, **{flag: True})
    ...
    def test_commit_failure_keeps_committed_state_and_stale_backups_are_swept(self):
        with tempfile.TemporaryDirectory() as name:
            ...
            with self.assertRaisesRegex(RuntimeError, "post-commit failure"):
                publish_rg05(source, expected, output, manifest, fail_after_commit=True)
            ...
            self.assertFalse(recover_rg05_publication(manifest))
            retried = publish_rg05(source, expected, output, manifest)
    ...
    def test_corrupt_journal_is_rejected_and_preserved(self):
        with tempfile.TemporaryDirectory() as name:
            ...
            output.write_bytes(original_output)
            original_manifest = manifest.read_bytes()
            journal_path = directory / ".rg-05-publication.journal.json"
            raw_cases = {
                "truncated": b'{"format_version": 2, "case": "RG-05", "phase": "prep',
                "empty": b"",
            ...
            journal_path.unlink()
            published = publish_rg05(source, expected, output, manifest)
        ...
        with tempfile.TemporaryDirectory() as name, tempfile.TemporaryDirectory() as outside_name:
            ...
            for field in (
                ...
                with self.subTest(field=field):
                    _craft_journal(directory, "RG-05", "output_installed", **{field: victim})
                    with self.assertRaises(PublicationRecoveryError):
                        recover_rg05_publication(manifest)
        ...
        with tempfile.TemporaryDirectory() as name:
            ...
            crafted_cases = {
                "token_not_hex": lambda: _craft_journal(
                    directory, "RG-05", "output_backup", token="z" * 32
                ),
                ...
                "dotdot_traversal": lambda: _craft_journal(
                    directory, "RG-05", "output_backup",
                    output_temp=directory.parent / f".rg-05.json.{'a' * 32}.tmp",
                ),
            }
            for label, craft in crafted_cases.items():
                with self.subTest(label=label):
            ...
            output.unlink()
            crafted = _craft_journal(directory, "RG-05", "prepared")
            raw = crafted.read_bytes()
            with self.assertRaises(PublicationRecoveryError):
                recover_rg05_publication(manifest)
            self.assertEqual(raw, crafted.read_bytes())
            ...
            output.write_bytes(original_output)
            (directory / f".rg-05.json.{'a' * 32}.bak").write_bytes(original_output)
            crafted = _craft_journal(directory, "RG-05", "prepared")
            raw = crafted.read_bytes()
            with self.assertRaises(PublicationRecoveryError):
                recover_rg05_publication(manifest)
    ...
    def test_recovery_is_idempotent_and_converges(self):
        with tempfile.TemporaryDirectory() as name:
            ...
            _craft_journal(directory, "RG-05", "manifest_installed")
    ...
    def test_output_installed_and_manifest_backup_journals_recover(self):
        with tempfile.TemporaryDirectory() as name:
            ...
            with self.subTest(phase="output_installed"):
                ...
                output_backup.write_bytes(original_output)
                output.write_bytes(b"new output bytes")
                _craft_journal(directory, "RG-05", "output_installed")
                self.assertTrue(recover_rg05_publication(manifest))
                self.assertEqual(original_output, output.read_bytes())
                self.assertEqual(original_manifest, manifest.read_bytes())
                self.assertFalse(output_backup.exists())
            ...
            with self.subTest(phase="manifest_backup"):
                ...
                manifest_backup.write_bytes(original_manifest)
                manifest.unlink()
                _craft_journal(directory, "RG-05", "manifest_backup")
                self.assertTrue(recover_rg05_publication(manifest))
                self.assertEqual(original_output, output.read_bytes())
                self.assertEqual(original_manifest, manifest.read_bytes())
                self.assertFalse(output_backup.exists())
    ...
    def test_gate_failure_stops_publication_with_zero_mutation(self):
        with tempfile.TemporaryDirectory() as name:
            ...
            with patch(
                "golden_cases.rg05_publication.verify_publication_integrity",
                side_effect=PublicationIntegrityError("injected gate failure"),
            ) as gate_mock, patch(
                "golden_cases.rg05_publication._load_json"
            ) as load_mock:
                with self.assertRaises(PublicationIntegrityError):
                    publish_rg05(source, expected, output, manifest)
    ...
    def test_published_output_is_lf_only(self):
        with tempfile.TemporaryDirectory() as name:
            ...
            source, expected, manifest = self._copy_inputs(directory)
            output = directory / "rg-05.json"
            result = publish_rg05(source, expected, output, manifest)
    ...
    def test_expected_artifact_has_25_operations(self):
        document = json.loads(EXPECTED_PATH.read_text(encoding="utf-8"))
        self.assertEqual(25, len(document["operations"]))
        self.assertEqual(17, len(document["roots"]))
        self.assertEqual(42, len(document["states"]))
        counts: dict[str, int] = {}
        for operation in document["operations"]:
            status = operation["outcome"]["status"]
            counts[status] = counts.get(status, 0) + 1
        self.assertEqual({"accepted": 4, "no_change": 4, "rejected": 17}, counts)
