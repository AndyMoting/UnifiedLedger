...
from golden_cases.rg03_publication import (
    PublicationRecoveryError,
    publish_rg03,
    recover_rg03_publication,
)


ROOT = Path(__file__).resolve().parents[2]
SOURCE_PATH = ROOT / "golden" / "rules" / "rg-03.json"
...
class Rg03PublicationTests(unittest.TestCase):
    ...
    def test_publication_hashes_is_idempotent_and_preserves_source(self):
        with tempfile.TemporaryDirectory() as name:
            ...
            output = Path(name) / "rg-03.json"
            source_before = source.read_bytes()
            expected_bytes = expected.read_bytes()

            first = publish_rg03(source, expected, output, manifest)
            ...
            self.assertEqual(
                {"operations": 20, "roots": 13, "states": 33},
                first.object_counts,
            )
            self.assertEqual(
                {"accepted": 5, "no_change": 5, "rejected": 10},
                first.operation_status_counts,
            )

            output_after_first = output.read_bytes()
            manifest_after_first = manifest.read_bytes()
            second = publish_rg03(source, expected, output, manifest)
            ...
            self.assertIn("RG-03", entries)
            self.assertEqual(first.canonical_sha256, entries["RG-03"]["canonical_sha256"])
            self.assertEqual(first.expected_sha256, entries["RG-03"]["expected_byte_sha256"])
            self.assertEqual(first.output_sha256, entries["RG-03"]["hashes"]["output_sha256"])
            ...
            # the discovery record must carry the 20-operation full comparison
            # (D-086 manifest registration format).
            self.assertEqual(first.source_sha256, entries["RG-03"]["source_byte_sha256"])
            self.assertEqual(first.source_sha256, entries["RG-03"]["source_sha256"])
            self.assertEqual(first.source_sha256, entries["RG-03"]["hashes"]["source_sha256"])
            self.assertEqual(first.expected_sha256, entries["RG-03"]["hashes"]["expected_sha256"])
            self.assertEqual(first.canonical_sha256, entries["RG-03"]["hashes"]["canonical_sha256"])
            self.assertEqual(
                {"operations": 20, "roots": 13, "states": 33},
                entries["RG-03"]["object_counts"],
            )
            self.assertEqual(
                {"accepted": 5, "no_change": 5, "rejected": 10},
                entries["RG-03"]["operation_status_counts"],
            )
            self.assertEqual(
                "20-operation full comparison",
                entries["RG-03"]["discovery"]["comparison"],
            )
            self.assertEqual(
                "approved", entries["RG-03"]["approval_status"]
            )
            self.assertEqual("published", entries["RG-03"]["publication_status"])

    def test_output_swap_failure_rolls_back_and_isolates_other_cases(self):
        with tempfile.TemporaryDirectory() as name:
            source, expected, manifest = self._copy_inputs(Path(name))
            output = Path(name) / "rg-03.json"
            original_output = b"old RG-03 output\n"
            ...
            self.assertFalse((Path(name) / ".rg-03-publication.journal.json").exists())
    ...
    def test_output_backup_failure_keeps_original_files_and_allows_retry(self):
        with tempfile.TemporaryDirectory() as name:
            ...
            retried = publish_rg03(source, expected, output, manifest)
    ...
    def test_output_backup_moved_journal_recovery_restores_missing_original(self):
        with tempfile.TemporaryDirectory() as name:
            ...
            original_manifest = manifest.read_bytes()
            token = "a" * 32
            output_temp = directory / f".rg-03.json.{token}.tmp"
            manifest_temp = directory / f".manifest.json.{token}.tmp"
            output_backup = directory / f".rg-03.json.{token}.bak"
            ...
            output.unlink()
            journal_path = _craft_journal(directory, "RG-03", "output_backup")

            self.assertTrue(recover_rg03_publication(manifest))
            self.assertEqual(original_output, output.read_bytes())
            self.assertEqual(original_manifest, manifest.read_bytes())
    ...
    def test_prepared_journal_recovery_keeps_original_files(self):
        with tempfile.TemporaryDirectory() as name:
            ...
            source, expected, manifest = self._copy_inputs(directory)
            output = directory / "rg-03.json"
            output.write_bytes(b"old RG-03 output\n")
            ...
            output_temp.write_bytes(b"incomplete output")
            manifest_temp.write_bytes(b"incomplete manifest")
            _craft_journal(directory, "RG-03", "prepared")

            self.assertTrue(recover_rg03_publication(manifest))
            self.assertEqual(original_output, output.read_bytes())
            self.assertEqual(original_manifest, manifest.read_bytes())
            self.assertFalse((directory / ".rg-03-publication.journal.json").exists())
            self.assertFalse(output_temp.exists())
            self.assertFalse(manifest_temp.exists())
        ...
        for flag in flags:
            with self.subTest(flag=flag), tempfile.TemporaryDirectory() as name:
                ...
                with self.assertRaisesRegex(
                    ...
                    publish_rg03(source, expected, output, manifest, **{flag: True})
    ...
    def test_commit_failure_keeps_committed_state_and_stale_backups_are_swept(self):
        with tempfile.TemporaryDirectory() as name:
            ...
            with self.assertRaisesRegex(RuntimeError, "post-commit failure"):
                publish_rg03(source, expected, output, manifest, fail_after_commit=True)
            # The transaction is committed: the journal is gone and both
            # targets carry the new versions (no mixed state).
            self.assertFalse((directory / ".rg-03-publication.journal.json").exists())
            self.assertEqual(expected.read_bytes(), output.read_bytes())
            published = json.loads(manifest.read_text(encoding="utf-8"))
            self.assertIn("RG-03", {item["case"] for item in published["cases"]})
            ...
            self.assertFalse(recover_rg03_publication(manifest))
            retried = publish_rg03(source, expected, output, manifest)
    ...
    def test_corrupt_journal_is_rejected_and_preserved(self):
        with tempfile.TemporaryDirectory() as name:
            ...
            output.write_bytes(original_output)
            original_manifest = manifest.read_bytes()
            journal_path = directory / ".rg-03-publication.journal.json"
            raw_cases = {
                "truncated": b'{"format_version": 2, "case": "RG-03", "phase": "prep',
                "empty": b"",
            ...
            journal_path.unlink()
            published = publish_rg03(source, expected, output, manifest)
        ...
        with tempfile.TemporaryDirectory() as name, tempfile.TemporaryDirectory() as outside_name:
            ...
            for field in (
                ...
                with self.subTest(field=field):
                    _craft_journal(directory, "RG-03", "output_installed", **{field: victim})
                    with self.assertRaises(PublicationRecoveryError):
                        recover_rg03_publication(manifest)
        ...
        with tempfile.TemporaryDirectory() as name:
            ...
            crafted_cases = {
                "token_not_hex": lambda: _craft_journal(
                    directory, "RG-03", "output_backup", token="z" * 32
                ),
                ...
                "dotdot_traversal": lambda: _craft_journal(
                    directory, "RG-03", "output_backup",
                    output_temp=directory.parent / f".rg-03.json.{'a' * 32}.tmp",
                ),
            }
            for label, craft in crafted_cases.items():
                with self.subTest(label=label):
            ...
            output.unlink()
            crafted = _craft_journal(directory, "RG-03", "prepared")
            raw = crafted.read_bytes()
            with self.assertRaises(PublicationRecoveryError):
                recover_rg03_publication(manifest)
            self.assertEqual(raw, crafted.read_bytes())
            ...
            output.write_bytes(original_output)
            (directory / f".rg-03.json.{'a' * 32}.bak").write_bytes(original_output)
            crafted = _craft_journal(directory, "RG-03", "prepared")
            raw = crafted.read_bytes()
            with self.assertRaises(PublicationRecoveryError):
                recover_rg03_publication(manifest)
    ...
    def test_recovery_is_idempotent_and_converges(self):
        with tempfile.TemporaryDirectory() as name:
            ...
            _craft_journal(directory, "RG-03", "manifest_installed")
    ...
    def test_output_installed_and_manifest_backup_journals_recover(self):
        with tempfile.TemporaryDirectory() as name:
            ...
            with self.subTest(phase="output_installed"):
                ...
                output_backup.write_bytes(original_output)
                output.write_bytes(b"new output bytes")
                _craft_journal(directory, "RG-03", "output_installed")
                self.assertTrue(recover_rg03_publication(manifest))
                self.assertEqual(original_output, output.read_bytes())
                self.assertEqual(original_manifest, manifest.read_bytes())
                self.assertFalse(output_backup.exists())
            ...
            with self.subTest(phase="manifest_backup"):
                ...
                manifest_backup.write_bytes(original_manifest)
                manifest.unlink()
                _craft_journal(directory, "RG-03", "manifest_backup")
                self.assertTrue(recover_rg03_publication(manifest))
                self.assertEqual(original_output, output.read_bytes())
                self.assertEqual(original_manifest, manifest.read_bytes())
                self.assertFalse(output_backup.exists())
    ...
    def test_published_output_is_lf_only(self):
        with tempfile.TemporaryDirectory() as name:
            ...
            source, expected, manifest = self._copy_inputs(directory)
            output = directory / "rg-03.json"
            result = publish_rg03(source, expected, output, manifest)
    ...
    def test_expected_artifact_has_20_operations(self):
        document = json.loads(EXPECTED_PATH.read_text(encoding="utf-8"))
        self.assertEqual(20, len(document["operations"]))
        self.assertEqual(13, len(document["roots"]))
        self.assertEqual(33, len(document["states"]))
        counts: dict[str, int] = {}
        for operation in document["operations"]:
            status = operation["outcome"]["status"]
            counts[status] = counts.get(status, 0) + 1
        self.assertEqual({"accepted": 5, "no_change": 5, "rejected": 10}, counts)
