...
阶段 4“导入与对账闭环”已于 2026-08-23 收口；阶段 6（Android 视觉与平台适配）已于 2026-09-11 按 D-142 收口，交付项为：compileSdk/targetSdk 已升至 37（minSdk 34），FOUND-001 损坏数据库 fail-closed 已按 D-132 闭环，D1 工具链批（D-133）、D2 主题与组件批（D-134）、D3 平台回归批（D-135）、玻璃启用批（D-136）与对话框 Esc 修复批（D-137，缺陷 D131DESKESC-001 关闭）均已交付并推送；D-138 发生时间手输宽容解析批、D-139/D-140 键入保留缺陷修复批、技术栈升级批（D-141：CMP 1.12.0 / backdrop 2.0.1 / Kotlin 2.4.20 / AGP 9.3.1 / targetSdk 37 / kotlinx-datetime 0.8.0）均已交付并推送。阶段 7（Android 日常工作流）第一组 P7-01（账户与分类目录管理）、P7-02（录入基础）、P7-03（看账）与 P7-04（导入与草稿确认）已按 D-143/D-144/D-145/D-146 交付并合入本地 `main`（merge `4eb41ee`/`b56913d`/`b490bea`/`12e3057`），已推送 `origin/main`（push `a710c6c..12e3057`，head `12e3057` 同提交 CI run `34924060988` 三 job 全绿并上传 APK 工件）；其后 D-147 开卡枚举性能批（schema v30 覆盖索引、会话级批量查询与进度面）、A-PERF 导入读治理批（merge `e61b9a0`）与 A-04 CCB 网银 XLS 的 Android 验证与能力翻转（merge `b66bdf3`/`c93e18a`，微信 XLSX、支付宝 CSV、招行 CSV 与 CCB 网银 XLS 四个导入来源在双平台全部可用）已交付并推送，其后 D-150～D-154（A-DOC 正式状态同步与 CCB fixture 隐私清理、P7-02 A-02 置顶与确认页缺陷修复、P7-02 提交后刷新链缺陷修复、P7-03 导入批量确认完成触发刷新、P7-04 导入决策面按方向渲染分类源）亦已交付并推送，其后 P7-05 设计门（D-156）、A-05 收口裁决（D-157）、P7-05 首切片（片 1a 基底层，D-158，schema v31）、ktlint 修复与 §10.3 规模项承接复核（D-159）、A-DOC 批正式状态同步（D-160，schema v31）、§10.3 程序化滚动致动器与四阶段规模证据（D-161/D-162）、B5 测量方法复核（D-163）、单次会话全链实测（D-164）、§10.3 数据条件实测（D-165）与 postScroll 决定性取证及 61k 规模 OOM 缺陷登记（D-166）亦已登记并推送；P7-05 修错与回收站（D-156）的片 1b（产品接线）已全量交付并合入 `main`（Piece 1–6，`6f74ee4`/`02af67a`/`cfe0722`/`f814f7a`/`b4d6200`/`5ea7f67`），V-17 Android 人工验收已执行并按 D-181 收口（四项用户可观察期望与 V-16 设备部分 PASS；TalkBack 朗读巡检与再核对入口设备观察两项缺证经用户 2026-09-26 裁决接受延期/替代证据，V-17 判 PASS 含披露；V-19 以会话内人工再核对为满足口径判 PASS，片 1b 验收就此收口）；P7-06（整体备份与恢复）五个设计门已批准（06.0/06.A 容器格式 D-174、06.1 稳定存储与 runtime owner D-176、06.2/06.B 备份导出 D-177、06.3/06.C 恢复预检 D-179、06.4/06.D 恢复确认与切换 D-182，规格 `docs/specs/2026-09-26-p7-06-restore-confirm-switch-design.md`），其中 06.1 已实施并合入 `main`（`d6cffb6`），但其 Android 启动路径经设备实证存在两项 P0 缺陷（相对库名含路径分隔符致 `StartupError`；主线程阻塞打开致 ANR 并留下 `POINTER_MISSING` 无指针代际）并已修复合入（`0c0b319`，登记 D-178；残余崩溃窗口归 06.D，06.1 仍非完整 P706-A06 通过）；P7-06 06.B（备份导出）已实施并合入 `main`（`6c3c415`，跟进修复合并 `a18738f`/`aed0920`；面向用户的取消控件延期，规格 §9 开放项与未并入 PASS 的验收向量见 `CURRENT_STATE.md`）；**06.C（预检）已按 D-179 规格实施并合入 `main`（merge `3d99b48`，登记 D-180；组合根接线归 06.D）**；06.4/06.D（恢复确认与切换）设计门已按 D-182 批准、实施属后续实施批；P7-07～P7-09 尚未开始。当前 `origin/main` 为 `328bee0`（D-181 登记合并；此前 `b69ba7b`（D-180 登记合并）同提交 CI run `36246932378` 三 job 全 success）；剩余 A-03 未竟项（TalkBack 朗读巡检接受延期、A03-8b 缺证）与 A-05 收口项仍在推进。P4-01 acceptance contract、P4-02 shared import spine、P4-03 微信 XLSX 普通收支、P4-04 transfer formalization、P4-05 支付宝普通收支及 RL-04 余额宝转账路由均已闭环；P4-08 matcher 与 reconciliation persistence 已按 D-103、P4-07 duplicate candidate/closed-records 已按 D-104/D-105 于 2026-08-22 合入 `main`；此后 P4-08 第二批 normalized evidence projection 已按 D-112 完成并合入 `main`（merge `b8755dd`，schema v26，2026-08-27 推送）。此后 P4-08 correction / successor invalidation 已按 D-113 完成并合入 `main`（merge `a3d11bb`，schema v27）——激活预留的失效事件/后继链接与 MISSING/DIFFERENCE 对账结果态，并解除 D-103 登记的该项延期。此后 BP-01 银行 parser 门承接批已按 D-116 于 2026-08-29 交付并推送（CMB 网银 CSV + CCB 网银 XLS 解析器、余额镜像 `note` 诊断与 RL-07 镜像代表路径；银行 parser 门 D-109 O-8 就此关闭，零 schema 变更；PDF/其他银行/信用卡属 D-116 边界外待样本）。

Python 继续作为迁移、规则验证和黄金结果基线。仓库包含 `ledger-domain`、`ledger-application`、`ledger-data` 与 `app-ui` 四个可构建的 Kotlin Multiplatform 共享库模块，SQLDelight schema 当前为 v31（v1→v31 共 30 个迁移文件）。RG-01 至 RG-12 的 runtime 均已进入共享库（RG-01/02 完整 state/delta/status 比较已实现，D-087）：RG-04 全 26 项完整比较已合入（`Rg04FullStateOracleTest`，`88c9bfa`，17 roots/26 ops/43 states），RG-05 expected 已根据 `D-075` 批准，RG-06 已根据 `D-081` 完成 41-operation full-state replay 并发布 v2 工件，RG-07 expected 已根据 `D-079` 批准，完整状态 oracle 也已完成，RG-09 runtime/persistence 按 `D-082` 批准范围实现，mapping gate 已 approved，严格 9-root/50-operation/59-state runtime oracle 比较发布工件（merge `07986b0`），RG-10 runtime/oracle/persistence 按 `D-083` 批准范围实现并已合入 main（`22f3141`），RG-11/12 direct-v2 runtime 按 `D-085` 实现，RG-08 完整 lending runtime 按 `D-084` 实现。这不表示所有黄金场景或正式账务核心已经完成：RG-01/02 完整比较已实现且 v2 publication 已按 `D-089` A 批完成（`D-090` LF 验收已闭合），RG-08 的 v2 expected 工件与 publication 已按 `D-089` B 批完成；阶段 5 已交付双端最小外壳与 P5-03 演示面 B：`desktop-app` 与 `android-app` 两个组合根应用模块可构建，`app-ui` 提供共享启动、总览和手工支出流程，桌面端可经 `.\gradlew.bat :desktop-app:run` 启动，Android 调试 APK 经 `.\gradlew.bat :android-app:assembleDebug` 构建并由 CI artifact 完成人工门。
...
Kotlin 构建需要 JDK 21。使用仓库内的 Gradle Wrapper 分别运行四个共享库模块的 JVM 测试。16 GB Windows 主机上 Gradle/Kotlin 验证必须串行运行，使用单 worker 和 1 GB heap；每次运行结束后停止 daemon。完整约束与命令见[开发规范](docs/CONTRIBUTING.md)。
