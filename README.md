...
这不是完整的 `RG-01` 或正式账务核心实现。当前尚未实现 JSON `field_path`/`reason_code` 映射、原始十进制字符串解析或序列化 adapter；应用层的原子提交也仍只是端口契约，尚无数据库、schema、ORM、并发或持久化实现。报表、对账、导入、UI 和平台客户端运行时均尚未实现。Android 与 Desktop 工程仍未建立，具体数据库、UI、导航、依赖注入和同步实现将在相应接口与验收边界稳定后选择。
