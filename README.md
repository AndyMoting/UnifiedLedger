...
`ledger-application` 目前实现 `RG-01` 的最小共享应用边界：明确确认请求、请求身份与完整快照、只含身份的回执、原子提交端口契约，以及稀疏保存输入 wrapper。wrapper 只允许金额、分类和付款账户为空；缺失项返回无序的语义类型集合且不调用确认用例，完整输入和已提供的零金额继续委托既有确认与领域验证。应用层还实现了 typed decoded-field Golden adapter：区分字段省略、显式 `null` 与值，使用精确十进制字符串转换，校验 RG-01 case timezone，并把既有结果投影为冻结的 `field_path`/`reason_code`。当前共 `30` 项应用测试。回执不携带可回写的业务快照，因此重试不会将已替代的备注版本回退到初始版本。
...
这仍不是完整的 `RG-01` 或正式账务核心实现。typed decoded-field adapter 已覆盖上述输入和结果投影，但 raw JSON decoder、序列化方案，以及完整 state/report/reconciliation/delta comparison 仍未实现或授权；报表、导入、对账、UI 和平台客户端运行时也均尚未实现。仓库已有 `ledger-data` 的 Android 编译目标，但 Android app 与 Desktop app 仍未建立；当前数据库选择只适用于上述正式持久化边界，UI、导航、依赖注入、同步及更广泛查询方案仍需在各自验收边界明确后决定。
