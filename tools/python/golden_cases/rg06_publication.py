"""Transactional publication helpers for the approved RG-06 v2 candidate."""
...
def _operation_status_counts(document: dict[str, Any]) -> dict[str, int]:
    ...
    for operation in document["operations"]:
        ...
        if status not in counts:
            raise ValueError(f"unsupported RG-06 operation status: {status}")
        counts[status] += 1
    return counts
...
def _manifest_entry(
    ...
    return {
        ...
        "discovery": {
            ...
            "comparison": "41-operation full comparison",
        ...
        "expected_path": "docs/migrations/golden-v2/rg-06-expected.json",
        "hashes": hashes,
        "object_counts": object_counts,
        "operation_status_counts": operation_status_counts,
        "output_path": "golden/rules-v2/rg-06.json",
        "publication_status": "published",
        "source_byte_sha256": source_sha256,
        "source_path": "golden/rules/rg-06.json",
        "source_sha256": source_sha256,
    }
...
def _journal_path(manifest_path: Path) -> Path:
    return manifest_path.with_name(".rg-06-publication.journal.json")
...
def recover_rg06_publication(manifest_path: Path) -> bool:
    """Recover an interrupted RG-06 publication transaction, if its journal exists."""
...
def publish_rg06(
    ...
    *,
    ...
    """Publish one RG-06 candidate with rollback and idempotent replay."""
    source_path = source_path.resolve()
    expected_path = expected_path.resolve()
    output_path = output_path.resolve()
    manifest_path = manifest_path.resolve()
    if output_path == source_path or output_path == expected_path:
        raise ValueError("RG-06 publication target must be distinct from inputs")
    if output_path.parent != manifest_path.parent:
        raise ValueError("RG-06 output and manifest must share a publication directory")
    ...
    if source.get("schema_version") != 1 or source.get("case", {}).get("id") != CASE_ID:
        raise ValueError("RG-06 source is not the frozen v1 fixture")
    if expected.get("contract") != CONTRACT or expected.get("contract_version") != CONTRACT_VERSION:
        raise ValueError("RG-06 expected has the wrong contract")
    if expected.get("case", {}).get("id") != CASE_ID:
        raise ValueError("RG-06 expected has the wrong case id")
    if expected.get("case", {}).get("approval_status") != "approved":
        raise ValueError("RG-06 expected must be approved before publication")
    ...
    if not manifest_path.is_file():
        raise ValueError("RG-06 publication requires the existing v2 manifest")
    ...
    if not _same_bytes(source_path, source_bytes):
        raise ValueError("RG-06 source changed during publication preparation")
    ...
    try:
        ...
        if fail_after_manifest_backup:
            raise RuntimeError("injected RG-06 manifest backup failure")
        ...
        if fail_after_commit:
            raise RuntimeError("injected RG-06 post-commit failure")
...
__all__ = ["PublicationResult", "publish_rg06", "recover_rg06_publication"]
