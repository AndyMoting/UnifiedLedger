"""Transactional publication helpers for the approved RG-02 v2 candidate."""
...
def _operation_status_counts(document: dict[str, Any]) -> dict[str, int]:
    ...
    for operation in document["operations"]:
        ...
        if status not in counts:
            raise ValueError(f"unsupported RG-02 operation status: {status}")
        counts[status] += 1
    return counts
...
def _manifest_entry(
    ...
    return {
        ...
        "discovery": {
            ...
            "comparison": "13-operation full comparison",
        ...
        "expected_path": "docs/migrations/golden-v2/rg-02-expected.json",
        "hashes": hashes,
        "object_counts": object_counts,
        "operation_status_counts": operation_status_counts,
        "output_path": "golden/rules-v2/rg-02.json",
        "publication_status": "published",
        "source_byte_sha256": source_sha256,
        "source_path": "golden/rules/rg-02.json",
        "source_sha256": source_sha256,
    }
...
def _journal_path(manifest_path: Path) -> Path:
    return manifest_path.with_name(".rg-02-publication.journal.json")
...
def recover_rg02_publication(manifest_path: Path) -> bool:
    """Recover an interrupted RG-02 publication transaction, if its journal exists."""
...
def publish_rg02(
    ...
    *,
    ...
    """Publish one RG-02 candidate with rollback and idempotent replay."""
    source_path = source_path.resolve()
    expected_path = expected_path.resolve()
    output_path = output_path.resolve()
    manifest_path = manifest_path.resolve()
    if output_path == source_path or output_path == expected_path:
        raise ValueError("RG-02 publication target must be distinct from inputs")
    if output_path.parent != manifest_path.parent:
        raise ValueError("RG-02 output and manifest must share a publication directory")
    output_path.parent.mkdir(parents=True, exist_ok=True)
    publish_dir = manifest_path.parent

    recovered = recover_rg02_publication(manifest_path)
    ...
    if source.get("schema_version") != 1 or source.get("case", {}).get("id") != CASE_ID:
        raise ValueError("RG-02 source is not the frozen v1 fixture")
    if expected.get("contract") != CONTRACT or expected.get("contract_version") != CONTRACT_VERSION:
        raise ValueError("RG-02 expected has the wrong contract")
    if expected.get("case", {}).get("id") != CASE_ID:
        raise ValueError("RG-02 expected has the wrong case id")
    if expected.get("case", {}).get("approval_status") != "approved":
        raise ValueError("RG-02 expected must be approved before publication")
    ...
    if not manifest_path.is_file():
        raise ValueError("RG-02 publication requires the existing v2 manifest")
    ...
    if not _same_bytes(source_path, source_bytes):
        raise ValueError("RG-02 source changed during publication preparation")
    ...
    try:
        ...
        if fail_after_output_backup:
            raise RuntimeError("injected RG-02 output backup failure")
        journal["phase"] = "output_installed"
        _write_journal(journal_path, journal)
        os.replace(output_temp, output_path)
        if fail_after_output_swap:
            raise RuntimeError("injected RG-02 output swap failure")
        ...
        if fail_after_manifest_backup:
            raise RuntimeError("injected RG-02 manifest backup failure")
        journal["phase"] = "manifest_installed"
        _write_journal(journal_path, journal)
        os.replace(manifest_temp, manifest_path)
        if fail_after_manifest_swap:
            raise RuntimeError("injected RG-02 manifest swap failure")
        ...
        if fail_after_commit:
            raise RuntimeError("injected RG-02 post-commit failure")
...
__all__ = ["PublicationResult", "publish_rg02", "recover_rg02_publication"]
