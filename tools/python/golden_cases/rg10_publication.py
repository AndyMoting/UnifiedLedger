"""Transactional publication helpers for the approved RG-10 v2 candidate."""
...
def _operation_status_counts(document: dict[str, Any]) -> dict[str, int]:
    ...
    for operation in document["operations"]:
        ...
        if status not in counts:
            raise ValueError(f"unsupported RG-10 operation status: {status}")
        counts[status] += 1
    return counts
...
def _manifest_entry(
    ...
    return {
        ...
        "expected_path": "docs/migrations/golden-v2/rg-10-expected.json",
        "hashes": hashes,
        "object_counts": object_counts,
        "operation_status_counts": operation_status_counts,
        "output_path": "golden/rules-v2/rg-10.json",
        "publication_status": "published",
        "source_byte_sha256": source_sha256,
        "source_path": "golden/rules/rg-10.json",
        "source_sha256": source_sha256,
    }
...
def _journal_path(manifest_path: Path) -> Path:
    return manifest_path.with_name(".rg-10-publication.journal.json")
...
def publish_rg10(
    ...
    *,
    ...
    if output_path == source_path or output_path == expected_path:
        raise ValueError("RG-10 publication target must be distinct from inputs")
    ...
    if source.get("schema_version") != 1 or source.get("case", {}).get("id") != CASE_ID:
        raise ValueError("RG-10 source is not the frozen v1 fixture")
    if expected.get("contract") != CONTRACT or expected.get("contract_version") != CONTRACT_VERSION:
        raise ValueError("RG-10 expected has the wrong contract")
    if expected.get("case", {}).get("id") != CASE_ID:
        raise ValueError("RG-10 expected has the wrong case id")
    if expected.get("case", {}).get("approval_status") != "approved":
        raise ValueError("RG-10 expected must be approved before publication")
    ...
    if not manifest_path.is_file():
        raise ValueError("RG-10 publication requires the existing v2 manifest")
    ...
    if not _same_bytes(source_path, source_bytes):
        raise ValueError("RG-10 source changed during publication preparation")
...
__all__ = ["PublicationResult", "publish_rg10", "recover_rg10_publication"]
